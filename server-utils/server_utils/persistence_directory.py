...
class PersistenceResetter:
    """Mark a persistence directory for deletion on the next boot.

    Deletions are deferred to the next boot so that in-progress HTTP requests
    and background tasks are not disrupted.
    """
    ...
    async def mark_directory_reset(self) -> None:
        """Write a marker file that ``prepare_root`` will honour on the next boot."""
...
async def prepare_root(
    ...
    if await _is_marked_for_reset(directory_to_reset=persistence_directory_root):
        _log.info(f"{persistence_directory_root} was marked for reset. Deleting it.")
...
async def prepare_active_subdirectory(
    ...
    """Run pending migrations and return the active version subdirectory.

    This cleans up any stray temporary files left by a prior interrupted
    migration, then runs all outstanding migrations to bring the database
