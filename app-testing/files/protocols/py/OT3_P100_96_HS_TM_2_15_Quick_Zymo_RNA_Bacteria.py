...
def run(ctx):
    ...
    """
    # Protocol Parameters
    MAG_PLATE_SLOT = ctx.load_module('heaterShakerModuleV1','1')
    ...
    lysis_reservoir = ctx.load_labware(deepwell_type,'2') #deleted after use- replaced (by gripper) with wash2 res
    bind_reservoir = ctx.load_labware(deepwell_type,'6')
    wash1_reservoir = ctx.load_labware(deepwell_type,'5')
    wash2_reservoir = ctx.load_labware(deepwell_type,'4') #loaded on magplate- move to lysis location after lysis is used
    ...
    def grip_offset(action, item, slot = None):
        ...
        _pick_up_offsets = {
            "deck": Point(x=-0.2),
            "mag-plate": Point(x=-0.5,y=3),
            "heater-shaker": Point(x=-2.5,y=1.5),
            "temp-module": Point(),
            "thermo-cycler": Point(),
        }
        # EDIT these values
        ...
        _drop_offsets = {
            "deck": Point(z=-2),
            "mag-plate": Point(y=3,z=9.5),
            "heater-shaker": Point(x=-2.5,y=2,z=-2),
