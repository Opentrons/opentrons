def get_values(*names):
            import json
            _all_values = json.loads("""{"trash_chute":false,"USE_GRIPPER":true,"dry_run":false,"temp_mod":true,"heater_shaker":true,"tip_mixing":false,"wash_vol":600,"AL_vol":230,"sample_vol":180,"bind_vol":320,"elution_vol":100,"protocol_filename":"Omega_HDQ_DNA_Cells-Flex_96_channel"}""")
...
def run(ctx):
    ...
    try:
        [trash_chute,USE_GRIPPER,dry_run,temp_mod,heater_shaker,tip_mixing,wash_vol,sample_vol,bind_vol,AL_vol,elution_vol] = get_values(  # noqa: F821
    ...
    if not heater_shaker and tip_mixing:
        ctx.comment('This step is usually performed at 55C for the second 10 minutes, if results dont look right, look into our heater-shaker.')
        for x in range(3):
