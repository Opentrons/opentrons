def get_values(*names):
            import json
            _all_values = json.loads("""{"num_samp":8,"real_mode":true,"m50_mount":"left","m200_mount":"right","protocol_filename":"sigdx-part2"}""")
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    ctx.pause(f'You will need {200*num_col*1.15} of fast binding buffer in columns A2, A3, A4, A5 of the deepwell reagent plate on deck, B2.')
    ...
    ctx.pause(f'You will need {200*num_col*1.15} of wash buffer 1 in columns 1 and 2 of the deepwell reagent plate on temperature module, D1')
    ctx.pause(f'You will need {200*num_col*1.15} of wash buffer 2 in columns 3, 4, 5 of the deepwell reagent plate on temperature module, D1')
    ...
    for i in range (2):
        ...
        ctx.move_labware(
            labware=deep_plate,
            new_location=sample_temp_mod if i == 0 else 5,
            use_gripper=True
        )
    ...
    for i in range (3):
        ...
        ctx.move_labware(
            labware=deep_plate,
            new_location=sample_temp_mod if i <= 1 else 5,
            use_gripper=True
        )
    ...
    pause(f'''Water added. Store 22.5ul in freezer. Protocol continues with remaining 22.5ul in armadillo plate in D2.
    ...
    pause(f'''Primers and amp mix added. Plate on slot 2 ready for PCR according to section 2.4.
             ''')
    ...
    ctx.move_labware(
        labware=sample_plate,
        new_location=2,  # back to temp mod
        use_gripper=True
    )
