def get_values(*names):
            import json
            _all_values = json.loads("""{"VOL_REACTION":10,"NUM_COMBO":8,"insert_1":2,"vector_1":1,"insert_2":2,"vector_2":1,"insert_3":2,"vector_3":1,"insert_4":2,"vector_4":1,"insert_5":2,"vector_5":1,"insert_6":2,"vector_6":1,"insert_7":2,"vector_7":1,"insert_8":2,"vector_8":1,"protocol_filename":"Takara_InFusionSnapAssembly_Flex"}""")
            return [_all_values[n] for n in names]


metadata = {
    'protocolName': 'Takara In-Fusion Snap Assembly Kit on Flex',
...
def run(ctx):
    ...
    if VOL_REACTION == 10: 
        ...
        for i in range(num_row):
            ...
            if vol_water < 0: raise Exception('Invalid liqud volume')
    ...
    else: raise Exception('Invalid liqud volume') 
    ...
    tips = [ctx.load_labware('opentrons_flex_96_filtertiprack_50ul', slot, '50uL Tips')
               for slot in ['C2', 'B2']]
    ...
    for i in range(len(VOL_INSERT)): 
        ...
        insert_def = ctx.define_liquid(name="Insert# "+str(i+1), description="DNA Fragments to be Inserted", display_color="#704848")  ## Brown
