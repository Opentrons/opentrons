...
def run(protocol: protocol_api.ProtocolContext):
    ...
    tip1000 = [protocol.load_labware('opentrons_flex_96_filtertiprack_1000ul', 
                                     location=slot, adapter="opentrons_flex_96_tiprack_adapter") for slot in ["A1","A2", "B1"]]

    trash = protocol.load_trash_bin(location="A3")

    dwp = [protocol.load_labware("nest_96_wellplate_2ml_deep", location=slot, label=f"Sample Plate {no}") 
           for slot, no in zip(["C2","C3"][:plate_count],[1,2][:plate_count])]

    pcr = [protocol.load_labware("opentrons_96_wellplate_200ul_pcr_full_skirt", location=slot, label=f"Elution Plate {no}") 
