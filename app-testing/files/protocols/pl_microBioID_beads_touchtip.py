def get_values(*names):
            import json
            _all_values = json.loads("""{"num_samp":8,"digest_time_hours":12,"bead_incubation":15,"test_mode":false,"p50_mount":"right","m50_mount":"left","protocol_filename":"microBioID_beads_touchtip"}""")
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    [num_samp, digest_time_hours, bead_incubation,
        test_mode, p50_mount, m50_mount] = get_values(  # noqa: F821
        "num_samp", "digest_time_hours", "bead_incubation",
            "test_mode", "p50_mount", "m50_mount")
    ...
    reagent_deep_plate = ctx.load_labware(
        'nest_96_wellplate_2ml_deep', 'C3')
    dest_plate = ctx.load_labware("opentrons_96_wellplate_200ul_pcr_full_skirt",
                                  protocol_api.OFF_DECK)
    reagent_pcr_plate = ctx.load_labware(
        'opentrons_96_wellplate_200ul_pcr_full_skirt', 'C1')

    deck_slots = ['B1', 'B2', 'B3', 'A1', 'A2']
    staging_slots = ['A4', 'B4', 'C4', 'D4']
    tiprack_50 = [ctx.load_labware('opentrons_flex_96_tiprack_50ul',
                  slot) for slot in deck_slots]

    staging_racks = [ctx.load_labware('opentrons_flex_96_tiprack_50ul',
    ...
    m50 = ctx.load_instrument(
        "flex_8channel_50", m50_mount, tip_racks=tiprack_50)
    ...
    ctx.comment('\n---------------LYSING SAMPLES----------------\n\n')
    for starting_sample, dest in zip(start_sample_plate.rows()[0][:num_col],
                                  samples):
        pick_up(m50)
    ...
    for well in empty_digestion_buff_col:
        p50.transfer(digestion_vol_per_well,
                     digestion_tube,
                     well,
    ...
    ctx.pause("""Add a lid/seal to the pcr plate
    ...
    ctx.move_labware(labware=reagent_deep_plate,
