def get_values(*names):
            import json
            _all_values = json.loads("""{"num_samp":8,"real_mode":true,"end_prep_step":true,"first_wash_step":true,"add_tet_step":true,"second_wash_step":true,"add_formamide_step":true,"third_wash_step":true,"fourth_wash_step":true,"m50_mount":"left","m200_mount":"right","protocol_filename":"EM-seq_48Samples_AllSteps_Edits_150"}""")
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    [num_samp, real_mode, end_prep_step, first_wash_step, add_tet_step,
        second_wash_step, add_formamide_step, third_wash_step,
        fourth_wash_step, m50_mount, m200_mount] = get_values(  # noqa: F821
        "num_samp", "real_mode", "end_prep_step", "first_wash_step",
        "add_tet_step", "second_wash_step", "add_formamide_step",
        "third_wash_step", "fourth_wash_step", "m50_mount", "m200_mount")
    ...
    staging_racks_50 = [ctx.load_labware('opentrons_flex_96_tiprack_50ul',
                  slot) for slot in staging_slots[:2]]
    staging_slots_50 = ['A4', 'B4']
    staging_racks_200 = [ctx.load_labware('opentrons_flex_96_tiprack_200ul',
                  slot) for slot in staging_slots[2:]]
    ...
    def pick_up(pip):
        ...
        try:
            ...
            # if not tipracks in staging slot (second iteration), replenish
                ...
                tipracks = [ctx.load_labware('opentrons_flex_96_tiprack_50ul' if pip == m50 else 'opentrons_flex_96_tiprack_200ul', slot) for slot in deck_slots]
                staging_racks = [ctx.load_labware('opentrons_flex_96_tiprack_50ul' if pip == m50 else 'opentrons_flex_96_tiprack_200ul', slot) for slot in staging_slots]
    ...
    def remove_super(pip, vol, list_of_samples, round, eb_vol, eb_well, list_to_go, eb_mix_vol, plate, deck_location, pcr=False, ethanol=False):
    ...
    beads = deep_reag_plate['A1'] if num_col <= 3 else [deep_reag_plate['A1'], deep_reag_plate['A2']]*100  # BEADS DEPENDENT ON HOW MANY SAMPLES
    ...
    ctx.pause(f'''You will need {num_ethanol_wells} ethanol wells in the reservoir all with 10mL each, starting at A1.
    ...
    if end_prep_step:
        ...
        pause(f'''End Prep MMX added. Place plate in thermal cycler, then back on the temperature module on C1.
    ...
    if first_wash_step:
        ...
        pause('''Beads cleanup completed. Continue with 1.5 Oxidation of 5-Methylcytosines and 5-Hydroxymethylcytosines. Samples are on the right half of the plate, starting in column 7.''')

        ########################### END FIRST BEAD WASH #############################
        ########################### END FIRST BEAD WASH #############################
        ########################### END FIRST BEAD WASH #############################

        pause(f'''First wash completed.
    ...
    if second_wash_step:
        ...
        ctx.move_labware(
            labware=sample_plate,
            new_location=chute,  # OR MOVE IT WITH CHUTE?
            use_gripper=True
        )
        ...
        ctx.move_labware(
            labware=fresh_plate,
            new_location=sample_temp_adapter,
            use_gripper=True
        )

        pause('''Completed step 1.6. Samples are in left half of plate on temperature module.
    ...
    if third_wash_step:
        ...
        pause('''Samples are in right half of PCR plate. They can go overnight at -20C. Proceed on to 1.10 of SOP manually. Return plate to complete 1.11 of SOP.''')
