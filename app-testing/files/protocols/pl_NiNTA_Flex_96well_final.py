def get_values(*names):
            import json
            _all_values = json.loads("""{"NUM_COL":12,"INCUBATION_ON_DECK":1,"INCUBATION_MIN":60,"ELUTION_TIMES":1,"BEADS_PRELOAD":1,"protocol_filename":"NiNTA_Flex_96well_final"}""")
...
def run(ctx):
    ...
    def transfer(vol1, start, end, reagent=0):
        for i in range(NUM_COL):
                ...
                start_loc = start[i]
                end_loc = end[i]
                if reagent==2: p1000.mix(5, vol1*0.75, start_loc.bottom(z=ASP_HEIGHT*2))
                if reagent==3: 
                    p1000.flow_rate.aspirate = 100
                    p1000.aspirate(vol1, start_loc.bottom(z=ASP_HEIGHT))
                    p1000.flow_rate.aspirate = default_rate
                else: p1000.aspirate(vol1, start_loc.bottom(z=ASP_HEIGHT)) 
                p1000.air_gap(10)
