def get_values(*names):
            import json
            _all_values = json.loads("""{"NUM_COL":12,"INCUBATION_ON_DECK":1,"INCUBATION_MIN":60,"READY_FOR_SDSPAGE":1,"protocol_filename":"Dynabeads_IP_Flex_96well_final"}""")
            return [_all_values[n] for n in names]


metadata = {
    'protocolName': 'Immunoprecipitation by Dynabeads - 96-well setting on Opentrons Flex',
...
def run(ctx):
    ...
    def transfer_plate_to_plate(vol1, start, end, liquid, drop_height=-20):
        for i in range(NUM_COL):  
            ...
            p1000.blow_out()
            p1000.touch_tip()
            if USE_TRASH_BIN == 1: p1000.drop_tip()
    ...
    def transfer_well_to_plate(vol2, start, end, liquid, drop_height=-20):
        if liquid == 5:
            ...
            start_loc = start
            end_loc_gap = end[0]
            p1000.mix(10, vol2*NUM_COL*0.5, start_loc.bottom(z=ASP_HEIGHT))
            p1000.aspirate(vol2*NUM_COL, start_loc.bottom(z=ASP_HEIGHT))
            p1000.air_gap(10)
            p1000.dispense(10, end_loc_gap.top(z=-5))
            for j in range(NUM_COL):   
                end_loc = end[j]
                p1000.flow_rate.dispense = 500
                p1000.dispense(vol2, end_loc.top(z=drop_height))  
                p1000.flow_rate.dispense = default_rate 
                p1000.touch_tip()
            p1000.blow_out()
            p1000.drop_tip()
    ...
    transfer_well_to_plate(BEADS_VOL, beads, working_cols, 2)
    ...
    transfer_well_to_plate(AB_VOL, ab, working_cols, 3)
    ...
    transfer_well_to_plate(ELUTION_VOL, elu, working_cols, 4)
