...
metadata = {
	'protocolName':'Illumina PCR-Free DNA Prep',
...
################################################################################
	...
	def tiptrack(rack,reuse_col,reuse=None):
		...
		if rack == tip200:
			...
			if reuse == "REMOVE" and REUSE_REMOVE_TIPS:
				...
				tt_200 -= 1
				if report:
					ctx.comment('Column '+str(12-tt_200)+' (may be negative in certain conditions- use 12 as a base [i.e -5 = column 7])')
					ctx.comment('Available On Deck Slots:'+str(len(Available_on_deck_slots)))
					ctx.comment('Available Off Deck Slots:'+str(len(Available_off_deck_slots)))
				p200.pick_up_tip()
	...
	def run_tag_profile():
		...
		ctx.move_labware(
			sample_plate,
			tc_mod if ondeck_thermo else "A1",
			use_gripper = USE_GRIPPER
		)
		if heater_shaker:
			h_s.close_labware_latch()
...
################################################################################
	...
	def tagmentation():
		...
		if heater_shaker:
			...
			ctx.delay(minutes=2 if not dry_run else 0.25,msg='Please allow 2 minutes to mix beads \
		...
		if ondeck_thermo:
			...
			if heater_shaker:
				h_s.open_labware_latch()
			ctx.pause("Place plate on off-deck thermocycler and run pre-programmed TAG profile. Return to Heater-Shaker/ D1 and press resume.")
			if heater_shaker:
				h_s.close_labware_latch()
...
################################################################################
	...
	def post_tag_cleanup():
		...
		if heater_shaker:
			h_s.set_and_wait_for_shake_speed(1600)
			ctx.delay(minutes=2 if not dry_run else 0.25,msg='Please allow 2 minutes for stop solution to mix with sample.\
			 Shaking at 1600 rpm.')
			h_s.deactivate_shaker()
...
################################################################################
	...
	def index_ligation():
		...
		if heater_shaker:
			...
			ctx.delay(minutes=2 if not dry_run else 0.25,msg='Please allow 2 minutes \
		...
		if ondeck_thermo:
			...
			if heater_shaker:
				h_s.open_labware_latch()
			ctx.pause("Place plate on off-deck thermocycler and run pre-programmed ELM profile. Place FLP plate in A1. Return sample plate to Heater-Shaker/ D1 and press resume.")
			if heater_shaker:
				h_s.close_labware_latch()
		"""
		...
		if heater_shaker:
			if trash_tips:
				...
				ctx.comment('****Dropping Tip Back in Tip Box****')
			h_s.set_and_wait_for_shake_speed(1600)
			ctx.delay(minutes=2 if not dry_run else 0.25,msg='Please allow 2 minutes for \
...
################################################################################
	...
	def lib_cleanup():
		...
		for x,i in enumerate(samples):
			if standard_input:
				tiptrack(tip200,None,reuse=None)
				mix_beads(p200,ipb_res,40,7 if x == 0 else 2,num_cols-1) #num_cols-1 means there will be no calculation of volume, just use exact volume specified
