def get_values(*names):
            import json
            _all_values = json.loads("""{"NUM_COL":12,"INCUBATION_ON_DECK":1,"INCUBATION_MIN":60,"READY_FOR_SDSPAGE":1,"protocol_filename":"Dynabeads_IP_Flex_96well_RIT_final"}""")
...
def run(ctx):
    ...
    def transfer_well_to_plate(vol2, start, end, liquid, drop_height=-20):
        if liquid == 5:
            ...
            for j in range(NUM_COL): 
                end_loc_gap = end[j*8] 
                if liquid == 2: p1000_single.mix(2, vol*0.75, start_loc.bottom(z=ASP_HEIGHT*5))
