---
title: Labware
description: How to load and work with Opentrons-verified and custom labware in a Python protocol.
---
...
Here's how you use the `.ProtocolContext.load_labware` method to load labware on either Flex or OT-2.
...
You can load lids on compatible plates or tip racks with the optional `lid` parameter of `~.ProtocolContext.load_labware`. This example adds an Opentrons Tough Auto-Sealing Lid to a PCR plate:
...
You might need multiple lids during your protocol. Use `.ProtocolContext.load_lid_stack` to stack up to five Opentrons Tough Auto-Sealing Lids on a deck slot, or use `.Labware.load_lid_stack` to stack them on the riser or other compatible adapter.
...
:material-plus-circle-outline: *Added in version 2.23*
...
:material-plus-circle-outline: *Added in version 2.15:* 
...
!!! note
    ...
    :material-alert-octagon-outline: Deprecated in version 2.15
...
To use these optional methods, first create a liquid object with `.ProtocolContext.define_liquid` and then label individual wells by calling `.Labware.load_liquid`.
...
:material-plus-circle-outline: *Added in version 2.14*
...
You can also use `.Labware.load_empty` to label individual wells or an entire labware as empty at the beginning of your protocol.

:material-plus-circle-outline: *Added in version 2.14:* 
Use `Well.load_liquid()` to label liquid in individual wells.


:material-plus-circle-outline: *Added in version 2.22:* 
...
Although it's optional to define and load liquids in your protocol, you can use a starting liquid volume to specify pipette movements relative to a liquid location, like the `meniscus <well-meniscus>`, in your protocol.

## Well Dimensions<span id="v2-location-within-wells"></span>

The functions in the `new-well-access` section above return a single `.Well` object or a larger object representing many wells. `.Well` objects have attributes that provide information about their physical shape, such as the depth or diameter, as specified in their corresponding labware definition. These properties can be used for different applications, such as calculating the volume of a well or a `position relative to the well <position-relative-labware>`.
...
:material-plus-circle-outline: *Added in version 2.9*
