...
- Wells now have a `~.Well.meniscus` location that corresponds to the top of the liquid, either as set in the protocol or measured by probing with a pipette tip. See `well-meniscus`.
- Load and move labware lids with a new `lid` parameter of `~.ProtocolContext.load_labware` and standalone methods. See `loading-lids` and `moving-lids`.
...
- Adds `.AbsorbanceReaderContext` to support the `Absorbance Plate Reader Module <absorbance-plate-reader-module>`. Use the load name `absorbanceReaderV1` with `.ProtocolContext.load_module` to add an Absorbance Plate Reader to a protocol.
- `Liquid presence detection <lpd>` now only checks on the first aspiration of the `.mix` cycle.
- Improved the run log output of `.ThermocyclerContext.execute_profile`.

### Version 2.20

- Detect liquid presence within a well. The `.InstrumentContext.detect_liquid_presence()` and `.InstrumentContext.require_liquid_presence()` building block commands check for liquid any point in your protocol. You can also `enable liquid presence detection <lpd>` for all aspirations when loading a pipette, although this will add significant time to your protocol.
- Define CSV runtime parameters and use their contents in a protocol with new `data manipulation methods <rtp-csv-data>`. See the `cherrypicking use case <use-case-cherrypicking>` for a full example.
...
- Move the pipette to positions relative to the top of a trash container. See `position-relative-trash`. The default behavior of `.drop_tip` also accounts for this new possibility.
...
- New features
  - Use `.configure_nozzle_layout` to pick up a single column of tips with the 96-channel pipette. See `Partial Tip Pickup <partial-tip-pickup>`.
...
- Flex features
  - Write protocols for Opentrons Flex by declaring `"robotType": "Flex"` in the new `requirements` dictionary. See the `examples in the Tutorial <tutorial-requirements>`.
  - `.load_instrument` supports loading Flex 1-, 8-, and 96-channel pipettes. See `new-create-pipette`.
  ...
  - `.load_module` supports loading the `Magnetic Block <magnetic-block>`.
  ...
  - The new `.configure_for_volume` method can place Flex 50 µL pipettes in a low-volume mode for dispensing very small volumes of liquid. See `pipette-volume-modes`.
- Flex and OT-2 features
  ...
  - Use coordinates or numbers to specify `deck slots <deck-slots>`. These formats match physical labels on Flex and OT-2, but you can use either system, regardless of `robotType`.
  - The new module context `load_adapter()` methods let you load adapters and labware separately on modules, and `.ProtocolContext.load_adapter` lets you load adapters directly in deck slots. See `labware-on-adapters`.
  - Move labware manually using `.move_labware`, without having to stop your protocol.
  - Manual labware moves support moving to or from the new `~.protocol_api.OFF_DECK` location (outside of the robot).
  - `.ProtocolContext.load_labware` also accepts `~.protocol_api.OFF_DECK` as a location. This lets you prepare labware to be moved onto the deck later in a protocol.
...
- Known limitations
  ...
  - `.ProtocolContext.max_speeds` is not yet supported on the API version.
    Use `.InstrumentContext.default_speed` or the per-method <span class="title-ref">speed</span> argument, instead.
...
- You can now access certain geometry data regarding a labware's well via a Well Object. See `new-labware-well-properties` for more information.

### Version 2.8

- You can now pass in a list of volumes to distribute and consolidate. See `distribute-consolidate-volume-list` for more information.
  - Passing in a zero volume to any `complex command <v2-complex-commands>` will result in no actions taken for aspirate or dispense
- `.Well.from_center_cartesian` can be used to find a point within a well using normalized distance from the center in each axis.
  - Note that you will need to create a location object to use this function in a protocol. See `protocol-api-labware` for more information.
- You can now pass in a blowout location to transfer, distribute, and consolidate
  with the `blowout_location` parameter. See `.InstrumentContext.transfer` for more detail!
...
- GEN2 Single pipettes now default to flow rates equivalent to 10 mm/s plunger
  ...
  - For a comparison between API Versions, see `ot2-flow-rates`

### Version 2.5

- New `utility commands <new-utility-commands>` were added:
...
- You should now specify Magnetic Module engage height using the
  ...
  magnet from the base of the labware. For more, see `magnetic-module-engage`.
- Return tip will now use pre-defined heights from hardware testing. For more information, see `pipette-return-tip`.
- When using the return tip function, tips are no longer added back into the tip tracker. For more information, see `pipette-return-tip`.
...
- When loading labware onto a module, you can now specify a label with the `label` parameter of
  ...
  just like you can when loading labware onto the deck with `.ProtocolContext.load_labware`.
