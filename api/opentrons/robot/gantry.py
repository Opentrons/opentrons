...
def resolve_all_coordinates(
        ...
        z=None):
    """
    Deals with situations where you want x, y, z but are not passed values
    for all three. To deal with this, it assumes the other coordinates are
    equal to their current values and returns full XYZ coords
    """
...
class InstrumentMover(object):
    ...
    def move(self, x=None, y=None, z=None):
        ...
        - Determine the target gantry position given the instrument offset for
        the gantry in x, y (since the gantry can only move in x,y)
        - Move the gantry to the target x,y
        - Determine the target mount position given the instrument offset for
        the gantry in z (since the mount can only move in z)
        - Move the mount to the target z
        """
        print('--[InstrumentMover.move] move: {}'.format((x, y, z)))

        goal_inst_pos = resolve_all_coordinates(
            self.instrument,
            self.gantry._pose_tracker,
            x,
            y,
            z)

        print(
            '--[InstrumentMover.move] absolute goal: {}'.format(goal_inst_pos)
        )
        ...
        print(
            '--[InstrumentMover.move] gantry target x,y: {}'.format(
                (goal_x, goal_y))
        )
...
class Gantry:
    ...
    def _setup_mounts(self):
        ...
        self.right_mount = Mount(
            ...
            RIGHT_MOUNT_OFFSET)
        self._pose_tracker.track_object(
            self,
            self.left_mount,
            **LEFT_MOUNT_OFFSET)
        self._pose_tracker.track_object(
    ...
    def _publish_position(self):
        new_position = resolve_all_coordinates(
            self,
            self._pose_tracker,
            **self._position_from_driver())
    ...
    def mount_instrument(self, instrument, instrument_mount):
        if instrument_mount is 'left':
            ...
            raise RuntimeError(
                'Invalid instrument mount. Valid mounts are "right" and "left"'
            )
