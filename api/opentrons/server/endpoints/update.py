...
async def _update_firmware(filename, loop):
    ...
    log.info('Running command: "{}"'.format(update_cmd))
    proc = await asyncio.create_subprocess_shell(
        update_cmd,
        stdout=asyncio.subprocess.PIPE,
        loop=loop)
    log.info('Waiting for firmware to flash...')
    rd = await proc.stdout.read()
    res = rd.decode().strip()
    await proc.wait()
    log.info('Finished flashing firmware')

    # reconnect to port
    driver._smoothie_reset()
    driver._connection.open()
    driver._setup()
    log.info('Reset Smoothieware')
...
async def update_firmware(request):
    ...
    try:
        ...
        log.info('Preparing to flash image: {}'.format(filename))
        content = data['hex'].file.read()
        log.info('Read in HEX file')

        with open(filename, 'wb') as wf:
            wf.write(content)

        log.info('Wrote HEX file to local system')
