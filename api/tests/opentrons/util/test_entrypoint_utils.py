...
def test_labware_from_paths(
    ...
    with open(os.path.join(tmp_path, "path-1", "labware1.json"), "w") as lwtemp:
        json.dump(lw1, lwtemp)
    with open(os.path.join(tmp_path, "path-1", "labware2.json"), "w") as lwtemp:
        json.dump(lw2, lwtemp)
    with open(os.path.join(tmp_path, "path-2", "labware3.json"), "w") as lwtemp:
        json.dump(lw3, lwtemp)
    with open(os.path.join(tmp_path, "path-2", "invalid.json"), "w") as lwtemp:
        lwtemp.write("asdjkashdkajvka")
    with open(os.path.join(tmp_path, "path-2", "notevenjson"), "w") as lwtemp:
        lwtemp.write("bgbbabcba")

    res = labware_from_paths(
        [
            os.path.realpath(os.path.join(tmp_path, "path-1")),
            pathlib.Path(os.path.join(tmp_path, "path-2")),
        ]
    )
...
def test_datafiles_from_paths(tmp_path: pathlib.Path) -> None:
    os.mkdir(os.path.join(tmp_path, "path-1"))
    os.mkdir(os.path.join(tmp_path, "path-2"))
