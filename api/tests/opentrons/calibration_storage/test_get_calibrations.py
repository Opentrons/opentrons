...
def _get(
    ...
    if robot_type == "ot3":
        yield importlib.import_module(
            "opentrons.calibration_storage.ot3.get"
        ), robot_type
    else:
        yield importlib.import_module(
            "opentrons.calibration_storage.ot2.get"
        ), robot_type
...
def starting_calibration_data(
    request: pytest.FixtureRequest, ot_config_tempdir: Any
) -> None:
    """
...
def test_get_deck_calibration(
    _get: Tuple[ModuleType, str], starting_calibration_data: Any, schema: ModuleType
) -> None:
    """
