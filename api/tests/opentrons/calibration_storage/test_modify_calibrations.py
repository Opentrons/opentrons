...
def _cache(request: pytest.FixtureRequest) -> Generator[Tuple[ModuleType, str], None, None]:
    ...
    robot_type = request.param
    if robot_type == "ot3":
        yield importlib.import_module("opentrons.calibration_storage.ot3.cache"), robot_type
    else:
        yield importlib.import_module("opentrons.calibration_storage.ot2.cache"), robot_type
...
@pytest.mark.parametrize(
    ...
    indirect=True)
def test_save_deck_attitude(ot_config_tempdir: Any, modify: ModuleType, _cache: ModuleType) -> None:
    """
...
@pytest.mark.parametrize(
    ...
    argvalues=[["ot2", "ot2"], ["ot3", "ot3"]],
    indirect=True)
def test_save_pipette_calibration(ot_config_tempdir: Any, modify: ModuleType, _cache: ModuleType) -> None:
    """
    Test saving pipette calibrations.
    """
    cache, robot_type = _cache
    assert cache._pipette_offset_calibrations() == {MountType.LEFT: {}, MountType.RIGHT: {}}
    if robot_type == "ot3":
    ...
    assert cache._pipette_offset_calibrations()[MountType.LEFT]['pip1'].offset == [1, 1, 1]
    assert cache._pipette_offset_calibrations()[MountType.RIGHT]['pip2'].offset == [1, 1, 1]
...
@pytest.mark.parametrize(
    ...
    argvalues=[["ot2", "ot2"], ["ot3", "ot3"]],
    indirect=True)
def test_save_tip_length_calibration(ot_config_tempdir: Any, modify: ModuleType, _cache: ModuleType) -> None:
    """
...
@pytest.mark.parametrize(
    ...
    argvalues=[["ot3", "ot3"]],
    indirect=True)
def test_save_gripper_calibration(ot_config_tempdir: Any, modify: ModuleType, _cache: ModuleType) -> None:
    """
    ...
    assert cache._gripper_offset_calibrations() != {}
    assert cache._gripper_offset_calibrations()['gripper1'].offset == [1, 1, 1]
