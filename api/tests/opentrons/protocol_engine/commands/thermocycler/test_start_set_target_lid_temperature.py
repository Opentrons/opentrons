"""Test Thermocycler start set lid temperature command implementation."""
...
from opentrons.protocol_engine.commands.thermocycler.start_set_target_lid_temperature import (
    StartSetTargetLidTemperatureImpl,
    StartSetTargetLidTemperatureResult,
)
from opentrons.protocol_engine.types.tasks import Task


async def test_start_set_target_lid_temperature(
    decoy: Decoy,
    ...
    subject = StartSetTargetLidTemperatureImpl(
        state_view=state_view, equipment=equipment, task_handler=real_task_handler
    )

    data = tc_commands.StartSetTargetLidTemperatureParams(
        moduleId="input-thermocycler-id", celsius=12.3, taskId="taskId"
    )
    tc_commands.StartSetTargetLidTemperatureResult(
    ...
    assert result == SuccessData(
        public=StartSetTargetLidTemperatureResult(
            targetLidTemperature=45.6, taskId="taskId"
        )
    )
