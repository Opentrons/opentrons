...
from opentrons.protocol_engine.commands.thermocycler.start_set_target_block_temperature import (
...
async def test_start_set_target_block_temperature(
    decoy: Decoy,
    ...
    subject = StartSetTargetBlockTemperatureImpl(
        state_view=state_view, equipment=equipment, task_handler=real_task_handler
    )

    data = tc_commands.StartSetTargetBlockTemperatureParams(
        moduleId="input-thermocycler-id",
        celsius=12.3,
        blockMaxVolumeUl=50.2,
        taskId="taskId",
    )
    tc_commands.StartSetTargetBlockTemperatureResult(
