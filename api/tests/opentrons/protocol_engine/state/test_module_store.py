...
def test_handle_shake_commands(heater_shaker_v1_def: ModuleDefinition) -> None:
    """It should update `target_speed_set` correctly."""
    load_module_cmd = commands.LoadModule.construct(  # type: ignore[call-arg]
        params=commands.LoadModuleParams(
            model=ModuleModel.HEATER_SHAKER_MODULE_V1,
            location=DeckSlotLocation(slotName=DeckSlotName.SLOT_1),
    ...
    set_shake_cmd = hs_commands.SetTargetShakeSpeed.construct(  # type: ignore[call-arg]
        params=hs_commands.SetTargetShakeSpeedParams(moduleId="module-id", rpm=500),
        result=hs_commands.SetTargetShakeSpeedResult(),
    )
    stop_cmd = hs_commands.StopShake.construct(  # type: ignore[call-arg]
        params=hs_commands.StopShakeParams(moduleId="module-id"),
        result=hs_commands.StopShakeResult(),
    )
    subject = ModuleStore()
    ...
    subject.handle_action(actions.UpdateCommandAction(command=stop_cmd))
    assert subject.state.hardware_module_by_slot == {
        DeckSlotName.SLOT_1: HardwareModule(
            serial_number="serial-number",
            definition=heater_shaker_v1_def,
