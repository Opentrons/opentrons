...
def test_load_simulating_module(
    ctx: papi.ProtocolContext, loadname: str, klass: Type[ModuleTypes], model: str
) -> None:
    ...
    assert mod._core._sync_module_hardware.model() == model  # type: ignore[union-attr]
...
def test_magdeck(
    ctx_with_magdeck: papi.ProtocolContext, mock_module_controller: mock.MagicMock
) -> None:
...
def test_thermocycler_lid_status(
    ...
    mod = cast(
        papi.ThermocyclerContext, ctx_with_thermocycler.load_module("thermocycler")
    )
...
def test_thermocycler_profile_no_temperature(
    ...
    with pytest.raises(ValueError, match="temperature must be defined"):
        mod.execute_profile(
            steps=[
                {"temperature": 10, "hold_time_seconds": 30},
                {"hold_time_seconds": 90},  # type: ignore[typeddict-item]
            ],
            repetitions=5,
        )
...
def test_thermocycler_flag_unsafe_move(
    ...
    mod = cast(
        papi.ThermocyclerContext,
        ctx_with_thermocycler.load_module("thermocycler", configuration="semi"),
    )
    labware_name = "nest_96_wellplate_100ul_pcr_full_skirt"
    tc_labware = mod.load_labware(labware_name)
    ...
    with pytest.raises(RuntimeError, match="Cannot move to labware"):
        mod._core.flag_unsafe_move(with_tc_labware, without_tc_labware)  # type: ignore[attr-defined]
    with pytest.raises(RuntimeError, match="Cannot move to labware"):
        mod._core.flag_unsafe_move(without_tc_labware, with_tc_labware)  # type: ignore[attr-defined]
...
def test_deprecated_module_load_labware_by_name(
    ...
    mod = ctx_with_tempdeck.load_module("Temperature Module", 1)
    mod.load_labware = mock.MagicMock()  # type: ignore[assignment]
...
async def test_magdeck_gen1_labware_props(ctx: papi.ProtocolContext) -> None:
    ...
    mod = cast(papi.MagneticModuleContext, ctx.load_module("magdeck", 1))
    assert mod.labware is None
...
def test_magdeck_gen2_labware_props(ctx: papi.ProtocolContext) -> None:
    mod = cast(papi.MagneticModuleContext, ctx.load_module("magnetic module gen2", 1))
