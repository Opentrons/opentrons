...
class InstrumentContextImplementation(AbstractInstrument):
    ...
    def move_and_aspirate(self,
        ...
        # FIX BEFORE MERGE: I don't understand what this check is for.
        if self.get_current_volume() == 0:
            # Make sure we're at the top of the labware and clear of any
            # liquid to prepare the pipette for aspiration
            if (not self._plunger_is_prepared_to_aspirate) or force_move_to_top:
                if location.labware.is_well:
                    self.move_to(
                        location.labware.as_well().top(),
                        force_direct=False,
                    ...
                    self._log.warning(
                        ...
                        "blow_out.")
                self._prepare_plunger_to_aspirate()
            self.move_to(
                location,
                force_direct=False,
                minimum_z_height=None,
        ...
        elif location != self._ctx.location_cache:
            self.move_to(
                location,
                force_direct=False,
                minimum_z_height=None,
