...
class DeckStart(BaseModel):
    ...
    force: bool = Field(False, description="Set to true to refresh the session")


class PipetteDeckCalibration(BaseModel):
    """Details of the pipette the system has selected for use in deck calibration"""
    mount: Mount
    model: str = Field(..., description="The model of the pipette attached in this mount")


class DeckStartResponse(BaseModel):
    """None"""
    token: UUID = Field(..., description="The token to send along in future deck calibration actions")
...
class DeckCalibrationPoint(str, Enum):
    """
    The name of a point relative to deck calibration. The number points are calibration crosses ("1" in slot 1, "2"
    in slot 3, "3" in slot 7); "safeZ" is a safe height above the deck, "attachTip" is a good place to go for the
...
class DeckCalibrationDispatch(BaseModel):
    ...
    tipLength: typing.Optional[float] = \
        Field(None, description="The length of the tip you are prompting the user to attach. Required for command \"attach tip\"")
    point: typing.Optional[DeckCalibrationPoint] = \
        Field(None, description="Required for commands \"save xy\" and \"move\"")
    axis: typing.Optional[JogAxis] = \
        Field(None, description="The axis to jog. Required for \"jog\" command.")
    direction: typing.Optional[JogDirection] =\
        Field(None, description="The direction to jog. Required for \"jog\" command.")
    step: typing.Optional[float] =\
        Field(None, description="The distance to jog. Required for \"jog\" command.")
