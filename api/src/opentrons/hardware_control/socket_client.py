...
class JsonRpcAdapter(HardwareAPILike):
    ...
    def __init__(self,
        ...
        def sync_only(p):
            return inspect.isfunction(p) and not inspect.iscoroutinefunction(p)

        def async_only(p):
            return inspect.iscoroutinefunction(p)

        self._coromap = generate_proxies(API, self, async_only)
        self._funcmap = generate_proxies(API, self, sync_only)
    ...
    def __getattr__(self, attr_name):
        ...
        if attr_name in self._propmap:
            ...
            try:
                return self._loop.run_until_complete(
                    self._client.request(getter)).data.result
            except RuntimeError:
                raise RuntimeError(
                    f"Can't get remote properties from an async context;"
                    f" try calling {getter}")
        ...
        elif attr_name in self._funcmap:
            dummy_meth, dummy_sig = self._funcmap[attr_name]

            @functools.wraps(dummy_meth)
            def _wrapper(*args, **kwargs):
                bound = dummy_sig.bind(*args, **kwargs)
                res = self._loop.run_until_complete(
                    self._client.request(attr_name,
                                         **bound.arguments))
                return res.data.result
        else:
