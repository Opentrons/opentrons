...
class OT3API(
    ...
    async def get_tip_status(self, mount: Union[top_types.Mount, OT3Mount]) -> Dict[Axis, bool]:
        checked_mount = OT3Mount.from_mount(mount)
        res = await self._backend.get_tip_present_state(checked_mount)
        return res
    ...
    async def _force_pick_up_tip(
        ...
        for press in pipette_spec.presses:
            ...
            await self._update_position_estimation(axes=[Axis.by_mount(mount)])
