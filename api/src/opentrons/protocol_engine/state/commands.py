...
@dataclass
class _CommandAnnotation:
    """Holds a command annotation and information about its status."""
...
class CommandState:
    ...
    command_annotations: Dict[str, _CommandAnnotation]
    """A map of annotation IDs to command annotations that have been generated by the protocol."""
...
class CommandStore(HasState[CommandState], HandlesActions):
    ...
    def handle_action(self, action: Action) -> None:  # noqa: C901
        ...
        match action:
            ...
            case CreateUserCommandAnnotation():
                self._handle_create_user_command_annotation(action)
            case CloseUserCommandAnnotation():
                self._handle_close_user_command_annotation(action)
            case _:
                pass
    ...
    def _handle_create_user_command_annotation(
        ...
        self._state.command_annotations[action.annotation_id] = _CommandAnnotation(
            annotation=annotation, enabled=True
        )

    def _handle_close_user_command_annotation(
        self, action: CloseUserCommandAnnotation
    ) -> None:
        try:
