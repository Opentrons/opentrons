...
class MotionView:
    ...
    def get_edge_waypoints(
        ...
        """Get the movement waypoints from origin (center of well) to its four edges."""
        edge_points = self._geometry.get_well_edges(
            labware_id, well_name, well_location
        )
        ...
        # TODO if we are applying this only to the current well, then logic from
        #  get_movement_waypoints can be short-circuited, and movement type will be
        #  direct and min_z will be from labware id
        min_travel_z = self._geometry.get_labware_highest_z(labware_id)
        try:
            waypoints = []
            for dest in edge_points:
                waypoints.extend(
                    get_waypoints(
                ...
                origin = dest  # NOTE: This is most likely right
                origin_cp = dest_cp  # TODO but is this?
            return waypoints
        except MotionPlanningError as error:
            raise errors.FailedToPlanMoveError(str(error))
