...
from ..errors import HardwareNotSupportedError, InvalidPipetteSpecificationsError
...
class LoadPipetteImplementation(
    ...
    def __init__(self, equipment: EquipmentHandler, state_view: StateView, **kwargs: object) -> None:
    ...
    async def execute(
        ...
        pipette_generation = convert_to_pipette_name_type(
            params.pipetteName.value).pipette_generation
        robot_type = RobotTypeEnum.robot_literal_to_enum(self._state_view.config.robot_type)
        if (
                (robot_type == RobotTypeEnum.FLEX
                 and pipette_generation != PipetteGenerationType.FLEX)
        ) or (
                (robot_type == RobotTypeEnum.OT2
                 and pipette_generation not in [PipetteGenerationType.GEN1,
                                                PipetteGenerationType.GEN2])
        ):
            raise InvalidPipetteSpecificationsError(
