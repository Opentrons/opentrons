...
class LoadLabwareImplementation(
    ...
    async def execute(
        ...
        if isinstance(params.location, AddressableAreaLocation):
            ...
            if not (fixture_validation.is_deck_slot(params.location.addressableAreaName) or fixture_validation.is_abs_reader_lid_dock(params.location.addressableAreaName)):
                raise LabwareIsNotAllowedInLocationError(
