...
class RunStoreProvider:
    ...
    async def insert_command(self, command_index: int, command: Command) -> None:
        ...
        if self._run_id and self._store_insert_command:
            ...
            self._all_commands_by_id[command.id] = CommandEntry(command=command, index=command_index)
    ...
    async def insert_command_annotation(self, command_annotation: CommandAnnotation) -> None:
        ...
        if self._run_id and self._store_insert_command_annotation:
            ...
            self._all_command_annotations_by_id[command_annotation.id] = command_annotation
    ...
    async def get_command(self, command_id: str) -> Command:
        ...
        if self._run_id and self._store_get_command:
            ...
            try:
                return self._all_commands_by_id[command_id]
            except KeyError:
    ...
    async def get_commands_slice(self, cursor: int, length: int, include_fixit_commands: bool) -> CommandSlice:
        """get commands slice"""
        if self._run_id and self._store_get_commands_slice:
            return await self._store_get_commands_slice(self._run_id, cursor, length, include_fixit_commands)
        else:
            command_ids = []
            for command_id, command_entry in self._all_commands_by_id.items():
                if include_fixit_commands:
                    command_ids.append(command_id)
                elif not include_fixit_commands and command_entry.command.intent != CommandIntent.FIXIT:
                    command_ids.append(command_id)
            ...
            command_list = [self._all_commands_by_id[command].command for command in commands]
            return CommandSlice(
    ...
    async def get_command_by_index(self, index: int) -> CommandEntry:
        """Get a command by a given index."""
        if self._run_id and self._store_get_commands_slice:
            pass
        # CASEY NOTE: TO DO
        else:
            for command_entry in self._all_commands_by_id.values():
                if command_entry.index == index:
                    return command_entry
            raise CommandDoesNotExistError(f"Command at index {index} does not exist")
