...
class TLSCAManager:
    ...
    def get_current_certificate_der_bytes(self) -> bytes:
        """Get the currently-valid CA certificate in DER-encoded form. This is not HTTP safe."""
        return cryptography_utils.get_cert_bytes_der(self._current_ca.cert)

    def get_next_certificate_der_bytes(self) -> bytes | None:
        """Get the next valid CA certificate, if there is one, in DER-encoded form. This is not HTTP safe."""
        return (
