...
def _parse_args() -> _ParsedArgs:
    parser = argparse.ArgumentParser(description="Opentrons key server")
    parser.add_argument(
        "-p",
        "--port",
        dest="port",
...
if __name__ == "__main__":
    ...
    if args.uds is not None:
        _log.info(f"Starting key server on {args.uds}.")
    else:
        _log.info(f"Starting key server on {args.host}:{args.port}.")
