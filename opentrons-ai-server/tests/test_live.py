...
def test_get_bad_endpoint_with_good_auth(client: Client) -> None:
    ...
    assert response.status_code == 405, "nonexistent endpoint with good auth should return HTTP 405"
...
def test_get_bad_endpoint_with_bad_auth(client: Client) -> None:
    ...
    assert response.status_code == 405, "nonexistent endpoint with bad auth should return HTTP 405"
