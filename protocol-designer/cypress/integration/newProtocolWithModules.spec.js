...
describe('Protocols with Modules', () => {
  ...
  describe('build a new protocol with mag deck', () => {
    it('sets up pipettes, tips, and modules', () => {
      ...
      cy.contains('Left Pipette').next().contains('None').click()
      cy.contains('Left Pipette').next().contains('P300').click()
      cy.contains('Right Pipette').next().contains('None').click()
      ...
      // Edit modules
      cy.get(modulesSection).contains('Edit').click()
      cy.get(editModuleModal).within(() => {
        cy.contains('GEN1')
        cy.get('select[disabled]').contains('Slot 1').should('exist')
    ...
    it('design protocol with mag and temp decks', () => {
      ...
      cy.get(designPageModal).within(() => {
        ...
        cy.get('h3').contains('Tip Rack').click()
      ...
      cy.get(designPageModal).within(() => {
        ...
        cy.contains('Aluminum Block').should('exist').click()
        cy.get('ul').within(() => {
      ...
      // Add first liquid
      cy.get(slotOne).contains('Name & Liquids').click()
      ...
      // Add second liquid
      cy.get(slotThree).contains('Name & Liquids').click()
      rowLetters.forEach(element => {
        cy.get(`[data-wellname="${element}1"]`).click()
      })
      ...
      cy.get('[data-test="StepItem_1"]').contains('temperature').should('exist')
      cy.get(designPageModal).within(() => {
        ...
        cy.contains('Change to temperature').click()
        cy.get(defaultTextInput).focus().blur()
        cy.get(alertTitleContainer).contains('error').should('exist')
        ...
        cy.get('button[disabled]')
          .contains('save', { matchCase: false })
          .should('exist')
        cy.get(defaultTextInput).clear().type('3').blur()
        cy.get('button[disabled]')
          .contains('save', { matchCase: false })
          .should('exist')
        cy.contains('Min is 4')
        cy.get(defaultTextInput).clear().type('100').blur()
        cy.get('button[disabled]')
          .contains('save', { matchCase: false })
          .should('exist')
        cy.contains('Max is 95')
        cy.get(defaultTextInput).clear().type('37').blur()
        cy.get(alertTitleContainer).should('not.exist')
        cy.get('button').contains('save', { matchCase: false }).click()
        cy.get('button').contains('pause later').click()
      })
      ...
      cy.addStep('pause')
      cy.get('[data-test="StepItem_2"]').contains('pause').should('exist')
      cy.get(designPageModal).within(() => {
        ...
        cy.get('button[disabled]')
          .contains('save', { matchCase: false })
          .should('exist')
        cy.get(defaultTextInput).type('37').blur()
        cy.get(alertTitleContainer).should('not.exist')
      ...
      cy.addStep('transfer')
      cy.get('[data-test="StepItem_3"]').contains('transfer').should('exist')
      cy.get(designPageModal).within(() => {
      ...
      cy.addStep('magnet')
      cy.get('[data-test="StepItem_4"]').contains('magnet').should('exist')
      cy.get(designPageModal).within(() => {
        cy.contains('Module', { matchCase: false }).should('exist')
        cy.contains('Magnet', { matchCase: false }).should('exist')
        cy.get(defaultTextInput).clear().blur()
        cy.get(alertTitleContainer).contains('error').should('exist')
        cy.get('button[disabled]')
          .contains('save', { matchCase: false })
          .should('exist')
        cy.get(defaultTextInput).type('16').blur()
        cy.get(alertTitleContainer).should('not.exist')
        cy.get('button[disabled]').should('not.exist')
        cy.get('button').contains('save', { matchCase: false }).click()
      })
      ...
      cy.get('[data-test="StepItem_5"]').contains('pause').should('exist')
      ...
      cy.get('[data-test="StepItem_7"]').contains('magnet').should('exist')

      // Add Temperature Step w/ Pause
      cy.addStep('temperature')
      cy.get(designPageModal).within(() => {
        cy.contains('Change to temperature').click()
        cy.get(defaultTextInput).clear().type('4').blur()
        cy.get('button').contains('save', { matchCase: false }).click()
        cy.get('button').contains('Pause protocol now').click()
      })
      cy.get('[data-test="StepItem_8"]').contains('Temperature').should('exist')
    ...
    it('deletes and replaces modules', () => {
      ...
      cy.openDesignPage()
      cy.get('[data-test="StepItem_4"]').children().first().click()
      cy.get(designPageModal).within(() => {
        cy.get(alertTitleContainer).contains('error').should('exist')
        cy.get('button[disabled]')
          .contains('save', { matchCase: false })
      ...
      cy.get(editModuleModal).within(() => {
        cy.get('select[disabled]').contains('Slot 3').should('exist')
        cy.get(moduleModelDropdown).select('GEN2')
        cy.get('select[disabled]').should('not.exist')
        cy.get('button').contains('save', { matchCase: false }).click()
      })
