...
const supportedPipetteNames = ['p10_single', 'p10_multi', 'p50_single', 'p50_multi', 'p300_single', 'p300_multi', 'p1000_single'];
// TODO: should a version of pipetteOptions be moved to shared-data,
// and used for both PD and Run App?
export const pipetteOptions: Options = supportedPipetteNames.map((name: string) => {
...
export function getPipetteCapacity(pipetteEntity: PipetteEntity): number {
  ...
  assert(false, `Expected spec and tiprack def for pipette ${pipetteEntity ? pipetteEntity.id : '???'}`);
  return NaN;
}
export function getMinPipetteVolume(pipetteEntity: PipetteEntity): number {
  ...
  assert(false, `Expected spec for pipette ${pipetteEntity ? pipetteEntity.id : '???'}`);
  return NaN;
}