...
import type { ChangeTipOptions, LabwareEntity, PipetteEntity } from "@opentrons/step-generation";
...
export type StepType = "moveLiquid" | "mix" | "pause" | "manualIntervention" | "magnet" | "temperature" | "thermocycler";
...
export type HydratedMoveLiquidFormDataLegacy = AnnotationFields & $PropertyType<HydratedMoveLiquidFormData, "fields"> & {
  id: string;
  stepType: "moveLiquid";
};
...
export function getIsTouchTipField(fieldName: StepFieldName): boolean {
  const touchTipFields = ['aspirate_touchTip_mmFromBottom', 'dispense_touchTip_mmFromBottom', 'mix_touchTip_mmFromBottom'];
  return touchTipFields.includes(fieldName);
}
export function getIsDelayPositionField(fieldName: string): boolean {
  const delayPositionFields = ['aspirate_delay_mmFromBottom', 'dispense_delay_mmFromBottom'];
