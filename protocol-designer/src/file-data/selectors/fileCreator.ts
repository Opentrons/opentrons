...
import { DEFAULT_MM_FROM_BOTTOM_ASPIRATE, DEFAULT_MM_FROM_BOTTOM_DISPENSE, DEFAULT_MM_TOUCH_TIP_OFFSET_FROM_TOP, DEFAULT_MM_BLOWOUT_OFFSET_FROM_TOP } from "../../constants";
import type { ModuleEntity, PipetteEntity, LabwareEntities, PipetteEntities, Timeline } from "@opentrons/step-generation";
import type { FilePipette, FileLabware, FileModule } from "@opentrons/shared-data/protocol/flowTypes/schemaV4";
...
export const getLabwareDefinitionsInUse = (labware: LabwareEntities, pipettes: PipetteEntities, allLabwareDefsByURI: LabwareDefByDefURI): LabwareDefByDefURI => {
  const labwareDefURIsOnDeck: Array<string> = Object.keys(labware).map((labwareId: string) => labware[labwareId].labwareDefURI);
  const tiprackDefURIsInUse: Array<string> = Object.keys(pipettes).map(id => pipettes[id]).map((pipetteEntity: PipetteEntity) => pipetteEntity.tiprackDefURI);
  const labwareDefURIsInUse = uniq([...tiprackDefURIsInUse, ...labwareDefURIsOnDeck]);
  return labwareDefURIsInUse.reduce<LabwareDefByDefURI>((acc, labwareDefURI: string) => ({ ...acc,
...
const _isV3Command = (command: Command): boolean => command.command === 'aspirate' || command.command === 'dispense' || command.command === 'blowout' || command.command === 'touchTip' || command.command === 'pickUpTip' || command.command === 'dropTip' || command.command === 'moveToSlot' || command.command === 'delay';
...
export const getRobotStateTimelineWithoutAirGapDispenseCommand: Selector<Timeline> = createSelector(getRobotStateTimeline, robotStateTimeline => {
  const timeline = robotStateTimeline.timeline.map(frame => ({ ...frame,
...
export const getRequiresAtLeastV4: Selector<boolean> = createSelector(getRobotStateTimelineWithoutAirGapDispenseCommand, stepFormSelectors.getModuleEntities, (robotStateTimeline, moduleEntities) => {
  const noModules = isEmpty(moduleEntities);
  const hasOnlyV3Commands = robotStateTimeline.timeline.every(timelineFrame => timelineFrame.commands.every(command => _isV3Command(command)));
...
const _requiresV5 = (command: Command): boolean => command.command === 'moveToWell' || command.command === 'airGap';

export const getRequiresAtLeastV5: Selector<boolean> = createSelector(getRobotStateTimelineWithoutAirGapDispenseCommand, robotStateTimeline => {
  return robotStateTimeline.timeline.some(timelineFrame => timelineFrame.commands.some(command => _requiresV5(command)));
});
export const getExportedFileSchemaVersion: Selector<number> = createSelector(getRequiresAtLeastV4, getRequiresAtLeastV5, (requiresV4, requiresV5) => {
  if (requiresV5) {
    return 5;
  } else if (requiresV4) {
    return 4;
...
export const createFile: Selector<PDProtocolFile> = createSelector(getFileMetadata, getInitialRobotState, getRobotStateTimelineWithoutAirGapDispenseCommand, dismissSelectors.getAllDismissedWarnings, ingredSelectors.getLiquidGroupsById, ingredSelectors.getLiquidsByLabwareId, stepFormSelectors.getSavedStepForms, stepFormSelectors.getOrderedStepIds, stepFormSelectors.getLabwareEntities, stepFormSelectors.getModuleEntities, stepFormSelectors.getPipetteEntities, uiLabwareSelectors.getLabwareNicknamesById, labwareDefSelectors.getLabwareDefsByURI, getRequiresAtLeastV4, getRequiresAtLeastV5, (fileMetadata, initialRobotState, robotStateTimeline, dismissedWarnings, ingredients, ingredLocations, savedStepForms, orderedStepIds, labwareEntities, moduleEntities, pipetteEntities, labwareNicknamesById, labwareDefsByURI, requiresAtLeastV4Protocol, requiresAtLeastV5Protocol) => {
  const {
    author,
    description,
    created
  ...
  const pipettes = mapValues(initialRobotState.pipettes, (pipette: $Values<typeof initialRobotState.pipettes>, pipetteId: string): FilePipette => ({
    mount: pipette.mount,
    name: pipetteEntities[pipetteId].name
  }));
  const labware: Record<string, FileLabware> = mapValues(initialRobotState.labware, (l: $Values<typeof initialRobotState.labware>, labwareId: string): FileLabware => ({
    slot: l.slot,
    displayName: labwareNicknamesById[labwareId],
    definitionId: labwareEntities[labwareId].labwareDefURI
  }));
  const modules: Record<string, FileModule> = mapValues(moduleEntities, (moduleEntity: ModuleEntity, moduleId: string): FileModule => ({
  ...
  const labwareDefinitions = getLabwareDefinitionsInUse(labwareEntities, pipetteEntities, labwareDefsByURI);
  const commands: Array<Command> = flatMap(robotStateTimeline.timeline, timelineFrame => timelineFrame.commands);
  const protocolFile = {
    metadata: {
      protocolName: name,
      author,
    ...
    designerApplication: {
      ...
      data: {
        ...
        pipetteTiprackAssignments: mapValues(pipetteEntities, (p: $Values<typeof pipetteEntities>): string | null | undefined => p.tiprackDefURI),
        dismissedWarnings,
        ingredients,
        ingredLocations,
        savedStepForms,
