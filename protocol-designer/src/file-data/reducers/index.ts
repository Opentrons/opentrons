...
export const computedRobotStateTimeline: Reducer<Timeline, any> = handleActions({
  COMPUTE_ROBOT_STATE_TIMELINE_SUCCESS: (state, action: ComputeRobotStateTimelineSuccessAction) => action.payload.standardTimeline
}, {
  timeline: []
});
export const computedSubsteps: Reducer<Substeps, any> = handleActions({
  COMPUTE_ROBOT_STATE_TIMELINE_SUCCESS: (state, action: ComputeRobotStateTimelineSuccessAction) => action.payload.substeps
}, {});
const defaultFields = {
  protocolName: '',
  author: '',
...
const updateMetadataFields = (state: FileMetadataFields, action: LoadFileAction): FileMetadataFields => {
  const {
    file
  } = action.payload;
...
function newProtocolMetadata(state: FileMetadataFields, action: {
...
const fileMetadata = handleActions({
  ...
  SAVE_FILE_METADATA: (state: FileMetadataFields, action: SaveFileMetadataAction): FileMetadataFields => ({ ...state,
    ...action.payload
  }),
