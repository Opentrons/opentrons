...
import { createFile, getRequiresAtLeastV5, getLabwareDefinitionsInUse } from "../selectors";
import { fileMetadata, dismissedWarnings, ingredients, ingredLocations, labwareEntities, labwareNicknamesById, labwareDefsByURI, pipetteEntities } from "../__fixtures__/createFile/commonFields";
...
describe('createFile selector', () => {
  it('should return a schema-valid JSON V3 protocol, if the protocol has NO modules', () => {
    ...
    const result = createFile.resultFunc(fileMetadata, noModules.initialRobotState, noModules.robotStateTimeline, dismissedWarnings, ingredients, ingredLocations, noModules.savedStepForms, noModules.orderedStepIds, labwareEntities, noModules.moduleEntities, pipetteEntities, labwareNicknamesById, labwareDefsByURI, false, // isV4Protocol
    false // requiresV5
    );
  ...
  it('should return a schema-valid JSON V4 protocol, if the protocol does have modules', () => {
    // $FlowFixMe TODO(IL, 2020-02-25): Flow doesn't have type for resultFunc
    const result = createFile.resultFunc(fileMetadata, engageMagnet.initialRobotState, engageMagnet.robotStateTimeline, dismissedWarnings, ingredients, ingredLocations, engageMagnet.savedStepForms, engageMagnet.orderedStepIds, labwareEntities, engageMagnet.moduleEntities, pipetteEntities, labwareNicknamesById, labwareDefsByURI, true, // isV4Protocol
    false // requiresV5
    );
  ...
  it('should return a schema-valid JSON V5 protocol, if getRequiresAtLeastV5 returns true', () => {
    // $FlowFixMe TODO(IL, 2020-02-25): Flow doesn't have type for resultFunc
    const result = createFile.resultFunc(fileMetadata, v5Fixture.initialRobotState, v5Fixture.robotStateTimeline, dismissedWarnings, ingredients, ingredLocations, v5Fixture.savedStepForms, v5Fixture.orderedStepIds, labwareEntities, v5Fixture.moduleEntities, pipetteEntities, labwareNicknamesById, labwareDefsByURI, true, // isV4Protocol
    true // requiresV5
    );
...
describe('getRequiresAtLeastV5', () => {
  ...
  it('should return false if protocol has no airGap and no moveToWell', () => {
    ...
    expect( // $FlowFixMe TODO(IL, 2020-02-25): Flow doesn't have type for resultFunc
    getRequiresAtLeastV5.resultFunc(engageMagnet.robotStateTimeline)).toBe(false);
  });
});
describe('getLabwareDefinitionsInUse util', () => {
  it('should exclude definitions that are neither on the deck nor assigned to a pipette', () => {
    ...
    const result = getLabwareDefinitionsInUse(labwareEntities, pipetteEntities, allLabwareDefsByURI);
