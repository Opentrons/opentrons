...
import { consolidate, distribute, transfer, mix, curryCommandCreator } from "@opentrons/step-generation";
...
import type { CurriedCommandCreator, InvariantContext, RobotState, ConsolidateArgs, DistributeArgs, MixArgs, TransferArgs } from "@opentrons/step-generation";
import type { StepIdType } from "../form-types";
import type { NamedIngred, StepArgsAndErrors, StepItemSourceDestRow, SourceDestSubstepItem, SubstepItemData, SubstepTimelineFrame, LabwareNamesByModuleId } from "./types";
export type GetIngreds = (labware: string, well: string) => Array<NamedIngred>;
type TransferLikeArgs = ConsolidateArgs | DistributeArgs | TransferArgs | MixArgs;

function getCommandCreatorForTransferlikeSubsteps(stepArgs: TransferLikeArgs): CurriedCommandCreator | null {
  ...
  if (stepArgs.commandCreatorFnName === 'transfer') {
    const commandCallArgs = { ...stepArgs,
    ...
    console.warn(`getStepArgsForSubsteps got unsupported stepType "${stepArgs.commandCreatorFnName}"`);
    return null;
  }
}
...
export const mergeSubstepRowsSingleChannel = (args: {
  ...
  return steplistUtils.mergeWhen(substepRows, (currentRow, nextRow) => // NOTE: if aspirate then dispense rows are adjacent, collapse them into one row
  currentRow.source && nextRow.dest, (currentRow, nextRow) => ({ ...currentRow,
    source: {
...
export const mergeSubstepRowsMultiChannel = (args: {
  ...
  return steplistUtils.mergeWhen(substepRows, (currentMultiRow: SubstepTimelineFrame, nextMultiRow: SubstepTimelineFrame) => {
    // aspirate then dispense multirows adjacent
    // (inferring from first channel row in each multirow)
    return currentMultiRow && currentMultiRow.source && nextMultiRow && nextMultiRow.dest;
  ...
  }, currentMultiRow => range(channels).map(channelIndex => {
...
function transferLikeSubsteps(args: {
  ...
  const initialRobotState = { ...args.robotState,
    tipState
  };
  const {
    pipette: pipetteId
  ...
  if (!pipetteSpec) {
    assert(false, `Pipette "${pipetteId}" does not exist, step ${stepId} can't determine channels`);
    return null;
  }
  ...
  // Call appropriate command creator with the validateForm fields.
  // Disable any mix args so those aspirate/dispenses don't show up in substeps
  const substepCommandCreator = getCommandCreatorForTransferlikeSubsteps(stepArgs);
  ...
  if (pipetteSpec.channels > 1) {
    const substepRows: Array<SubstepTimelineFrame> = substepTimeline(substepCommandCreator, invariantContext, initialRobotState, pipetteSpec.channels);
    const mergedMultiRows: Array<Array<StepItemSourceDestRow>> = mergeSubstepRowsMultiChannel({
      substepRows,
    ...
    // single channel
    const substepRows = substepTimeline(substepCommandCreator, invariantContext, initialRobotState, 1);
...
export function generateSubstepItem(stepArgsAndErrors: StepArgsAndErrors | null | undefined, invariantContext: InvariantContext, robotState: RobotState | null | undefined, stepId: string, labwareNamesByModuleId: LabwareNamesByModuleId): SubstepItemData | null | undefined {
  if (!robotState) {
    console.info(`No robot state, could not generate substeps for step ${stepId}.` + `There was probably an upstream error.`);
    return null;
  }
  ...
  if (stepArgs.commandCreatorFnName === 'consolidate' || stepArgs.commandCreatorFnName === 'distribute' || stepArgs.commandCreatorFnName === 'transfer' || stepArgs.commandCreatorFnName === 'mix') {
  ...
  const labwareNames = stepArgs.module ? labwareNamesByModuleId[stepArgs.module] : null;

  if (stepArgs.commandCreatorFnName === 'disengageMagnet' || stepArgs.commandCreatorFnName === 'engageMagnet') {
    return {
      substepType: 'magnet',
  ...
  if (stepArgs.commandCreatorFnName === 'setTemperature' || stepArgs.commandCreatorFnName === 'deactivateTemperature') {
    const temperature = stepArgs.commandCreatorFnName === 'setTemperature' ? stepArgs.targetTemperature : null;
    return {
  ...
  console.warn("generateSubsteps doesn't support commandCreatorFnName: ", stepArgs.commandCreatorFnName, stepId);
  return null;
}