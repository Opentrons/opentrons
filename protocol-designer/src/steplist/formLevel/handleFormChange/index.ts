...
export function handleFormChange(patch: FormPatch, rawForm: FormData | null | undefined, pipetteEntities: PipetteEntities, labwareEntities: LabwareEntities): FormPatch {
  if (rawForm == null) {
    return patch;
  }
  ...
  if (rawForm.stepType === 'temperature') {
    const dependentFieldsPatch = dependentFieldsUpdateTemperature(patch, rawForm);
    return { ...patch,
      ...dependentFieldsPatch
    };
  }
  ...
  if (rawForm.stepType === 'thermocycler') {
    const dependentFieldsPatch = dependentFieldsUpdateThermocycler(patch, rawForm);
    return { ...patch,
      ...dependentFieldsPatch
    };
  }
