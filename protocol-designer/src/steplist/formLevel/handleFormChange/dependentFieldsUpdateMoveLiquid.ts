...
import { chainPatchUpdaters, fieldHasChanged, getChannels, getDefaultWells, getAllWellsFromPrimaryWells, getMaxDisposalVolumeForMultidispense, volumeInCapacityForMulti, DISPOSAL_VOL_DIGITS } from "./utils";
import type { LabwareEntities, PipetteEntities } from "@opentrons/step-generation";
import type { FormData, StepFieldName } from "../../../form-types";
import type { FormPatch } from "../../actions/types";
import { getMinPipetteVolume, getPipetteCapacity } from "../../../pipettes/pipetteData";

// TODO: Ian 2019-02-21 import this from a more central place - see #2926
const getDefaultFields = (...fields: Array<StepFieldName>): FormPatch => pick(getDefaultsForStepType('moveLiquid'), fields);
...
const wellRatioUpdatesMap = [{
  ...
  prevValue: 'n:n',
  nextValue: 'many:1',
  dependentFields: [// no updates, all possible values are OK
  ]
}, {
  prevValue: '1:many',
  nextValue: 'n:n',
...
export function updatePatchPathField(patch: FormPatch, rawForm: FormData, pipetteEntities: PipetteEntities): FormPatch {
  const {
    id,
    stepType,
    ...stepData
  } = rawForm;
  const appliedPatch = { ...(stepData as FormPatch),
    ...patch
  };
  const {
    path,
  ...
  if (appliedPatch.volume && typeof appliedPatch.pipette === 'string' && appliedPatch.pipette in pipetteEntities) {
...
const updatePatchOnLabwareChange = (patch: FormPatch, rawForm: FormData, labwareEntities: LabwareEntities, pipetteEntities: PipetteEntities): FormPatch => {
  ...
  const sourceLabwarePatch: FormPatch = sourceLabwareChanged ? { ...getDefaultFields('aspirate_mmFromBottom', 'aspirate_touchTip_mmFromBottom'),
  ...
  const destLabwarePatch: FormPatch = destLabwareChanged ? { ...getDefaultFields('dispense_mmFromBottom', 'dispense_touchTip_mmFromBottom'),
  ...
  return { ...sourceLabwarePatch,
    ...destLabwarePatch
  };
};

const updatePatchOnPipetteChange = (patch: FormPatch, rawForm: FormData, pipetteEntities: PipetteEntities) => {
  ...
  if (fieldHasChanged(rawForm, patch, 'pipette')) {
    ...
    return { ...patch,
      ...getDefaultFields('aspirate_flowRate', 'dispense_flowRate', 'aspirate_mix_volume', 'dispense_mix_volume', 'disposalVolume_volume', 'aspirate_mmFromBottom', 'dispense_mmFromBottom'),
...
const getClearedDisposalVolumeFields = () => getDefaultFields('disposalVolume_volume', 'disposalVolume_checkbox');

const clampAspirateAirGapVolume = (patch: FormPatch, rawForm: FormData, pipetteEntities: PipetteEntities): FormPatch => {
  const patchedAspirateAirgapVolume = patch.aspirate_airGap_volume ?? rawForm?.aspirate_airGap_volume;
  const pipetteId = patch.pipette ?? rawForm.pipette;

  if (patchedAspirateAirgapVolume && typeof pipetteId === 'string' && pipetteId in pipetteEntities) {
    ...
    const clampedAirGapVolume = clamp(Number(patchedAspirateAirgapVolume), minAirGapVolume, maxAirGapVolume);
    if (clampedAirGapVolume === Number(patchedAspirateAirgapVolume)) return patch;
    return { ...patch,
...
const clampDispenseAirGapVolume = (patch: FormPatch, rawForm: FormData, pipetteEntities: PipetteEntities): FormPatch => {
  const {
    id,
    stepType,
    ...stepData
  ...
  // $FlowFixMe(mc, 2020-02-19): appliedPatch.pipette is type ?mixed. Address in #3161
  const pipetteId: string = appliedPatch.pipette;
  const disposalVolume = appliedPatch.disposalVolume_checkbox ? Number(appliedPatch.disposalVolume_volume) || 0 : 0;
  const transferVolume = Number(appliedPatch.volume);
  const dispenseAirGapVolume = Number(appliedPatch.dispense_airGap_volume);

  if (appliedPatch.dispense_airGap_volume && typeof pipetteId === 'string' && pipetteId in pipetteEntities) {
    const pipetteEntity = pipetteEntities[pipetteId];
    const capacity = getPipetteCapacity(pipetteEntity);
    const minAirGapVolume = 0; // NOTE: a form level warning will occur if the air gap volume is below the pipette min volume

    const maxAirGapVolume = appliedPatch.path === 'multiDispense' ? capacity - disposalVolume - transferVolume : capacity;
    const clampedAirGapVolume = clamp(dispenseAirGapVolume, minAirGapVolume, maxAirGapVolume);
...
const updatePatchDisposalVolumeFields = (patch: FormPatch, rawForm: FormData, pipetteEntities: PipetteEntities) => {
  const {
    id,
    stepType,
    ...stepData
  ...
  const pathChangedFromMultiDispense = patch.path && patch.path !== 'multiDispense' && rawForm.path === 'multiDispense';
  ...
  const shouldReinitializeDisposalVolume = patch.path === 'multiDispense' && rawForm.path !== 'multiDispense' || patch.pipette && patch.pipette !== rawForm.pipette || patch.disposalVolume_checkbox;

  if (shouldReinitializeDisposalVolume && typeof appliedPatch.pipette === 'string') {
...
const clampDisposalVolume = (patch: FormPatch, rawForm: FormData, pipetteEntities: PipetteEntities) => {
  const {
    id,
    stepType,
    ...stepData
  ...
  const maxDisposalVolume = getMaxDisposalVolumeForMultidispense( // $FlowFixMe(IL, 2021-03-22): appliedPath isn't well-typed, address in #3161
  appliedPatch, pipetteEntities);
  ...
  const nextDisposalVolume = clamp(round(candidateDispVolNum, DISPOSAL_VOL_DIGITS), 0, maxDisposalVolume);
  ...
  return rawForm.path === 'multiDispense' ? { ...patch,
...
const updatePatchOnPipetteChannelChange = (patch: FormPatch, rawForm: FormData, labwareEntities: LabwareEntities, pipetteEntities: PipetteEntities) => {
  ...
  const nextChannels = typeof patch.pipette === 'string' ? getChannels(patch.pipette, pipetteEntities) : null;
  const {
    id,
    stepType,
    ...stepData
  ...
  if (patch.pipette === null || singleToMulti) {
    ...
    update = {
      aspirate_wells: getAllWellsFromPrimaryWells( // $FlowFixMe(mc, 2020-02-21): appliedPatch.aspirate_wells is type ?mixed. Address in #3161
      appliedPatch.aspirate_wells, // $FlowFixMe(mc, 2020-04-29): sourceLabwareDef is not typed properly. Address in #3161
      sourceLabwareDef),
      dispense_wells: getAllWellsFromPrimaryWells( // $FlowFixMe(mc, 2020-02-21): appliedPatch.dispense_wells is type ?mixed. Address in #3161
...
function updatePatchOnWellRatioChange(patch: FormPatch, rawForm: FormData): FormPatch {
  ...
  if (!nextWellRatio || !prevWellRatio) {
    ...
    const resetPath = { ...patch,
      path: 'single'
    };
    return resetChangeTip ? { ...resetPath,
      changeTip: 'always'
    } : resetPath;
  }
  ...
  return { ...patch,
    ...(wellRatioUpdater(prevWellRatio, nextWellRatio, appliedPatch) as FormPatch)
  };
}
...
function updatePatchMixFields(patch: FormPatch, rawForm: FormData): FormPatch {
  if (patch.path) {
    if (patch.path === 'multiAspirate') {
      return { ...patch,
        ...getDefaultFields('aspirate_mix_checkbox', 'aspirate_mix_times', 'aspirate_mix_volume')
      };
    }

    if (patch.path === 'multiDispense') {
      return { ...patch,
        ...getDefaultFields('dispense_mix_checkbox', 'dispense_mix_times', 'dispense_mix_volume')
      };
    }
  }
...
export function updatePatchBlowoutFields(patch: FormPatch, rawForm: FormData): FormPatch {
  ...
  if (fieldHasChanged(rawForm, patch, 'path')) {
    ...
    const shouldResetBlowoutLocation = path === 'multiAspirate' && blowout_location === SOURCE_WELL_BLOWOUT_DESTINATION || path === 'multiDispense' && blowout_location === DEST_WELL_BLOWOUT_DESTINATION;
...
export function dependentFieldsUpdateMoveLiquid(originalPatch: FormPatch, rawForm: FormData, // raw = NOT hydrated
