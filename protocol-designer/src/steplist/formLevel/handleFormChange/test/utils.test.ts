import { volumeInCapacityForMulti, volumeInCapacityForMultiAspirate, volumeInCapacityForMultiDispense } from "../utils";
...
describe('utils', () => {
    ...
    describe('multi dispense path', () => {
      ...
      describe('ensure that 2x volume + air gap volume can fit in pipette', () => {
        testCases.forEach(({
          ...
          it(msg, () => {
            expect(volumeInCapacityForMulti({ ...sharedForm,
              ...form
            }, pipetteEntities)).toBe(expected);
          });
        });
      });
    });
  ...
  describe('volumeInCapacityForMultiDispense', () => {
    it('should return false when air gap is too large', () => {
      ...
      expect(volumeInCapacityForMultiDispense({
        volume,
        pipetteCapacity,
        airGapVolume
      })).toBe(false);
  ...
  describe('volumeInCapacityForMultiAspirate', () => {
    it('should return false when air gap is too large', () => {
      ...
      expect(volumeInCapacityForMultiAspirate({
        volume,
        pipetteCapacity,
        airGapVolume
      })).toBe(false);
