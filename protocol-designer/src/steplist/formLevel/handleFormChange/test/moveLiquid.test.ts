...
beforeEach(() => {
  ...
  handleFormHelper = (patch, baseForm) => dependentFieldsUpdateMoveLiquid(patch, baseForm, pipetteEntities, labwareEntities);
});
afterEach(() => {
  jest.resetAllMocks();
});
...
describe('path should update...', () => {
  ...
  describe('if path is multi and volume*2 + air gap volume exceeds pipette/tip capacity', () => {
    ...
    multiPaths.forEach(path => {
      const testCases = [{
        description: 'should not reset path when aspirate > air gap checkbox is unchecked',
        volume: '3',
        ...
        description: 'should not reset path when aspirate > air gap volume is not a number',
        volume: '3',
        ...
        description: 'should not reset path when aspirate > air gap volume is small',
        volume: '3',
        aspirate_airGap_checkbox: true,
        ...
        description: 'should reset path when aspirate > air gap volume is large',
        volume: '3',
        aspirate_airGap_checkbox: true,
      ...
      testCases.forEach(({
        ...
        it(`${description} and path is ${path}`, () => {
          ...
          const airGapChange = handleFormHelper( // patch
          {
            aspirate_airGap_volume
          }, // form
          { ...pathFields,
          ...
          const volumeChange = handleFormHelper( // patch
          {
            volume
          }, // form
          { ...pathFields,
          ...
          const pathPatch = path === expectedPath ? {} : {
            path: expectedPath
          };
          const volumeChangeExpected = {
            volume,
            ...pathPatch
          };
...
describe('aspirate > air gap volume', () => {
  describe('when the path is single', () => {
    ...
    it('should update the aspirate > air gap volume to 0 when the raw form volume is less than 0', () => {
      const result = handleFormHelper({}, { ...form,
        aspirate_airGap_volume: '-1'
      });
      expect(result.aspirate_airGap_volume).toEqual('0');
    });
...
describe('air gap > dispense volume', () => {
  ...
  paths.forEach(path => {
    ...
    it(`should clamp max dispense > air gap when dispense_airGap_volume field is changed, for ${path} path`, () => {
      ...
      const result = handleFormHelper({
        dispense_airGap_volume: '29'
      }, form);
      // clamp to max, which should be 10 for the P10
      expect(result).toMatchObject({
        dispense_airGap_volume: '10'
      });
    });
  });
  ...
  multiDispenseCases.forEach(({
    update,
    expected
  }) => {
    it(`should clamp max air gap > dispense volume, if path is multi-dispense, when field '${Object.keys(update).join(', ')}' updated`, () => {
      const form = {
        path: 'multiDispense',
        aspirate_wells: ['A1'],
        dispense_wells: ['B2', 'B3'],
