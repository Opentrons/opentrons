...
beforeEach(() => {
  ...
  handleFormHelper = (patch, baseForm) => dependentFieldsUpdateMix(patch, baseForm, pipetteEntities, labwareEntities);
});
...
describe('well selection should update', () => {
  ...
  it('pipette cleared', () => {
    ...
    expect(handleFormHelper(patch, form)).toEqual({ ...patch,
      wells: [],
  ...
  it('pipette multi -> single', () => {
    ...
    expect(handleFormHelper(patch, multiChForm)).toEqual({ ...patch,
  ...
  it('select labware with multiple wells', () => {
    const trashLabwareForm = { ...form,
      labware: 'trashId'
    };
    const patch = {
      labware: 'plateId'
    };
    expect(handleFormHelper(patch, trashLabwareForm)).toEqual({ ...patch,
      wells: [],
      mix_mmFromBottom: DEFAULT_MM_FROM_BOTTOM_DISPENSE,
      mix_touchTip_mmFromBottom: null
    });
