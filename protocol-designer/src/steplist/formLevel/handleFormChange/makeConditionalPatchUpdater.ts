...
export const makeConditionalPatchUpdater: MakeConditionalPatchUpdater = updateMaps => (prevValue, nextValue, dependentFields) => {
  // get relevant update map (if any) via key values
  const updateMap = updateMaps.find(u => u.prevValue === prevValue && u.nextValue === nextValue);
  ...
  return fieldUpdates.reduce((patchAcc, {
    name,
    prevValue,
    nextValue
  }) => {
    return dependentFields[name] !== undefined && dependentFields[name] === prevValue ? { ...patchAcc,
      [name]: nextValue
    } : patchAcc;
  }, {});
};