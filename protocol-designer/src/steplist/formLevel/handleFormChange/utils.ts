...
import type { LabwareDefinition2, PipetteChannels } from "@opentrons/shared-data";
...
export function chainPatchUpdaters(initialPatch: FormPatch, fns: Array<(arg0: FormPatch) => FormPatch>): FormPatch {
...
export function getAllWellsFromPrimaryWells(primaryWells: Array<string>, labwareDef: LabwareDefinition2): Array<string> {
...
export function getChannels(pipetteId: string, pipetteEntities: PipetteEntities): PipetteChannels | null | undefined {
...
export function getMaxDisposalVolumeForMultidispense(values: {
  ...
  assert(values.path === 'multiDispense', `getMaxDisposalVolumeForMultidispense expected multiDispense, got path ${values.path}`);
...
export function volumeInCapacityForMulti(rawForm: FormData, pipetteEntities: PipetteEntities): boolean {
  assert(rawForm.pipette in pipetteEntities, `volumeInCapacityForMulti expected pipette ${rawForm.pipette} to be in pipetteEntities`);
  ...
  airGapVolume = Number.isFinite(airGapVolume) ? airGapVolume : 0;
  return rawForm.path === 'multiAspirate' ? volumeInCapacityForMultiAspirate({
    volume,
    pipetteCapacity,
    airGapVolume
...
export function volumeInCapacityForMultiAspirate(args: {
  ...
  return volume > 0 && pipetteCapacity > 0 && volume * 2 + airGapVolume * 2 <= pipetteCapacity;
}
export function volumeInCapacityForMultiDispense(args: {
  ...
  return volume > 0 && pipetteCapacity > 0 && volume * 2 + airGapVolume <= pipetteCapacity;
}
...
export function getDefaultWells(args: GetDefaultWellsArgs): Array<string> {
  ...
  if (!labwareId || !labwareEntities[labwareId] || !pipetteId || !pipetteEntities[pipetteId]) return [];
  const labwareDef = labwareEntities[labwareId].def;
  const pipetteCanUseLabware = canPipetteUseLabware(pipetteEntities[pipetteId].spec, labwareDef);
  if (!pipetteCanUseLabware) return [];
  const isSingleWellLabware = labwareDef.ordering.length === 1 && labwareDef.ordering[0].length === 1;
...
export function fieldHasChanged(rawForm: FormData, patch: FormPatch, fieldName: StepFieldName): boolean {
  return Boolean(patch[fieldName] !== undefined && patch[fieldName] !== rawForm[fieldName]);
}