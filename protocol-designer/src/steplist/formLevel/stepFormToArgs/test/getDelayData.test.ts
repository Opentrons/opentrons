...
describe('getMoveLiquidDelayData', () => {
  it('should return null if checkbox field is false', () => {
    expect(getMoveLiquidDelayData({
      checkboxField: false,
      secondsField: 3,
      offsetField: 2
    }, 'checkboxField', 'secondsField', 'offsetField')).toBe(null);
  });
  ...
  it('should return seconds & mmFromBottom if checkbox is checked', () => {
    expect(getMoveLiquidDelayData({
      ...
      secondsField: 30,
      offsetField: 2
    }, 'checkboxField', 'secondsField', 'offsetField')).toEqual({
      seconds: 30,
      mmFromBottom: 2
    });
  });
...
describe('getMixDelayData', () => {
  it('should return null if the checkbox field is false', () => {
    expect(getMixDelayData({
      checkboxField: false,
      secondsField: 3
    }, 'checkboxField', 'secondsField')).toBe(null);
  });
  ...
  it('should return the seconds field if checckbox is checked and the seconds field is > 0', () => {
    expect(getMixDelayData({
      checkboxField: true,
      secondsField: 10
    }, 'checkboxField', 'secondsField')).toEqual(10);
  });
});