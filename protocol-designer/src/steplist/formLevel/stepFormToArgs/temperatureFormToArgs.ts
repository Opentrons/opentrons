...
import type { SetTemperatureArgs, DeactivateTemperatureArgs } from "@opentrons/step-generation";
import type { HydratedTemperatureFormData } from "../../../form-types";
type TemperatureArgs = SetTemperatureArgs | DeactivateTemperatureArgs;
export const temperatureFormToArgs = (hydratedFormData: HydratedTemperatureFormData): TemperatureArgs => {
  const {
    moduleId
  } = hydratedFormData;
  // cast values
  ...
  assert(setTemperature ? !Number.isNaN(targetTemperature) : true, 'temperatureFormToArgs expected (hydrated) targetTemperature to be a number when setTemperature is "true"');
