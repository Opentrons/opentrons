...
import type { ThermocyclerProfileStepArgs, ThermocyclerStateStepArgs } from "@opentrons/step-generation";
import { PROFILE_STEP } from "../../../form-types";
import type { FormData, ProfileItem, ProfileStepItem } from "../../../form-types";
type FlatProfileSteps = $PropertyType<ThermocyclerProfileStepArgs, "profileSteps">;
...
export const thermocyclerFormToArgs = (formData: FormData): ThermocyclerProfileStepArgs | ThermocyclerStateStepArgs | null => {
  const {
    ...
    case THERMOCYCLER_STATE:
      {
        return {
          ...
          blockTargetTemp: formData.blockIsActive && formData.blockTargetTemp !== null ? Number(formData.blockTargetTemp) : null,
          lidTargetTemp: formData.lidIsActive && formData.lidTargetTemp !== null ? Number(formData.lidTargetTemp) : null,
    ...
    case THERMOCYCLER_PROFILE:
      {
        ...
        return {
          ...
          blockTargetTempHold: formData.blockIsActiveHold && formData.blockTargetTempHold !== null ? Number(formData.blockTargetTempHold) : null,
          lidOpenHold: formData.lidOpenHold,
          lidTargetTempHold: formData.lidIsActiveHold && formData.lidTargetTempHold !== null ? Number(formData.lidTargetTempHold) : null,
          meta: {
            rawProfileItems: formData.orderedProfileItems.map(itemId => formData.profileItemsById[itemId])
          },
          profileSteps,
