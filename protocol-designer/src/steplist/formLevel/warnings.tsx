...
export type WarningChecker = (arg0: unknown) => FormWarning | null | undefined;
...
export const belowPipetteMinimumVolume = (fields: HydratedFormData): FormWarning | null | undefined => {
  const {
    pipette,
    volume
  } = fields;
  if (!(pipette && pipette.spec)) return null;
  return volume < pipette.spec.minVolume ? belowPipetteMinVolumeWarning(pipette.spec.minVolume) : null;
};
export const maxDispenseWellVolume = (fields: HydratedFormData): FormWarning | null | undefined => {
  const {
    dispense_labware,
    dispense_wells,
    volume
...
export const minDisposalVolume = (fields: HydratedFormData): FormWarning | null | undefined => {
  ...
  return isBelowMin ? belowMinDisposalVolumeWarning(pipette.spec.minVolume) : null;
};
// both aspirate and dispense air gap volumes have the same minimums
export const _minAirGapVolume: (checkboxField: "aspirate_airGap_checkbox" | "dispense_airGap_checkbox", volumeField: "aspirate_airGap_volume" | "dispense_airGap_volume") => (arg0: HydratedFormData) => FormWarning | null | undefined = (checkboxField, volumeField) => fields => {
...
export const minAspirateAirGapVolume: (fields: HydratedFormData) => FormWarning | null | undefined = _minAirGapVolume('aspirate_airGap_checkbox', 'aspirate_airGap_volume');
export const minDispenseAirGapVolume: (fields: HydratedFormData) => FormWarning | null | undefined = _minAirGapVolume('dispense_airGap_checkbox', 'dispense_airGap_volume');

/*******************
 **     Helpers    **
 ********************/
type ComposeWarnings = (...warningCheckers: Array<WarningChecker>) => (formData: unknown) => Array<FormWarning>;
export const composeWarnings: ComposeWarnings = (...warningCheckers: Array<WarningChecker>) => formData => warningCheckers.reduce((acc, checker) => {
