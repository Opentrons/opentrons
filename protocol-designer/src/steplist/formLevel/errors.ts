...
import { MIN_ENGAGE_HEIGHT_V1, MAX_ENGAGE_HEIGHT_V1, MIN_ENGAGE_HEIGHT_V2, MAX_ENGAGE_HEIGHT_V2, PAUSE_UNTIL_RESUME, PAUSE_UNTIL_TIME, PAUSE_UNTIL_TEMP, THERMOCYCLER_PROFILE } from "../../constants";
...
const PAUSE_TYPE_REQUIRED: FormError = {
  title: 'Must either pause for amount of time, until told to resume, or until temperature reached',
...
const MODULE_ID_REQUIRED: FormError = {
  title: 'Module is required. Ensure the appropriate module is present on the deck and selected for this step',
...
export const incompatibleLabware = (fields: HydratedFormData): FormError | null | undefined => {
  const {
    labware,
    pipette
  } = fields;
  if (!labware || !pipette) return null;
  return !canPipetteUseLabware(pipette.spec, labware.def) ? INCOMPATIBLE_LABWARE : null;
};
export const incompatibleDispenseLabware = (fields: HydratedFormData): FormError | null | undefined => {
  const {
    dispense_labware,
    pipette
  } = fields;
  if (!dispense_labware || !pipette) return null;
  return !canPipetteUseLabware(pipette.spec, dispense_labware.def) ? INCOMPATIBLE_DISPENSE_LABWARE : null;
};
export const incompatibleAspirateLabware = (fields: HydratedFormData): FormError | null | undefined => {
  const {
    aspirate_labware,
    pipette
  } = fields;
  if (!aspirate_labware || !pipette) return null;
  return !canPipetteUseLabware(pipette.spec, aspirate_labware.def) ? INCOMPATIBLE_ASPIRATE_LABWARE : null;
};
export const pauseForTimeOrUntilTold = (fields: HydratedFormData): FormError | null | undefined => {
  const {
    pauseAction,
    pauseHour,
    pauseMinute,
...
export const wellRatioMoveLiquid = (fields: HydratedFormData): FormError | null => {
  const {
    aspirate_wells,
    dispense_wells
  } = fields;
  if (!aspirate_wells || !dispense_wells) return null;
  return getWellRatio(aspirate_wells, dispense_wells) ? null : WELL_RATIO_MOVE_LIQUID;
};
export const volumeTooHigh = (fields: HydratedFormData): FormError | null => {
  ...
  if (!Number.isNaN(volume) && !Number.isNaN(pipetteCapacity) && volume > pipetteCapacity) {
    return VOLUME_TOO_HIGH(pipetteCapacity);
  }

  return null;
};
export const magnetActionRequired = (fields: HydratedFormData): FormError | null => {
  const {
    magnetAction
  } = fields;
...
export const engageHeightRequired = (fields: HydratedFormData): FormError | null => {
  const {
    magnetAction,
    engageHeight
  } = fields;
  return magnetAction === 'engage' && !engageHeight ? ENGAGE_HEIGHT_REQUIRED : null;
};
export const moduleIdRequired = (fields: HydratedFormData): FormError | null => {
  const {
    moduleId
  } = fields;
...
export const targetTemperatureRequired = (fields: HydratedFormData): FormError | null => {
  const {
    setTemperature,
    targetTemperature
  } = fields;
  return setTemperature === 'true' && !targetTemperature ? TARGET_TEMPERATURE_REQUIRED : null;
};
export const profileVolumeRequired = (fields: HydratedFormData): FormError | null => {
  const {
    thermocyclerFormType,
    profileVolume
  } = fields;
  return thermocyclerFormType === THERMOCYCLER_PROFILE && !profileVolume ? PROFILE_VOLUME_REQUIRED : null;
};
export const profileTargetLidTempRequired = (fields: HydratedFormData): FormError | null => {
  const {
    thermocyclerFormType,
    profileTargetLidTemp
  } = fields;
  return thermocyclerFormType === THERMOCYCLER_PROFILE && !profileTargetLidTemp ? PROFILE_LID_TEMPERATURE_REQUIRED : null;
};
export const blockTemperatureRequired = (fields: HydratedFormData): FormError | null => {
  const {
    blockIsActive,
    blockTargetTemp
  } = fields;
  return blockIsActive === true && !blockTargetTemp ? BLOCK_TEMPERATURE_REQUIRED : null;
};
export const lidTemperatureRequired = (fields: HydratedFormData): FormError | null => {
  const {
    lidIsActive,
    lidTargetTemp
  } = fields;
  return lidIsActive === true && !lidTargetTemp ? LID_TEMPERATURE_REQUIRED : null;
};
export const blockTemperatureHoldRequired = (fields: HydratedFormData): FormError | null => {
  const {
    blockIsActiveHold,
    blockTargetTempHold
  } = fields;
  return blockIsActiveHold === true && !blockTargetTempHold ? BLOCK_TEMPERATURE_HOLD_REQUIRED : null;
};
export const lidTemperatureHoldRequired = (fields: HydratedFormData): FormError | null => {
  const {
    lidIsActiveHold,
    lidTargetTempHold
  } = fields;
  return lidIsActiveHold === true && !lidTargetTempHold ? LID_TEMPERATURE_HOLD_REQUIRED : null;
};
export const engageHeightRangeExceeded = (fields: HydratedFormData): FormError | null => {
  const {
    magnetAction,
    engageHeight
  } = fields;
 ...
 **     Helpers    **
 ********************/
type ComposeErrors = (...errorCheckers: Array<FormErrorChecker>) => (arg0: unknown) => Array<FormError>;
export const composeErrors: ComposeErrors = (...errorCheckers: Array<FormErrorChecker>) => value => errorCheckers.reduce((acc, errorChecker) => {
