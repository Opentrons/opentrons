...
export function getNextDefaultMagnetAction(savedForms: Record<StepIdType, FormData>, orderedStepIds: Array<StepIdType>): MagnetAction {
  const prevMagnetSteps = orderedStepIds.map(stepId => savedForms[stepId]).filter(form => form && form.magnetAction);
  ...
  if (lastMagnetStep && lastMagnetStep.magnetAction) {
    nextDefaultMagnetAction = lastMagnetStep.magnetAction === 'engage' ? 'disengage' : 'engage';
  }
