...
export function getNextDefaultPipetteId(savedForms: Record<StepIdType, FormData>, orderedStepIds: Array<StepIdType>, equippedPipettesById: Record<string, PipetteOnDeck>): string {
  const prevPipetteSteps = orderedStepIds.map(stepId => savedForms[stepId]).filter(form => form && form.pipette);
  ...
  const nextDefaultPipette: string | null | undefined = lastPipetteStep && lastPipetteStep.pipette || findKey(equippedPipettesById, p => p.mount === 'left') || findKey(equippedPipettesById, p => p.mount === 'right');
