...
import { makeContext } from "@opentrons/step-generation/src/__fixtures__";
...
describe('generateSubstepItem', () => {
  ...
  it('null is returned when no robotState', () => {
    ...
    const result = generateSubstepItem(stepArgsAndErrors, invariantContext, robotState, stepId, labwareNamesByModuleId);
  ...
  [{
    ...
    it(testName, () => {
      const result = generateSubstepItem(args, invariantContext, robotState, stepId, labwareNamesByModuleId);
      expect(result).toBeNull();
    });
  });
  ...
  it('null is returned when no matching command', () => {
    ...
    const result = generateSubstepItem(stepArgsAndErrors, invariantContext, robotState, stepId, {});
    expect(result).toBeNull();
  });
});