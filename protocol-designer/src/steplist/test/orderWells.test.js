...
describe('orderWells', () => {
  ...
  describe('regular labware', () => {
    ...
    orderTuples.forEach(tuple => {
      test(`first ${tuple[0]} then ${tuple[1]}`, () => {
        expect(orderWells(regularOrdering, ...tuple)).toEqual(regularAnswerMap[tuple[0]][tuple[1]])
      })
    })
  })
  ...
  describe('irregular labware', () => {
    ...
    orderTuples.forEach(tuple => {
      test(`first ${tuple[0]} then ${tuple[1]}`, () => {
        expect(orderWells(irregularOrdering, ...tuple)).toEqual(irregularAnswerMap[tuple[0]][tuple[1]])
      })
    })
  })
})
