...
describe('mergeWhen', () => {
  ...
  it('single array uses alternative when predicate is false', () => {
    const result = mergeWhen(['a'], (current, next) => false, (current, next) => null, current => current.toUpperCase());
  ...
  it('predicate true on index=0 only', () => {
    const result = mergeWhen(['1', '2', '3'], (current, next) => current === 1, concat);
    expect(result).toEqual(['1', '2', '3']);
  });
  it('merge at end', () => {
    const result = mergeWhen(['1', '2', '3'], (current, next) => current === '3', concat);
    expect(result).toEqual(['1', '2', '3']);
  });
  it('merge when "a*" before "b*"', () => {
    const result = mergeWhen(['a1', 'a2', 'b3', 'a4', 'b5', 'b6', 'a7', 'b8'], (prev: string, current: string) => prev.startsWith('a') && current.startsWith('b'), concat);
