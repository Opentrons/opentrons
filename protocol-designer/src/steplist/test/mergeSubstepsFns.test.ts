import { mergeSubstepRowsSingleChannel, mergeSubstepRowsMultiChannel } from "../generateSubstepItem";
const ingred1Id = 'ingred1Id';

const wellNamesForCol = (isMulti: boolean, colNum: string): Array<string> => isMulti ? 'ABCDEFGH'.split('').map(s => `${s}${colNum}`) : [`A${colNum}`];

type Ingreds = Record<string, number>;

const repeatIngreds = (isMulti: boolean, colNum: string, _ingreds: Ingreds | null | undefined): Ingreds | Array<Ingreds> => {
  const ingreds = _ingreds || {};
  return isMulti ? wellNamesForCol(true, colNum).reduce((acc, wellName) => ({ ...acc,
    [wellName]: ingreds
  }), {}) : ingreds;
};
...
const getFixtures = ({
  ...
  const makeIngreds = (volume: number | null, colNum: string) => repeatIngreds(isMulti, colNum, volume ? {
    [ingred1Id]: volume
  } : null);
  ...
  return {
    activeTips,
    transferRowsFixture: [// from 'aspirate' command
    {
      activeTips,
      source: {
    ...
    consolidateRowsFixture: [// from 'aspirate' command
    {
      activeTips,
      source: {
        wells: ['A1'],
    ...
    distributeRowsFixture: [// from 'aspirate' command
    {
      activeTips,
      source: {
        wells: wellNamesForCol(isMulti, '1'),
...
describe('mergeSubstepRowsSingleChannel', () => {
  ...
  const testCases = [{
    ...
    expected: [// merged into single row
    {
      activeTips,
      source: {
        well: 'A1',
    ...
    expected: [// first aspirate stands alone
    {
      activeTips,
      source: {
        well: 'A1',
    ...
    expected: [// first aspirate + disp merged into single row
    {
      activeTips,
      source: {
        well: 'A1',
