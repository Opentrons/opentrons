...
import type { FileUploadErrorType, FileUploadMessage, LoadFileAction, NewProtocolFields } from "./types";
...
export const loadProtocolFile = (event: React.SyntheticEvent<HTMLInputElement>): ThunkAction<any> => (dispatch: ThunkDispatch<any>, getState: GetState) => {
  const fileError = (errorType: FileUploadErrorType, errorMessage?: string) => dispatch(fileUploadMessage({
    isError: true,
    errorType,
    errorMessage
  }));
...
export const saveProtocolFile: () => ThunkAction<SaveProtocolFileAction> = () => (dispatch, getState) => {
  ...
  const protocolName = fileDataSelectors.getFileMetadata(state).protocolName || 'untitled';
