...
type Version = string;
type MigrationsByVersion = Record<Version, (arg0: Record<string, any>) => Record<string, any>>;
...
export const migration = (file: any): {
  file: PDProtocolFile;
  didMigrate: boolean;
  ...
  const applicationVersion: string = designerApplication.applicationVersion || designerApplication.version || OLDEST_MIGRATEABLE_VERSION;
  const migrationVersionsToRun = getMigrationVersionsToRunFromVersion(allMigrationsByVersion, applicationVersion);
  const migratedFile = flow(migrationVersionsToRun.map(version => allMigrationsByVersion[version]))(file);
  return {
    file: migratedFile,
