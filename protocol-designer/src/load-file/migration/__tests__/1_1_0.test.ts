...
import { renameOrderedSteps, addInitialDeckSetupStep, INITIAL_DECK_SETUP_STEP_ID, updateStepFormKeys, TCD_DEPRECATED_FIELD_NAMES, MIX_DEPRECATED_FIELD_NAMES, replaceTCDStepsWithMoveLiquidStep } from "../1_1_0";
describe('renameOrderedSteps', () => {
  ...
  it('the rest of file should be unaltered', () => {
    const oldWithout = { ...oldProtocol,
      'designer-application': { ...oldProtocol['designer-application'],
        data: omit(oldProtocol['designer-application'].data, ['orderedSteps', 'orderedStepIds'])
      }
    };
    const migratedWithout = { ...migratedFile,
      'designer-application': { ...migratedFile['designer-application'],
...
describe('addInitialDeckSetupStep', () => {
  ...
  describe('adds well formed savedStepForm value', () => {
    ...
    it('constructs labware location update object', () => {
      expect(deckSetupStepForm.labwareLocationUpdate).toEqual({ ...wellFormedSetupStep.labwareLocationUpdate,
        ...mapValues(oldProtocol.labware, l => l.slot)
      });
    });
    it('constructs pipette location update object', () => {
      expect(deckSetupStepForm.pipetteLocationUpdate).toEqual({ ...wellFormedSetupStep.pipetteLocationUpdate,
  ...
  describe('for TCD stepTypes', () => {
    ...
    it('creates non-existent new fields', () => {
      ...
      each(Object.keys(addedFields), fieldName => {
        ...
        each(migratedFile['designer-application'].data.savedStepForms, stepForm => expect(stepForm[fieldName]).toEqual(addedFields[fieldName]));
      });
    });
  });
  describe('for mix stepType', () => {
    ...
    it('deprecates all indicated field names', () => {
      each(MIX_DEPRECATED_FIELD_NAMES, fieldName => {
        each(stubbedMixStepFile['designer-application'].data.savedStepForms, stepForm => expect(stepForm[fieldName]).not.toEqual(undefined));
        each(migratedFile['designer-application'].data.savedStepForms, stepForm => expect(stepForm[fieldName]).toEqual(undefined));
      });
    });
    it('creates non-existent new fields', () => {
