...
import type { FileLabware, FilePipette, ProtocolFile } from "@opentrons/shared-data/protocol/flowTypes/schemaV1";
...
export type PDMetadata = {
  ...
  ingredLocations: Record<string, Record<string, Record<string, {
    volume: number;
  }>>>;
...
type LegacyPipetteEntities = Record<string, {
  id: string;
  tiprackModel: string;
  mount: string;
  name?: string;
...
function getPipetteCapacityLegacy(pipette: $Values<LegacyPipetteEntities>): number {
  // hacky model to name ('p10_single_v1.3' -> 'p10_single') fallback
  const pipetteName = pipette.name || (pipette.model || '').split('_v')[0];

  if (!pipetteName) {
    throw new Error(`expected pipette name or model in migration. Pipette: "${JSON.stringify(pipette)}"`);
  }
  ...
  assert(tiprackDef?.metadata?.tipVolume, `expected tiprack volume for tiprack def ${JSON.stringify(tiprackDef?.metadata || 'undefined')}`);
  return NaN;
}
...
function _updatePatchPathField(patch: FormPatch, rawForm: FormData, pipetteEntities: LegacyPipetteEntities) {
  // $FlowFixMe(IL, 2020-02-24): address in #3161, patch may overwrite explicit keys in rawForm
  const appliedPatch = { ...rawForm,
    ...patch
  };
  const {
    path,
  ...
  const pipetteCapacity = getPipetteCapacityLegacy(pipetteEntities[appliedPatch.pipette]);
  ...
  const incompatiblePath = changeTip === 'perSource' && path === 'multiAspirate' || changeTip === 'perDest' && path === 'multiDispense';
...
export function addInitialDeckSetupStep(fileData: PDProtocolFile): PDProtocolFile {
  ...
  const deckSetupStep: any = { ...initialDeckSetupStepForm,
    labwareLocationUpdate: { ...initialDeckSetupStepForm.labwareLocationUpdate,
      ...additionalLabware
    },
    pipetteLocationUpdate: { ...initialDeckSetupStepForm.pipetteLocationUpdate,
      ...pipetteLocations
    }
  };
  return { ...fileData,
    'designer-application': { ...fileData['designer-application'],
      data: { ...fileData['designer-application'].data,
        savedStepForms: { ...savedStepForms,
...
export const MIX_DEPRECATED_FIELD_NAMES = ['step-name', 'step-details', 'aspirate_changeTip', 'dispense_mmFromBottom', 'aspirate_wellOrder_first', 'aspirate_wellOrder_second', 'dispense_blowout_checkbox', 'dispense_blowout_labware', 'dispense_blowout_location', 'touchTip', 'mix_touchTipMmFromBottom'];
export function updateStepFormKeys(fileData: PDProtocolFile): PDProtocolFile {
  ...
  const migratedStepForms = mapValues(savedStepForms, formData => {
    if (['transfer', 'consolidate', 'distribute'].includes(formData.stepType)) {
      ...
      return { ...omitBy(updatedFields, isUndefined),
...
export function replaceTCDStepsWithMoveLiquidStep(fileData: PDProtocolFile): PDProtocolFile {
  ...
  const migratedStepForms = mapValues(savedStepForms, formData => {
    ...
    const pipetteEntities = mapValues(fileData['pipettes'], (pipette, pipetteId) => ({ ...pipette,
...
export const migrateFile = (fileData: PDProtocolFile): PDProtocolFile => flow([renameOrderedSteps, addInitialDeckSetupStep, updateStepFormKeys, replaceTCDStepsWithMoveLiquidStep])(fileData);