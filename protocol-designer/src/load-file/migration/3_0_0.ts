...
import type { ProtocolFile, FileLabware, FilePipette } from "@opentrons/shared-data/protocol/flowTypes/schemaV3";
...
function migrateMetadata(metadata: $PropertyType<PDProtocolFileV1, "metadata">): $PropertyType<PDProtocolFile, "metadata"> {
  const metadataExtraKeys: typeof metadata = { ...metadata,
...
function migratePipettes(pipettes: $PropertyType<PDProtocolFileV1, "pipettes">): FilePipettes {
  return Object.keys(pipettes).reduce<FilePipettes>((acc, pipetteId: string) => {
...
function migrateLabware(labware: $PropertyType<PDProtocolFileV1, "labware">): FileLabwares {
...
function getLabwareDefinitions(labware: $PropertyType<PDProtocolFileV1, "labware">): LabwareDefinitions {
  const allLabwareModels = uniq(Object.keys(labware).map((labwareId: string) => labware[labwareId].model));
  const allLabwareDefs = allLabwareModels.map(v1LabwareModelToV2Def);
  return reduce(allLabwareDefs, (acc, def): LabwareDefinitions => ({ ...acc,
...
function migrateAppData(appData: PDMetadata): PDMetadata {
  ...
  return { ...appData,
    pipetteTiprackAssignments: mapValues(appData.pipetteTiprackAssignments, (model: string): string => getLabwareDefURI(v1LabwareModelToV2Def(model)))
  };
}
