...
export const SUPPORTED_MODULE_TYPES: Array<ModuleRealType> = [MAGNETIC_MODULE_TYPE, TEMPERATURE_MODULE_TYPE, THERMOCYCLER_MODULE_TYPE];
type SupportedSlotMap = Record<ModuleRealType, Array<DropdownOption>>;
...
export function getAllModuleSlotsByType(moduleType: ModuleRealType): Array<DropdownOption> {
  ...
  const allOtherSlots = ALL_MODULE_SLOTS.filter(s => s.value !== supportedSlotOption[0].value);
