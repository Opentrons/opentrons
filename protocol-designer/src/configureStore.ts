...
function getRootReducer() {
  ...
  return (state: any, action) => {
    if (action.type === 'LOAD_FILE' || action.type === 'CREATE_NEW_PROTOCOL' || action.type === 'UNDO_LOAD_FILE') {
...
export function configureStore(): Store<BaseState, Action, ThunkDispatch<Action>> {
  const reducer = getRootReducer();
  const composeEnhancers: any = window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__ || compose;
  const store = createStore<BaseState, Action, ThunkDispatch<any>>(reducer,
  /* preloadedState, */
  composeEnhancers(applyMiddleware(trackEventMiddleware, timelineMiddleware, thunk)));
  ...
  if (module.hot) {
    ...
    // $FlowFixMe no module.hot
    module.hot.accept(['./analytics/reducers', './dismiss/reducers', './feature-flags/reducers', './file-data/reducers', './labware-defs/reducers', // NOTE: labware-defs is nested inside step-forms, so it doesn't need to go directly into getRootReducer fn above
    './labware-ingred/reducers', './load-file/reducers', './navigation/reducers', './step-forms/reducers', './tutorial/reducers', './ui/steps/reducers', './well-selection/reducers'], replaceReducers);
  }
