import { MAGNETIC_MODULE_TYPE, MAGNETIC_MODULE_V1, TEMPERATURE_MODULE_TYPE, TEMPERATURE_MODULE_V1, THERMOCYCLER_MODULE_TYPE, THERMOCYCLER_MODULE_V1 } from "@opentrons/shared-data";
import { SPAN7_8_10_11_SLOT, FIXED_TRASH_ID } from "../../../constants";
import { getDisposalLabwareOptions, getLabwareOptions, _sortLabwareDropdownOptions } from "../selectors";
...
describe('labware selectors', () => {
  ...
  describe('getDisposalLabwareOptions', () => {
    it('returns an empty list when labware is NOT provided', () => {
      expect( // $FlowFixMe(IL, 2020-03-12): resultFunc
      getDisposalLabwareOptions.resultFunc([], names)).toEqual([]);
    });
    it('returns empty list when trash is NOT present', () => {
      const labwareEntities = { ...tipracks
      };
      expect( // $FlowFixMe(IL, 2020-03-12): resultFunc
      getDisposalLabwareOptions.resultFunc(labwareEntities, names)).toEqual([]);
    });
  ...
  describe('getLabwareOptions', () => {
    ...
    it('should return labware options when no modules are present, with no tipracks', () => {
      ...
      expect( // $FlowFixMe(IL, 2020-03-12): resultFunc
      getLabwareOptions.resultFunc(labwareEntities, names, initialDeckSetup)).toEqual([{
        name: 'Source Plate',
        value: 'wellPlateId'
      }, {
        name: 'Trash',
    ...
    it('should return labware options with module prefixes when a labware is on module', () => {
      const labware = {
        wellPlateId: { ...otherLabware.wellPlateId,
          slot: 'magModuleId' // On magnetic module

        },
        tempPlateId: { ...otherLabware.wellPlateId,
          id: 'tempPlateId',
          slot: 'tempModuleId' // On temperature module

        },
        tcPlateId: { ...otherLabware.wellPlateId,
          id: 'tcPlateId',
          slot: 'thermocyclerId' // On thermocycler

        }
      };
      const labwareEntities = { ...trash,
        ...labware
      };
      const initialDeckSetup = {
        pipettes: {},
      ...
      const nicknames: Record<string, string> = { ...names,
        wellPlateId: 'Well Plate',
        tempPlateId: 'Temp Plate',
        tcPlateId: 'TC Plate'
      };
      expect( // $FlowFixMe(IL, 2020-03-12): resultFunc
      getLabwareOptions.resultFunc(labwareEntities, nicknames, initialDeckSetup)).toEqual([{
        name: 'MAG Well Plate',
        value: 'wellPlateId'
      }, {
  ...
  describe('_sortLabwareDropdownOptions', () => {
    ...
    it('should sort labware ids in alphabetical order but with fixed trash at the bottom', () => {
      const result = _sortLabwareDropdownOptions([trashOption, aaaPlateOption, zzzPlateOption]);
