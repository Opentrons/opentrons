...
import { END_TERMINAL_ITEM_ID, PRESAVED_STEP_ID, START_TERMINAL_ITEM_ID } from "../../../steplist/types";
import { SINGLE_STEP_SELECTION_TYPE, MULTI_STEP_SELECTION_TYPE, TERMINAL_ITEM_SELECTION_TYPE } from "../reducers";
import { getHoveredStepLabware, getSelectedStepTitleInfo, getActiveItem, getMultiSelectLastSelected, _getSavedMultiSelectFieldValues, getMultiSelectFieldValues, getMultiSelectDisabledFields, getCountPerStepType, getBatchEditSelectedStepTypes } from "../selectors";
...
describe('getHoveredStepLabware', () => {
  ...
  it('no labware is returned when no hovered step', () => {
    ...
    const result = getHoveredStepLabware.resultFunc(argsByStepId, hoveredStep, initialDeckState);
  ...
  it('no labware is returned when no step arguments', () => {
    const stepArgs = null;
    const argsByStepId = createArgsForStepId(hoveredStepId, stepArgs);
    const result = getHoveredStepLabware.resultFunc(argsByStepId, hoveredStepId, initialDeckState);
    expect(result).toEqual([]);
  });
...
describe('getSelectedStepTitleInfo', () => {
  it('should return title info of the presaved form when the presaved terminal item is selected', () => {
    ...
    const result = getSelectedStepTitleInfo.resultFunc(unsavedForm, {}, null, PRESAVED_STEP_ID);
    expect(result).toEqual({
  ...
  it('should return title info of the saved step when a saved step is selected', () => {
    ...
    const result = getSelectedStepTitleInfo.resultFunc(null, {
      [stepId]: savedForm
    }, stepId, null);
    expect(result).toEqual({
...
describe('getActiveItem', () => {
  const testCases = [{
    ...
    title: 'should return null, if nothing is hovered and multi-select is selected',
    selected: {
      selectionType: MULTI_STEP_SELECTION_TYPE,
      ids: ['notTheseSteps', 'nope']
    },
    ...
    title: 'should show the single-selected terminal item, if nothing is hovered',
    selected: {
...
describe('_getSavedMultiSelectFieldValues', () => {
  ...
  beforeEach(() => {
    mockSavedStepForms = { ...getMockMoveLiquidStep(),
      // just doing this so the ids are not the exact same
      another_move_liquid_step_id: { ...getMockMoveLiquidStep().move_liquid_step_id
      }
    };
    mockmultiSelectItemIds = ['move_liquid_step_id', 'another_move_liquid_step_id'];
  });
  afterEach(() => {});
  it('should return null if any of the forms are an unhandled type', () => {
    const savedStepForms = { ...mockSavedStepForms,
      another_move_liquid_step_id: { ...mockSavedStepForms.another_move_liquid_step_id,
        stepType: 'someOtherThing'
      }
    };
    expect(_getSavedMultiSelectFieldValues.resultFunc(savedStepForms, mockmultiSelectItemIds)).toBe(null);
  });
  it('should return null if some forms are moveLiquid and others are mix', () => {
    const savedStepForms = { ...mockSavedStepForms,
      ...getMockMixStep()
    };
    expect(_getSavedMultiSelectFieldValues.resultFunc(savedStepForms, ['move_liquid_step_id', 'mix_step_id'])).toBe(null);
  });
  describe('moveLiquid: when fields are NOT indeterminate', () => {
    it('should return the fields with the indeterminate boolean', () => {
      expect(_getSavedMultiSelectFieldValues.resultFunc(mockSavedStepForms, mockmultiSelectItemIds)).toEqual({
        // aspirate settings
        aspirate_labware: {
  ...
  describe('moveLiquid: when fields are indeterminate', () => {
    ...
    beforeEach(() => {
      mockSavedStepFormsIndeterminate = { ...getMockMoveLiquidStep(),
        // just doing this so the ids are not the exact same
        another_move_liquid_step_id: { ...getMockMoveLiquidStep().move_liquid_step_id,
    ...
    it('should return the fields with the indeterminate boolean', () => {
      expect(_getSavedMultiSelectFieldValues.resultFunc(mockSavedStepFormsIndeterminate, mockmultiSelectItemIds)).toEqual({
        // aspirate settings
        aspirate_labware: {
          isIndeterminate: true
        },
  ...
  describe('mix: when fields are NOT indeterminate', () => {
    ...
    beforeEach(() => {
      mockMixSavedStepForms = { ...getMockMixStep(),
    ...
    it('should return the fields with the indeterminate boolean', () => {
      expect(_getSavedMultiSelectFieldValues.resultFunc(mockMixSavedStepForms, mockMixMultiSelectItemIds)).toEqual({
        volume: {
          value: '100',
          isIndeterminate: false
        },
  ...
  describe('mix: when fields are indeterminate', () => {
    ...
    let mockMixMultiSelectItemIds;
    beforeEach(() => {
      mockMixSavedStepFormsIndeterminate = { ...getMockMixStep(),
        another_mix_step_id: { ...getMockMixStep().mix_step_id,
          volume: '123',
          times: '6',
          changeTip: 'never',
    ...
    it('should return the fields with the indeterminate boolean', () => {
      expect(_getSavedMultiSelectFieldValues.resultFunc(mockMixSavedStepFormsIndeterminate, mockMixMultiSelectItemIds)).toEqual({
        volume: {
          isIndeterminate: true
        },
        times: {
...
describe('getMultiSelectDisabledFields', () => {
  describe('disabled field tooltips', () => {
    it('should exist', () => {
      ...
      const disabledReasons = ['pipette-different', 'aspirate-labware-different', 'dispense-labware-different', 'multi-aspirate-present', 'multi-aspirate-present-pipette-different', 'multi-dispense-present', 'multi-dispense-present-pipette-different'];
      expect.assertions(7);
  ...
  describe('when all forms are of type moveLiquid', () => {
    ...
    it('should return an empty object when no fields are different and path is single', () => {
      expect(getMultiSelectDisabledFields.resultFunc(mockSavedStepForms, mockmultiSelectItemIds)).toEqual({});
    });
    describe('when pipettes are different', () => {
      ...
      it('should return fields being disabled with associated reasons', () => {
        expect(getMultiSelectDisabledFields.resultFunc(savedStepForms, mockmultiSelectItemIds)).toEqual({
          aspirate_mix_checkbox: i18n.t('tooltip.step_fields.batch_edit.disabled.pipette-different'),
          aspirate_mix_volume: i18n.t('tooltip.step_fields.batch_edit.disabled.pipette-different'),
          aspirate_mix_times: i18n.t('tooltip.step_fields.batch_edit.disabled.pipette-different'),
          aspirate_flowRate: i18n.t('tooltip.step_fields.batch_edit.disabled.pipette-different'),
          aspirate_airGap_checkbox: i18n.t('tooltip.step_fields.batch_edit.disabled.pipette-different'),
          aspirate_airGap_volume: i18n.t('tooltip.step_fields.batch_edit.disabled.pipette-different'),
          dispense_mix_checkbox: i18n.t('tooltip.step_fields.batch_edit.disabled.pipette-different'),
          dispense_mix_volume: i18n.t('tooltip.step_fields.batch_edit.disabled.pipette-different'),
          dispense_mix_times: i18n.t('tooltip.step_fields.batch_edit.disabled.pipette-different'),
          dispense_flowRate: i18n.t('tooltip.step_fields.batch_edit.disabled.pipette-different'),
          dispense_airGap_checkbox: i18n.t('tooltip.step_fields.batch_edit.disabled.pipette-different'),
          dispense_airGap_volume: i18n.t('tooltip.step_fields.batch_edit.disabled.pipette-different')
        });
      });
    });
    describe('when aspirate labware are different', () => {
      ...
      it('should return fields being disabled with associated reasons', () => {
        const aspirateLabwareDifferentText = i18n.t('tooltip.step_fields.batch_edit.disabled.aspirate-labware-different');
        expect(getMultiSelectDisabledFields.resultFunc(savedStepForms, mockmultiSelectItemIds)).toEqual({
    ...
    describe('when dispense labware are different', () => {
      ...
      it('should return fields being disabled with associated reasons', () => {
        const dispenseLabwareDifferentText = i18n.t('tooltip.step_fields.batch_edit.disabled.dispense-labware-different');
        expect(getMultiSelectDisabledFields.resultFunc(savedStepForms, mockmultiSelectItemIds)).toEqual({
    ...
    describe('when a form includes a multi aspirate path', () => {
      ...
      it('should return fields being disabled with associated reasons', () => {
        expect(getMultiSelectDisabledFields.resultFunc(savedStepForms, mockmultiSelectItemIds)).toEqual({
          aspirate_mix_checkbox: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-aspirate-present'),
          aspirate_mix_volume: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-aspirate-present'),
          aspirate_mix_times: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-aspirate-present')
        });
      });
    });
    describe('when a form includes a multi dispense path', () => {
      ...
      it('should return fields being disabled with associated reasons', () => {
        expect(getMultiSelectDisabledFields.resultFunc(savedStepForms, mockmultiSelectItemIds)).toEqual({
          dispense_mix_checkbox: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-dispense-present'),
          dispense_mix_volume: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-dispense-present'),
          dispense_mix_times: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-dispense-present'),
          blowout_checkbox: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-dispense-present'),
          blowout_location: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-dispense-present')
        });
      });
    });
    describe('when pipettes are different AND a form includes a multi aspirate path', () => {
      ...
      it('should return aspirate mix being disabled for both reasons', () => {
        expect(getMultiSelectDisabledFields.resultFunc(savedStepForms, mockmultiSelectItemIds)).toEqual(expect.objectContaining({
          aspirate_mix_checkbox: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-aspirate-present-pipette-different'),
          aspirate_mix_volume: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-aspirate-present-pipette-different'),
          aspirate_mix_times: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-aspirate-present-pipette-different')
        }));
      });
    });
    describe('when pipettes are different AND a form includes a multi dispense path', () => {
      ...
      it('should return aspirate mix being disabled for both reasons', () => {
        expect(getMultiSelectDisabledFields.resultFunc(savedStepForms, mockmultiSelectItemIds)).toEqual(expect.objectContaining({
          dispense_mix_checkbox: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-dispense-present-pipette-different'),
          dispense_mix_volume: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-dispense-present-pipette-different'),
          dispense_mix_times: i18n.t('tooltip.step_fields.batch_edit.disabled.multi-dispense-present-pipette-different')
        }));
      });
    });
  });
  describe('when all forms are of type mix', () => {
    ...
    let mockmultiSelectItemIds;
    beforeEach(() => {
      mockSavedStepForms = { ...getMockMixStep(),
        // just doing this so the ids are not the exact same
        another_mix_step_id: { ...getMockMixStep().mix_step_id
        }
      };
    ...
    describe('when pipettes are different', () => {
      ...
      beforeEach(() => {
        savedStepForms = { ...mockSavedStepForms,
          another_mix_step_id: { ...mockSavedStepForms.another_mix_step_id,
            pipette: 'different_pipette_id'
          }
        };
      });
    ...
    describe('when labware are different', () => {
      ...
      it('should return fields being disabled with associated reasons', () => {
        const labwareDifferentText = i18n.t('tooltip.step_fields.batch_edit.disabled.labware-different');
        expect(getMultiSelectDisabledFields.resultFunc(savedStepForms, mockmultiSelectItemIds)).toEqual({
  ...
  it('should return null if when forms are not all uniformly moveliquid OR mix', () => {
    const savedStepForms = { ...getMockMoveLiquidStep(),
      ...getMockMixStep()
    };
    const multiSelectItemIds = ['move_liquid_step_id', 'mix_step_id'];
    expect(getMultiSelectDisabledFields.resultFunc(savedStepForms, multiSelectItemIds)).toBe(null);
  });
});
describe('getCountPerStepType', () => {
  it('should return an object representing counts of all selected step types', () => {
    ...
    const result = getCountPerStepType.resultFunc(multiSelectItemIds, savedStepForms);
    expect(result).toEqual({
      magnet: 2,
      moveLiquid: 1
    });
