...
const mockGetDeckHasLiquid: JestMockFn<[Record<string, any>], any> = labwareIngredSelectors.getDeckHasLiquid;
const mockGetMagnetModuleHasLabware: JestMockFn<[Record<string, any>], any> = uiModuleSelectors.getMagnetModuleHasLabware;
const mockGetTemperatureModuleHasLabware: JestMockFn<[Record<string, any>], any> = uiModuleSelectors.getTemperatureModuleHasLabware;
const mockGetThermocyclerModuleHasLabware: JestMockFn<[Record<string, any>], any> = uiModuleSelectors.getThermocyclerModuleHasLabware;
const mockGetSingleTemperatureModuleId: JestMockFn<[Record<string, any>], any> = uiModuleSelectors.getSingleTemperatureModuleId;
const mockGetSingleThermocyclerModuleId: JestMockFn<[Record<string, any>], any> = uiModuleSelectors.getSingleThermocyclerModuleId;
const mockGetRobotStateTimeline: JestMockFn<[Record<string, any>], any> = fileDataSelectors.getRobotStateTimeline;
beforeEach(() => {
  jest.clearAllMocks();
  ...
  describe('ADD_HINT "module_without_labware"', () => {
    ...
    [{
      ...
      it(`should be dispatched (after addStep thunk is dispatched) for ${testName}`, () => {
        mockGetMagnetModuleHasLabware.mockReturnValue(selectorValues.getMagnetModuleHasLabware);
        mockGetTemperatureModuleHasLabware.mockReturnValue(selectorValues.getTemperatureModuleHasLabware);
        mockGetThermocyclerModuleHasLabware.mockReturnValue(selectorValues.getThermocyclerModuleHasLabware);
        mockGetSingleTemperatureModuleId.mockReturnValue(selectorValues.getSingleTemperatureModuleId);
        mockGetSingleThermocyclerModuleId.mockReturnValue(selectorValues.getSingleThermocyclerModuleId);
        const payload = {
          stepType
        };
        addAndSelectStepWithHints(payload)(dispatch, getState);
