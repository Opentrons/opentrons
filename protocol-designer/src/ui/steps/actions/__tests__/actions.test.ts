...
const mockGetSavedStepForms: JestMockFn<[Record<string, any>], any> = stepFormSelectors.getSavedStepForms;
const mockGetOrderedStepIds: JestMockFn<[Record<string, any>], any> = stepFormSelectors.getOrderedStepIds;
const mockGetMultiSelectLastSelected: JestMockFn<[Record<string, any>], any> = getMultiSelectLastSelected;
describe('steps actions', () => {
  describe('selectStep', () => {
    const stepId = 'stepId';
    beforeEach(() => {
      when(mockGetSavedStepForms).calledWith(expect.anything()).mockReturnValue({
        stepId: {
  ...
  describe('selectAllSteps', () => {
    ...
    beforeEach(() => {
      ids = ['id_1', 'id_2'];
      when(mockGetOrderedStepIds).calledWith(expect.anything()).mockReturnValue(ids);
    });
    afterEach(() => {
      resetAllWhenMocks();
    });
  ...
  describe('deselectAllSteps', () => {
    const id = 'some_id';
    beforeEach(() => {
      when(mockGetMultiSelectLastSelected).calledWith(expect.anything()).mockReturnValue(id);
    });
    afterEach(() => {
      resetAllWhenMocks();
    });
    ...
    it('should console warn when NOT in multi select mode', () => {
      when(mockGetMultiSelectLastSelected).calledWith(expect.anything()).mockReturnValue(null);
      const consoleWarnSpy = jest.spyOn(global.console, 'warn').mockImplementation(() => null);
      const store = mockStore();
      // $FlowFixMe(SA, 2021-01-21): redux-mock-store dispatch types not cooperating. Related TypeScript issue: https://github.com/reduxjs/redux-mock-store/issues/148
      store.dispatch(deselectAllSteps());
      expect(consoleWarnSpy).toHaveBeenCalledWith('something went wrong, cannot deselect all steps if not in multi select mode');
  ...
  describe('duplicateMultipleSteps', () => {
    ...
    beforeEach(() => {
      ids = ['id_1', 'id_2', 'id_3'];
      when(mockGetOrderedStepIds).calledWith(expect.anything()).mockReturnValue(ids);
      when(mockGetMultiSelectLastSelected).calledWith(expect.anything()).mockReturnValue('id_3');
    });
    afterEach(() => {
      resetAllWhenMocks();
      jest.restoreAllMocks();
    ...
    it('should duplicate multiple steps with a new step ids, and select the new duplicated steps', () => {
      jest.spyOn(utils, 'uuid').mockReturnValueOnce('dup_1').mockReturnValueOnce('dup_2').mockReturnValueOnce('dup_3');
      const store = mockStore();
      // $FlowFixMe(SA, 2021-02-01): redux-mock-store dispatch types not cooperating. Related TypeScript issue: https://github.com/reduxjs/redux-mock-store/issues/148
      ...
      expect(store.getActions()).toEqual([duplicateStepsAction, selectMultipleStepsAction]);
    });
