...
import { getUnsavedForm, getUnsavedFormIsPristineSetTempForm, getOrderedStepIds } from "../../../../step-forms/selectors";
...
import { actions as tutorialActions, selectors as tutorialSelectors } from "../../../../tutorial";
...
import type { DuplicateStepAction, DuplicateMultipleStepsAction, SelectMultipleStepsAction } from "../types";
export const addAndSelectStepWithHints: (arg0: {
  ...
  const magnetModuleHasLabware = uiModuleSelectors.getMagnetModuleHasLabware(state);
  const temperatureModuleHasLabware = uiModuleSelectors.getTemperatureModuleHasLabware(state);
  const thermocyclerModuleHasLabware = uiModuleSelectors.getThermocyclerModuleHasLabware(state);
  const temperatureModuleOnDeck = uiModuleSelectors.getSingleTemperatureModuleId(state);
  const thermocyclerModuleOnDeck = uiModuleSelectors.getSingleThermocyclerModuleId(state);
  ...
  const stepModuleMissingLabware = stepMagnetNeedsLabware && !magnetModuleHasLabware || stepTemperatureNeedsLabware && (temperatureModuleOnDeck && !temperatureModuleHasLabware || thermocyclerModuleOnDeck && !thermocyclerModuleHasLabware);
...
export const reorderSelectedStep: (delta: number) => ThunkAction<ReorderSelectedStepAction> = delta => (dispatch, getState) => {
...
export const duplicateStep: (stepId: StepIdType) => ThunkAction<DuplicateStepAction> = stepId => (dispatch, getState) => {
...
export const duplicateMultipleSteps: (stepIds: Array<StepIdType>) => ThunkAction<DuplicateMultipleStepsAction | SelectMultipleStepsAction> = stepIds => (dispatch, getState) => {
...
export const _saveStepForm = (form: FormData): SaveStepFormAction => {
  ...
  const payload = form.id === PRESAVED_STEP_ID ? { ...form,
    id: uuid()
  } : form;
  return {
    type: SAVE_STEP_FORM,
...
export const saveStepForm: () => ThunkAction<any> = () => (dispatch, getState) => {
  ...
  if (!unsavedForm) {
    assert(false, 'Tried to saveStepForm with falsey unsavedForm. This should never be able to happen.');
    return;
  }
...
export const saveSetTempFormWithAddedPauseUntilTemp: () => ThunkAction<any> = () => (dispatch, getState) => {
  const initialState = getState();
  const unsavedSetTemperatureForm = getUnsavedForm(initialState);
  const isPristineSetTempForm = getUnsavedFormIsPristineSetTempForm(initialState);

  // this check is only for Flow. At this point, unsavedForm should always be populated
  if (!unsavedSetTemperatureForm) {
    assert(false, 'Tried to saveSetTempFormWithAddedPauseUntilTemp with falsey unsavedForm. This should never be able to happen.');
    return;
  }

  const {
    id
  } = unsavedSetTemperatureForm;
  ...
  if (!isPristineSetTempForm) {
    ...
    assert(false, `tried to saveSetTempFormWithAddedPauseUntilTemp but form ${id} is not a pristine set temp form`);
    return;
  }

  const temperature = unsavedSetTemperatureForm?.targetTemperature;
  assert(temperature != null && temperature !== '', `tried to auto-add a pause until temp, but targetTemperature is missing: ${temperature}`);
