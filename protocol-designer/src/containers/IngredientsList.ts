...
function mapStateToProps(state: BaseState): SP {
  ...
  const labwareWellContents = selectedLabwareId && labwareIngredSelectors.getLiquidsByLabwareId(state)[selectedLabwareId] || {};
  return {
...
function mergeProps(stateProps: SP, dispatchProps: {
  ...
  return { ...passThruProps,
    removeWellsContents: args => {
      if (_labwareId) {
        dispatch(removeWellsContents({ ...args,
...
export const IngredientsList: React.AbstractComponent<{}> = connect<Props, {}, SP, {}, _, _>(mapStateToProps, null, mergeProps)(IngredientsListComponent);