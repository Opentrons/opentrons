...
import { getSelectedStepTitleInfo, getSelectedTerminalItemId, getWellSelectionLabwareKey, actions as stepsActions } from "../ui/steps";
...
type TitleWithBetaTagProps = {
  text?: string | null | undefined;
};

const TitleWithBetaTag = (props: TitleWithBetaTagProps) => <div className={styles.title_wrapper}>
...
function mapStateToProps(state: BaseState): SP {
  ...
  const labwareNickname = selectedLabwareId != null ? labwareNames[selectedLabwareId].replace('µL', 'uL') : null;
  const labwareEntity = selectedLabwareId != null ? stepFormSelectors.getLabwareEntities(state)[selectedLabwareId] : null;
    ...
    default:
      {
        ...
        if (liquidPlacementMode) {
          return {
            ...
            // TODO(mc, 2019-06-27): µL to uL replacement needed to handle CSS capitalization
            subtitle: labwareEntity && getLabwareDisplayName(labwareEntity.def).replace('µL', 'uL'),
        ...
        if (selectedTerminalId === START_TERMINAL_ITEM_ID) {
          ...
          if (drilledDownLabwareId) {
            backButtonLabel = 'Deck';
            const labwareDef = stepFormSelectors.getLabwareEntities(state)[drilledDownLabwareId].def;
            const nickname = uiLabwareSelectors.getLabwareNicknamesById(state)[drilledDownLabwareId];
            // TODO(mc, 2019-06-27): µL to uL replacement needed to handle CSS capitalization
            title = nickname.replace('µL', 'uL');
            subtitle = labwareDef && getLabwareDisplayName(labwareDef).replace('µL', 'uL');
          }
        } else if (selectedStepInfo) {
          const stepTitle = selectedStepInfo.stepName || i18n.t(`application.stepType.${selectedStepInfo.stepType}`);
          ...
          if (wellSelectionLabwareKey) {
            ...
            return {
              ...
              title: <TitleWithIcon iconName={stepIconsByType[selectedStepInfo.stepType]} text={stepTitle} />,
...
const StickyTitleBar = (props: TitleBarProps) => <TitleBar id="TitleBar_main" {...props} className={styles.sticky_bar} />;

export const ConnectedTitleBar: React.AbstractComponent<{}> = connect<Props, {}, SP, {}, _, _>(mapStateToProps, null, mergeProps)(StickyTitleBar);