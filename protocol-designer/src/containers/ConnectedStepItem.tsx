...
import { getCollapsedSteps, getHoveredSubstep, getHoveredStepId, getSelectedStepId, getMultiSelectItemIds, getMultiSelectLastSelected, getIsMultiSelectMode, actions as stepsActions } from "../ui/steps";
...
import { ConfirmDeleteModal, CLOSE_STEP_FORM_WITH_CHANGES, CLOSE_UNSAVED_STEP_FORM, CLOSE_BATCH_EDIT_FORM } from "../components/modals/ConfirmDeleteModal";
import type { SubstepIdentifier } from "../steplist/types";
import type { StepIdType } from "../form-types";
type Props = {
  stepId: StepIdType;
  stepNumber: number;
  onStepContextMenu?: () => unknown;
};

const nonePressed = (keysPressed: Array<boolean>): boolean => keysPressed.every(keyPress => keyPress === false);

const getUserOS = () => new UAParser().getOS().name;

const getMouseClickKeyInfo = (event: React.MouseEvent): {
  isShiftKeyPressed: boolean;
  isMetaKeyPressed: boolean;
} => {
  const isMac: boolean = getUserOS() === 'Mac OS';
  const isShiftKeyPressed: boolean = event.shiftKey;
  const isMetaKeyPressed: boolean = isMac && event.metaKey || !isMac && event.ctrlKey;
  return {
    isShiftKeyPressed,
    isMetaKeyPressed
  };
...
export const ConnectedStepItem = (props: Props): React.ReactNode => {
  const {
    stepId,
    stepNumber
  } = props;
  const step = useSelector(stepFormSelectors.getSavedStepForms)[stepId];
  const argsAndErrors = useSelector(stepFormSelectors.getArgsAndErrorsByStepId)[stepId];
  ...
  const hasTimelineWarningsPerStep = useSelector(timelineWarningSelectors.getHasTimelineWarningsPerStep);
  const hasFormLevelWarningsPerStep = useSelector(dismissSelectors.getHasFormLevelWarningsPerStep);
  const hasWarnings = hasTimelineWarningsPerStep[stepId] || hasFormLevelWarningsPerStep[stepId];
  ...
  const selected: boolean = multiSelectItemIds?.length ? multiSelectItemIds.includes(stepId) : selectedStepId === stepId;
  ...
  const labwareNicknamesById = useSelector(uiLabwareSelectors.getLabwareNicknamesById);
  const currentFormIsPresaved = useSelector(stepFormSelectors.getCurrentFormIsPresaved);
  const singleEditFormHasUnsavedChanges = useSelector(stepFormSelectors.getCurrentFormHasUnsavedChanges);
  const batchEditFormHasUnsavedChanges = useSelector(stepFormSelectors.getBatchEditFormHasUnsavedChanges);
  // Actions
  const dispatch = useDispatch();

  const highlightSubstep = (payload: SubstepIdentifier) => dispatch(stepsActions.hoverOnSubstep(payload));

  const selectStep = () => dispatch(stepsActions.selectStep(stepId));

  const selectMultipleSteps = (steps: Array<StepIdType>, lastSelected: StepIdType) => dispatch(stepsActions.selectMultipleSteps(steps, lastSelected));

  const toggleStepCollapsed = () => dispatch(stepsActions.toggleStepCollapsed(stepId));
  ...
  const handleStepItemSelection = (event: React.MouseEvent): void => {
    ...
    const noModifierKeys = nonePressed([isShiftKeyPressed, isMetaKeyPressed]) || toggledLastSelected;
    ...
    } else if ((isMetaKeyPressed || isShiftKeyPressed) && currentFormIsPresaved) {
      // current form is presaved, enter batch edit mode with only the clicked
      stepsToSelect = [stepId];
    } else {
      if (isShiftKeyPressed) {
        stepsToSelect = getShiftSelectedSteps(selectedStepId, orderedStepIds, stepId, multiSelectItemIds, lastMultiSelectedStepId);
      } else if (isMetaKeyPressed) {
        stepsToSelect = getMetaSelectedSteps(multiSelectItemIds, stepId, selectedStepId);
      }
    }
  ...
  const {
    ...
    showConfirmation,
    cancel
  } = useConditionalConfirm(handleStepItemSelection, isMultiSelectMode ? batchEditFormHasUnsavedChanges : currentFormIsPresaved || singleEditFormHasUnsavedChanges);
  ...
  return <>
      {showConfirmation && <ConfirmDeleteModal modalType={getModalType()} onContinueClick={confirmWithPersistedEvent} onCancelClick={cancel} />}
      <StepItem {...stepItemProps} onStepContextMenu={props.onStepContextMenu}>
        <StepItemContents {...stepItemContentsProps} />
      </StepItem>
    </>;
};
export function getMetaSelectedSteps(multiSelectItemIds: Array<StepIdType> | null, stepId: StepIdType, selectedStepId: StepIdType | null): Array<StepIdType> {
  let stepsToSelect: Array<StepIdType> = [];

  if (multiSelectItemIds?.length) {
    // already have a selection, add/remove the meta-clicked item
    stepsToSelect = multiSelectItemIds.includes(stepId) ? multiSelectItemIds.filter(id => id !== stepId) : [...multiSelectItemIds, stepId];
...
function getShiftSelectedSteps(selectedStepId, orderedStepIds, stepId, multiSelectItemIds, lastMultiSelectedStepId) {
  let stepsToSelect: Array<StepIdType>;

  if (selectedStepId) {
    stepsToSelect = getOrderedStepsInRange(selectedStepId, stepId, orderedStepIds);
  } else if (multiSelectItemIds?.length && lastMultiSelectedStepId) {
    const potentialStepsToSelect = getOrderedStepsInRange(lastMultiSelectedStepId, stepId, orderedStepIds);
    const allSelected: boolean = potentialStepsToSelect.slice(1).every(stepId => multiSelectItemIds.includes(stepId));

    if (allSelected) {
      // if they're all selected, deselect them all
      if (multiSelectItemIds.length - potentialStepsToSelect.length > 0) {
        stepsToSelect = multiSelectItemIds.filter(id => !potentialStepsToSelect.includes(id));
      } else {
...
function getOrderedStepsInRange(lastSelectedStepId: StepIdType, stepId: StepIdType, orderedStepIds: Array<StepIdType>) {
