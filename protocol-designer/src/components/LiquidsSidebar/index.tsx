...
function LiquidsSidebarComponent(props: Props) {
  ...
  return <SidePanel title="Liquids">
      {liquids.map(({
      ingredientId,
      name
    }) => <PDTitledList key={ingredientId} selected={selectedLiquid === ingredientId} onClick={() => selectLiquid(ingredientId)} iconName="circle" iconProps={{
      style: {
        fill: swatchColors(ingredientId)
      }
...
function mapDispatchToProps(dispatch: ThunkDispatch<any>): DP {
  return {
    selectLiquid: liquidGroupId => dispatch(labwareIngredActions.selectLiquidGroup(liquidGroupId)),
    createNewLiquid: () => dispatch(labwareIngredActions.createNewLiquidGroup())
  };
}

export const LiquidsSidebar: React.AbstractComponent<{}> = connect<Props, {}, SP, DP, _, _>(mapStateToProps, mapDispatchToProps)(LiquidsSidebarComponent);