...
import { FormGroup, Text, Tooltip, useHoverTooltip, FONT_WEIGHT_SEMIBOLD, FONT_SIZE_BODY_1, C_LIGHT_GRAY } from "@opentrons/components";
...
export const WellOrderField = (props: Props): React.ReactNode => {
  const {
    firstValue,
    secondValue,
    firstName,
  ...
  return <>
      ...
      <div {...targetProps}>
        <FormGroup label={props.label} className={className}>
          <WellOrderModal prefix={props.prefix} closeModal={handleClose} isOpen={isModalOpen} updateValues={updateValues} firstValue={firstValue} secondValue={secondValue} firstName={firstName} secondName={secondName} />
          {firstValue != null && secondValue != null ? <img onClick={handleOpen} src={ZIG_ZAG_IMAGE} className={cx(styles.well_order_icon, {
          [styles.icon_with_label]: props.label
        }, getIconClassNames())} id={`WellOrderField_button_${props.prefix}`} data-test={`WellOrderField_button_${String(firstValue)}_${String(secondValue)}`} /> : <Text onClick={handleOpen} css={mixedWellOrderStyles} id={`WellOrderField_button_${props.prefix}`} data-test={`WellOrderField_button_${String(firstValue)}_${String(secondValue)}`}>
