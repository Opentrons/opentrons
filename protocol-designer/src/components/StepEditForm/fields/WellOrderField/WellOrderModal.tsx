...
import { Modal, OutlineButton, PrimaryButton, FormGroup, DropdownField } from "@opentrons/components";
...
type Props = {
  ...
  firstName: string;
  secondName: string;
  updateValues: (firstValue: WellOrderOption | null | undefined, secondValue: WellOrderOption | null | undefined) => void;
};
type State = {
...
export const ResetButton = (props: {
  onClick: () => void;
}): React.ReactNode => <OutlineButton className={modalStyles.button_medium} onClick={props.onClick}>
    {i18n.t('button.reset')}
  </OutlineButton>;
export const CancelButton = (props: {
  onClick: () => void;
}): React.ReactNode => <PrimaryButton className={cx(modalStyles.button_medium, modalStyles.button_right_of_break)} onClick={props.onClick}>
    {i18n.t('button.cancel')}
  </PrimaryButton>;
export const DoneButton = (props: {
  onClick: () => void;
}): React.ReactNode => <PrimaryButton className={modalStyles.button_medium} onClick={props.onClick}>
    {i18n.t('button.done')}
  </PrimaryButton>;
export class WellOrderModal extends React.Component<Props, State> {
  ...
  makeOnChange: (ordinality: "first" | "second") => (event: React.SyntheticEvent<HTMLSelectElement>) => void = ordinality => event => {
    const {
      value
    } = event.currentTarget;
    let nextState = {
      [`${ordinality}Value`]: value
    };

    if (ordinality === 'first') {
      if (VERTICAL_VALUES.includes(value) && VERTICAL_VALUES.includes(this.state.secondValue)) {
        nextState = { ...nextState,
          secondValue: HORIZONTAL_VALUES[0]
        };
      } else if (HORIZONTAL_VALUES.includes(value) && HORIZONTAL_VALUES.includes(this.state.secondValue)) {
        nextState = { ...nextState,
          secondValue: VERTICAL_VALUES[0]
        };
      }
    }
  ...
  isSecondOptionDisabled: (arg0: WellOrderOption) => boolean = (value: WellOrderOption) => {
  ...
  render(): React.ReactNode {
    ...
    return <Portal>
        <Modal className={modalStyles.modal} contentsClassName={cx(modalStyles.modal_contents)} onCloseClick={this.handleCancel}>
          ...
          <div className={styles.main_row}>
            <FormGroup label={i18n.t('modal.well_order.field_label')}>
              <div className={styles.field_row}>
                <DropdownField name={firstName} value={firstValue} className={cx(stepEditStyles.field, styles.well_order_dropdown)} onChange={this.makeOnChange('first')} options={WELL_ORDER_VALUES.map(value => ({
                value,
                ...
                <DropdownField name={secondName} value={secondValue} className={cx(stepEditStyles.field, styles.well_order_dropdown)} onChange={this.makeOnChange('second')} options={WELL_ORDER_VALUES.map(value => ({
                value,
                name: i18n.t(`form.step_edit_form.field.well_order.option.${value}`),
