...
const getFieldDefaultTooltipSpy = jest.spyOn(stepEditFormUtils, 'getFieldDefaultTooltip');
const getSingleSelectDisabledTooltipSpy = jest.spyOn(stepEditFormUtils, 'getSingleSelectDisabledTooltip');
const getDisabledFieldsMock: JestMockFn<any, Set<string>> = getDisabledFields;
const getDefaultsForStepTypeMock: JestMockFn<[any], any> = getDefaultsForStepType;
const getFieldErrorsMock: JestMockFn<[string, unknown], Array<string>> = getFieldErrors;
beforeEach(() => {
...
describe('makeSingleEditFieldProps', () => {
  it('should create correct props for all fields in the given stepType', () => {
    ...
    getFieldErrorsMock.mockImplementation((name, value) => {
      ...
      if (name === 'pristine_error_field' || name === 'dirty_error_field' || name === 'focused_error_field') {
    ...
    const result = makeSingleEditFieldProps(focusHandlers, formData, handleChangeFormInput);
    ...
    ;
    ['some_field', 'disabled_field', 'pristine_error_field', 'dirty_error_field', 'focused_error_field'].forEach(name => {
      const {
        onFieldBlur,
        onFieldFocus,
        updateValue
