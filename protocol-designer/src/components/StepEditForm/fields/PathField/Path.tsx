...
const PathButton = (buttonProps: ButtonProps) => {
  ...
  const pathButtonData = `PathButton_${selected ? 'selected' : 'deselected'}_${disabled ? 'disabled' : 'enabled'}`;
  return <>
      {tooltip}
      <li {...targetProps} className={cx(styles.path_option, {
      [styles.selected]: selected,
      [styles.disabled]: disabled
    })} onClick={disabled ? null : onClick} id={id} data-test={pathButtonData}>
        {children}
      </li>
    </>;
};

const getSubtitle = (path: PathOption, disabledPathMap: DisabledPathMap): string => {
  const reasonForDisabled = disabledPathMap && disabledPathMap[path];
  return reasonForDisabled || '';
};

export const Path = (props: PathFieldProps): React.ReactNode => {
  const {
    disabledPathMap,
    value,
    updateValue
  } = props;
  return <FormGroup label="Path">
      <ul className={styles.path_options}>
        {ALL_PATH_OPTIONS.map(option => <PathButton id={`PathButton_${option.name}`} key={option.name} selected={option.name === value} path={option.name} disabled={disabledPathMap !== null && option.name in disabledPathMap} subtitle={getSubtitle(option.name, disabledPathMap)} onClick={() => updateValue(option.name)}>
