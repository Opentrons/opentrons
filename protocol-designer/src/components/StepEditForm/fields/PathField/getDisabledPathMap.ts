...
import { volumeInCapacityForMultiDispense, volumeInCapacityForMultiAspirate } from "../../../../steplist/formLevel/handleFormChange/utils";
...
export function getDisabledPathMap(values: ValuesForPath, pipetteEntities: PipetteEntities): DisabledPathMap {
  const {
  ...
  if (changeTip === 'perDest') {
    disabledPathMap = { ...disabledPathMap,
      multiDispense: i18n.t('form.step_edit_form.field.path.subtitle.incompatible_with_per_dest')
    };
  } else if (changeTip === 'perSource') {
    disabledPathMap = { ...disabledPathMap,
      multiAspirate: i18n.t('form.step_edit_form.field.path.subtitle.incompatible_with_per_source')
    };
  }
