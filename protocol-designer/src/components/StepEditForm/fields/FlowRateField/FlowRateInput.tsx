...
import { AlertModal, FormGroup, RadioGroup, InputField } from "@opentrons/components";
...
export const FlowRateInput = (props: FlowRateInputProps): React.ReactNode => {
  const {
    className,
    defaultFlowRate,
    disabled,
  ...
  const [isPristine, setIsPristine] = React.useState<$PropertyType<State, "isPristine">>(initialState.isPristine);
  const [modalFlowRate, setModalFlowRate] = React.useState<$PropertyType<State, "modalFlowRate">>(initialState.modalFlowRate);
  const [modalUseDefault, setModalUseDefault] = React.useState<$PropertyType<State, "modalUseDefault">>(initialState.modalUseDefault);
  const [showModal, setShowModal] = React.useState<$PropertyType<State, "showModal">>(initialState.showModal);
  ...
  const handleChangeRadio = (e: React.SyntheticEvent): void => {
    setModalUseDefault(e.target.value !== 'custom');
  };

  const handleChangeNumber = (e: React.SyntheticEvent) => {
  ...
  const outOfBounds = modalFlowRateNum === 0 || minFlowRate > modalFlowRateNum || modalFlowRateNum > maxFlowRate;
  ...
  if (!modalUseDefault) {
    if (!Number.isNaN(modalFlowRateNum) && !correctDecimals) {
      errorMessage = `a max of ${DECIMALS_ALLOWED} decimal place${DECIMALS_ALLOWED > 1 ? 's' : ''} is allowed`;
    } else if (!isPristine && outOfBounds) {
      errorMessage = `accepted range is ${displayMinFlowRate} to ${maxFlowRate}`;
    }
  }

  const FlowRateInputField = <InputField disabled={disabled} caption={rangeDescription} error={errorMessage} isIndeterminate={isIndeterminate && modalFlowRate === null} name={`${name}_customFlowRate`} onChange={handleChangeNumber} units={i18n.t('application.units.microliterPerSec')} value={`${modalFlowRate || ''}`} />;
  const FlowRateModal = pipetteDisplayName && <Portal>
      <AlertModal alertOverlay className={modalStyles.modal} buttons={[{
      children: 'Cancel',
      onClick: cancelModal
    }, {
      children: 'Done',
      ...
      disabled: isPristine ? false : !allowSave
        ...
        <RadioGroup inline value={modalUseDefault ? 'default' : 'custom'} onChange={handleChangeRadio} options={[{
        name: `${defaultFlowRate || '?'} ${i18n.t('application.units.microliterPerSec')} (default)`,
        value: 'default'
      }, {
        name: 'Custom',
        value: 'custom',
  ...
  return <React.Fragment>
      <FormGroup label={label || DEFAULT_LABEL} disabled={disabled}>
        <InputField className={className || stepFormStyles.small_field} disabled={disabled} isIndeterminate={isIndeterminate} name={name} onClick={openModal} readOnly units={i18n.t('application.units.microliterPerSec')} value={props.value ? String(props.value) : 'default'} />
      </FormGroup>
