...
export const getDisabledChangeTipOptions = (args: DisabledChangeTipArgs): Set<ChangeTipOptions> | null | undefined => {
  const {
    path,
    aspirateWells,
    dispenseWells,
  ...
  switch (stepType) {
    ...
    default:
      {
        console.warn(`getChangeTipOptions for stepType ${String(stepType)} not yet implemented!`);
        return null;
      }
  }
};