...
import { AlertModal, Flex, HandleKeypress, Icon, InputField, OutlineButton, RadioGroup } from "@opentrons/components";
...
type Props = {
  ...
  name: StepFieldName;
  updateValue: (arg0: number | null | undefined) => unknown;
  wellDepthMm: number;
};
...
export const TipPositionModal = (props: Props): React.ReactNode => {
  const {
    isIndeterminate,
    name,
    wellDepthMm
  ...
  const [value, setValue] = React.useState<string | null>(props.mmFromBottom === null ? null : String(props.mmFromBottom));
  const [isDefault, setIsDefault] = React.useState<boolean>(!isIndeterminate && props.mmFromBottom === null);
  ...
  const hasVisibleErrors = isPristine ? errors.includes(TOO_MANY_DECIMALS) : hasErrors;
  ...
  const handleChange = (newValueRaw: string | number): void => {
    ...
    const newValue = typeof newValueRaw === 'string' ? newValueRaw.replace(/[^.0-9]/, '') : String(newValueRaw);
  ...
  const handleInputFieldChange = (e: React.SyntheticEvent<HTMLInputElement>): void => {
  ...
  const TipPositionInputField = !isDefault && <InputField caption={`between ${minMmFromBottom} and ${maxMmFromBottom}`} className={styles.position_from_bottom_input} error={errorText} id={'TipPositionModal_custom_input'} isIndeterminate={value === null && isIndeterminate} onChange={handleInputFieldChange} units="mm" value={value !== null ? String(value) : ''} />;
  // Mix Form's asp/disp tip position field has different default value text
  const isMixAspDispField = name === 'mix_mmFromBottom';
  return <Portal>
      <HandleKeypress preventDefault handlers={[{
      key: 'ArrowUp',
      shiftKey: false,
      ...
      onPress: makeHandleDecrement(LARGE_STEP_MM)
        ...
        <AlertModal alertOverlay buttons={[{
        ...
        disabled: hasVisibleErrors
      }]} className={modalStyles.modal} contentsClassName={cx(modalStyles.modal_contents)} onCloseClick={handleCancel}>
          ...
          <div className={styles.main_row}>
            <Flex alignItems="flex-start">
              <div>
                <RadioGroup value={isDefault ? 'default' : 'custom'} onChange={e => {
                setIsDefault(e.currentTarget.value === 'default');
              }} options={[{
                name: isMixAspDispField ? `Aspirate 1mm, Dispense 0.5mm from the bottom (default)` : `${defaultMmFromBottom} mm from the bottom (default)`,
                value: 'default'
              }, {
                name: 'Custom',
                value: 'custom'
              ...
              <div className={styles.viz_group}>
                {!isDefault && <div className={styles.adjustment_buttons}>
                    <OutlineButton id="Increment_tipPosition" className={styles.adjustment_button} onClick={makeHandleIncrement(SMALL_STEP_MM)}>
                      <Icon name="plus" />
                    </OutlineButton>
                    <OutlineButton id="Decrement_tipPosition" className={styles.adjustment_button} onClick={makeHandleDecrement(SMALL_STEP_MM)}>
                      <Icon name="minus" />
                    </OutlineButton>
                  </div>}
                <TipPositionZAxisViz mmFromBottom={value !== null ? Number(value) : defaultMmFromBottom} wellDepthMm={wellDepthMm} />
              </div>
            </Flex>
          </div>
        </AlertModal>
