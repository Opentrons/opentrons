...
import { getDisabledFields, getDefaultsForStepType } from "../../../steplist/formLevel";
import { getFieldDefaultTooltip, getSingleSelectDisabledTooltip } from "../utils";
...
export const showFieldErrors = ({
  ...
  focusedField,
  dirtyFields
}: ShowFieldErrorParams): boolean | void | Array<StepFieldName> => !(name === focusedField) && dirtyFields && dirtyFields.includes(name);
export const makeSingleEditFieldProps = (focusHandlers: FocusHandlers, formData: FormData, handleChangeFormInput: (name: string, value: unknown) => void): FieldPropsByName => {
  const {
    dirtyFields,
    blur,
    focusedField,
  ...
  const fieldNames: Array<string> = Object.keys(getDefaultsForStepType(formData.stepType));
  return fieldNames.reduce<FieldPropsByName>((acc, name) => {
    ...
    const disabledTooltip = getSingleSelectDisabledTooltip(name, formData.stepType);
