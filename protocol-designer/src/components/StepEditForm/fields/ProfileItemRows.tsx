...
import { Icon, InputField, OutlineButton, Tooltip, useConditionalConfirm, useHoverTooltip, TOOLTIP_TOP, TOOLTIP_TOP_END } from "@opentrons/components";
import { i18n } from "../../../localization";
import * as steplistActions from "../../../steplist/actions";
import { PROFILE_CYCLE } from "../../../form-types";
import { getProfileFieldErrors, maskProfileField } from "../../../steplist/fieldLevel";
import { ConfirmDeleteModal, DELETE_PROFILE_CYCLE } from "../../modals/ConfirmDeleteModal";
...
}): boolean => !(fieldId === focusedField) && dirtyFields && dirtyFields.includes(fieldId);
type ProfileCycleRowProps = {
  cycleItem: ProfileCycleItem;
  focusHandlers: FocusHandlers;
  stepOffset: number;
};
export const ProfileCycleRow = (props: ProfileCycleRowProps): React.ReactNode => {
  const {
    cycleItem,
    focusHandlers,
    stepOffset
  ...
  const addStepToCycle = () => {
    dispatch(steplistActions.addProfileStep({
      cycleId: cycleItem.id
    }));
  };

  // TODO IMMEDIATELY make conditional
  const deleteProfileCycle = () => dispatch(steplistActions.deleteProfileCycle({
    id: cycleItem.id
  }));

  const [addStepToCycleTargetProps, addStepToCycleTooltipProps] = useHoverTooltip({
  ...
  return <>
      {showConfirmDeleteCycle && <ConfirmDeleteModal modalType={DELETE_PROFILE_CYCLE} onContinueClick={confirmDeleteCycle} onCancelClick={cancelConfirmDeleteCycle} />}

      <div className={styles.profile_cycle_wrapper}>
        <div className={styles.profile_cycle_group}>
          {cycleItem.steps.length > 0 && <div className={styles.cycle_steps}>
              <div className={styles.cycle_row}>
                {cycleItem.steps.map((stepItem, index) => {
              return <ProfileStepRow profileStepItem={stepItem} focusHandlers={focusHandlers} key={stepItem.id} stepNumber={stepOffset + index} isCycle />;
            })}
              </div>

              <ProfileField name="repetitions" focusHandlers={focusHandlers} profileItem={cycleItem} units={i18n.t('application.units.cycles')} className={cx(styles.small_field, styles.cycles_field)} updateValue={(name, value) => dispatch(steplistActions.editProfileCycle({
            id: cycleItem.id,
            fields: {
              [name]: value
            }
...
export const ProfileItemRows = (props: ProfileItemRowsProps): React.ReactNode => {
  const {
    orderedProfileItems,
    profileItemsById
  } = props;
  ...
  const rows = orderedProfileItems.map((itemId, index) => {
    ...
    if (itemFields.type === PROFILE_CYCLE) {
      const cycleRow = <ProfileCycleRow cycleItem={itemFields} focusHandlers={props.focusHandlers} key={itemId} stepOffset={counter + 1} />;
      counter += itemFields.steps.length;
      return cycleRow;
    }

    counter++;
    return <ProfileStepRow profileStepItem={itemFields} focusHandlers={props.focusHandlers} key={itemId} stepNumber={counter} />;
  });
  return <>
      {rows.length > 0 && <div className={styles.profile_step_labels}>
          <div>Name:</div>
          <div>Temperature:</div>
          <div>Time:</div>
        </div>}
      ...
      <div className={styles.profile_button_group}>
        <OutlineButton hoverTooltipHandlers={addStepTargetProps} onClick={addProfileStep}>
          {i18n.t('form.step_edit_form.field.thermocyclerProfile.add_step_button')}
        </OutlineButton>
        <OutlineButton hoverTooltipHandlers={addCycleTargetProps} onClick={addProfileCycle}>
          {i18n.t('form.step_edit_form.field.thermocyclerProfile.add_cycle_button')}
        </OutlineButton>
      </div>
    </>;
};
...
const ProfileField = (props: ProfileFieldProps) => {
  ...
  return <div className={styles.step_input_wrapper}>
      <InputField className={cx(styles.step_input, className)} error={errorToShow} units={units} {...{
      name,
      onChange,
      onBlur,
      onFocus,
...
const ProfileStepRow = (props: ProfileStepRowProps) => {
  ...
  const updateStepFieldValue = (name: string, value: unknown) => {
    dispatch(steplistActions.editProfileStep({
  ...
  const deleteProfileStep = () => {
    dispatch(steplistActions.deleteProfileStep({
      id: profileStepItem.id
    }));
  };
  ...
  const fields = names.map(name => {
    ...
    return <ProfileField key={name} units={units[name]} className={className} {...{
      name,
      focusHandlers,
  ...
  return <div className={cx(styles.profile_step_row, {
    [styles.cycle]: isCycle
  })}>
      <div className={cx(styles.profile_step_fields, {
      [styles.profile_cycle_fields]: isCycle
    })}>
        <span className={styles.profile_step_number}>{props.stepNumber}. </span>
        {fields}
      </div>
      <div onClick={deleteProfileStep} className={cx({
      [styles.cycle_step_delete]: isCycle
    })} {...targetProps}>
