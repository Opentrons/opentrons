...
export const StepFormDropdown = (props: StepFormDropdownProps): React.ReactNode => {
  const {
    options,
    name,
    className,
  ...
  return <DropdownField name={name} error={errorToShow} className={cx(styles.large_field, className)} options={options} onBlur={onFieldBlur} onFocus={onFieldFocus} value={fieldValue} onChange={(e: React.SyntheticEvent<HTMLSelectElement>) => {
