...
import type { FormData, ProfileItem, StepFieldName, StepType, PathOption } from "../../form-types";
...
export const getDirtyFields = (isNewStep: boolean, formData: FormData | null | undefined): Array<string> => {
  let dirtyFields = [];
...
export const getVisibleFormErrors = (args: {
  ...
  return errors.filter(error => {
    const dependentFieldsAreNotFocused = !error.dependentFields.includes(focusedField);
    const dependentFieldsAreDirty = difference(error.dependentFields, dirtyFields).length === 0;
  ...
  return errors.filter(error => {
    return profileItemIds.every(itemId => {
      ...
      return steps.every(step => {
        const fieldsForStep = error.dependentProfileFields.map(fieldName => getDynamicFieldFocusHandlerId({
          id: step.id,
          name: fieldName
        }));
        const dependentFieldsAreNotFocused = !fieldsForStep.includes(focusedField);
        const dependentProfileFieldsAreDirty = difference(fieldsForStep, dirtyFields).length === 0;
        return dependentFieldsAreNotFocused && dependentProfileFieldsAreDirty;
      });
    });
  });
};
export const getFieldDefaultTooltip = (name: string): string => i18n.t([`tooltip.step_fields.defaults.${name}`, '']);
export const getFieldIndeterminateTooltip = (name: string): string => i18n.t([`tooltip.step_fields.indeterminate.${name}`, '']);
export const getSingleSelectDisabledTooltip = (name: string, stepType: string): string => i18n.t([`tooltip.step_fields.${stepType}.disabled.${name}`, `tooltip.step_fields.${stepType}.disabled.$generic`, '']);
...
export function getLabwareFieldForPositioningField(name: StepFieldName): StepFieldName {
