...
import { ConfirmDeleteModal, DELETE_STEP_FORM, CLOSE_STEP_FORM_WITH_CHANGES, CLOSE_UNSAVED_STEP_FORM } from "../modals/ConfirmDeleteModal";
...
const StepEditFormManager = (props: StepEditFormManagerProps) => {
  ...
  const [showMoreOptionsModal, setShowMoreOptionsModal] = React.useState<boolean>(false);
  const [focusedField, setFocusedField] = React.useState<string | null>(null);
  const [dirtyFields, setDirtyFields] = React.useState<Array<StepFieldName>>(getDirtyFields(isNewStep, formData));
  ...
  const handleDelete = () => {
    if (stepId != null) {
      ...
      console.error(`StepEditForm: tried to delete step with no step id, this should not happen`);
    }
  };
  ...
  } = useConditionalConfirm(saveSetTempFormWithAddedPauseUntilTemp, isPristineSetTempForm);
  ...
  const propsForFields = makeSingleEditFieldProps(focusHandlers, formData, handleChangeFormInput);
  return <>
      {showConfirmDeleteModal && <ConfirmDeleteModal modalType={DELETE_STEP_FORM} onCancelClick={cancelDelete} onContinueClick={confirmDelete} />}
      {showConfirmCancelModal && <ConfirmDeleteModal modalType={isNewStep ? CLOSE_UNSAVED_STEP_FORM : CLOSE_STEP_FORM_WITH_CHANGES} onCancelClick={cancelClose} onContinueClick={confirmClose} />}
      {showAddPauseUntilTempStepModal && <AutoAddPauseUntilTempStepModal displayTemperature={formData?.targetTemperature ?? '?'} handleCancelClick={saveStepForm} handleContinueClick={confirmAddPauseUntilTempStep} />}
      ...
      handleSave: isPristineSetTempForm ? confirmAddPauseUntilTempStep : saveStepForm,
      propsForFields,
...
const mapStateToProps = (state: BaseState): SP => {
  return {
    ...
    isPristineSetTempForm: stepFormSelectors.getUnsavedFormIsPristineSetTempForm(state)
  };
};
...
const mapDispatchToProps = (dispatch: ThunkDispatch<any>): DP => {
  ...
  const saveSetTempFormWithAddedPauseUntilTemp = () => dispatch(stepsActions.saveSetTempFormWithAddedPauseUntilTemp());
  ...
  const handleChangeFormInput = (name: string, value: unknown) => {
    ...
    dispatch(actions.changeFormInput({
      update: {
        [name]: maskedValue
      }
    }));
...
export const StepEditForm: React.AbstractComponent<{}> = connect<StepEditFormManagerProps, {}, _, _, _, _>(mapStateToProps, mapDispatchToProps)((props: StepEditFormManagerProps) => // key by ID so manager state doesn't persist across different forms
