...
import { getVisibleFormErrors, getVisibleFormWarnings, getVisibleProfileFormLevelErrors } from "./utils";
...
const mapStateToProps = (state: BaseState, ownProps: OP): SP => {
  ...
  const formLevelErrors = stepFormSelectors.getFormLevelErrorsForUnsavedForm(state);
  ...
  const {
    profileItemsById
  } = stepFormSelectors.getHydratedUnsavedForm(state);
  let visibleDynamicFieldFormErrors = [];

  if (profileItemsById != null) {
    const dynamicFieldFormErrors = stepFormSelectors.getDynamicFieldFormErrorsForUnsavedForm(state);
  ...
  return {
    errors: [...visibleErrors.map(error => ({
      title: error.title,
...
const mergeProps = (stateProps: SP, dispatchProps: {
  ...
  return { ...stateProps,
    dismissWarning: (dismissId: string) => {
      if (stepId) dispatch(dismissActions.dismissFormWarning({
        type: dismissId,
        stepId
      }));
    }
...
export const FormAlerts: React.AbstractComponent<OP> = connect<Props, OP, SP, {}, _, _>(mapStateToProps, null, mergeProps)(Alerts);