...
describe('MixForm', () => {
  ...
  const render = _props => mount(<MixForm {..._props} />, {
  ...
  describe('when advanced settings are visible', () => {
    it('should render the aspirate delay fields', () => {
      ...
      expect(aspirateDelayFields.prop('checkboxFieldName')).toBe('aspirate_delay_checkbox');
      expect(aspirateDelayFields.prop('secondsFieldName')).toBe('aspirate_delay_seconds');
    ...
    it('should render the dispense delay fields', () => {
      ...
      expect(aspirateDelayFields.prop('checkboxFieldName')).toBe('dispense_delay_checkbox');
      expect(aspirateDelayFields.prop('secondsFieldName')).toBe('dispense_delay_seconds');
      // no tip position field
      expect(aspirateDelayFields.prop('tipPositionFieldName')).toBe(undefined);
    });
    it('should render the mix well order field', () => {
      ...
      expect(wellOrderField.props()).toMatchObject({
        ...
        firstValue: 'r2l',
        secondValue: 'b2t',
        updateFirstWellOrder: props.propsForFields['mix_wellOrder_first'].updateValue,
        updateSecondWellOrder: props.propsForFields['mix_wellOrder_second'].updateValue
      });
    });
  });
});