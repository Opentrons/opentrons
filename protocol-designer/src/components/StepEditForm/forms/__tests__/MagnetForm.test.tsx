...
const getUnsavedFormMock: JestMockFn<[BaseState], any> = stepFormSelectors.getUnsavedForm;
...
describe('MagnetForm', () => {
  ...
  beforeEach(() => {
    ...
    fields.ConditionalOnField = jest.fn().mockImplementation(props => <div>{props.children}</div>);
    fields.TextField = jest.fn().mockImplementation(props => <div />);
    fields.RadioGroupField = jest.fn().mockImplementation(props => <div />);
    (uiModuleSelectors.getMagneticLabwareOptions as JestMockFn<[BaseState], Options>).mockReturnValue([{
      name: 'magnet module v1',
      value: 'magnetV1',
      disabled: false
    }, {
      name: 'magnet module v2',
      value: 'magnetV2',
      disabled: false
    }]);
  ...
  it('engage height caption is displayed with proper height to decimal scale', () => {
    ;
    (uiModuleSelectors.getMagnetLabwareEngageHeight as JestMockFn<[BaseState], number | null>).mockReturnValue(10.9444);
    const wrapper = render(props);
    expect(wrapper.find(fields.TextField).prop('caption')).toEqual('Recommended: 10.9');
  });
  it('engage height caption is null when no engage height', () => {
    ;
    (uiModuleSelectors.getMagnetLabwareEngageHeight as JestMockFn<[BaseState], number | null>).mockReturnValue(null);
    const wrapper = render(props);
  ...
  models.forEach(({
    ...
    it(`should show appropriate engage height image for ${model}`, () => {
      ...
      expect(wrapper.find('.engage_height_diagram').hasClass(`engage_height_diagram_gen${modelNum}`)).toBe(true);
    });
  });
});