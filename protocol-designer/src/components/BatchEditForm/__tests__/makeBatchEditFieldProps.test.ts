...
const getIndeterminateTooltipSpy = jest.spyOn(stepEditFormUtils, 'getFieldIndeterminateTooltip');
beforeEach(() => {
...
describe('makeBatchEditFieldProps', () => {
  it('should create correct props for all fields with the given MultiselectFieldValues obj', () => {
    ...
    const result = makeBatchEditFieldProps(fieldValues, disabledFields, handleChangeFormInput);
    ...
    expect(handleChangeFormInput).toHaveBeenCalledWith('aspirate_flowRate', '42');
  });
  it('should make field disabled if it is represented in disabledFields, and show disabled explanation tooltip', () => {
    ...
    expect(result.aspirate_flowRate.tooltipContent).toBe('Disabled explanation text here');
  });
  ...
  it('should show indeterminate tooltip content for indeterminate checkboxes', () => {
    ...
    expect(result.preWetTip.tooltipContent).toBe('Not all selected steps are using this setting');
  });
  it('should override indeterminate tooltip content if field is also disabled', () => {
    ...
    expect(result.preWetTip.tooltipContent).toBe('Disabled explanation text here');
  });
});