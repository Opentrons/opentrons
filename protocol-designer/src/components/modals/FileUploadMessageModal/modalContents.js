...
export default function getModalContents (
  ...
  if (uploadResponse.isError) {
    switch (uploadResponse.errorType) {
      ...
      case 'INVALID_JSON_FILE': return invalidJsonModal(uploadResponse.errorMessage)
      default: {
  ...
  switch (uploadResponse.messageKey) {
    case 'didMigrate': return didMigrateModal
    default: {
      assert(false, `invalid messageKey ${uploadResponse.messageKey} specified for modal`)
