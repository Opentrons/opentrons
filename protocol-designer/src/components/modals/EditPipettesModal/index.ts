...
const makeUpdatePipettes = (prevPipettes, orderedStepIds, dispatch, closeModal) => ({
  ...
  const nextPipettes: Record<string, {
    mount: string;
    name: string;
  ...
  newPipetteArray.forEach(newPipette => {
    if (newPipette && newPipette.name && newPipette.tiprackDefURI) {
      ...
      if (pipetteId) {
        ...
        const newId = uuid();
        nextPipettes[newId] = { ...newPipette,
          id: newId
        };
      }
    }
  });
  dispatch(stepFormActions.createPipettes(mapValues(nextPipettes, (p: $Values<typeof nextPipettes>): NormalizedPipette => ({
    id: p.id,
    name: p.name,
  ...
  const pipetteReplacementMap: SubstitutionMap = pipetteIdsToDelete.reduce((acc: SubstitutionMap, deletedId: string): SubstitutionMap => {
    const deletedPipette = prevPipettes[deletedId];
    const replacementId = Object.keys(nextPipettes).find(newId => nextPipettes[newId].mount === deletedPipette.mount);
    return replacementId && replacementId !== -1 ? { ...acc,
      [deletedId]: replacementId
    } : acc;
  }, {});
  const pipettesWithNewTipracks: Array<string> = filter(nextPipettes, (nextPipette: $Values<typeof nextPipettes>) => {
    const newPipetteId = nextPipette.id;
    const tiprackChanged = newPipetteId in prevPipettes && nextPipette.tiprackDefURI !== prevPipettes[newPipetteId].tiprackDefURI;
  ...
  const pipettesWithNewTiprackIdentityMap: SubstitutionMap = pipettesWithNewTipracks.reduce((acc: SubstitutionMap, id: string): SubstitutionMap => {
    return { ...acc,
      ...{
        [id]: id
      }
  ...
  const substitutionMap = { ...pipetteReplacementMap,
...
const mergeProps = (stateProps: SP, dispatchProps: {
  ...
  const updatePipettes = makeUpdatePipettes(_prevPipettes, _orderedStepIds, dispatch, closeModal);
  return { ...passThruStateProps,
...
export const EditPipettesModal: React.AbstractComponent<OP> = connect<Props, OP, SP, {}, _, _>(mapSTP, null, mergeProps)(FilePipettesModal);