...
import { Modal, FormGroup, BUTTON_TYPE_SUBMIT, OutlineButton, Tooltip, useHoverTooltip } from "@opentrons/components";
import { THERMOCYCLER_MODULE_TYPE, MAGNETIC_MODULE_TYPE, THERMOCYCLER_MODULE_V1 } from "@opentrons/shared-data";
import { i18n } from "../../../localization";
import { getSlotsBlockedBySpanning, getSlotIsEmpty, getLabwareOnSlot } from "../../../step-forms/utils";
import { getLabwareIsCompatible } from "../../../utils/labwareModuleCompatibility";
import { selectors as stepFormSelectors, actions as stepFormActions } from "../../../step-forms";
import { SUPPORTED_MODULE_SLOTS, getAllModuleSlotsByType } from "../../../modules/moduleData";
...
export const EditModulesModal = (props: EditModulesModalProps): React.ReactNode => {
  const {
    moduleType,
  ...
  const hasSlotIssue = (selectedSlot): boolean => {
    ...
    const isSlotBlocked = getSlotsBlockedBySpanning(initialDeckSetup).includes(selectedSlot);
    ...
    const isLabwareCompatible = labwareOnSlot && getLabwareIsCompatible(labwareOnSlot.def, moduleType);
  ...
  const onSaveClick = (values: EditModulesFormValues): void => {
    ...
    if (!selectedModel) {
      console.warn('Cannot edit module without a module on the deck. This should not happen');
      return;
    }
    ...
    if (moduleOnDeck) {
      ...
      dispatch(stepFormActions.createModule({
        slot: selectedSlot,
        type: moduleType,
        model: selectedModel
      }));
  ...
  return <Formik onSubmit={onSaveClick} initialValues={initialValues} initialErrors={validator(initialValues)} validate={validator}>
      <EditModulesModalComponent {...props} supportedModuleSlot={supportedModuleSlot} />
    </Formik>;
};
...
const EditModulesModalComponent = (props: EditModulesModalComponentProps) => {
  ...
  const disabledModuleRestriction = useSelector(featureFlagSelectors.getDisableModuleRestrictions);
  const noCollisionIssue = selectedModel && !isModuleWithCollisionIssue(selectedModel);
  const enableSlotSelection = disabledModuleRestriction || noCollisionIssue;
  const slotOptionTooltip = <div className={styles.slot_tooltip}>
  ...
  const slotIssue = errors?.selectedSlot && errors.selectedSlot.includes('occupied');
  useResetSlotOnModelChange(supportedModuleSlot);
  const [targetProps, tooltipProps] = useHoverTooltip({
    placement: 'top'
  });
  return <Modal heading={i18n.t(`modules.module_long_names.${moduleType}`)} className={cx(modalStyles.modal, styles.edit_module_modal)} contentsClassName={styles.modal_contents}>
      <>
        {slotIssue && <PDAlert alertType="warning" title={i18n.t('alert.module_placement.SLOT_OCCUPIED.title')} description={''} />}
        <Form>
          <div className={styles.form_row}>
            <FormGroup label="Model*" className={styles.option_model}>
              <ModelDropdown fieldName={'selectedModel'} tabIndex={0} options={MODELS_FOR_MODULE_TYPE[moduleType]} />
            </FormGroup>
            {showSlotOption && <>
                {!enableSlotSelection && <Tooltip {...tooltipProps}>{slotOptionTooltip}</Tooltip>}

                <div {...targetProps} className={styles.option_slot}>
                  <FormGroup label="Position">
                    <SlotDropdown fieldName={'selectedSlot'} options={getAllModuleSlotsByType(moduleType)} disabled={!enableSlotSelection} tabIndex={1} />
                  </FormGroup>
                </div>
          ...
          <div className={modalStyles.button_row}>
            ...
            <OutlineButton className={styles.button_margin} disabled={!isValid} type={BUTTON_TYPE_SUBMIT}>
              {i18n.t('button.save')}
            </OutlineButton>
          </div>
        </Form>
