...
import { MAGNETIC_MODULE_TYPE, TEMPERATURE_MODULE_TYPE, MAGNETIC_MODULE_V2, TEMPERATURE_MODULE_V2 } from "@opentrons/shared-data";
import { getMockDeckSetup, getMockMagneticModule, getMockTemperatureModule } from "../../../../../fixtures/state/deck";
import { actions as stepFormActions, selectors as stepFormSelectors } from "../../../../step-forms";
import { getLabwareOnSlot, getSlotsBlockedBySpanning, getSlotIsEmpty } from "../../../../step-forms/utils";
...
const getDisableModuleRestrictionsMock: JestMockFn<any, any> = featureSelectors.getDisableModuleRestrictions;
const isModuleWithCollisionIssueMock: JestMockFn<any, any> = isModuleWithCollisionIssue;
const getSlotsBlockedBySpanningMock: JestMockFn<any, any> = getSlotsBlockedBySpanning;
...
describe('Edit Modules Modal', () => {
  ...
  const render = props => mount(<Provider store={mockStore}>
  ...
  describe('Slot Dropdown', () => {
    it('should pass the correct options and field name', () => {
      ...
      jest.spyOn(moduleData, 'getAllModuleSlotsByType').mockImplementation(() => mockSlots);
      const wrapper = render(props);
    ...
    it('should error when labware is incompatible', () => {
      getLabwareIsCompatibleMock.mockReturnValue(false);
      const wrapper = render(props);
      expect(wrapper.find(SlotDropdown).childAt(0).prop('error')).toMatch('labware incompatible');
    });
  ...
  describe('Form Submission', () => {
    ...
    it('creates a new module if no module is registered', () => {
      ...
      const expected = { ...createModuleAction,
        payload: { ...createModuleAction.payload,
