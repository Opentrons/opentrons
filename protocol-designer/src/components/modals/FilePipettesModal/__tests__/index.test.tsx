...
describe('FilePipettesModal', () => {
  ...
  function renderFormComponent(props) {
    return shallow(<FilePipettesModal {...props} />).find(Formik).dive();
  }
  ...
  describe('form buttons', () => {
    it('does not allow save btn to be clicked when no pipette selected', () => {
      const saveButton = renderFormComponent(props).find(OutlineButton).filterWhere(n => n.render().text() === 'save');
    ...
    it('is not disabled when pipette is selected', async () => {
      ...
      props.initialModuleValues = initialModuleValues;
      const wrapper = renderFormComponent(props);
      const saveButton = wrapper.find(OutlineButton).filterWhere(n => n.render().text() === 'save');
    ...
    it('closes the modal when clicking cancel button', () => {
      const cancel = renderFormComponent(props).find(OutlineButton).filterWhere(n => n.render().text() === 'cancel');
  ...
  describe('step changes confirm modal', () => {
    it('displays edit confirmation when editing pipettes', () => {
      ...
      expect(wrapper.find(Formik).dive().find(StepChangesConfirmModal)).toHaveLength(1);
    });
    it('does not display edit confirmation when new protocol', () => {
      expect(renderFormComponent(props).find(StepChangesConfirmModal)).toHaveLength(0);
    });
  });
  describe('render', () => {
    ...
    it('displays heading for new protocol when creating new protocol', () => {
      ...
      const wrapper = shallow(<FilePipettesModal {...props} />).find(Formik).dive();
      const newProtocolHeader = wrapper.find('h2').filterWhere(n => n.render().text() === i18n.t('modal.new_protocol.title.PROTOCOL_FILE'));
    ...
    it('displays edit pipettes title when editing pipettes', () => {
      ...
      const wrapper = shallow(<FilePipettesModal {...props} />).find(Formik).dive();
      const editPipettesHeader = wrapper.find('h2').filterWhere(n => n.render().text() === i18n.t('modal.edit_pipettes.title'));
