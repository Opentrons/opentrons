...
import { MAGNETIC_MODULE_V2, THERMOCYCLER_MODULE_TYPE, TEMPERATURE_MODULE_TYPE, MAGNETIC_MODULE_TYPE } from "@opentrons/shared-data";
...
describe('ModuleFields', () => {
  let magnetModuleOnDeck, temperatureModuleNotOnDeck, thermocyclerModuleNotOnDeck, props;
  beforeEach(() => {
    magnetModuleOnDeck = {
      onDeck: true,
      slot: '1',
  ...
  it('adds module to protocol when checkbox is selected and resets the model field', () => {
    ...
    const wrapper = shallow(<ModuleFields {...props} />);
    const temperatureSelectChange = wrapper.find({
      name: checkboxTargetName
    }).prop('onChange');
  ...
  it('displays specific module img when model has been selected', () => {
    ...
    const wrapper = shallow(<ModuleFields {...props} />);
    const magnetModuleDiagramProps = wrapper.find(ModuleDiagram).filter({
