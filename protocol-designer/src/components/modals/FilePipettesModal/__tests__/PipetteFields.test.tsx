...
import { PipetteSelect, DropdownField, OutlineButton } from "@opentrons/components";
...
const getOnlyLatestDefsMock: JestMockFn<[], LabwareDefByDefURI> = getOnlyLatestDefs;
const createCustomTiprackDefMock: JestMockFn<[React.SyntheticEvent<HTMLInputElement>], ThunkAction<any>> = labwareDefActions.createCustomTiprackDef;
describe('PipetteFields', () => {
  ...
  it('renders a selection for left and right pipette with disabled tiprack select', () => {
    ...
    expect(wrapper.find(DropdownField).filter({
      name: leftTiprackKey
    }).prop('disabled')).toBe(true);
    expect(wrapper.find(DropdownField).filter({
      name: rightTiprackKey
    }).prop('disabled')).toBe(true);
  });
  it('selects a pipette and clears tiprack fields that has been touched', () => {
    ...
    expect(props.onSetFieldValue.mock.calls).toEqual([[leftPipetteName, 'p50'], [leftTiprackKey, null]]);
  });
  ...
  it('selects a tiprack for the pipette', () => {
    ...
    const wrapper = render(props);
    const leftTiprackSelect = wrapper.find(DropdownField).filter({
      name: leftTiprackKey
    });
