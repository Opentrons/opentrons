...
export function PipetteDiagram(props: Props): React.ReactNode {
  const {
    leftPipette,
    rightPipette
  } = props;
...
function PipetteGroup(props: Props) {
  ...
  return <>
      {leftPipette && leftSpecs ? <InstrumentDiagram pipetteSpecs={leftSpecs} className={styles.left_pipette} mount="left" /> : <div className={styles.left_pipette} />}{' '}
      {rightPipette && rightSpecs ? <InstrumentDiagram pipetteSpecs={rightSpecs} className={styles.right_pipette} mount="right" /> : <div className={styles.right_pipette} />}
    </>;
}