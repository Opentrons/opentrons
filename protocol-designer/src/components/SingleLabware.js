...
export default function SingleLabware(props: Props) {
  ...
  const width = showLabels
    ? SLOT_RENDER_WIDTH + LABEL_OFFSET
    : SLOT_RENDER_WIDTH
  const height = showLabels
    ? SLOT_RENDER_HEIGHT + LABEL_OFFSET
    : SLOT_RENDER_HEIGHT
  return (
    <div className={cx(styles.single_labware, className)}>
      <svg viewBox={`${minX} ${minY} ${width} ${height}`}>{children}</svg>
    </div>
  )
}
