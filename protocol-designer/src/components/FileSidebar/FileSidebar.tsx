...
import { PrimaryButton, AlertModal, OutlineButton, SidePanel } from "@opentrons/components";
...
import type { InitialDeckSetup, SavedStepFormState, ModuleOnDeck, PipetteOnDeck } from "../../step-forms";
type Props = {
  loadFile: (event: React.SyntheticEvent<HTMLInputElement>) => unknown;
...
function getWarningContent({
  ...
  const pipettesDetails = pipettesWithoutStep.map(pipette => `${pipette.mount} ${pipette.spec.displayName}`).join(' and ');
  const modulesDetails = modulesWithoutStep.map(moduleOnDeck => i18n.t(`modules.module_long_names.${moduleOnDeck.type}`)).join(' and ');
  ...
  if (pipettesWithoutStep.length && modulesWithoutStep.length) {
    return {
      ...
      heading: i18n.t('alert.export_warnings.unused_pipette_and_module.heading')
    };
  }
  ...
  if (modulesWithoutStep.length) {
    const moduleCase = modulesWithoutStep.length > 1 ? 'unused_modules' : 'unused_module';
    return {
      content: <>
          <p>
...
export const v4WarningContent: React.ReactNode = <div>
    <p>
...
export const v5WarningContent: React.ReactNode = <div>
    <p>
...
export function FileSidebar(props: Props): React.ReactNode {
  const {
    canDownload,
    fileData,
    loadFile,
  ...
  const [showExportWarningModal, setShowExportWarningModal] = React.useState<boolean>(false);
  ...
  const pipettesWithoutStep = getUnusedEntities(pipettesOnDeck, savedStepForms, 'pipette');
  const modulesWithoutStep = getUnusedEntities(modulesOnDeck, savedStepForms, 'moduleId');
  const hasWarning = noCommands || modulesWithoutStep.length || pipettesWithoutStep.length;
  const warning = hasWarning && getWarningContent({
    noCommands,
    pipettesWithoutStep,
    modulesWithoutStep
  });
  ...
  const getExportHintContent = (): {
    ...
    return {
      hintKey: schemaVersion === 5 ? 'export_v5_protocol_3_20' : 'export_v4_protocol_3_18',
  ...
  return <>
      {blockingExportHint}
      {showExportWarningModal && <Portal>
          <AlertModal alertOverlay className={modalStyles.modal} heading={warning && warning.heading} onCloseClick={cancelModal} buttons={[{
        children: 'CANCEL',
        onClick: cancelModal
      }, {
