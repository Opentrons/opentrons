...
export type Props = {
  ...
  iconProps?: $Diff<React.ElementProps<typeof Icon>, {
    name: unknown;
  }>;
...
export function TitledStepList(props: Props): React.ReactNode {
  const {
    iconName,
    'data-test': dataTest,
    onCollapseToggle,
  ...
  const multiSelectIconName = props.selected ? 'checkbox-marked' : 'checkbox-blank-outline';
  return <div className={className} data-test={dataTest} {...{
    ...
    onContextMenu
      ...
      <div onClick={onClick} className={titleBarClass}>
        {isMultiSelectMode && <div className={cx(styles.multiselect_wrapper, {
        [styles.last_selected]: isLastSelected
      })}>
            <Icon name={multiSelectIconName} className={styles.icon_multiselect} />
          </div>}
        {iconName && <Icon {...iconProps} className={iconClass} name={iconName} />}
        <h3 className={styles.title}>{props.title}</h3>
        {collapsible && <div onClick={handleCollapseToggle} className={styles.title_bar_carat}>
            <Icon className={styles.title_bar_icon} name={props.selected ? 'chevron-right' : props.collapsed ? 'chevron-down' : 'chevron-up'} />
          </div>}
      </div>
      {!props.collapsed && props.description}
      {!props.collapsed && hasValidChildren && <ol className={styles.list}>{props.children}</ol>}
    </div>;
}