...
function LiquidEditFormWrapper(props: WrapperProps) {
  ...
  return showForm ? <LiquidEditForm {...formProps} key={formKey} /> : <LiquidsPageInfo />;
}
...
function mergeProps(stateProps: SP, dispatchProps: {
  ...
  return {
    ...
    formProps: { ...passThruFormProps,
      deleteLiquidGroup: () => _liquidGroupId && dispatch(labwareIngredActions.deleteLiquidGroup(_liquidGroupId)),
      cancelForm: () => dispatch(labwareIngredActions.deselectLiquidGroup()),
      saveForm: (formData: LiquidGroup) => dispatch(labwareIngredActions.editLiquidGroup({ ...formData,
...
export const LiquidsPage: React.AbstractComponent<{}> = connect<WrapperProps, {}, SP, {}, _, _>(mapStateToProps, null, mergeProps)(LiquidEditFormWrapper);