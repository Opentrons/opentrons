...
export type Props = {
  ...
  dismissWarning: (arg0: string) => unknown;
};
type MakeAlert = (alertType: AlertType, data: AlertData, key: number | string) => React.ReactNode;
...
const AlertsComponent = (props: Props) => {
  ...
  const makeAlert: MakeAlert = (alertType, data, key) => <PDAlert alertType={alertType} title={data.title} description={data.description} key={`${alertType}:${key}`} onDismiss={alertType === 'warning' ? makeHandleCloseWarning(data.dismissId) : null} />;
...
export const Alerts: React.AbstractComponent<Props> = React.memo(AlertsComponent);