...
const LiquidGroupCard = (props: LiquidGroupCardProps): React.ReactNode => {
  ...
  const wellsWithIngred = Object.keys(labwareWellContents).sort(sortWells).filter(well => labwareWellContents[well][groupId]);
  ...
  return <PDTitledList title={ingredGroup.name || i18n.t('card.unnamedLiquid')} iconProps={{
    style: {
      fill: swatchColors(groupId)
    }
  }} iconName="circle" onCollapseToggle={toggleAccordion} collapsed={!expanded} selected={selected} description={<TitledListNotes notes={ingredGroup.description} />}>
      ...
      return <IngredIndividual key={well} name={showName ? `${ingredGroup.name || ''} ${i + 1}` : null} wellName={well} canDelete volume={volume} groupId={groupId} removeWellsContents={removeWellsContents} />;
    })}
    </PDTitledList>;
};
...
function IngredIndividual(props: IndividProps) {
  ...
  return <PDListItem border hoverable>
      <span>{wellName}</span>
      <span>
        {volume ? volume + ` ${i18n.t('application.units.microliter')}` : '-'}
      </span>
      {name && <span>{name}</span>}
      {canDelete && <IconButton className={styles.close_icon} name="close" onClick={() => window.confirm(`Are you sure you want to delete well ${wellName} ?`) && removeWellsContents({
      liquidGroupId: groupId,
      wells: [wellName]
    })} />}
    </PDListItem>;
}
...
export function IngredientsList(props: Props): React.ReactNode {
  const {
    labwareWellContents,
    liquidGroupsById,
    removeWellsContents,
  ...
  return <SidePanel title={i18n.t('nav.nameAndLiquids')}>
      <LabwareDetailsCard />

      {Object.keys(liquidGroupsById).map(groupIdForCard => <LiquidGroupCard key={groupIdForCard} removeWellsContents={removeWellsContents} labwareWellContents={labwareWellContents} ingredGroup={liquidGroupsById[groupIdForCard]} groupId={groupIdForCard} selected={selectedIngredientGroupId === groupIdForCard} />)}
    </SidePanel>;
}