...
function mapStateToProps(state: BaseState): SP {
  ...
  const labwareDefDisplayName = labwareId && getLabwareDisplayName(stepFormSelectors.getLabwareEntities(state)[labwareId].def);
...
function mergeProps(stateProps: SP, dispatchProps: {
  ...
  const renameLabware = (name: string) => {
    ...
    if (_labwareId) {
      dispatch(labwareIngredActions.renameLabware({
...
export const LabwareDetailsCard: React.AbstractComponent<{}> = connect<Props, {}, SP, {}, _, _>(mapStateToProps, null, mergeProps)(LabwareDetailsCardComponent);