...
export const EditModules = (props: EditModulesProps): React.ReactNode => {
  const {
    onCloseClick,
    moduleToEdit
  } = props;
  ...
  const [changeModuleWarningInfo, displayModuleWarning] = React.useState<null | ModelModuleInfo>(null);
  const dispatch = useDispatch();

  const editModuleModel = (selectedModel: ModuleModel) => {
    if (moduleOnDeck?.id != null) {
      dispatch(stepFormActions.editModule({
        id: moduleOnDeck.id,
        model: selectedModel
      }));
    } else {
      console.error(`cannot edit module model without module id. This shouldn't be able to happen`);
    }
  };
  ...
  return changeModuleWarning || <EditModulesModal moduleType={moduleType} moduleOnDeck={moduleOnDeck} onCloseClick={onCloseClick} editModuleSlot={editModuleSlot} editModuleModel={editModuleModel} displayModuleWarning={displayModuleWarning} />;
};