...
import { actions as labwareDefActions, selectors as labwareDefSelectors } from "../../labware-defs";
...
function mapStateToProps(state: BaseState): SP {
  ...
  const initialModules: Array<ModuleOnDeck> = Object.keys(modulesById).map(moduleId => modulesById[moduleId]);
  const parentModule = slot != null && initialModules.find(moduleOnDeck => moduleOnDeck.id === slot) || null;
  return {
...
function mergeProps(stateProps: SP, dispatchProps: {
  ...
  return { ...stateProps,
    ...
    onUploadLabware: fileChangeEvent => dispatch(labwareDefActions.createCustomLabwareDef(fileChangeEvent)),
...
export const LabwareSelectionModal: React.AbstractComponent<{}> = connect<Props, {}, SP, {}, _, _>(mapStateToProps, null, mergeProps)(LabwareSelectionModalComponent);