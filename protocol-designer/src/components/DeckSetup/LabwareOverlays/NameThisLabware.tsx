...
const NameThisLabwareComponent = (props: Props) => {
  ...
  return <div className={cx(styles.slot_overlay, styles.with_form)} ref={wrapperRef}>
      <div className={styles.name_input_wrapper}>
        <input className={styles.name_input} onChange={handleChange} onKeyUp={handleKeyUp} placeholder={i18n.t('deck.overlay.name_labware.nickname_placeholder')} value={inputValue} />
      </div>
...
const mapDispatchToProps = (dispatch: ThunkDispatch<any>, ownProps: OP): DP => {
  ...
  return {
    setLabwareName: (name: string | null | undefined) => dispatch(renameLabware({
      labwareId: id,
      name
    }))
  };
...
export const NameThisLabware: React.AbstractComponent<OP> = connect<Props, OP, _, DP, BaseState, ThunkDispatch<any>>(null, mapDispatchToProps)(NameThisLabwareComponent);