...
import { openIngredientSelector, deleteContainer, duplicateLabware, moveDeckItem } from "../../../labware-ingred/actions";
...
type DP = {
  ...
  moveDeckItem: (arg0: DeckSlot, arg1: DeckSlot) => unknown;
};
type DNDP = {
  draggedLabware: LabwareOnDeck | null | undefined;
  isOver: boolean;
  connectDragSource: (arg0: React.ReactNode) => React.ReactNode;
  connectDropTarget: (arg0: React.ReactNode) => React.ReactNode;
};
...
const EditLabwareComponent = (props: Props) => {
  ...
  const {
    isTiprack
  } = labwareOnDeck.def.parameters;

  if (isYetUnnamed && !isTiprack) {
    return <NameThisLabware labwareOnDeck={labwareOnDeck} editLiquids={editLiquids} />;
  } else {
    ...
    if (swapBlocked) {
      ...
      contents = <div className={cx(styles.overlay_button, {
      ...
      contents = <>
          {!isTiprack ? <a className={styles.overlay_button} onClick={editLiquids}>
              <Icon className={styles.overlay_icon} name="pencil" />
              {i18n.t('deck.overlay.edit.name_and_liquids')}
            </a> : <div className={styles.button_spacer} />}
    ...
    return connectDragSource(connectDropTarget(<div className={cx(styles.slot_overlay, {
...
const DragEditLabware = DragSource(DND_TYPES.LABWARE, labwareSource, collectLabwareSource)(EditLabwareComponent);
const labwareDropTarget = {
  canDrop: (props: OP & SP & DP, monitor) => {
    const draggedItem = monitor.getItem();
    const draggedLabware = draggedItem?.labwareOnDeck;
    const isDifferentSlot = draggedLabware && draggedLabware.slot !== props.labwareOnDeck.slot;
  ...
  drop: (props, monitor) => {
    ...
    if (draggedItem) {
      props.moveDeckItem(draggedItem.labwareOnDeck.slot, props.labwareOnDeck.slot);
    }
  }
};
...
const DragDropEditLabware = DropTarget(DND_TYPES.LABWARE, labwareDropTarget, collectLabwareDropTarget)(DragEditLabware);

const mapStateToProps = (state: BaseState, ownProps: OP): SP => {
  const {
    id
  } = ownProps.labwareOnDeck;
...
const mapDispatchToProps = (dispatch: ThunkDispatch<any>, ownProps: OP): DP => ({
  editLiquids: () => dispatch(openIngredientSelector(ownProps.labwareOnDeck.id)),
  duplicateLabware: () => dispatch(duplicateLabware(ownProps.labwareOnDeck.id)),
  deleteLabware: () => {
    window.confirm(`Are you sure you want to permanently delete this ${getLabwareDisplayName(ownProps.labwareOnDeck.def)}?`) && dispatch(deleteContainer({
      labwareId: ownProps.labwareOnDeck.id
    }));
  },
  moveDeckItem: (sourceSlot, destSlot) => dispatch(moveDeckItem(sourceSlot, destSlot))
});

export const EditLabware: React.AbstractComponent<OP> = connect<OP & SP & DP, OP, SP, DP, BaseState, ThunkDispatch<any>>(mapStateToProps, mapDispatchToProps)(DragDropEditLabware);