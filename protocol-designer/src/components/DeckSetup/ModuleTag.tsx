...
import { TEMPERATURE_AT_TARGET, TEMPERATURE_APPROACHING_TARGET, TEMPERATURE_DEACTIVATED } from "@opentrons/step-generation";
...
import type { ModuleTemporalProperties, TemperatureModuleState } from "../../step-forms";
type Props = {
  x: number;
  y: number;
...
function getTempStatus(temperatureModuleState: TemperatureModuleState): string {
  ...
  if (status === TEMPERATURE_APPROACHING_TARGET) {
    return `Going to ${targetTemperature} ${i18n.t('application.units.degrees')}`;
  }
...
export const ModuleStatus = ({
  ...
  switch (moduleState.type) {
    case MAGNETIC_MODULE_TYPE:
      return <div className={styles.module_status_line}>
          {i18n.t(`modules.status.${moduleState.engaged ? 'engaged' : 'disengaged'}`)}
        </div>;
    ...
    default:
      console.warn(`ModuleStatus doesn't support module type ${moduleState.type}`);
      return null;
  }
};
...
const ModuleTagComponent = (props: Props) => {
  ...
  const moduleEntity = useSelector(stepFormSelectors.getModuleEntities)[props.id];
  const moduleState: $PropertyType<ModuleTemporalProperties, "moduleState"> | null | undefined = timelineFrame?.robotState.modules[props.id]?.moduleState;
  ...
  const isHoveredModuleStep = moduleLabware && hoveredLabwares.length ? hoveredLabwares[0] === moduleLabware.id : false;

  if (moduleType == null || moduleState == null) {
    // this should never happen, but better to have an empty tag than to whitescreen
    console.error(`nullsy moduleType or moduleState for module "${props.id}" in the selected timeline frame`);
    return null;
  }
  ...
  return <RobotCoordsForeignDiv x={props.x + (props.orientation === 'left' ? childXOffset - tagWidth : STD_SLOT_X_DIM + childXOffset)} y={props.y + childYOffset + (STD_SLOT_Y_DIM - tagHeight) / 2} height={tagHeight} width={tagWidth} innerDivProps={{
    'data-test': `ModuleTag_${moduleType}`,
    className: cx(styles.module_tag, {
      [styles.highlighted_border_right_none]: isHoveredModuleStep && props.orientation === 'left',
      [styles.highlighted_border_left_none]: isHoveredModuleStep && props.orientation === 'right'
    })
  }}>
...
export const ModuleTag: React.AbstractComponent<Props> = React.memo(ModuleTagComponent);