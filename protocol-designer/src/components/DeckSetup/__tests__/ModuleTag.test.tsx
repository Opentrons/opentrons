import { MAGNETIC_MODULE_TYPE, TEMPERATURE_MODULE_TYPE, TEMPERATURE_MODULE_V1 } from "@opentrons/shared-data";
import React from "react";
import { Provider } from "react-redux";
import { render, mount } from "enzyme";
import fixture_tiprack_10_ul from "@opentrons/shared-data/labware/fixtures/2/fixture_tiprack_10_ul.json";
import { TEMPERATURE_APPROACHING_TARGET, TEMPERATURE_AT_TARGET, TEMPERATURE_DEACTIVATED } from "@opentrons/step-generation";
...
const timelineFrameBeforeActiveItemMock: JestMockFn<[BaseState], CommandsAndRobotState | null> = timelineFramesSelectors.timelineFrameBeforeActiveItem;
const getModuleEntitiesMock: JestMockFn<[BaseState], ModuleEntities> = stepFormSelectors.getModuleEntities;
const getHoveredStepLabwareMock: JestMockFn<[BaseState], Array<string>> = uiSelectors.getHoveredStepLabware;
const getInitialDeckSetup: JestMockFn<[BaseState], InitialDeckSetup> = stepFormSelectors.getInitialDeckSetup;
describe('ModuleTag', () => {
  ...
  describe('ModuleTagComponent', () => {
    ...
    it('adds a border when the step is is a module step type', () => {
      ...
      expect(wrapper.find('RobotCoordsForeignDiv').prop('innerDivProps').className).toContain('highlighted_border_right_none');
    });
    it('does not add a border when the step is not a module step', () => {
      const wrapper = render();
      expect(wrapper.find('RobotCoordsForeignDiv').prop('innerDivProps').className).not.toContain('highlighted_border_right_none');
    });
