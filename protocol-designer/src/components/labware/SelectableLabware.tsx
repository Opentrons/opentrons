...
import { arrayToWellGroup, getCollidingWells, getWellSetForMultichannel } from "../../utils";
...
export type Props = {
  labwareProps: $Diff<React.ElementProps<typeof SingleLabware>, {
    selectedWells: any;
  }>;
...
export class SelectableLabware extends React.Component<Props> {
  ...
  _wellsFromSelected: (selectedWells: WellGroup) => WellGroup = selectedWells => {
    ...
    if (this.props.pipetteChannels === 8) {
      ...
      const primaryWells: WellGroup = reduce(selectedWells, (acc: WellGroup, _, wellName: string): WellGroup => {
  ...
  handleSelectionMove: (e: MouseEvent, rect: GenericRect) => void = (e, rect) => {
    const labwareDef = this.props.labwareProps.definition;
    ...
    if (!e.shiftKey) {
      if (this.props.pipetteChannels === 8) {
        ...
        const allWellsForMulti: WellGroup = reduce(selectedWells, (acc: WellGroup, _, wellName: string): WellGroup => {
          const wellSetForMulti = getWellSetForMultichannel(labwareDef, wellName) || [];
  ...
  handleSelectionDone: (e: MouseEvent, rect: GenericRect) => void = (e, rect) => {
  ...
  render(): React.ReactNode {
    ...
    const allSelectedWells = pipetteChannels === 8 ? reduce<WellGroup, WellGroup>(selectedPrimaryWells, (acc, _, wellName): WellGroup => {
      const wellSet = getWellSetForMultichannel(this.props.labwareProps.definition, wellName);
      if (!wellSet) return acc;
      return { ...acc,
        ...arrayToWellGroup(wellSet)
      };
    }, {}) : selectedPrimaryWells;
    return <SelectionRect onSelectionMove={this.handleSelectionMove} onSelectionDone={this.handleSelectionDone}>
        <WellTooltip ingredNames={ingredNames}>
          {({
          makeHandleMouseEnterWell,
          handleMouseLeaveWell,
          tooltipWellName
        }) => <SingleLabware {...labwareProps} selectedWells={allSelectedWells} onMouseLeaveWell={mouseEventArgs => {
          this.handleMouseLeaveWell(mouseEventArgs);
          handleMouseLeaveWell(mouseEventArgs.event);
        }} selectableWellClass={SELECTABLE_WELL_CLASS} onMouseEnterWell={({
          wellName,
          event
        }) => {
          if (wellContents !== null) {
            this.handleMouseEnterWell({
              wellName,
              event
            });
            makeHandleMouseEnterWell(wellName, wellContents[wellName]?.ingreds)(event);
          }
        }} />}
        </WellTooltip>
      </SelectionRect>;
