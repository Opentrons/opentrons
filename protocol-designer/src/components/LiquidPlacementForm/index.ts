...
function mapStateToProps(state: BaseState): SP {
  ...
  const _selectionHasLiquids = Boolean(_labwareId && liquidLocations[_labwareId] && Object.keys(selectedWells).some(well => liquidLocations[_labwareId][well]));
...
function mergeProps(stateProps: SP, dispatchProps: {
  ...
  return { ...passThruProps,
    ...
    saveForm: (values: LiquidPlacementFormValues) => {
      ...
      assert(_labwareId != null, 'when saving liquid placement form, expected a selected labware ID');
      assert(_selectedWells && _selectedWells.length > 0, `when saving liquid placement form, expected selected wells to be array with length > 0 but got ${String(_selectedWells)}`);
      assert(selectedLiquidId != null, `when saving liquid placement form, expected selectedLiquidId to be non-nullsy but got ${String(selectedLiquidId)}`);
      assert(volume > 0, `when saving liquid placement form, expected volume > 0, got ${volume}`);
...
export const LiquidPlacementForm: React.AbstractComponent<{}> = connect<Props, {}, SP, {}, _, _>(mapStateToProps, null, mergeProps)(LiquidPlacementFormComponent);