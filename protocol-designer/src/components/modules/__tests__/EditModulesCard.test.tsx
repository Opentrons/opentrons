...
import { MAGNETIC_MODULE_TYPE, MAGNETIC_MODULE_V1, MAGNETIC_MODULE_V2, TEMPERATURE_MODULE_TYPE, TEMPERATURE_MODULE_V1 } from "@opentrons/shared-data";
...
const getDisableModuleRestrictionsMock: JestMockFn<[BaseState], boolean | null | undefined> = featureFlagSelectors.getDisableModuleRestrictions;
const getPipettesForEditPipetteFormMock: JestMockFn<[BaseState], FormPipettesByMount> = stepFormSelectors.getPipettesForEditPipetteForm;
describe('EditModulesCard', () => {
  let store, crashableMagneticModule, nonCrashableMagneticModule, crashablePipette, noncrashablePipette, props;
  beforeEach(() => {
    ...
    nonCrashableMagneticModule = { ...crashableMagneticModule,
  ...
  it('displays module row with added module', () => {
    ...
    const wrapper = render(props);
    expect(wrapper.find(ModuleRow).filter({
      type: MAGNETIC_MODULE_TYPE
    }).props()).toEqual({
      type: MAGNETIC_MODULE_TYPE,
