...
import { MAGNETIC_MODULE_TYPE, MAGNETIC_MODULE_V1, MAGNETIC_MODULE_V2 } from "@opentrons/shared-data";
import { Tooltip, SlotMap, LabeledValue, OutlineButton } from "@opentrons/components";
...
describe('ModuleRow', () => {
  ...
  it('displays the correct module diagram for the selected module model', () => {
    ...
    const wrapper = render(props);
    expect(wrapper.find(ModuleDiagram).prop('model')).toEqual(MAGNETIC_MODULE_V2);
  });
  it('displays the correct module model and slot when it is added to protocol', () => {
    ...
    const wrapper = render(props);
    expect(wrapper.find(LabeledValue).filter({
      label: 'Model'
    }).prop('value')).toBe('GEN1');
    expect(wrapper.find(LabeledValue).filter({
      label: 'Position'
    }).prop('value')).toBe('Slot 1');
  });
  ...
  describe('buttons', () => {
    it('opens edit modal when module is added', () => {
      ...
      const wrapper = render(props);
      const editButton = wrapper.find(OutlineButton).filterWhere(n => n.text() === 'Edit');
      editButton.simulate('click');
      expect(props.openEditModuleModal).toHaveBeenCalledWith(MAGNETIC_MODULE_TYPE, 'magnet123');
    });
    it('adds module when module has not been added yet', () => {
      ...
      const wrapper = render(props);
      const editButton = wrapper.find(OutlineButton).filterWhere(n => n.text() === 'add');
      editButton.simulate('click');
      expect(props.openEditModuleModal).toHaveBeenCalledWith(MAGNETIC_MODULE_TYPE, undefined);
    });
    it('removes module when module has been added', () => {
      ...
      const wrapper = render(props);
      const editButton = wrapper.find(OutlineButton).filterWhere(n => n.text() === 'remove');
      editButton.simulate('click');
      expect(store.dispatch).toHaveBeenCalledWith(stepFormActions.deleteModule('magnet123'));
    });
  });
});