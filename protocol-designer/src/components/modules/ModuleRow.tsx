...
import { LabeledValue, OutlineButton, SlotMap, Tooltip, useHoverTooltip } from "@opentrons/components";
import { i18n } from "../../localization";
import { actions as stepFormActions } from "../../step-forms";
import { SPAN7_8_10_11_SLOT, DEFAULT_MODEL_FOR_MODULE_TYPE } from "../../constants";
...
export function ModuleRow(props: Props): React.ReactNode {
  const {
    moduleOnDeck,
    openEditModuleModal,
  ...
  */
  ...
  const moduleHasCollisionIssue = model ? isModuleWithCollisionIssue(model) : false;
  ...
  if (collisionSlots && collisionSlots.includes('4')) {
    collisionTooltipText = i18n.t(`tooltip.edit_module_card.magnetic_module_collision`);
  } else if (collisionSlots && collisionSlots.includes('6')) {
    collisionTooltipText = i18n.t(`tooltip.edit_module_card.temperature_module_collision`);
  }

  const collisionTooltip = collisionTooltipText && <div className={styles.collision_tolltip}>{collisionTooltipText}</div>;

  const setCurrentModule = (moduleType: ModuleRealType, moduleId?: string) => () => openEditModuleModal(moduleType, moduleId);

  const addRemoveText = moduleOnDeck ? 'remove' : 'add';
  const dispatch = useDispatch();
  const handleAddOrRemove = moduleOnDeck ? () => dispatch(stepFormActions.deleteModule(moduleOnDeck.id)) : setCurrentModule(type);
  const handleEditModule = moduleOnDeck && setCurrentModule(type, moduleOnDeck.id);
  ...
  return <div>
      ...
      <div className={styles.module_row}>
        <div className={styles.module_diagram_container}>
          <ModuleDiagram type={type} model={model || DEFAULT_MODEL_FOR_MODULE_TYPE[type]} />
        </div>
        <div className={styles.module_col}>
          {model && <LabeledValue label="Model" value={i18n.t(`modules.model_display_name.${model}`)} />}
        </div>
        <div className={styles.module_col}>
          {slot && <LabeledValue label="Position" value={slotDisplayName} />}
        </div>
        <div className={styles.slot_map}>
          {collisionSlots.length > 0 && <Tooltip {...tooltipProps}>{collisionTooltip}</Tooltip>}
          {slot && <div {...targetProps}>
              <SlotMap occupiedSlots={occupiedSlotsForMap} collisionSlots={collisionSlots} />
            </div>}
        </div>
        <div className={styles.modules_button_group}>
          {moduleOnDeck && <OutlineButton className={styles.module_button} onClick={handleEditModule} name={`edit${upperFirst(type)}`}>
              Edit
            </OutlineButton>}
          <OutlineButton className={styles.module_button} onClick={handleAddOrRemove} name={`${addRemoveText}${upperFirst(type)}`}>
            {addRemoveText}
          </OutlineButton>
        </div>
      </div>
