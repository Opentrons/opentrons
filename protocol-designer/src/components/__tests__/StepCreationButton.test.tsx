...
import { TEMPERATURE_MODULE_TYPE, TEMPERATURE_MODULE_V1 } from "@opentrons/shared-data";
...
import { StepCreationButton, StepCreationButtonComponent, StepButtonItem } from "../StepCreationButton";
...
describe('StepCreationButton', () => {
  ...
  beforeEach(() => {
    ...
    when(getInitialDeckSetupMock).calledWith(expect.anything()).mockReturnValue({
      labware: {},
      pipettes: {},
      modules: {}
    });
  ...
  const render = () => mount(<Provider store={store}>
        <StepCreationButton />
      </Provider>);
  ...
  describe('when clicking add step', () => {
    it('expands StepCreationButtonComponent onClick', () => {
      ...
      expect(wrapper.find(StepCreationButtonComponent).prop('expanded')).toBe(true);
    });
    it('renders step button items when expanded', () => {
      ...
      const disabledModuleSteps = stepButtonItems.find({
        disabled: true
      });
      ...
      expect(disabledButtonTooltip.prop('children')).toBe('Add a relevant module to use this step');
    });
    it('enables module step types when present on the deck', () => {
      ...
      expect(enabledButtonTooltip.prop('children')).toBe('Set temperature command for Temperature module.');
    });
  });
  describe('StepButtonItem', () => {
    it('should dispatch add step action onClick', () => {
      const addStepSpy = jest.spyOn(stepsActions, 'addAndSelectStepWithHints').mockImplementation(() => () => null);
      // mockImplementation is just to avoid calling the real action creator
      const wrapper = render();
      const button = wrapper.find(PrimaryButton);
      act(() => {
        button.simulate('click');
