...
describe('StepSelectionBanner', () => {
  ...
  const render = store => mount(<StepSelectionBanner />, {
  ...
  describe('when clicking "exit batch edit"', () => {
    it('should exit batch edit mode when there are NOT unsaved form changes', () => {
      when(getBatchEditFormHasUnsavedChangesMock).calledWith(expect.anything()).mockReturnValue(false);
      const deselectAllStepsSpy = jest.spyOn(stepActions, 'deselectAllSteps').mockImplementation(() => () => null);
      const countPerStepType = {
        magnet: 1
      };
      when(getCountPerStepTypeMock).calledWith(expect.anything()).mockReturnValue(countPerStepType);
