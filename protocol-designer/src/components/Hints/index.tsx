...
type SP = {
  hintKey: HintKey | null | undefined;
};
type DP = {
  removeHint: (arg0: HintKey, arg1: boolean) => unknown;
  selectTerminalItem: (arg0: TerminalItemId) => unknown;
};
type Props = SP & DP;
type State = {
...
class HintsComponent extends React.Component<Props, State> {
  ...
  toggleRememberDismissal = () => {
    this.setState({
      rememberDismissal: !this.state.rememberDismissal
    });
  };
  ...
  render() {
    ...
    return <Portal>
        <AlertModal alertOverlay heading={hintIsAlert ? headingText : null}>
          {!hintIsAlert ? <div className={styles.heading}>{headingText}</div> : null}
          <div className={styles.hint_contents}>
            {this.renderHintContents(hintKey)}
          </div>
          <div>
            <CheckboxField className={styles.dont_show_again} label={i18n.t('alert.hint.dont_show_again')} onChange={this.toggleRememberDismissal} value={this.state.rememberDismissal} />
            <OutlineButton className={styles.ok_button} onClick={this.makeHandleCloseClick(hintKey)}>
              {i18n.t('button.ok')}
            </OutlineButton>
          </div>
        </AlertModal>
...
const mapDispatchToProps = (dispatch: ThunkDispatch<any>): DP => ({
  removeHint: (hintKey, rememberDismissal) => dispatch(actions.removeHint(hintKey, rememberDismissal)),
  selectTerminalItem: terminalId => dispatch(stepsActions.selectTerminalItem(terminalId))
});

export const Hints: React.AbstractComponent<{}> = connect<Props, {}, SP, DP, _, _>(mapStateToProps, mapDispatchToProps)(HintsComponent);