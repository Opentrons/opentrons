...
export const BlockingHint = (props: HintProps): React.ReactNode => {
  const {
    hintKey,
    handleCancel,
    handleContinue
  ...
  const [rememberDismissal, setRememberDismissal] = React.useState<boolean>(false);
  ...
  return <Portal>
      <ContinueModal alertOverlay heading={i18n.t(`alert.hint.${hintKey}.title`)} onCancelClick={onCancelClick} onContinueClick={onContinueClick}>
        <div className={styles.hint_contents}>{props.content}</div>
        <div>
          <CheckboxField className={styles.dont_show_again} label={i18n.t('alert.hint.dont_show_again')} onChange={toggleRememberDismissal} value={rememberDismissal} />
        </div>
      </ContinueModal>
    </Portal>;
};
...
export const useBlockingHint = (args: HintArgs): React.ReactNode => {
  const {
    enabled,
    hintKey,
    handleCancel,
  ...
  return <BlockingHint hintKey={hintKey} handleCancel={handleCancel} handleContinue={handleContinue} content={content} />;
};