...
export class EditableTextField extends React.Component<Props, State> {
  ...
  enterEditMode: () => void = () => this.setState({
    editing: true,
    transientValue: this.props.value
  });
  handleCancel: () => void = () => {
  ...
  handleFormSubmit: (e: React.SyntheticEvent) => void = e => {
  ...
  updateValue: (e: React.SyntheticEvent<HTMLInputElement>) => void = e => {
    this.setState({
      transientValue: e.currentTarget.value
    });
  };
  ...
  render(): React.ReactNode {
    ...
    if (this.state.editing) {
      return <ClickOutside onClickOutside={this.handleSubmit}>
          {({
          ref
        }) => <form className={className} onKeyUp={this.handleKeyUp} onSubmit={this.handleFormSubmit} ref={ref}>
              <InputField autoFocus value={this.state.transientValue} onChange={this.updateValue} units={<Icon name="pencil" className={styles.edit_icon} />} />
            </form>}
        </ClickOutside>;
    }

    return <div onClick={this.enterEditMode} className={className}>
