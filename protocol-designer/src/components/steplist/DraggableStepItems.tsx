...
type DragDropStepItemProps = React.ElementProps<typeof ConnectedStepItem> & {
  connectDragSource: (arg0: unknown) => React.ReactElement<any>;
  connectDropTarget: (arg0: unknown) => React.ReactElement<any>;
  stepId: StepIdType;
  stepNumber: number;
  isDragging: boolean;
  findStepIndex: (arg0: StepIdType) => number;
  onDrag: () => void;
  moveStep: (arg0: StepIdType, arg1: number) => void;
};

const DragSourceStepItem = (props: DragDropStepItemProps) => props.connectDragSource(props.connectDropTarget(<div style={{
...
class StepItems extends React.Component<StepItemsProps, StepItemsState> {
  constructor(props) {
    super(props);
    this.state = {
      stepIds: this.props.orderedStepIds
    };
  }
  ...
  moveStep = (stepId: StepIdType, targetIndex: number) => {
    ...
    const currentRemoved = [...stepIds.slice(0, currentIndex), ...stepIds.slice(currentIndex + 1, stepIds.length)];
    const currentReinserted = [...currentRemoved.slice(0, targetIndex), stepId, ...currentRemoved.slice(targetIndex, currentRemoved.length)];
    this.setState({
      stepIds: currentReinserted
    });
  };
  ...
  render() {
    ...
    return this.props.connectDropTarget(<div>
        ...
        }) => currentIds.map((stepId: StepIdType, index: number) => <DragDropStepItem key={stepId} stepNumber={index + 1} stepId={stepId} onStepContextMenu={makeStepOnContextMenu(stepId)} findStepIndex={this.findStepIndex} onDrag={this.onDrag} moveStep={this.moveStep} />)}
        </ContextMenu>
...
const StepDragPreview = (props: StepDragPreviewProps) => {
  ...
  return <div className={styles.step_drag_preview} style={{
    left: currentOffset.x - NAV_OFFSET,
    top: currentOffset.y
  }}>
      <PDTitledList iconName={stepIconsByType[stepType]} title={stepName || ''} onCollapseToggle={() => {}} // NOTE: necessary to render chevron
    collapsed />
    </div>;
};
...
export const StepDragPreviewLayer: React.AbstractComponent<{}> = DragLayer(monitor => ({
...
export const DraggableStepItems: React.AbstractComponent<$Diff<StepItemsProps, {
  isOver: unknown;
  connectDropTarget: unknown;
