...
export class MultiChannelSubstep extends React.PureComponent<MultiChannelSubstepProps, MultiChannelSubstepState> {
  state: MultiChannelSubstepState = {
  ...
  render(): React.ReactNode {
    ...
    return <ol onMouseEnter={() => selectSubstep({
      stepId,
      substepIndex
    })} onMouseLeave={() => selectSubstep(null)} className={cx({
      [styles.highlighted]: highlighted
    })}>
        {
        /* Header row */
      }
        <PDListItem border className={cx(styles.step_subitem, {
        ...
        return <SubstepRow key={rowKey} className={styles.step_subitem_channel_row} volume={row.volume} ingredNames={this.props.ingredNames} source={row.source} dest={row.dest} stepId={stepId} substepIndex={substepIndex} />;
      })}
      </ol>;
  }
