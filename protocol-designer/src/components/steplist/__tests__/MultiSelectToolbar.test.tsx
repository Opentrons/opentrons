...
import { MultiSelectToolbar, ClickableIcon, Accordion } from "../MultiSelectToolbar";
import { ConfirmDeleteModal, CLOSE_BATCH_EDIT_FORM, DELETE_MULTIPLE_STEP_FORMS } from "../../modals/ConfirmDeleteModal";
...
describe('MultiSelectToolbar', () => {
  ...
  beforeEach(() => {
    ...
    when(getOrderedStepIdsMock).calledWith(expect.anything()).mockReturnValue([]);
    when(getMultiSelectItemIdsMock).calledWith(expect.anything()).mockReturnValue([]);
  ...
  it('should have a checked checkbox when all steps are selected, and deselect them all when clicked', () => {
    when(getOrderedStepIdsMock).calledWith(expect.anything()).mockReturnValue(['id_1', 'id_2']);
    when(getMultiSelectItemIdsMock).calledWith(expect.anything()).mockReturnValue(['id_1', 'id_2']);
  ...
  it('should have a minus box when not all steps are selected, and select them all when clicked', () => {
    when(getOrderedStepIdsMock).calledWith(expect.anything()).mockReturnValue(['id_1', 'id_2']);
    when(getMultiSelectItemIdsMock).calledWith(expect.anything()).mockReturnValue(['id_1']);
  ...
  describe('when clicking on expand/collapse', () => {
    ...
    it('should expand/collapse the selected steps', () => {
      when(getMultiSelectItemIdsMock).calledWith(expect.anything()).mockReturnValue(['id_1', 'id_2']);
      const expandMultipleStepsSpy = jest.spyOn(stepActions, 'expandMultipleSteps');
      const wrapper = render();
      ...
      wrapper.update();
      const collapseMultipleStepsSpy = jest.spyOn(stepActions, 'collapseMultipleSteps');
  ...
  describe('when clicking on delete', () => {
    it('should show a confirm delete modal before deleting the steps', () => {
      when(getOrderedStepIdsMock).calledWith(expect.anything()).mockReturnValue(['id_1', 'id_2']);
      when(getMultiSelectItemIdsMock).calledWith(expect.anything()).mockReturnValue(['id_1']);
      const deleteMultipleStepsSpy = jest.spyOn(stepListActions, 'deleteMultipleSteps');
      const wrapper = render();
      ...
      expect(confirmDeleteModal.prop('modalType')).toBe(DELETE_MULTIPLE_STEP_FORMS);
      act(() => {
  ...
  describe('when clicking on duplicate', () => {
    it('should duplicate all of the steps selected when there are NO changes to the batch edit form', () => {
      when(getOrderedStepIdsMock).calledWith(expect.anything()).mockReturnValue(['id_1', 'id_2']);
      when(getMultiSelectItemIdsMock).calledWith(expect.anything()).mockReturnValue(['id_1']);
      const duplicateMultipleStepsSpy = jest.spyOn(stepActions, 'duplicateMultipleSteps');
      const wrapper = render();
