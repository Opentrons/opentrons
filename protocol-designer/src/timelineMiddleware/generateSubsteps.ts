import type { Timeline, RobotState, InvariantContext } from "@opentrons/step-generation";
import { generateSubstepItem } from "../steplist/generateSubstepItem";
import type { LabwareNamesByModuleId, StepArgsAndErrorsById, Substeps } from "../steplist/types";
...
export const generateSubsteps = (args: GenerateSubstepsArgs): Substeps => {
  ...
  return orderedStepIds.reduce((acc: Substeps, stepId, timelineIndex) => {
    ...
    const substeps = generateSubstepItem(allStepArgsAndErrors[stepId], invariantContext, robotState, stepId, labwareNamesByModuleId);
    return { ...acc,
      [stepId]: substeps
    };
  }, {});
