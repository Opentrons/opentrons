import { getArgsAndErrorsByStepId, getOrderedStepIds, getInvariantContext } from "../step-forms/selectors";
...
import { computeRobotStateTimelineRequest, computeRobotStateTimelineSuccess } from "../file-data/actions";
...
const hasChanged = (nextValues: { [key in any]?: any }, memoizedValues: { [key in any]?: any }): boolean => Object.keys(nextValues).some((selectorKey: string) => nextValues[selectorKey] !== memoizedValues?.[selectorKey]);
...
export const makeTimelineMiddleware: () => Middleware<BaseState, any> = () => {
  ...
  const substepsNeedsRecompute = (state: BaseState): boolean => {
    ...
    const needsRecompute = hasChanged(nextSubstepSelectorResults, prevSubstepsArgs);
  ...
  return ({
    ...
    if (shouldRecomputeTimeline) {
      ...
      if (prevTimelineArgs !== null && prevSubstepsArgs !== null) {
        ...
        console.error('something weird happened, prevTimelineArgs and prevSubstepsArgs should never be null here');
      }
