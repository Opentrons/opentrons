...
export const generateRobotStateTimeline = (args: GenerateRobotStateTimelineArgs): StepGeneration.Timeline => {
  const {
  ...
  // $FlowFixMe
  const continuousStepArgs: Array<StepGeneration.CommandCreatorArgs> = takeWhile(allStepArgs, stepArgs => stepArgs);
  const curriedCommandCreators = continuousStepArgs.reduce((acc: Array<StepGeneration.CurriedCommandCreator>, args: StepGeneration.CommandCreatorArgs, stepIndex): Array<StepGeneration.CurriedCommandCreator> => {
    ...
    if (pipetteId) {
      const nextStepArgsForPipette = continuousStepArgs.slice(stepIndex + 1).find(stepArgs => stepArgs.pipette && stepArgs.pipette === pipetteId);
      const willReuseTip = nextStepArgsForPipette?.changeTip && nextStepArgsForPipette.changeTip === 'never';

      if (!willReuseTip) {
        return [...acc, (_invariantContext, _prevRobotState) => StepGeneration.reduceCommandCreators([curriedCommandCreator, StepGeneration.curryCommandCreator(StepGeneration.dropTip, {
          pipette: pipetteId
  ...
  const timeline = StepGeneration.commandCreatorsTimeline(curriedCommandCreators, invariantContext, initialRobotState);
  return timeline;
};