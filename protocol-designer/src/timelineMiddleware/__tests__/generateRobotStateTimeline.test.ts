...
import { getInitialRobotStateStandard, makeContext, DEFAULT_PIPETTE, MULTI_PIPETTE, SOURCE_LABWARE, DEST_LABWARE } from "@opentrons/step-generation/src/__fixtures__";
...
describe('generateRobotStateTimeline', () => {
  it('performs eager tip dropping', () => {
    ...
    const commandOverview = result.timeline.map(frame => frame.commands.map(command => command.command));
