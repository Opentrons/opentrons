...
const definitionsContext = require.context('@opentrons/shared-data/labware/definitions/2', true, // traverse subdirectories
/\.json$/, // import filter
...
export function getAllDefinitions(): LabwareDefByDefURI {
  ...
  if (!_definitions) {
    _definitions = definitionsContext.keys().reduce((acc, filename) => {
      ...
      return PD_DO_NOT_LIST.includes(def.parameters.loadName) ? acc : { ...acc,
        [labwareDefURI]: def
      };
    }, {});
  }
...
export function getOnlyLatestDefs(): LabwareDefByDefURI {
  if (!_latestDefs) {
    ...
    const labwareDefGroups: Record<string, Array<LabwareDefinition2>> = groupBy(allURIs.map((uri: string) => allDefs[uri]), d => `${d.namespace}/${d.parameters.loadName}`);
    _latestDefs = Object.keys(labwareDefGroups).reduce((acc, groupKey: string) => {
...
export function _getSharedLabware(labwareDefURI: string): LabwareDefinition2 | null | undefined {
