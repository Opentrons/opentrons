...
const getSchemaDefForProtocol = (protocol: any): any => {
  ...
  const errorMessage = `bad schema for protocol!: ${n ? `'${n}'. Does a new schema need to be added to validateProtocolFixtures.test.js?` : 'Could not determine schema.'}`;
  throw new Error(errorMessage);
};

describe('Protocol fixtures should validate under their JSON schemas', () => {
  const fixtureDirsGlobPath = path.join(__dirname, '../../fixtures/protocol/**/*.json');
  ...
  fixturePaths.forEach(protocolPath => {
    ...
    it(`${relativeName} should validate under its schema`, () => {
      expectResultToMatchSchema(protocolPath, protocol, getSchemaDefForProtocol(protocol));
    });
    ...
    it(`${relativeName} should be in a folder matching its PD major version, ${expectedVersion}`, () => {
      const designerApplication = protocol.designerApplication || protocol['designer-application'];
      ...
      const pdVersion: string = designerApplication?.applicationVersion || designerApplication?.version || '1';
