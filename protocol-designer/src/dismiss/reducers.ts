...
import type { CancelStepFormAction, DeleteStepAction, DeleteMultipleStepsAction } from "../steplist/actions";
import type { StepIdType } from "../form-types";
export type WarningType = string;
export type DismissedWarningsAllSteps = Record<StepIdType, Array<WarningType> | null | undefined>;
...
const dismissedWarnings: Reducer<DismissedWarningState, any> = handleActions({
  DISMISS_FORM_WARNING: (state: DismissedWarningState, action: DismissFormWarning): DismissedWarningState => {
    const {
      type
    } = action.payload;
  ...
  DISMISS_TIMELINE_WARNING: (state: DismissedWarningState, action: DismissTimelineWarning): DismissedWarningState => {
    const {
      type
    } = action.payload;
    const stepId = action.payload.stepId;
  ...
  DELETE_MULTIPLE_STEPS: (state: DismissedWarningState, action: DeleteMultipleStepsAction): DismissedWarningState => {
  ...
  LOAD_FILE: (state: DismissedWarningState, action: LoadFileAction): DismissedWarningState => getPDMetadata(action.payload.file).dismissedWarnings,
  CANCEL_STEP_FORM: (state: DismissedWarningState, action: CancelStepFormAction): DismissedWarningState => ({
