...
import { MAGNETIC_MODULE_TYPE, THERMOCYCLER_MODULE_TYPE } from "@opentrons/shared-data";
import { createBlankForm, getNextDefaultEngageHeight, getNextDefaultMagnetAction, getNextDefaultPipetteId, getNextDefaultTemperatureModuleId, getNextDefaultThermocyclerModuleId, handleFormChange } from "../../steplist/formLevel";
import { getModuleOnDeckByType, getMagnetLabwareEngageHeight } from "../../ui/modules/utils";
import { maskField } from "../../steplist/fieldLevel";
import type { PipetteEntities, LabwareEntities, RobotState, Timeline } from "@opentrons/step-generation";
...
const _patchDefaultPipette = (args: {
  ...
  const defaultPipetteId = getNextDefaultPipetteId(savedStepForms, orderedStepIds, initialDeckSetup.pipettes);
  ...
  if (formHasPipetteField && defaultPipetteId !== '') {
    ...
    }, formData, pipetteEntities, labwareEntities);
...
const _patchDefaultMagnetFields = (args: {
  ...
  const moduleId = getModuleOnDeckByType(initialDeckSetup, MAGNETIC_MODULE_TYPE)?.id || null;
  const magnetAction = getNextDefaultMagnetAction(savedStepForms, orderedStepIds);
  const defaultEngageHeight = getMagnetLabwareEngageHeight(initialDeckSetup, moduleId);
  const stringDefaultEngageHeight = defaultEngageHeight ? maskField('engageHeight', defaultEngageHeight) : null;
  const prevEngageHeight = getNextDefaultEngageHeight(savedStepForms, orderedStepIds);
  // 'magnet' steps only.
  //
...
const _patchTemperatureModuleId = (args: {
  ...
  const hasTemperatureModuleId = stepType === 'pause' || stepType === 'temperature';

  // Auto-populate moduleId field of 'pause' and 'temperature' steps.
  //
  // Bypass dependent field changes, do not use handleFormChange
  if (hasTemperatureModuleId) {
    const moduleId = getNextDefaultTemperatureModuleId(savedStepForms, orderedStepIds, initialDeckSetup.modules);
    return {
      moduleId
    };
  }
...
const _patchThermocyclerFields = (args: {
  ...
  const lastRobotState: RobotState | null | undefined = last(robotStateTimeline.timeline)?.robotState;
...
export const createPresavedStepForm = ({
  ...
  return [updateDefaultPipette, updateTemperatureModuleId, updateThermocyclerFields, updateMagneticModuleId].reduce<FormData>((acc, updater: FormUpdater) => {
