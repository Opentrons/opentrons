...
export type NestedCombineReducers<S, A> = (getNextState: GetNextState) => Reducer<S, A>;

const getUndefinedStateErrorMessage = (key: string, action: Record<string, any>) => {
...
export function nestedCombineReducers<S, A>(getNextState: GetNextState): Reducer<S, A> {
  ...
  return (state, action) => {
    ...
    if (state !== null && typeof state === 'object' && Object.keys(nextState).every((key: string) => state[key] === nextState[key])) {
      // no change
      return state;
    }
