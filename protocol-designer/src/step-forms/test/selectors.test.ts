import { _hasFieldLevelErrors, getEquippedPipetteOptions, getBatchEditFormHasUnsavedChanges } from "../selectors";
...
const mockGetFieldErrors: JestMockFn<[string, unknown], Array<string>> = getFieldErrors;
const mockGetProfileItemsHaveErrors: JestMockFn<[any], boolean> = getProfileItemsHaveErrors;
beforeEach(() => {
  jest.clearAllMocks();
});
...
describe('getBatchEditFormHasUnsavedChanges', () => {
  it('should return true if there are unsaved changes ', () => {
    expect(getBatchEditFormHasUnsavedChanges.resultFunc({
      someField: 'someVal'
    })).toBe(true);
  });
