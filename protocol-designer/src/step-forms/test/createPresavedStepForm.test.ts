import { MAGNETIC_MODULE_TYPE, MAGNETIC_MODULE_V2, TEMPERATURE_MODULE_TYPE, TEMPERATURE_MODULE_V2, THERMOCYCLER_MODULE_TYPE, THERMOCYCLER_MODULE_V1 } from "@opentrons/shared-data";
...
import { DEFAULT_DELAY_SECONDS, DEFAULT_MM_FROM_BOTTOM_DISPENSE } from "../../constants";
...
beforeEach(() => {
  ...
  defaultArgs = {
    ...
    initialDeckSetup: {
      labware: {
        labwareOnMagModule: { ...labwareOnMagModule,
...
describe('createPresavedStepForm', () => {
  ...
  [true, false].forEach(hasTempModule => {
    it(`should populate initial values for a new pause step (with ${hasTempModule ? '' : 'NO'} temp module)`, () => {
      const args: CreatePresavedStepFormArgs = { ...defaultArgs,
        stepType: 'pause',
        initialDeckSetup: hasTempModule ? defaultArgs.initialDeckSetup : { ...defaultArgs.initialDeckSetup,
          modules: {}
        }
      };
  ...
  [true, false].forEach(timelineHasErrors => {
    ...
    [true, false].forEach(isFirstThermocyclerStep => {
      const testName = `should set a default thermocycler module and set TC state defaults when ${isFirstThermocyclerStep ? 'the first Thermocycler step is added' : 'a following Thermocycler step is added'}${timelineHasErrors ? ' and timeline has errors' : ''}`;
      it(testName, () => {
        ...
        if (timelineHasErrors) {
          ...
          const thermocyclerModuleState = defaultArgs.robotStateTimeline.timeline[0].robotState.modules['someThermocyclerModuleId'];
          thermocyclerModuleState.moduleState = { ...thermocyclerModuleState.moduleState,
            blockTargetTemp: 42,
            lidTargetTemp: 43,
            lidOpen: true
          };
