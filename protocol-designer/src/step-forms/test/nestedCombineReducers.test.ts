...
describe('nestedCombineReducers', () => {
  ...
  it('should throw an error when any sub-reducer returns undefined', () => {
    ...
    const badCombinedReducer = nestedCombineReducers(({
      ...
      // warnings gets the full state
      warnings: warnings(state, action),
      badCountReducer: badCountReducer(prevStateFallback.badCountReducer, action)
    }));
    expect(() => {
      badCombinedReducer({
        fruits: [],
    ...
    }).toThrowError('Given action "UNHANDLED_ACTION", reducer "badCountReducer" returned undefined. To ignore an action, you must explicitly return the previous state. If you want this reducer to hold no value, you can return null instead of undefined.');
  });
  it('should throw an error when any sub-reducer initializes to undefined', () => {
    ...
    }).toThrowError(`Reducer "badCountReducer" returned undefined during initialization. If the state passed to the reducer is undefined, you must explicitly return the initial state. The initial state may not be undefined. If you don't want to set a value for this reducer, you can use null instead of undefined.`);
  });
});