import { MAGNETIC_MODULE_TYPE, TEMPERATURE_MODULE_TYPE, THERMOCYCLER_MODULE_TYPE, MAGNETIC_MODULE_V1, MAGNETIC_MODULE_V2 } from "@opentrons/shared-data";
import { orderedStepIds, labwareInvariantProperties, moduleInvariantProperties, presavedStepForm, savedStepForms, unsavedForm, batchEditFormChanges } from "../reducers";
...
import { INITIAL_DECK_SETUP_STEP_ID, SPAN7_8_10_11_SLOT, PAUSE_UNTIL_TEMP } from "../../constants";
...
const mockCreatePresavedStepForm: JestMockFn<[CreatePresavedStepFormArgs], any> = createPresavedStepForm;
const handleFormChangeMock: JestMockFn<[Record<string, any>, Record<string, any>, any, any], Record<string, any>> = handleFormChange;
const getLabwareIsCompatibleMock: JestMockFn<[any], boolean> = getLabwareIsCompatible;
const mock_getPipetteEntitiesRootState: JestMockFn<[any], any> = _getPipetteEntitiesRootState;
const mock_getLabwareEntitiesRootState: JestMockFn<[any], any> = _getLabwareEntitiesRootState;
const mock_getInitialDeckSetupRootState: JestMockFn<[any], any> = _getInitialDeckSetupRootState;
afterEach(() => {
  jest.clearAllMocks();
});
...
describe('moduleInvariantProperties reducer', () => {
  ...
  it('edit module (change its model)', () => {
    ...
    expect(result).toEqual({
      [existingModuleId]: { ...prevState.existingModuleId,
        model: newModel
      }
    });
  });
...
describe('savedStepForms reducer: initial deck setup step', () => {
  ...
  describe('move deck item', () => {
    const testCases: Array<{
      ...
      testName: 'move labware to slot with occupied module -> swap labware, module stays',
      sourceSlot: '1',
      destSlot: moduleId,
      makeStateArgs: {
        labwareLocationUpdate: {
      ...
      testName: 'move empty module to slot with incompatible labware -> swap slots, do not add labware to module',
      sourceSlot: '1',
      destSlot: '3',
      makeStateArgs: {
        labwareLocationUpdate: {
      ...
      testName: 'move empty module to slot with compatible labware -> put module under labware',
      sourceSlot: '1',
      destSlot: '3',
      makeStateArgs: {
        labwareLocationUpdate: {
      ...
      testName: 'move occupied module to slot with labware -> swap slots, do not change labware on module (even if compatible)',
      sourceSlot: '1',
      destSlot: '3',
      makeStateArgs: {
        labwareLocationUpdate: {
    ...
    testCases.forEach(({
      ...
      it(testName, () => {
        ...
        if (expectedLabwareLocations) {
          expect(result[INITIAL_DECK_SETUP_STEP_ID].labwareLocationUpdate).toEqual(expectedLabwareLocations);
        }

        if (expectedModuleLocations) {
          expect(result[INITIAL_DECK_SETUP_STEP_ID].moduleLocationUpdate).toEqual(expectedModuleLocations);
        }
      });
    });
  });
  ...
  it('delete pipettes -> removes pipette(s) from initial deck setup step', () => {
    ...
    testCases.forEach(({
      ...
      const result = savedStepForms(prevRootState, action);
      expect(result[INITIAL_DECK_SETUP_STEP_ID].pipetteLocationUpdate).toEqual(expected);
    });
  });
  describe('create module', () => {
    describe('NO existing steps', () => {
      const destSlot = '3';
      const testCases = [{
        testName: 'create module in empty deck slot (labware in unrelated slot unaffected)',
        makeStateArgs: {
          labwareLocationUpdate: {
            [existingLabwareId]: '6'
          }
        ...
        testName: 'create module in deck slot occupied with labware -> move that labware to the new module',
        makeStateArgs: {
          labwareLocationUpdate: {
    ...
    describe('existing steps', () => {
      ...
      beforeEach(() => {
        prevRootStateWithMagAndTCSteps = {
          savedStepForms: { ...makePrevRootState().savedStepForms,
            ...{
              mag_step_form_id: {
                stepType: 'magnet',
                moduleId: 'magdeckId'
      ...
      magneticStepCases.forEach(({
        ...
        it(testName, () => {
          ...
          if (action.payload.type) expect(result.mag_step_form_id.moduleId).toBe(expectedModuleId);
        });
      });
      TCStepCases.forEach(({
        testName,
        ...
        it(testName, () => {
          const result = savedStepForms(prevRootStateWithMagAndTCSteps, action);
          if (action.payload.type) expect(result.TC_step_form_id.moduleId).toBe(expectedModuleId);
        });
      });
    });
  });
  describe('delete module -> removes module from initial deck setup step', () => {
    const testCases = [{
      ...
      testName: 'delete occupied module in span7_8_10_11 slot -> labware goes into slot 7',
      makeStateArgs: {
        labwareLocationUpdate: {
          [labwareOnModuleId]: moduleId
        },
  ...
  describe('delete module -> removes references to module from step forms', () => {
    ...
    testCases.forEach(({
      ...
      it(testName, () => {
        ...
        expect(result[stepId]).toEqual({ ...step,
          moduleId: null
        });
      });
    });
  ...
  describe('EDIT_MODULE', () => {
    it('should set engageHeight to null for all Magnet > Engage steps when a magnet module has its model changed, unless height matches default', () => {
      ...
      expect(result).toEqual({
        ...
        magnetDisengageStepId: prevRootState.savedStepForms.magnetDisengageStepId,
        nonDefaultMagnetEngageStepId: {
          stepType: 'magnet',
          magnetAction: 'engage',
          moduleId: 'magModuleId',
        ...
        unrelatedMagnetEngageStepId: prevRootState.savedStepForms.unrelatedMagnetEngageStepId
      });
    });
  });
  describe('saving multiple steps', () => {
    it('should apply the form patch to all of the step ids', () => {
      ...
      expect(savedStepForms(prevState, action)).toEqual(expectedSavedStepFormState);
    });
  });
});
describe('unsavedForm reducer', () => {
  ...
  it('should use handleFormChange to update the state with CHANGE_FORM_INPUT action', () => {
    ...
    mock_getPipetteEntitiesRootState.mockReturnValue('pipetteEntitiesPlaceholder');
    mock_getLabwareEntitiesRootState.mockReturnValue('labwareEntitiesPlaceholder');
    ...
    expect(handleFormChangeMock.mock.calls).toEqual([[action.payload.update, rootState.unsavedForm, 'pipetteEntitiesPlaceholder', 'labwareEntitiesPlaceholder']]);
    expect(result).toEqual({
      existingField: 123,
      someField: 42
    });
  ...
  it("should switch out pipettes via handleFormChange in response to SUBSTITUTE_STEP_FORM_PIPETTES if the unsaved form's ID is in range", () => {
    ...
    expect(handleFormChangeMock.mock.calls).toEqual([[{
      pipette: 'newPipetteId'
    }, rootState.unsavedForm, 'pipetteEntitiesPlaceholder', 'labwareEntitiesPlaceholder']]);
    expect(result).toEqual({
      id: '4',
      pipette: 'newPipetteId',
      otherField: 'blah'
  ...
  const actionTypes = ['CANCEL_STEP_FORM', 'CREATE_MODULE', 'DELETE_MODULE', 'DELETE_STEP', 'DELETE_MULTIPLE_STEPS', 'EDIT_MODULE', 'SAVE_STEP_FORM', 'SELECT_TERMINAL_ITEM', 'SELECT_MULTIPLE_STEPS'];
  ...
  it('should return the result createPresavedStepForm util upon ADD_STEP action', () => {
    mockCreatePresavedStepForm.mockReturnValue('createPresavedStepFormMockResult');
  ...
  it('should add a profile step item to the specified cycle upon ADD_PROFILE_STEP action with cycleId payload', () => {
    ...
    expect(result).toEqual({
      ...
      orderedProfileItems: [cycleId],
      profileItemsById: {
        [cycleId]: { ...state.unsavedForm.profileItemsById[cycleId],
          steps: [{
            id: stepId,
            type: PROFILE_STEP,
            title: '',
...
describe('presavedStepForm reducer', () => {
  ...
  const clearingActions = ['CANCEL_STEP_FORM', 'DELETE_STEP', 'DELETE_MULTIPLE_STEPS', 'SAVE_STEP_FORM', 'SELECT_STEP', 'SELECT_MULTIPLE_STEPS'];
  clearingActions.forEach(actionType => {
    it(`should clear upon ${actionType}`, () => {
      ...
      expect(presavedStepForm(prevState, {
        type: actionType
      })).toEqual(null);
    });
  });
...
describe('batchEditFormChanges reducer', () => {
  it('should add the new fields into empty state on CHANGE_BATCH_EDIT_FIELD', () => {
    ...
    expect(batchEditFormChanges(state, { ...action
    })).toEqual({
      someFieldName: 'someFieldValue'
    });
  });
