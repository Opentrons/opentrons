...
export const _rehydrate = (path: string): any => {
  assert(PERSISTED_PATHS.includes(path), `Path "${path}" is missing from PERSISTED_PATHS! The changes to this reducer will not be persisted.`);
  return getLocalStorageItem(path);
};
export const _rehydrateAll = (): $PropertyType<RehydratePersistedAction, "payload"> => {
...
const PERSISTED_PATHS = ['analytics.hasOptedIn', 'tutorial.dismissedHints', 'featureFlags.flags'];
...
export const setLocalStorageItem = (path: string, value: any) => {
  try {
    global.localStorage.setItem(_addStoragePrefix(path), JSON.stringify(transformBeforePersist(path, value)));
  } catch (e) {
...
export const makePersistSubscriber = (store: Store<any, any>): PersistSubscriber => {
