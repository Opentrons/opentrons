...
function _wellsForPipette(pipetteEntity: PipetteEntity, labwareEntity: LabwareEntity, wells: Array<string>): Array<string> {
...
function _getSelectedWellsForStep(stepArgs: StepGeneration.CommandCreatorArgs, labwareId: string, frame: StepGeneration.CommandsAndRobotState, invariantContext: StepGeneration.InvariantContext): Array<string> {
  if (StepGeneration.getHasNoWellsFromCCArgs(stepArgs)) {
    return [];
  }

  const pipetteId = StepGeneration.getPipetteIdFromCCArgs(stepArgs);
  const pipetteEntity = pipetteId ? invariantContext.pipetteEntities[pipetteId] : null;
  ...
  const getWells = (wells: Array<string>) => _wellsForPipette(pipetteEntity, labwareEntity, wells);
  ...
  frame.commands.forEach((c: Command) => {
    if (c.command === 'pickUpTip' && c.params.labware === labwareId) {
      ...
      if (pipetteSpec.channels === 1) {
        ...
        const wellSet = getWellSetForMultichannel(invariantContext.labwareEntities[labwareId].def, commandWellName) || [];
        wells.push(...wellSet);
      } else {
...
function _getSelectedWellsForSubstep(stepArgs: StepGeneration.CommandCreatorArgs, labwareId: string, substeps: SubstepItemData | null | undefined, substepIndex: number, invariantContext: StepGeneration.InvariantContext): Array<string> {
  if (substeps === null) {
    return [];
  }
  ...
  if (substeps && substeps.substepType === 'sourceDest') {
    ...
    if (substeps.multichannel) {
      const {
        activeTips
      } = substeps.multiRows[substepIndex][0]; // just use first multi row

      if (activeTips && activeTips.labware === labwareId) {
        const multiTipWellSet = getWellSetForMultichannel(invariantContext.labwareEntities[labwareId].def, activeTips.well);
      ...
      const {
        activeTips
      } = substeps.rows[substepIndex];
      if (activeTips && activeTips.labware === labwareId && activeTips.well) tipWellSet = [activeTips.well];
    }
...
export const wellHighlightsByLabwareId: Selector<Record<string, WellGroup>> = createSelector(fileDataSelectors.getRobotStateTimeline, stepFormSelectors.getInvariantContext, stepFormSelectors.getArgsAndErrorsByStepId, getHoveredStepId, getHoveredSubstep, fileDataSelectors.getSubsteps, stepFormSelectors.getOrderedStepIds, (robotStateTimeline, invariantContext, allStepArgsAndErrors, hoveredStepId, hoveredSubstep, substepsById, orderedStepIds) => {
  ...
  return mapValues(robotState.liquidState.labware, (labwareLiquids: StepGeneration.SingleLabwareLiquidState, labwareId: string): AllWellHighlights => {
    let selectedWells: Array<string> = [];

    if (hoveredSubstep != null) {
      // wells for hovered substep
      selectedWells = _getSelectedWellsForSubstep(stepArgs, labwareId, substepsById[stepId], hoveredSubstep.substepIndex, invariantContext);
    } else {
      // wells for step overall
      selectedWells = _getSelectedWellsForStep(stepArgs, labwareId, frame, invariantContext);
    }

    // return selected wells eg {A1: null, B4: null}
    return selectedWells.reduce((acc: AllWellHighlights, well) => ({ ...acc,
      [well]: null
    }), {});
  });
});