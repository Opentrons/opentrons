...
function _wellContentsForWell(liquidVolState: StepGeneration.LocationLiquidState, well: string): WellContents {
  ...
  const ingredGroupIdsWithContent = Object.keys(liquidVolState || {}).filter(groupId => liquidVolState[groupId] && liquidVolState[groupId].volume > 0);
  return {
    wellName: well,
...
export function _wellContentsForLabware(labwareLiquids: StepGeneration.SingleLabwareLiquidState, labwareDef: LabwareDefinition2): ContentsByWell {
  const allWellsForContainer = getAllWellsForLabware(labwareDef);
  return reduce(allWellsForContainer, (wellAcc, well: string): Record<string, WellContents> => {
    const wellHasContents = labwareLiquids && labwareLiquids[well];
    return { ...wellAcc,
      [well]: wellHasContents ? _wellContentsForWell(labwareLiquids[well], well) : {}
    };
  }, {});
}
export const getAllWellContentsForActiveItem: Selector<WellContentsByLabware | null> = createSelector(stepFormSelectors.getLabwareEntities, timelineFrameBeforeActiveItem, (labwareEntities, timelineFrame) => {
  if (timelineFrame == null) return null;
  const liquidState = timelineFrame.robotState.liquidState.labware;
  const wellContentsByLabwareId = mapValues(liquidState, (labwareLiquids: StepGeneration.SingleLabwareLiquidState, labwareId: string) => _wellContentsForLabware(labwareLiquids, labwareEntities[labwareId].def));
  return wellContentsByLabwareId;
});
export const getSelectedWellsMaxVolume: Selector<number> = createSelector(getSelectedWells, labwareIngredSelectors.getSelectedLabwareId, stepFormSelectors.getLabwareEntities, (selectedWells, selectedLabwareId, labwareEntities) => {
...
export const getSelectedWellsCommonValues: Selector<CommonWellValues> = createSelector(getSelectedWells, labwareIngredSelectors.getSelectedLabwareId, labwareIngredSelectors.getLiquidsByLabwareId, (selectedWells, labwareId, allIngreds) => {
  if (!labwareId) return {
    ingredientId: null,
    volume: null
  };
  const ingredsInLabware = allIngreds[labwareId];
  if (!ingredsInLabware || isEmpty(selectedWells)) return {
    ingredientId: null,
    volume: null
  };
  const initialWellContents: StepGeneration.LocationLiquidState | null | undefined = ingredsInLabware[Object.keys(selectedWells)[0]];
  // TODO IMMEDIATELY why arbitrary 0th???
  const initialIngredId: string | null | undefined = initialWellContents && Object.keys(initialWellContents)[0];
  ...
  if (!hasCommonIngred || !initialIngredId || !initialWellContents) {
    ...
    const initialVolume: number | null | undefined = initialWellContents[initialIngredId].volume;
    const hasCommonVolume = Object.keys(selectedWells).every((well: string) => {
...
export const getSelectedWellsCommonIngredId: Selector<string | null | undefined> = createSelector(getSelectedWellsCommonValues, commonValues => commonValues.ingredientId || null);
export const getSelectedWellsCommonVolume: Selector<number | null | undefined> = createSelector(getSelectedWellsCommonValues, commonValues => commonValues.volume || null);