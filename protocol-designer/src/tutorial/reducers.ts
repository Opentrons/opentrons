...
const hints = handleActions({
  ADD_HINT: (state: HintReducerState, action: AddHintAction): HintReducerState => uniq([...state, action.payload.hintKey]),
  // going to the steplist page triggers 'deck_setup_explanation' hint
  NAVIGATE_TO_PAGE: (state: HintReducerState, action: NavigateToPageAction): HintReducerState => action.payload === 'steplist' ? uniq([...state, 'deck_setup_explanation']) : state
}, []);
...
const dismissedHints: Reducer<DismissedHintReducerState, any> = handleActions({
  ...
  REHYDRATE_PERSISTED: (state: DismissedHintReducerState, action: RehydratePersistedAction) => {
...
export const dismissedHintsPersist = (state: DismissedHintReducerState): DismissedHintReducerState => {
  // persist only 'rememberDismissal' hints
  return pickBy(state, (h: $Values<DismissedHintReducerState>) => h && h.rememberDismissal);
};
const _allReducers = {
  hints,
  dismissedHints
...
export const rootReducer: Reducer<RootState, Action> = combineReducers(_allReducers);