...
export const getHint: Selector<HintKey | null | undefined> = createSelector(rootSelector, tutorial => {
...
export const getDismissedHints: Selector<Array<HintKey>> = createSelector(rootSelector, tutorial => {
  const dismissedKeys = Object.keys(tutorial.dismissedHints);
  return dismissedKeys;
});
export const getCanClearHintDismissals: Selector<boolean> = createSelector(rootSelector, tutorial => !isEmpty(tutorial.dismissedHints));
export const shouldShowCoolingHint: Selector<boolean> = createSelector(timelineFrameBeforeActiveItem, // TODO(IL, 2020-12-15): this shouldn't use activeItem bc that is tied to hover state
getUnsavedForm, (prevTimelineFrame, unsavedForm) => {
  ...
  const prevModuleState = prevTimelineFrame.robotState.modules[moduleId]?.moduleState;
...
export const shouldShowBatchEditHint: Selector<boolean> = createSelector(getOrderedStepIds, orderedStepIds => orderedStepIds.length >= 1);