...
import type { HighlightWellsAction, SelectWellsAction, DeselectWellsAction } from "./actions";
type SelectedWellsState = {
  highlighted: WellGroup;
  selected: WellGroup;
};

function deleteWells(initialWells: WellGroup, wellsToRemove: WellGroup): WellGroup {
...
const selectedWells = handleActions({
  HIGHLIGHT_WELLS: (state, action: HighlightWellsAction): SelectedWellsState => ({ ...state,
  ...
  DESELECT_WELLS: (state, action: DeselectWellsAction): SelectedWellsState => ({
    highlighted: {},
