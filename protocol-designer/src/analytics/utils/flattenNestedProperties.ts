...
const _innerFnFlattenNested = (innerProperties: Record<string, unknown>, prefix: string): any => {
  return Object.keys(innerProperties).reduce((acc, key) => {
    ...
    if (isPlainObject(nestedValue)) {
      return {
        ...acc,
        ..._innerFnFlattenNested(nestedValue as Record<string, unknown>, `${prefix}${SEPARATOR}${key}`),
      }
    }
