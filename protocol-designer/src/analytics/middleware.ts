...
import { getArgsAndErrorsByStepId, getPipetteEntities, getSavedStepForms } from "../step-forms/selectors";
...
export const reduxActionToAnalyticsEvent = (state: BaseState, action: any): AnalyticsEvent | null => {
  if (action.type === 'SAVE_STEP_FORM') {
    ...
    if (stepArgs !== null) {
      ...
      additionalProperties.__dateCreated = dateCreatedTimestamp != null && Number.isFinite(dateCreatedTimestamp) ? new Date(dateCreatedTimestamp).toISOString() : null;
  ...
  if (action.type === 'SAVE_STEP_FORMS_MULTI') {
    ...
    const batchEditedStepForms: Array<FormData> = stepIds.map(id => savedStepForms[id]);
    let stepType = null;
    const uniqueStepTypes: Array<StepType> = uniq(batchEditedStepForms.map(form => form.stepType));

    if (uniqueStepTypes.length === 1) {
      stepType = uniqueStepTypes[0];
    } else {
      console.warn(`Something went wrong, expected one step type in the batch edit form, but got ${String(uniqueStepTypes)} `);
    }
