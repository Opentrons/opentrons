...
const MIXPANEL_ID = getIsProduction() ? process.env.OT_PD_MIXPANEL_ID : process.env.OT_PD_MIXPANEL_DEV_ID;
const MIXPANEL_OPTS = {
  // opt out by default
...
export function initializeMixpanel(state: BaseState) {
  ...
  if (MIXPANEL_ID) {
    ...
    trackEvent({
      name: 'appOpen',
      properties: {}
    }, optedIn); // TODO IMMEDIATELY: do we want this?
  } else {
...
export function setMixpanelTracking(optedIn: boolean) {
  if (MIXPANEL_ID) {
    if (optedIn) {
      ...
      console.debug('User has opted out of analytics; stopping Mixpanel tracking');
