...
const initialFlags: Flags = {
  ...
  OT_PD_DISABLE_MODULE_RESTRICTIONS: process.env.OT_PD_DISABLE_MODULE_RESTRICTIONS === '1' || false
};
...
const flags: Reducer<Flags, any> = handleActions({
  SET_FEATURE_FLAGS: (state: Flags, action: SetFeatureFlagAction): Flags => {
    ...
    if (action.payload.PRERELEASE_MODE === false) {
      ...
      return mapValues(nextState, (value, flagName) => userFacingFlags.includes(flagName) ? value : false);
    }
  ...
  REHYDRATE_PERSISTED: (state: Flags, action: RehydratePersistedAction): Flags => ({ ...state,
