...
export const closeLabwareSelector: () => CloseLabwareSelectorAction = createAction('CLOSE_LABWARE_SELECTOR');
// ===== Open and close Ingredient Selector modal ====
export type OpenIngredientSelectorAction = {
  type: "OPEN_INGREDIENT_SELECTOR";
  payload: string;
};
export const openIngredientSelector: (payload // $FlowFixMe(mc, 2020-06-04): creatActions doesn't return exact actions
: string) => OpenIngredientSelectorAction = createAction('OPEN_INGREDIENT_SELECTOR');
export type CloseIngredientSelectorAction = {
  type: "CLOSE_INGREDIENT_SELECTOR";
};
// @ts-expect-error(mc, 2020-06-04): creatActions doesn't return exact actions
export const closeIngredientSelector: () => CloseIngredientSelectorAction = createAction('CLOSE_INGREDIENT_SELECTOR');
// ===== Drill Down on Labware ====
export type DrillDownOnLabwareAction = {
  type: "DRILL_DOWN_ON_LABWARE";
  payload: string;
};
export const drillDownOnLabware: (payload // $FlowFixMe(mc, 2020-06-04): creatActions doesn't return exact actions
: string) => DrillDownOnLabwareAction = createAction('DRILL_DOWN_ON_LABWARE');
export type DrillUpFromLabwareAction = {
  type: "DRILL_UP_FROM_LABWARE";
};
// @ts-expect-error(mc, 2020-06-04): creatActions doesn't return exact actions
export const drillUpFromLabware: () => DrillUpFromLabwareAction = createAction('DRILL_UP_FROM_LABWARE');
...
export const moveDeckItem = (sourceSlot: DeckSlot, destSlot: DeckSlot): SwapSlotContentsAction => ({
  type: 'MOVE_DECK_ITEM',
  payload: {
    sourceSlot,
    destSlot
...
export const removeWellsContents: (payload: $PropertyType<RemoveWellsContentsAction, "payload">) => RemoveWellsContentsAction = payload => ({
  type: 'REMOVE_WELLS_CONTENTS',
  payload
});
...
export const deleteLiquidGroup: (liquidGroupId: string) => ThunkAction<DeleteLiquidGroupAction> = liquidGroupId => (dispatch, getState) => {
  ...
  const okToDelete = liquidIsOnDeck ? global.confirm('This liquid has been placed on the deck, are you sure you want to delete it?') : true;
...
export const editLiquidGroup: (args: IngredInputs & {
  liquidGroupId: string | null | undefined;
}) => ThunkAction<EditLiquidGroupAction> = args => (dispatch, getState) => {
  const {
    liquidGroupId,
    ...payloadArgs
  } = args; // NOTE: separate liquidGroupId for flow to understand unpacking :/
