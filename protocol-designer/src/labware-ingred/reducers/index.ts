...
import type { SingleLabwareLiquidState, LocationLiquidState, LabwareLiquidState } from "@opentrons/step-generation";
import type { Action, DeckSlot } from "../../types";
import type { LiquidGroupsById, DisplayLabware } from "../types";
import type { LoadFileAction } from "../../load-file";
import type { RemoveWellsContentsAction, CreateContainerAction, DeleteLiquidGroupAction, DuplicateLabwareAction, EditLiquidGroupAction, SelectLiquidAction, SetWellContentsAction, RenameLabwareAction, DeleteContainerAction, OpenAddLabwareModalAction, OpenIngredientSelectorAction, CloseIngredientSelectorAction, DrillDownOnLabwareAction, DrillUpFromLabwareAction } from "../actions";
// REDUCERS
...
const modeLabwareSelection: Reducer<DeckSlot | false, any> = handleActions({
  OPEN_ADD_LABWARE_MODAL: (state, action: OpenAddLabwareModalAction) => action.payload.slot,
...
const selectedContainerId: Reducer<SelectedContainerId, any> = handleActions({
  OPEN_INGREDIENT_SELECTOR: (state, action: OpenIngredientSelectorAction): SelectedContainerId => action.payload,
  CLOSE_INGREDIENT_SELECTOR: (state, action: CloseIngredientSelectorAction): SelectedContainerId => null
}, null);
...
const drillDownLabwareId: Reducer<DrillDownLabwareId, any> = handleActions({
  DRILL_DOWN_ON_LABWARE: (state, action: DrillDownOnLabwareAction): DrillDownLabwareId => action.payload,
  DRILL_UP_FROM_LABWARE: (state, action: DrillUpFromLabwareAction): DrillDownLabwareId => null
}, null);
...
export const containers: Reducer<ContainersState, any> = handleActions({
  ...
  DELETE_CONTAINER: (state: ContainersState, action: DeleteContainerAction): ContainersState => pickBy(state, (value: DisplayLabware, key: string) => key !== action.payload.labwareId),
  RENAME_LABWARE: (state: ContainersState, action: RenameLabwareAction): ContainersState => {
    const {
      labwareId,
      name
    } = action.payload;
  ...
  LOAD_FILE: (state: ContainersState, action: LoadFileAction): ContainersState => {
    ...
    const sortedLabwareIds: Array<string> = Object.keys(allFileLabware).sort((a, b) => Number(allFileLabware[a].slot) - Number(allFileLabware[b].slot));
    return sortedLabwareIds.reduce((acc: ContainersState, id): ContainersState => {
      ...
      const disambiguationNumber = Object.keys(acc).filter((filterId: string) => allFileLabware[filterId].displayName === nickname).length + 1;
      return { ...acc,
        [id]: {
          nickname,
...
export const savedLabware: Reducer<SavedLabwareState, any> = handleActions({
  DELETE_CONTAINER: (state: SavedLabwareState, action: DeleteContainerAction) => ({ ...state,
    [action.payload.labwareId]: false
  }),
  RENAME_LABWARE: (state: SavedLabwareState, action: RenameLabwareAction) => ({ ...state,
    [action.payload.labwareId]: true
  }),
  DUPLICATE_LABWARE: (state: SavedLabwareState, action: DuplicateLabwareAction) => ({ ...state,
    [action.payload.duplicateLabwareId]: true
  }),
  LOAD_FILE: (state: SavedLabwareState, action: LoadFileAction): SavedLabwareState => mapValues(action.payload.file.labware, () => true)
}, {});
...
export const ingredients: Reducer<IngredientsState, any> = handleActions({
  EDIT_LIQUID_GROUP: (state: IngredientsState, action: EditLiquidGroupAction): IngredientsState => {
    ...
    return { ...state,
      [liquidGroupId]: { ...state[liquidGroupId],
        ...action.payload
      }
    };
  },
  DELETE_LIQUID_GROUP: (state: IngredientsState, action: DeleteLiquidGroupAction): IngredientsState => {
    const liquidGroupId = action.payload;
    return omit(state, liquidGroupId);
  },
  LOAD_FILE: (state: IngredientsState, action: LoadFileAction): IngredientsState => getPDMetadata(action.payload.file).ingredients
}, {});
...
export const ingredLocations: Reducer<LocationsState, any> = handleActions({
  SET_WELL_CONTENTS: (state: LocationsState, action: SetWellContentsAction): LocationsState => {
    ...
    const updatedWells = wells.reduce<SingleLabwareLiquidState>((acc, wellName) => ({ ...acc,
  ...
  DUPLICATE_LABWARE: (state: LocationsState, action: DuplicateLabwareAction): LocationsState => {
    ...
    return { ...state,
      [duplicateLabwareId]: { ...state[templateLabwareId]
      }
    };
  },
  REMOVE_WELLS_CONTENTS: (state: LocationsState, action: RemoveWellsContentsAction): LocationsState => {
    const {
      wells,
      labwareId
    } = action.payload;
    return { ...state,
      [labwareId]: { ...omit(state[labwareId], wells)
      }
    };
  },
  DELETE_LIQUID_GROUP: (state: LocationsState, action: DeleteLiquidGroupAction): LocationsState => {
    const liquidGroupId = action.payload;
    return mapValues(state, labwareContents => mapValues(labwareContents, well => omit(well, liquidGroupId)));
  },
  DELETE_CONTAINER: (state: LocationsState, action: DeleteContainerAction): LocationsState => omit(state, action.payload.labwareId),
  LOAD_FILE: (state: LocationsState, action: LoadFileAction): LocationsState => getPDMetadata(action.payload.file).ingredLocations
}, {});
