...
const mockGetLabwareNicknamesById: JestMockFn<any, any> = getLabwareNicknamesById;
const mockUuid: JestMockFn<[], string> = uuid;
const mockGetInitialDeckSetup: JestMockFn<any, InitialDeckSetup> = getInitialDeckSetup;
const mockGetNextAvailableDeckSlot: JestMockFn<[InitialDeckSetup], string | null | undefined> = getNextAvailableDeckSlot;
const mockGetNextNickname: JestMockFn<[Array<string>, string], string> = getNextNickname;
...
describe('renameLabware thunk', () => {
  ...
  it('should dispatch RENAME_LABWARE with a nickname from getNextNickname, with the nickname specified in the `name` arg', () => {
    ...
    store.dispatch( // $FlowFixMe(IL. 2020-11-13): flow hates thunks
    renameLabware({
      labwareId: 'someLabwareId',
      name: 'Specified Name'
    }));
