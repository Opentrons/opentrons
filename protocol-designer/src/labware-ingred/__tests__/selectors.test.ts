...
describe('allIngredientNamesIds selector', () => {
  it('selects names & ids from allIngredients selector result', () => {
    expect(selectors.allIngredientNamesIds.resultFunc(( // flow def for resultFunc is wrong and/or resultFun isn't typeable
    allIngredientsXXSingleIngred as any))).toEqual([{
      ingredientId: '0',
      name: 'Some Ingred'
    }]);
  });
...
describe('allIngredientGroupFields', () => {
  ...
  it('select fields from all ingred groups', () => {
    expect(selectors.allIngredientGroupFields.resultFunc(( // flow def for resultFunc is wrong and/or resultFun isn't typeable
    allIngredientsXXSingleIngred as any))).toEqual({
      '0': { ...baseIngredFields
      }
    });
