...
export type UseUpdateRobotNameMutationResult = UseMutationResult<
  ...
  string
> & {
  updateRobotName: UseMutateFunction<UpdatedRobotName, AxiosError<ErrorResponse>, string>
}
...
export function useUpdateRobotNameMutation(
    ...
    const mutation = useMutation<UpdatedRobotName, AxiosError<ErrorResponse>, string>(
        [host, 'server/name'],
        (newName: string) => updateRobotName(host as HostConfig, newName).then(response => {
