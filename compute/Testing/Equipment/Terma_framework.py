#! /usr/bin/env python
"""
Mitutoyo Digimatic Dial indicator framework.

Written by Carlos Chinchilla
based on Renee Beauvoir and Ben Rockhold's CMM scripts.
"""
import sys
import os

sys.path.insert(0, os.path.abspath('./submodules/s3g'))
sys.path.insert(0, os.path.abspath('./../../submodules/pyserial'))
...
class MitutoyoError(Exception):
    def __init__(self, value):
        self.value = value
    def __str__(self):
        return repr(self.value)

class MitutoyoGauge(serial.Serial):
    def gauge_read(self, bytesize=6):
        ...
        if self.raise_exceptions:
            raise MitutoyoError("%d consecutive failed attemps to read data. Last data received: '%s'" % (self.max_errors,self.reading_raw))
        return 'no reading'
...
class MitutoyoGaugeNull(serial.Serial):
    """ Fake mitutoyo connection """
...
if __name__ == '__main__':
    port = raw_input("Enter port number, leave blank for '/dev/ttyUSB0/'\n").strip()
    if port == '':
        port='/dev/ttyUSB0'
    gauge = MitutoyoGauge(port=port)
