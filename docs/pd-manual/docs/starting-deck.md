...
For the example slot D2, you can add labware, like a NEST well plate, by clicking the deck slot and **Add labware**. In the menu, search for or select a labware type, like reservoirs or well plates, and view available options from the [Labware Library](https://labware.opentrons.com "Labware Library"). At the bottom of the labware list, you can upload a JSON file to use custom labware in your protocol. 
