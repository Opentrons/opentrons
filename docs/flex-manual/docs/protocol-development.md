...
check out the , which includes topic-based articles as well as a of all
...
2.  Declaring the requirements and metadata in their respective
    dictionaries.

3.  Defining a run() function that contains all of the instructions to
    the robot, including:
    ...
    - types and locations (optional).

    - Commands the system will physically execute (e.g., or liquid
      handling commands, commands, or commands).

Python

from opentrons import protocol_api

requirements = {'robotType': 'Flex', 'apiLevel': '2.15'} def
run(protocol):

\# labware

plate = protocol.load_labware('corning_96_wellplate_360ul_flat',
location='D1') tiprack = protocol.load_labware(

'opentrons_flex_96_tiprack_200ul', location='D2')

\# pipettes

left_pipette = protocol.load_instrument( 'flex_1channel_1000',
mount='left', tip_racks=\[tiprack\])

\# commands left_pipette.pick_up_tip()

left_pipette.aspirate(100, plate\['A1'\]) left_pipette.dispense(100,
plate\['B2'\]) left_pipette.drop_tip()

If you're running a protocol via the Opentrons App or the touchscreen,
you don't need to call the run()

function, because the robot software does it for you.

However, one of the advanced features of the Python API is to control a
robot outside of the usual flow for setting up and running a protocol.
Opentrons Flex runs a Jupyter Notebook server, which can execute
discrete blocks of code (called *cells*), rather than a complete
protocol file. When organizing your code into cells, you can define a
run() function (and then call it) or run commands without one. It's also
possible

to execute complete protocols in a Jupyter terminal session or when
connected to Flex via SSH. For more information, see the of the Software
and Operation chapter.

### Python-exclusive features

Certain features are only available in Python protocols, either because
...
pipette. The InstrumentContext.configure\_ nozzle_layout() method
supports these additional layouts:
...
Certain configurations allow changing which nozzles are used. For
...
extra control in the hands of the technician running the protocol ---
without forcing them to switch between lots of protocol files or write
code themselves.

Runtime parameters can customize Boolean, numerical, and string values
in your protocol. And starting in API version 2.20, you can require a
CSV file of data to be parsed and used in the protocol. See the

for information on writing them into protocols, and see the Runtime
...
*non-blocking* manner. This lets your protocol save time by continuing
...
on the deck, except the Temperature Module. allows placing any powered
module in any column 1 or 3 slot (except the Thermocycler Module, which
...
means that it can perform any computation that relies on the Python
standard libraries or the suite of libraries included in the Flex system
software.

You can even install additional Python packages on Flex. and install the
package with pip. To avoid analysis errors in the Opentrons App, install
the packages on your computer as well. In the Opentrons App settings, go
to **Advanced** and click **Add override path** in the Override Path to
...
can perform all the basic actions that the OT-2 can, but OT-2 protocols
aren't directly compatible with Flex. However, with a little effort, you
can adapt an OT-2 protocol so it will run on Flex. This lets you have
parity across different Opentrons robots in your lab, or you can extend
older protocols to take advantage of new features only offered on Flex.
...
should also specify an apiLevel of 2.15 or higher. You can specify
apiLevel either in the metadata dictionary or the requirements
dictionary.
...
capacity or larger (or whatever you've outfitted your Flex with).

For example, you could convert an OT-2 protocol that uses a P300
Single-Channel GEN2 pipette and 300 µL tips to a Flex protocol that uses
a Flex 1-Channel 1000 µL pipette and 1000 µL tips:
...
The only necessary changes are the new arguments of load_labware() and
load_instrument(). Keep in mind that if you use smaller capacity tips
...
the example in the previous subsection), but it's not required. The
correspondence between deck slot numbers is as follows:
...
Currently, Protocol Designer can't convert an OT-2 protocol to a Flex
protocol. You have to choose which robot the protocol will run on when
you create it.

Since Flex protocols support nearly all the features of OT-2 protocols,
you can create a new protocol that performs all the same steps, but is
designed to run on Flex. The simplest way to do this is:
...
4.  Set up the Flex hardware as similarly as possible as the OT-2
    hardware. For example, choose pipettes of the same capacity or
    larger, and choose modules of the same type.

5.  Replicate the liquid setup and steps from the OT-2 protocol.

6.  Export your Flex protocol. Import it into the Opentrons App and
    check the run preview to see that it performs the same steps as your
    OT-2 protocol.

You can make bigger changes if your Flex configuration differs
significantly from your OT-2 configuration, but you may need to
re-verify your protocol.

### Magnetic Module protocols

Note that there is no direct analogue of the Magnetic Module on Flex.
You'll have to use the Magnetic Block and Flex Gripper instead. This
will require reworking some of your protocol steps, and you should
verify that your new protocol design achieves similar results.
