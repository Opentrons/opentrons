...
After initial robot setup, the next step is to attach instruments to the robot and calibrate them. The OT-2 has two pipette mounts. Each mount can hold a single 1-channel and/or 8-channel pipette. additionally, all pipettes have an automated calibration procedure, which you should perform immediately after installation.

## Determining pipette type

The OT-2 accepts GEN1 and GEN2 pipettes. The installation process is different depending on pipette generation so you'll need to know which pipettes you have when attaching them to the robot.
