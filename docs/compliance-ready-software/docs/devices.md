...
User actions, like setting up and running a protocol, require [documentation](../features/documentation.md) on a compliance ready Flex. You can use a built-in or external keyboard to make adding documentation easier on the Flex touchscreen.
...
The Flex also supports external keyboards connect via the [front USB port](../../../flex/docs/system-description/connections.md#usb-and-auxiliary-connections). When you use an external keyboard, the keyboard on the Flex touchscreen is hidden by default.

If you choose to download [files](../files.md) from the Flex touchscreen, you'll need to use an external storage device. 
