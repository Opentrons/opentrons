...
!!! note
    By default, only administrator accounts can sign for a protocol run in the Opentrons App. Use Compliance Ready settings to: 
...
- the information in this section is currently VERY confusing and is based on the images I took from designs. both things can't or shouldn't be true: 1) users are prompted and REQUIRED to download "audit logs" immediately post-signing because they AREN't saved on the robot 2)users can download compliance ready files, which I thought were the same thing/new name for audit logs, in the file manager...seemingly anytime before or when storage fills up...will be coming back to this
-------------->