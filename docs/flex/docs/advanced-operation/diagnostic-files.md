...
Starting in robot software version 10.0.0, you can access these files from the [file mananger](#managing-flex-files), where you'll be able to download two types of files: troubleshooting logs or calibration logs. 

The following table summarizes the data captured in each troubleshooting log.


| Diagnostic file | Description |
|---|---|
| API | <ul><li>The `api.log` records all movement and system activities performed by the robot.</li></ul> |
| Audit server | <ul><li>The `audit_server.log` records the robot's audit log periods for Opentrons Flex Compliance Ready Software.</li></ul> |
| Auth server | <ul><li>The `auth_server.log` records authentication processes on your Flex for Compliance Ready Software.</li></ul> |
| CAN Bus | <ul><li>The `can_bus.log` records communications among the robot's motor controller boards.</li></ul> |
| Kernel | <ul><li>The `kernel.log` records operating system data, like connected devices and network connections.</li></ul> |
| Remote access | <ul><li>The `remote_access.log` records whether remote access is allowed, including activity in Jupyter notebook or through a SSH connection.</li></ul> |
| Serial | <ul><li>The `serial.log` records communication between a Flex and its attached instruments and modules.</li></ul> |
| Server | <ul><li>The `server.log` records the HTTP requests (e.g., `GET`, `POST`, `DELETE`) sent and received by the robot server.</li></ul> |
| Touchscreen | <ul><li>The `touchscreen.log` records the status of the front display panel.</li></ul> |
| Update server | <ul><li>The `update_server.log` records activity related to robot software system updates.</li></ul> |

Your Flex also generates calibration logs. These `.json` files contain pipette and module calibration data. 

The records in both troubleshooting and calibration logs can be difficult to interpret and understand, but they're valuable for troubleshooting purposes. If you ever need to download the log files, the following instructions will step you through that process.
...
!!! note
    ...
    Compliance ready Flex robots also generate [additional files](../../compliance-ready-software/using/files.md#file-types) called audit logs.
...
!!! Note
    In previous robot software versions, you can download files from your robot's settings in the Opentrons App. Click the **Advanced** tab, then **Download logs.**
