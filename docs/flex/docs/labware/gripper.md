...
!!! note
    For best results, use the Flex Gripper only with the labware listed above. If you need to use the gripper with other ANSI/SLAS automation compliant labware, you need to create a custom labware definition by manually editing the JSON file to include gripper-specific information. See the [Labware Definitions section](definitions.md) or contact Opentrons Support for more information. 
