...
The App will notify you when the downgrade installation is complete. You should also check the touchscreen on your Flex before using it. The touchscreen might show the robot going through additional firmware checks and updates that the App does not. Flex will restart after these firmware checks finish. After restarting, your Flex will be running on an earlier version of the robot operating system.

It also includes instructions for manually downgrading your robot and App software, which may be required for troubleshooting issues or maintaining regulatory compliance.