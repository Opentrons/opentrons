...
async def _publisher_server_task(address: str, queue: Queue) -> None:
    """
    A task that reads multipart messages: topic, data. Received messages
...
async def _subscriber_server_task(address: str, queue: Queue) -> None:
    """
    A task that publishes messages to subscribers.
...
async def run(settings) -> None:
    ...
    t1 = asyncio.create_task(_subscriber_server_task(settings.subscriber_address, queue))
    t2 = asyncio.create_task(_publisher_server_task(settings.publisher_address, queue))
