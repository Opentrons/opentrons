...
from opentrons_shared_data.liquid_classes.liquid_class_definition import (
    ...
    MixProperties,
    TouchTipProperties,
    BlowoutProperties, BlowoutLocation,
)
...
def test_validate_delay_properties_dict() -> None:
    ...
    assert obj.params.duration == 2

    with pytest.raises(ValueError, match="Delay properties should specify either duration or params, not both"):
        DelayProperties.model_validate({'enable': False, 'params': {'duration': 2}, 'duration': 2})
...
def test_validate_touchtip_properties_dict() -> None:
    ...
    obj = TouchTipProperties.model_validate({'enable': True, 'z_offset': 2, 'mm_from_edge': 3, 'speed': 4})
...
def test_validate_blowout_properties_dict() -> None:
    ...
    obj = BlowoutProperties.model_validate({'enable': True, 'location': 'source', 'flow_rate': 3})
