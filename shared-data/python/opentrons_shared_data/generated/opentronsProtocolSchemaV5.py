...
#   filename:  opentronsProtocolSchemaV5
...
from . import opentronsLabwareSchemaV2
...
class MmOffset(BaseModel):
    __root__: float = Field(..., description='Millimeters for pipette location offsets')
...
class MoveToSlotCommand(BaseModel):
    """
    Move to slot command. NOTE: this is an EXPERIMENTAL command, its behavior is subject to change in future releases.
    """
...
class TemperatureModuleSetTargetCommand(BaseModel):
    """
    Temperature module set target temperature command. Module will begin moving to the target temperature. This command is non-blocking, it does not delay until the temperature is reached.
    """
...
class TemperatureModuleAwaitTemperatureCommand(BaseModel):
    """
    Temperature module await temperature command. Delay further protocol execution until the specified temperature is reached.
    """
...
class TemperatureModuleDeactivateCommand(BaseModel):
    """
    Temperature module deactivate command. Module will stop actively controlling its temperature and drift to ambient temperature.
    """
...
class ThermocyclerSetTargetBlockTemperatureCommand(BaseModel):
    """
    Thermocycler set target block temperature command. Lid will begin moving to the target temperature. This command is non-blocking, it does not delay until the temperature is reached.
    """
...
class ThermocyclerSetTargetLidTemperatureCommand(BaseModel):
    """
    Thermocycler set target lid temperature command. Lid will begin moving to the target temperature. This command is non-blocking, it does not delay until the temperature is reached.
    """
...
class ThermocyclerAwaitBlockTemperatureCommand(BaseModel):
    """
    Thermocycler await block temperature command. Delay further protocol execution until the specified temperature is reached.
    """
...
class ThermocyclerAwaitLidTemperatureCommand(BaseModel):
    """
    Thermocycler await lid temperature command. Delay further protocol execution until the specified temperature is reached.
    """
...
class ThermocyclerDeactivateBlockCommand(BaseModel):
    """
    Thermocycler deactivate block command. Module will stop actively controlling its block temperature.
    """
...
class ThermocyclerOpenLidCommand(BaseModel):
    """
    Thermocycler open lid command. This command will block until the lid is fully open.
    """
...
class ThermocyclerCloseLidCommand(BaseModel):
    """
    Thermocycler close lid command. This command will block until the lid is fully closed.
    """
...
class ThermocyclerRunProfile(BaseModel):
    """
    Thermocycler run profile command. Begin running the specified profile steps on the thermocycler. This command is non-blocking, it does not delay protocol execution outside of thermocycler steps. No more thermocycler commands should be given until a 'thermocycler/awaitProfileComplete' command is executed.
    """
...
class ThermocyclerAwaitProfileCompleteCommand(BaseModel):
    """
    Thermocycler await profile complete command. Blocks further protocol execution until profile execution is complete.
    """
...
class MoveToWellCommand(BaseModel):
    """
    Move to well command. Move the pipette's critical point to the specified well in a labware, with an optional offset. The pipette's critical point is a reference point on the pipette. The critical point can be one of the following: (1) Single-channel pipette with no tip: end of nozzle. (2) Multi-channel pipette with no tip: end of backmost nozzle. (3) Single-channel pipette with a tip: end of tip. (4) Multi-channel pipette with tip: end of tip on backmost nozzle.
    """
...
class AllCommands(BaseModel):
    """
    An array of command objects representing steps to be executed on the robot
    """
...
class LabwareDefinitions(BaseModel):
    __root__: opentronsLabwareSchemaV2.Model
...
class Model(BaseModel):
    ...
    metadata: Metadata2 = Field(..., description='Optional metadata about the protocol')
    ...
    labwareDefinitions: Dict[str, LabwareDefinitions] = Field(
        ...,
