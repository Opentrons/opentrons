...
class PipettePhysicalPropertiesDefinition(BaseModel):
    ...
    def convert_plunger_positions(
        ...
        return {pip_types.LiquidClasses(int(key)) if isinstance(key, str) and key.isdigit() else pip_types.LiquidClasses[key]: value for key, value in v.items()}
...
class PipetteLiquidPropertiesDefinition(BaseModel):
    ...
    def convert_aspirate_key_to_channel_type(
        ...
        return {pip_types.PipetteTipType(int(key)) if isinstance(key, str) and key.isdigit() else pip_types.PipetteTipType[key]: value for key, value in v.items()}
