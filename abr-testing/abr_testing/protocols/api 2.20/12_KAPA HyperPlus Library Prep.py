...
def tc_auto_seal_lid_and_close(
...
def run(ctx: ProtocolContext) -> None:
    ...
    heater_shaker = ctx.params.heater_shaker  # type: ignore[attr-defined]
    ...
    PCRCYCLES = ctx.params.PCRCYCLES  # type: ignore[attr-defined]
    ...
    magblock = ctx.load_module("magneticBlockV1", "D2")  # type: ignore[assignment]
    ...
    temp_plate = temp_adapter.load_labware(
        pcr_plate_type, "Temp Module Reservoir Plate"
    )

    if not dry_run:
        temp_mod.set_temperature(4)
    tc_mod_str = "thermocycler module gen2"
    tc_mod: ThermocyclerContext = ctx.load_module(tc_mod_str)  # type: ignore[assignment]
    # Just in case
    tc_mod.open_lid()

    FLP_plate = magblock.load_labware(pcr_plate_type, "FLP Plate")
    ...
    if heater_shaker:
        ...
        sample_plate = h_s_adapter.load_labware(pcr_plate_type, "Sample Plate")
        # Just in Case
        h_s.close_labware_latch()

    else:
        sample_plate = ctx.load_labware(pcr_plate_type, "D1", "Sample Pate")

    sample_plate_2 = ctx.load_labware(pcr_plate_type, "B2", "Sample Pate")
    samples_2 = sample_plate_2.rows()[0][:num_cols]
    samples: List[Well] = sample_plate.rows()[0][:num_cols]
    ...
    if disposable_lid:
        unused_lids: List[Labware] = [
            ctx.load_labware("opentrons_tough_pcr_auto_sealing_lid", "C3")
        ]
        for i in range(2):
            unused_lids.append(
    ...
    global tip50, tip200, p50_rack_count, p200_rack_count, tt_50, tt_200
    ...
    p50_racks_to_dump: List[Labware] = []
    p200_racks_to_dump: List[Labware] = []
    ...
    adapters: List[Well] = temp_plate.rows()[0][:num_cols]  # used for filling liquids
        ...
        if rack == tip50:
            if (
                ...
                if len(Available_on_deck_slots) > 0:
                    # If there are open deck slots, add a new tip box before pickup
                    p50_rack_count += 1
                    tt_50 += 12
                    avail_deck_slot = Available_on_deck_slots[0]
                    ...
                    ctx.comment(
                        f"Adding 50 ul tip rack #{p50_rack_count} to slot {avail_deck_slot}"
                    )
            ...
            if reuse and REUSE_RSB_TIPS and reuse_col is not None:
        ...
        if rack == tip200:
            if (
                ...
                if len(Available_on_deck_slots) > 0:
                    ...
                    p200_rack_count += 1
                    tt_200 += 12
                    avail_deck_slot_2 = Available_on_deck_slots[0]
                    addtiprack = ctx.load_labware(
                    ...
                    ctx.comment(
                        f"Adding 200 ul tip rack #{p200_rack_count} to slot {avail_deck_slot_2}"
                    )
                    Available_on_deck_slots.pop(0)
            ...
            # Call where tips will actually be picked up
            if reuse and REUSE_REMOVE_TIPS and reuse_col is not None:
    ...
    def TipSwap(tipvol: float) -> None:
        """Determine what tip to use."""
        if tipvol == 50:
    ...
    def run_tag_profile() -> None:
        ...
        if disposable_lid:
            lid_on_plate, unused_lids, used_lids = tc_auto_seal_lid_and_close(
                ctx, unused_lids, used_lids, sample_plate, tc_mod  # type: ignore
            )
    ...
    def run_amplification_profile() -> None:
        ...
        # Presetting Thermocycler Temps
        ctx.comment(
            "Starting Amplification Profile (37C for 5 minutes, then 50C for 5 minutes with 100C lid)"
        )
        tc_mod.set_lid_temperature(100)
        ...
        if disposable_lid:
            lid_on_plate, unused_lids, used_lids = tc_auto_seal_lid_and_close(
                ctx, unused_lids, used_lids, sample_plate_2, tc_mod  # type: ignore
            )
    ...
    def remove_supernatant(well: Well, vol: float, waste_: Well, column: int):
    ...
    def index_ligation() -> None:
        ...
        """Litigation Profile Sequence.
        Moves FLP plate off TC --> Heats TC --> moves sample plate to TC --> 
        ELM Profile --> removes sample plate from TC --> 
        places FLP plate back on TC
        """
    ...
    def lib_cleanup() -> None:
        ...
        for i_int, (s_well, e_well) in enumerate(zip(samp_list, samples_2)):
            tiptrack(tip50, i_int, reuse=True if REUSE_RSB_TIPS else False)
            p50.aspirate(elution_vol, s_well)
            p50.air_gap(5)
            p50.dispense(p50.current_volume, e_well.bottom(1), push_out=3)
            p50.air_gap(5)
            if trash_tips:
                p50.drop_tip()
                ctx.comment("****Dropping Tip in Waste shoot****")
    ...
    def lib_amplification_func() -> None:
    ...
    def lib_cleanup_2() -> None:
        ...
        p200.flow_rate.aspirate = 10
        for i_int2, (s_well2, e_well2) in enumerate(zip(samp_list_2, samples_flp)):
            tiptrack(tip50, i_int2, reuse=True if REUSE_RSB_TIPS else False)
            p50.aspirate(elution_vol_2, s_well2)
            p50.air_gap(5)
            p50.dispense(p50.current_volume, e_well2.bottom(1), push_out=3)
            p50.air_gap(5)
            if trash_tips:
                p50.drop_tip()
                ctx.comment("****Dropping Tip in Waste shoot****")
