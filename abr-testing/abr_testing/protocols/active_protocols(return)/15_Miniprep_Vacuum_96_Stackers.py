...
def _column_tip_well(tip_rack, tip_counter):
...
def _pick_column_tip(pipette, tip_rack, tip_counter):
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    manifold_collar = ctx.load_adapter(ctx.params.collar, VACUUM_MANIFOLD_DOCK)
    ...
    reservoir_12well['A1'].load_liquid(liquid=p1_buffer, volume=p_buffer_volume_reservoir_A1)
    reservoir_12well['A2'].load_liquid(liquid=p2_buffer, volume=p_buffer_volume_reservoir_A2)
    ...
    if ctx.params.liquid_run == True:
        ctx.comment("Checking liquid presence")
        pip_96_1000.configure_nozzle_layout(style=SINGLE, start="A1", tip_racks=tip_racks)
    ...
    with ctx.group_steps(name="Step 4 - Vacuum Filter Plate", description="Vacuum the filter plate"):
        ...
        ctx.move_labware(manifold_collar, VACUUM_MANIFOLD_DOCK, use_gripper=True) # Move the filter plate + collar from the manifold to the dock
        ctx.move_labware(nunc_filter_plate, filter_plate_parking_base, use_gripper=True) # Park filter plate for the reverse protocol
    ...
    with ctx.group_steps(name="Step 5 - Transfer from Collection Plate to Silica Membrane Plate", description="Transfer the collection plate to the silica membrane plate"):
        ...
        ctx.move_labware(sample_plate, 'B3', use_gripper=True) # Park sample plate for the reverse protocol
    ...
    with ctx.group_steps(name="Step 7 - Elute DNA from Silica Plate", description="Elute DNA from the silica plate"):
        ...
        ctx.move_labware(manifold_collar, VACUUM_MANIFOLD_DOCK, use_gripper=True) # Move the silica plate + collar from the manifold to the dock
