...
def run(protocol: ProtocolContext) -> None:
    ...
    MODETRASH = "RECYCLE"  # RECYCLE stores tip racks in stackers; plates stay on-deck
    SCP_Position = "C2"
    if DRYRUN:
        HYBRIDTIME = 18.0
    else:
    ...
    if STEP_RNA:
        ...
        if ONDECK_THERMO:
            thermocycler.open_lid()
        protocol.comment("MOVING: Plate Lid #1 = sample_plate_1 --> lids stack")
        protocol.move_lid(sample_plate_1, lids, use_gripper=True)
    ...
    if STEP_TAG:
        ...
        # ============================================================================================
        if MODETRASH == "RECYCLE":
            protocol.comment("MOVING: sample_plate_1 --> OFF_DECK (retained)")
            protocol.move_labware(
                labware=sample_plate_1,
    ...
    if STEP_WASH:
        ...
        if ONDECK_THERMO:
            thermocycler.open_lid()
        protocol.comment("MOVING: Plate Lid #1 = sample_plate_2 --> lids stack")
        protocol.move_lid(sample_plate_2, lids, use_gripper=True)
    ...
    if STEP_CLEANUP_1:
        ...
        if MODETRASH == "RECYCLE":
            protocol.comment("MOVING: sample_plate_2 --> OFF_DECK (retained)")
            protocol.move_labware(
                labware=sample_plate_2,
                new_location=OFF_DECK,
                use_gripper=False,
    ...
    if STEP_CAPTURE:
        ...
        if MODETRASH == "RECYCLE":
            protocol.comment("MOVING: CleanupPlate_1 --> OFF_DECK (retained)")
            protocol.move_labware(
                labware=CleanupPlate_1,
                new_location=OFF_DECK,
                use_gripper=False,
