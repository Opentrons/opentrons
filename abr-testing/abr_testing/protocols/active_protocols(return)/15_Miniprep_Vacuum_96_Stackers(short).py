...
metadata = {
    'protocolName': 'Minipreps with 96-Channel Pipette (Return Tips) - Short Collar',
...
def vacuum(ctx, vm_mod, pressure, time):
        # NOTE: VacuumModuleContext vacuum methods require API 2.31.
        # At apiLevel 2.30 wait for the hold time only.
        del vm_mod, pressure
        ctx.delay(
            time, msg=f"Vacuum hold placeholder ({time}s) — real vacuum needs API 2.31"
        )
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    # Empty refill source for the reverse protocol: the vacuum module pulls
    # liquid off the Flex, so recovered water is poured in here between runs and
    # the reverse redistributes it into reservoir_12well.
