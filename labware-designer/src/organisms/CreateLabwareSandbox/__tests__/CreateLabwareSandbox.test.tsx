...
describe('CreateLabwareSandbox', () => {
  it('renders irregular form by default', () => {
    ...
    expect(getByRole('heading', {name: 'Render Irregular Labware'})).toBeTruthy()

    const inputTextArea: HTMLTextAreaElement = getByRole('textbox', {name: 'input options'}) as any
    expect(inputTextArea.value).toContain('\"format\": \"irregular\"')
    const outputTextArea: HTMLTextAreaElement = getByRole('textbox', {name: 'output definition'}) as any
  ...
  it('renders regular form when selected', () => {
    ...
    expect(getByRole('heading', {name: 'Render Regular Labware'})).toBeTruthy()
  ...
  it('renders labware on deck by default and by itself after selected', () => {
    const { getByTestId, getByRole, queryByTestId } = render(<CreateLabwareSandbox />)
  ...
  it('renders labware in first deck slot by default and by changes with selection', () => {
    const { getByTestId, getByRole, queryByTestId, rerender } = render(<CreateLabwareSandbox />)
    let labwareRender: SVGGElement = getByTestId('lw_on_deck') as any
    expect(labwareRender.getAttributeNS(null, 'transform')).toBe('translate(0, 0)')

    fireEvent.change(getByRole('combobox'), { target: { value: 4 } })
    expect(labwareRender.getAttributeNS(null, 'transform')).not.toBe('translate(0, 0)')
  })
})
