...
eight_partial_column_bottom_left = PartialTipConfig(
    ...
    pipette_load_name="flex_8channel_1000",
...
def run(ctx):
    ...
    source = ctx.load_labware(target_labware_loadname, "D2")
    destination = ctx.load_labware(target_labware_loadname, "D3")
    ...
    if key == "return_tip_error":
        ...
        pipette.pick_up_tip()
        comment_column_has_tip(ctx, tip_rack, 1)
        pipette.drop_tip(tip_rack["B1"])  # this should raise an error
    else:
