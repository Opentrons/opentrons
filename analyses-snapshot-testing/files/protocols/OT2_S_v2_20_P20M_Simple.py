...
def run(protocol):
    ...
    pipette = protocol.load_instrument(instrument_name="p20_multi_gen2", mount="left", tip_racks=[tip_rack])
    ...
    # comment_tip_rack_status(protocol, tip_rack)
    # Invalid source for multichannel transfer: [H4 of PCR Plate on slot 2]
    # pipette.distribute(volume=volume, source=destination_labware_2["H4"], dest=[destination_labware_2["H5"], destination_labware_2["H6"]])
    # comment_tip_rack_status(protocol, tip_rack)
    # Invalid source for multichannel transfer: [H7 of PCR Plate on slot 2, H8 of PCR Plate on slot 2]
    # pipette.consolidate(volume=volume, source=[destination_labware_2["H7"], destination_labware_2["H8"]], dest=destination_labware_2["H9"])
    # comment_tip_rack_status(protocol, tip_rack)
