...
def add_parameters(parameters):
    ...
    parameters.add_bool(
        variable_name="DRYRUN", display_name="Dry Run", description="Skip incubation delays and shorten mix steps.", default=False
    )

    parameters.add_bool(variable_name="TIP_TRASH", display_name="Trash tip", description="tip thrases after every use", default=True)
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    DEACTIVATE_TEMP = (
        protocol.params.DEACTIVATE_TEMP
    )  # True = Deactivates Temp Block and Thermocycler, False = Leaves Temp Block and Thermocycler on (if leaving finished plates on deck)
    ...
    )  # 52AAFF = 'Sample Blue'                              #8400FF = 'Base Purple'
    ...
    for loop in range(RUN):
        ...
        if STEP_ERATDECK == 1:
            ...
            if DRYRUN == False:
                ...
                profile_ERAT = [{"temperature": 20, "hold_time_minutes": 5}, {"temperature": 65, "hold_time_minutes": 5}]
        ...
        if STEP_POSTERAT == 1:
            ...
            heatershaker.open_labware_latch()
            protocol.move_labware(labware=sample_plate_1, new_location=heatershaker, use_gripper=True)
            heatershaker.close_labware_latch()
            # ============================================================================================
            ...
            heatershaker.open_labware_latch()
            protocol.move_labware(labware=sample_plate_1, new_location=MAG_PLATE_SLOT, use_gripper=True)
            heatershaker.close_labware_latch()
        ...
        if STEP_POSTLIG == 1:
            ...
            heatershaker.open_labware_latch()
            protocol.move_labware(labware=sample_plate_1, new_location=thermocycler, use_gripper=True)
            heatershaker.close_labware_latch()
            # ============================================================================================
        if STEP_ELUTDECK == 1:
            ############################################################################################################################################
