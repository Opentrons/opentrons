def get_values(*names):
    import json
    _all_values = json.loads("""[{"name": "DRYRUN", "type": "dropDown", "label": "Dry Run", "options": [{"label": "no", "value": false}, {"label": "yes", "value": true}]}, {"name": "USE_GRIPPER", "type": "dropDown", "label": "Use Gripper", "options": [{"label": "yes", "value": true}, {"label": "no", "value": false}]}, {"name": "TIP_TRASH", "type": "dropDown", "label": "Trash Tips", "options": [{"label": "yes", "value": true}, {"label": "no", "value": false}]}, {"name": "DEACTIVATE_TEMP", "type": "dropDown", "label": "Deactivate Temperatures", "options": [{"label": "yes", "value": true}, {"label": "no", "value": false}]}, {"name": "SAMPLES", "type": "int", "label": "Samples", "default": 8}, {"name": "PCRCYCLES", "type": "int", "label": "PCR Cycles", "default": 26}, {"name": "DEFAULT_OFFSETS", "type": "dropDown", "label": "Default Module Offsets", "options": [{"label": "yes", "value": true}, {"label": "no", "value": false}]}]""")
...
metadata = {'protocolName': 'QIAseq Targeted DNA 8x v8','author': 'Opentrons <protocols@opentrons.com>','source': 'Protocol Library',}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    STEP_POSTTARGAMPDECK  = True      # Set to 0 to skip block of commands
 ...
 # ========== FIRST ROW ===========
    ...
    if ONDECK_HEATERSHAKER == True:
        ...
        sample_plate_2 = heatershaker.load_labware('opentrons_96_wellplate_200ul_pcr_full_skirt', 'Sample Plate 2')
    else:
        sample_plate_2 = protocol.load_labware('opentrons_96_wellplate_200ul_pcr_full_skirt','D1', 'Sample Plate 2')
    ...
    # ========================== REAGENT PLATE_1 ============================
    PCR                 = reagent_plate_1.wells_by_name()['A1']
    FRERAT              = reagent_plate_1.wells_by_name()['A2']
    LIG_Adapt           = reagent_plate_1.wells_by_name()['B2']
    LIG                 = reagent_plate_1.wells_by_name()['C2']
    LIG_EC              = reagent_plate_1.wells_by_name()['D2']
    TARGAMP_Primer      = reagent_plate_1.wells_by_name()['E2']
    TARGAMP             = reagent_plate_1.wells_by_name()['F2']
    TARGAMP_EC          = reagent_plate_1.wells_by_name()['G2']
    ...
    sample_1_list = ['A1','B1','C1','D1','E1','F1','G1','H1']           # Sample_plate_1 FRERAT/LIG/LIGEC/TARGAMP
    sample_2_list = ['A2','B2','C2','D2','E2','F2','G2','H2']           # Sample_plate_1 TARGAMPEC
    sample_3_list = ['A12','B12','C12','D12','E12','F12','G12','H12']   # Reagent_Plate_1 EXCESS SAMPLE
    sample_4_list = ['A1','B1','C1','D1','E1','F1','G1','H1']           # Sample_plate_2 POSTCLEANUP
    sample_5_list = ['A3','B3','C3','D3','E3','F3','G3','H3']           # Sample_plate_1 FINAL LIBRARY
    column_1_list = ['A1']                                              # Sample_plate_1 SAMPLES
    column_2_list = ['A2']                                              # Sample_plate_1 SAMPLES For TARGAMPEC
    column_3_list = ['A1']                                              # Sample_plate_2 for Cleanup 
    barcodes = ['A3','B3','C3','D3','E3','F3','G3','H3']
    ...
    if STEP_FRERAT == True:
        ...
        #===============================================
        for loop, X in enumerate(sample_1_list):
    ...
    if STEP_FRERATDECK == True:
        ...
        if DRYRUN == False:
            protocol.pause('Pausing to run FRERAT on an off deck Thermocycler ~25min')
        else:
            protocol.comment('Pausing to run FRERAT on an off deck Thermocycler ~25min')
        ############################################################################################################################################
    ...
    if STEP_LIGDECK == True:
        ...
        if ONDECK_THERMO == True:
            ...
            thermocycler.open_lid()
        if DRYRUN == False:
            protocol.pause('Pausing to run LIG on an off deck Thermocycler ~35min')
        else:
            protocol.comment('Pausing to run LIG on an off deck Thermocycler ~35min')
        ############################################################################################################################################

    if STEP_POSTLIG == True:
        protocol.comment('==============================================')
    ...
    if STEP_LIGDECK == True:
        ...
        if ONDECK_THERMO == True:
            ...
            thermocycler.open_lid()
        if DRYRUN == False:
            protocol.pause('Pausing to run ECR1 on an off deck Thermocycler ~25min')
        else:
            protocol.comment('Pausing to run ECR1 on an off deck Thermocycler ~25min')
        ############################################################################################################################################

    if STEP_TARGAMP == True:
        protocol.comment('==============================================')
        protocol.comment('--> Targeted Amplification')
        protocol.comment('==============================================')

        protocol.comment('--> Mixing TARGAMP')
        TARGAMP_PrimerVol = 10.25*SAMPLES
        ...
        if SAMPLES > 4:
            TipCheck(50,None,loop,None)
            p50.aspirate(TARGAMP_PrimerVol/2, TARGAMP_Primer.bottom(z=0.3), rate=0.2)
            p50.dispense(TARGAMP_PrimerVol/2, TARGAMP.bottom(z=0.3), rate=0.25)
            TipDone(50,None,loop,None)
            TipCheck(50,None,loop,None)
            p50.aspirate(TARGAMP_PrimerVol/2, TARGAMP_Primer.bottom(z=0.3), rate=0.2)
            p50.dispense(TARGAMP_PrimerVol/2, TARGAMP.bottom(z=0.3), rate=0.25)
    ...
    if STEP_POSTTARGAMP == True:
        ...
        for loop, X in enumerate(sample_1_list):
            ...
            protocol.delay(seconds=0.2)
            p50.default_speed = 400
            p50.dispense(SampleVol, sample_plate_1[sample_2_list[loop]].bottom(z=0.3),rate=0.2)
            p50.aspirate(SampleExcessVol, sample_plate_1[X].bottom(z=0.75))
            p50.default_speed = 5
            p50.move_to(sample_plate_1[X].top(5))
            protocol.delay(seconds=0.2)
            p50.default_speed = 400
            p50.dispense(SampleExcessVol, reagent_plate_1[sample_3_list[loop]].bottom(z=0.3))
            TipDone(50,None,loop,None)
            if loop == SAMPLES-1:
                break
        #===============================================

        protocol.comment('--> Adding TARGAMP_EC')
        TARGAMP_ECVol = 5
        TARGAMP_ECMixRep = 25 if DRYRUN == False else 1
        TARGAMP_ECMixVol = 20
        p50.flow_rate.aspirate = p50_flow_rate_aspirate_default*0.5
        p50.flow_rate.dispense = p50_flow_rate_dispense_default*0.5
        p50.flow_rate.blow_out = p50_flow_rate_blow_out_default*0.5
        #===============================================
        for loop, X in enumerate(sample_3_list):
            TipCheck(50,None,loop,None)
            p50.aspirate(TARGAMP_ECVol, TARGAMP_EC.bottom(z=1), rate=0.2)
            p50.default_speed = 5
            p50.move_to(TARGAMP_EC.top(5))
            protocol.delay(seconds=0.2)
            p50.default_speed = 400
            p50.dispense(TARGAMP_ECVol, sample_plate_1[X].bottom(z=0.3), rate=0.25)
            p50.mix(TARGAMP_ECMixRep,TARGAMP_ECMixVol, rate=0.5)
            p50.default_speed = 5
            p50.move_to(sample_plate_1[X].top(z=-5))
            protocol.delay(seconds=0.2)
            p50.default_speed = 400
    ...
    if STEP_POSTTARGAMPDECK == True:
        ...
        if ONDECK_THERMO == True:
            ...
            thermocycler.open_lid()
        if DRYRUN == False:
            protocol.pause('Pausing to run ECR2 on an off deck Thermocycler ~25min')
        else:
            protocol.comment('Pausing to run ECR2 on an off deck Thermocycler ~25min')
        ############################################################################################################################################
    ...
    if STEP_AMP == True:
        ...
        p50.flow_rate.blow_out = p50_flow_rate_blow_out_default*0.5
        #===============================================
        for loop, X in enumerate(sample_2_list):
            TipCheck(50,None,loop,None)
            p50.aspirate(BarcodeVol, reagent_plate_1.wells_by_name()[barcodes[loop]].bottom(z=0.3), rate=0.25)
            p50.dispense(BarcodeVol, sample_plate_1.wells_by_name()[X].bottom(z=0.3))
        ...
        for loop, X in enumerate(column_2_list):
            ...
            p1000.aspirate(PCRVol, PCR.bottom(z=0.3), rate=0.25)
    ...
    if STEP_CLEANUP == True:
        ...
        for loop, X in enumerate(column_2_list):
            TipCheck(200,None,loop,None)
            p1000.aspirate(SampleVol+2, sample_plate_1[X].bottom(z=0.3), rate=0.25)
            p1000.dispense(SampleVol+2, sample_plate_2[column_3_list[loop]].bottom(z=0.3), rate=0.25)
            p1000.blow_out(sample_plate_2[column_3_list[loop]].center())
            p1000.default_speed = 100
            p1000.move_to(sample_plate_2[column_3_list[loop]].top().move(types.Point(x=4,z=-3)))
            p1000.move_to(sample_plate_2[column_3_list[loop]].top().move(types.Point(x=-4,z=-3)))
        ...
        #============================================================================================
        # GRIPPER MOVE (sample_plate_2) HEATERSHAKER --> MAG BLOCK
        ...
        for loop, X in enumerate(column_3_list):
            ...
            p1000.aspirate(100, rate=0.25)
            p1000.default_speed = 200
            p1000.dispense(RemoveSup, Liquid_trash.top(z=0.5))
            protocol.delay(seconds=3)
            p1000.blow_out()
            p1000.default_speed = 400
            p1000.move_to(Liquid_trash.top(z=-5))
        ...
        p50.flow_rate.blow_out = p50_flow_rate_blow_out_default*0.5
        #===============================================
        for loop, X in enumerate(sample_4_list):
            TipCheck(50,None,loop,None)
            p50.move_to(sample_plate_2[X].bottom(z=0.3))
            p50.aspirate(TransferSup+1, rate=0.25)
            p50.dispense(TransferSup, sample_plate_1[sample_5_list[loop]].bottom(z=1))
            TipDone(50,None,loop,None)
        #===============================================
    ...
    if NOLABEL == False:
       # PROTOCOL SETUP - LABELING
        ...
        # ======== LABELING LIQUIDS =======
        Sample_Volume = 9.8
        QiaSeqBeads_Volume = round(SAMPLES*(80)*1.1,1)
        ETOH_Volume = round(SAMPLES*(150*2)*1.1,1)
        RSB_Volume = round(SAMPLES*(30)*1.1,1)
        PCR_Volume = round((70)*1.1,1)
        FRERAT_Volume = round(SAMPLES*(4.2)*1.1,1)
        LIG_Adapt_Volume = round(SAMPLES*(1.5)*1.1,1)
        LIG_Volume = round(SAMPLES*(12.5)*1.1,1)
        LIG_EC_Volume = round(SAMPLES*(2)*1.1,1)
        TARGAMP_Primer_Volume = round(SAMPLES*(10.25)*1.1,1)
        TARGAMP_Volume = round(SAMPLES*(12.4)*1.1,1)
        TARGAMP_EC_Volume = round(SAMPLES*(5)*1.1,1)
        QBBBuffer_Volume = SAMPLES*(80)*1.1
        H20_Volume = SAMPLES*(100)*1.1
        ...
        if RES_TYPE_96x == '12x15ml':
            ...
            reservoir.wells_by_name()['A6'].load_liquid(liquid=RSB, volume=RSB_Volume)
        ...
        if RES_TYPE_96x == '96x2ml':
            ...
            for loop, X in enumerate(SampleColumn):
                ...
                if loop == SAMPLES-1:
                    break
            for loop, X in enumerate(SampleColumn):
                reservoir.wells_by_name()[X+'6'].load_liquid(liquid=RSB, volume=RSB_Volume)
                if loop == SAMPLES-1:
                    break
            for loop, X in enumerate(SampleColumn):
        ...
        for loop, X in enumerate(SampleColumn):
            ...
            if loop == SAMPLES-1:
                break
        for loop, X in enumerate(SampleColumn):
            reagent_plate_1.wells_by_name()[X+'1'].load_liquid(liquid=PCR, volume=PCR_Volume)
            if loop == SAMPLES-1:
                break
        reagent_plate_1.wells_by_name()['A2'].load_liquid(liquid=FRERAT, volume=FRERAT_Volume)
        reagent_plate_1.wells_by_name()['B2'].load_liquid(liquid=LIG_Adapt, volume=LIG_Adapt_Volume)
        reagent_plate_1.wells_by_name()['C2'].load_liquid(liquid=LIG, volume=LIG_Volume)
        reagent_plate_1.wells_by_name()['D2'].load_liquid(liquid=LIG_EC, volume=LIG_EC_Volume)
        reagent_plate_1.wells_by_name()['E2'].load_liquid(liquid=TARGAMP_Primer, volume=TARGAMP_Primer_Volume)
        reagent_plate_1.wells_by_name()['F2'].load_liquid(liquid=TARGAMP, volume=TARGAMP_Volume)
        reagent_plate_1.wells_by_name()['G2'].load_liquid(liquid=TARGAMP_EC, volume=TARGAMP_EC_Volume)
        for loop, X in enumerate(SampleColumn):
            reagent_plate_1.wells_by_name()[X+'3'].load_liquid(liquid=Barcodes, volume=5)
            if loop == SAMPLES-1:
                break
        for loop, X in enumerate(SampleColumn):
            reagent_plate_1.wells_by_name()[X+'12'].load_liquid(liquid=Placeholder_Sample, volume=0)
            if loop == SAMPLES-1:
                break
        for loop, X in enumerate(SampleColumn):
            sample_plate_1.wells_by_name()[X+'3'].load_liquid(liquid=Placeholder_Sample, volume=0)
            if loop == SAMPLES-1:
                break