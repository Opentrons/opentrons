...
def run(ctx):
    ...
    def transfer_well_to_plate(vol2, start, end, liquid, drop_height=-20):
        if liquid == 5:
            ...
            start_loc = start
            end_loc_gap = end[0]
            p1k_8.mix(10, vol2*num_col*0.5, start_loc.bottom(z=ASP_HEIGHT))
            p1k_8.aspirate(vol2*num_col, start_loc.bottom(z=ASP_HEIGHT))
            p1k_8.air_gap(10)
            p1k_8.dispense(10, end_loc_gap.top(z=-5))
            for j in range(num_col):   
                end_loc = end[j]
                p1k_8.flow_rate.dispense = 500
                p1k_8.dispense(vol2, end_loc.top(z=drop_height))  
                p1k_8.flow_rate.dispense = default_rate 
                p1k_8.touch_tip()
            p1k_8.blow_out()
            p1k_8.drop_tip()
