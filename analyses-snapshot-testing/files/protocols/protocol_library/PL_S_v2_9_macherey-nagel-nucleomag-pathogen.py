def get_values(*names):
    import json
    _all_values = json.loads("""[{"name": "num_samples", "type": "int", "label": "Number of Samples [1-48 samples]", "default": 48}, {"name": "starting_vol", "type": "int", "label": "Starting Volume", "default": 400}, {"name": "elution_vol", "type": "int", "label": "Elution Volume [50-200\u00b5L]", "default": 100}, {"name": "binding_buffer_vol", "type": "int", "label": "Binding Buffer Volume", "default": 600}, {"name": "bead_vol", "type": "int", "label": "Bead Volume", "default": 20}, {"name": "wash1_vol", "type": "int", "label": "Wash 1 Volume", "default": 600}, {"name": "wash2_vol", "type": "int", "label": "Wash 2 Volume", "default": 600}, {"name": "wash3_vol", "type": "int", "label": "Wash 3 Volume", "default": 600}, {"name": "mix_reps_bind", "type": "int", "label": "Mix Repetitions During Binding [pretested default, changing is not recommended]", "default": 20}, {"name": "mix_reps_wash", "type": "int", "label": "Mix Repetitions during Wash [pretested default, changing is not recommended]", "default": 15}, {"name": "mix_reps_elu", "type": "int", "label": "Mix Repetitions during Elution [pretested default, changing is not recommended]", "default": 20}, {"name": "sep_time_bind", "type": "int", "label": "Separation Time during Binding [pretested default, changing is not recommended]", "default": 5}, {"name": "sep_time_wash", "type": "int", "label": "Separation Time during Wash [pretested default, changing is not recommended]", "default": 2}, {"name": "sep_time_elu", "type": "int", "label": "Separation Time during Elution [pretested default, changing is not recommended]", "default": 5}, {"name": "dry_time", "type": "int", "label": "Time for Air Dry [pretested default, changing is not recommended]", "default": 40}, {"name": "tip_track", "type": "dropDown", "label": "Track Tips Between Runs", "options": [{"label": "No", "value": 0}, {"label": "Yes", "value": 1}]}]""")
    return [_all_values[n] for n in names]


def get_values(*names):
    import json
    _all_values = json.loads("""{"num_samples":48,"starting_vol":400,"elution_vol":100,"binding_buffer_vol":600,"bead_vol":20,"wash1_vol":600,"wash2_vol":600,"wash3_vol":600,"mix_reps_bind":20,"mix_reps_wash":15,"mix_reps_elu":20,"sep_time_bind":5,"sep_time_wash":2,"sep_time_elu":5,"dry_time":40,"tip_track":0}""")
    return [_all_values[n] for n in names]
...
metadata = {'protocolName': 'NucleoMag_Pathogen_Rev01', 'author': 'Macherey-Nagel <automation-bio@mn-net.com>', 'apiLevel': '2.9'}
...
def run(ctx):
    ...
    [num_samples,
     ...
     sep_time_elu,
     dry_time,
     tip_track] = [48, 400, 100, 600, 20, 600, 600, 600, 20, 15, 20, 5, 2, 5, 40, False]
    ...
    '''
    if not 360 <= starting_vol <= 410:
        raise Exception('Starting volume should be from 360 - 410µl.')
    ...
    Define reagents
        ...
        Well 1 & 2 Binding Buffer NPB2
        Well 3 & 4 Wash Buffer NPW3
        Well 6 & 7 Wash Buffer NPW4
        Well 9 & 10 80% ethanol
        Well 12 Elution Buffer NPE5
    ...
    wash(wash2_vol, wash2, reuse=False, restore=False, resuspend=True, last=True, blowout=False)
