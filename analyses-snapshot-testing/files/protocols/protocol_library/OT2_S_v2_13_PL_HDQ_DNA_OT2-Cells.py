def get_values(*names):
    import json
    _all_values = json.loads("""{"dry_run": false, "mount": "left", "mag_mod_gen2": true, "res_type": "nest_12_reservoir_15ml", "num_samples": 8, "wash1_vol": 600, "wash2_vol": 600, "wash3_vol": 600, "AL_vol": 250, "sample_vol": 180, "bind_vol": 340, "elution_vol": 100}""")
...
metadata = {
    'protocolName': 'Omega HDQ DNA Extraction from Cells',
...
def run(ctx):
    ...
    settling_time= 4  if not dry_run else 0.25
    ...
    def lysis(vol, source):
        ...
        ctx.delay(minutes=9 if not dry_run else 0.25, msg='Shake at 1800 rpm for 10 minutes.')
        if not dry_run:
            h_s.set_and_wait_for_temperature(55)
        h_s.deactivate_shaker()
        ctx.delay(minutes=10 if not dry_run else 0.25,msg='Incubate at 55C for 10 minutes.')
    ...
    def bind(vol):
        ...
        for i, well in enumerate(hs_samples_m):
            ...
            #Wash walls of well after bead mixing- if it works well it can have a definition created for it
        ...
        for i, m in enumerate(mag_samples_m):
            ...
            for n in range(num_trans):
                ...
                if n < num_trans - 1:  # only air_gap if going back to source
                    m300.air_gap(20)
            resuspend_pellet(m, m300, 200, reps=12 if not dry_run else 1)
            m300.blow_out(m.top())
            m300.air_gap(20)
            m300.drop_tip() if not dry_run else m300.return_tip()
    ...
    def elute(vol):
        ...
        for i, (m, n) in enumerate(zip(mag_samples_m, hs_samples_m)):
            ...
            resuspend_pellet(m, m300, elution_vol)
