metadata = {
    'protocolName': 'Pierce BCA Peptide Assay',
...
def add_parameters(parameters):
    ...
    parameters.add_int(variable_name="heating",
...
def run(ctx):
    ...
    if dilution_on_deck == 1:
        ...
        if num_sample_initial > 6:
            def_undiluted_1 = ctx.define_liquid(name="Sample", description="Undiluted Samples, volume per tube (Slot C1)", display_color="#FF0000")  ## Red
            def_undiluted_2 = ctx.define_liquid(name="Sample", description="Undiluted Samples, volume per tube (Slot B1)", display_color="#FF0000")  ## Red
            for p in range(6):
    ...
    if heating == 1: hs.deactivate_heater()
    elif heating == 2: temp.deactivate()
