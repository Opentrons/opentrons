def get_values(*names):
    import json
    _all_values = json.loads("""[{"name": "trash_chute", "type": "dropDown", "label": "Trash Chute Present?", "options": [{"label": "no", "value": false}, {"label": "yes", "value": true}]}, {"name": "USE_GRIPPER", "type": "dropDown", "label": "Use Gripper", "options": [{"label": "yes", "value": true}, {"label": "no", "value": false}]}, {"name": "dry_run", "type": "dropDown", "label": "Dry Run", "options": [{"label": "no", "value": false}, {"label": "yes", "value": true}]}, {"name": "temp_mod", "type": "dropDown", "label": "Temperature Module Present?", "options": [{"label": "yes", "value": true}, {"label": "no", "value": false}]}, {"name": "heater_shaker", "type": "dropDown", "label": "Heater-Shaker Module Present?", "options": [{"label": "yes", "value": true}, {"label": "no", "value": false}]}, {"name": "tip_mixing", "type": "dropDown", "label": "Tip Mixing? (will heater-shaker be replaced with tip mixing-if h-s is present, this will be false)", "options": [{"label": "no", "value": false}, {"label": "yes", "value": true}]}, {"name": "wash_vol", "type": "int", "label": "Wash Volumes", "default": 150}, {"name": "lysis_vol", "type": "int", "label": "Lysis Volume", "default": 140}, {"name": "sample_vol", "type": "int", "label": "Sample Starting Volume", "default": 10}, {"name": "stop_vol", "type": "int", "label": "Stop Solution Volume", "default": 100}, {"name": "dnase_vol", "type": "int", "label": "DNAseI Volume", "default": 50}, {"name": "elution_vol", "type": "int", "label": "Elution Volume", "default": 50}]""")
...
def run(ctx):
    ...
    try:
        [trash_chute,USE_GRIPPER, dry_run,temp_mod,heater_shaker,tip_mixing,wash_vol,sample_vol,lysis_vol,dnase_vol,stop_vol,elution_vol] = get_values(  # noqa: F821
        'trash_chute','USE_GRIPPER','dry_run','temp_mod','heater_shaker','tip_mixing','wash_vol','sample_vol','lysis_vol','dnase_vol','stop_vol','elution_vol')
    ...
    def bind():
        ...
        if heater_shaker:
            ...
            ctx.move_labware(
                cell_plate,
                'C4' if USE_GRIPPER else protocol_api.OFF_DECK,
                use_gripper=False
            )
