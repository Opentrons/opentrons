def get_values(*names):
    import json
    _all_values = json.loads("""{"dry_run": false, "mount": "left", "mag_mod_gen2": true, "res_type": "nest_12_reservoir_15ml", "num_samples": 8, "bind_vol": 520, "wash1_vol": 1000, "wash2_vol": 1000, "lysis_vol": 220, "sample_vol": 200, "elution_vol": 110}""")
...
metadata = {
    'protocolName': 'Promega Magazorb DNA Mini Prep from Cells & Bacteria',
...
def run(ctx):
    ...
    try:
        [mount,mag_mod_gen2,dry_run,res_type,num_samples,binding_buffer_vol,wash1_vol,wash2_vol,elution_vol,sample_vol,lysis_vol] = get_values(  # noqa: F821
        'mount','mag_mod_gen2','dry_run','res_type','num_samples','binding_buffer_vol','wash1_vol','wash2_vol','elution_vol','sample_vol','lysis_vol')
    except (NameError):
        pass

    starting_vol= lysis_vol+sample_vol
    settling_time= 6 if not dry_run else 0.25
        ...
        for i in range(num_cols):
            ...
            for t in range(num_transfers):
                ...
                if t == num_transfers-1:
                    for x in range(7 if not dry_run else 1):
                        m300.aspirate(tvol,hs_samples_m[i].bottom(1))
                        m300.dispense(tvol,hs_samples_m[i].bottom(7))
            m300.drop_tip() if not dry_run else m300.return_tip()

        h_s.set_and_wait_for_shake_speed(2000)
        ctx.delay(minutes=1 if not dry_run else 0.25, msg='Shake at 1800 rpm for 2 minutes to mix and lyse.')
        h_s.set_and_wait_for_temperature(56)
        h_s.deactivate_shaker()
        ctx.delay(minutes=10 if not dry_run else 0.25,msg='Incubate at 56C for 10 minutes.')
    ...
    def bind(vol,src):
        ...
        for i, well in enumerate(hs_samples_m):
            ...
            if i == 0:
                ...
                reps = 4 if not dry_run else 1
            bead_mixing(source,m300,200,reps=reps)
        ...
        ctx.delay(minutes=10 if not dry_run else 0.25, msg='Shake at 1800 rpm for 10 minutes to allow DNA to bind to beads.')
        h_s.deactivate_shaker()

        #Transfer from H-S plate to Magdeck plate
        for x in range(num_cols):
            tiptrack(m300,tips)
            num_trans = math.ceil((vol+starting_vol)/200)
            ...
            for y in range(num_trans):
                ...
                m300.aspirate(vol_per_trans,hs_samples_m[x].bottom(0.3))
        ...
        for i, m in enumerate(mag_samples_m):
            ...
            for n in range(num_trans):
                ...
                if n < num_trans - 1:  # only air_gap if going back to source
                    m300.air_gap(20)
            resuspend_pellet(m, m300, 200, reps=5 if not dry_run else 1)
            bead_mixing(m, m300, 200, reps=5 if not dry_run else 1)
            m300.blow_out(m.top())
            m300.air_gap(20)
            m300.drop_tip() if not dry_run else m300.return_tip()
    ...
    def elute(vol):
        ...
        for i, (m,n) in enumerate(zip(mag_samples_m,hs_samples_m)):
            ...
            resuspend_pellet(m, m300, elution_vol/2,reps=7 if not dry_run else 1)
        ...
        h_s.set_and_wait_for_shake_speed(2000)
        ctx.delay(minutes=10 if not dry_run else 0.25,msg='Shake on H-S for 10 minutes at 2000 rpm.')
        h_s.deactivate_shaker()
        ...
        for i, (m, e) in enumerate(zip(mag_samples_m, elution_samples_m)):
            ...
            m300.flow_rate.aspirate = 15
            side = -1 if i % 2 == 0 else 1
            loc = m.bottom(0.3).move(Point(x=side*1.5))
            m300.transfer(vol-10, loc, e.bottom(5), air_gap=20, new_tip='never')
    ...
    bind(binding_buffer_vol,binding_buffer)
    wash(wash1_vol, wash1)
    wash(wash2_vol, wash2)
    ctx.delay(minutes=1 if not dry_run else 0.25, msg='Drying beads for 1 minutes')
    elute(elution_vol)
