...
def add_parameters(parameters):
    ...
    parameters.add_int(
        ...
        default=1,
        choices=[
            {"display_name": "Heater-shaker Module", "value": 1},
            {"display_name": "Temperature Module", "value": 2},
        ],
    )
    parameters.add_int(
    ...
    parameters.add_int(
        ...
        default=25,
        choices=[
            {"display_name": "25 uL", "value": 25},
            {"display_name": "10 uL", "value": 10},
        ],
    )
    parameters.add_int(
...
def run(ctx):
    ...
    sample_rack_1 = ctx.load_labware(
        "opentrons_24_tuberack_nest_1.5ml_snapcap", "C1", "SAMPLES"
    )
    if num_sample_final > 24:
        sample_rack_2 = ctx.load_labware(
            "opentrons_24_tuberack_nest_1.5ml_snapcap", "B1", "SAMPLES"
        )

    ctx.load_trash_bin("A3")

    if dilution_on_deck == 1:
        tips_200 = ctx.load_labware(
            "opentrons_flex_96_tiprack_200ul", "A1", "P200 TIPS"
        )
    ...
    tips_200 = ctx.load_labware("opentrons_flex_96_tiprack_200ul", "C3", "P50 TIPS")
    tips_200_loc = tips_200.wells()[:96]
    ...
    if reagent_prep_on_deck == 1:
        ...
        def_a = ctx.define_liquid(
            name="Reagent A", description=" ", display_color="#E5E4E2"
        )  ## Gray
        reagent_stock_rack.wells()[8].load_liquid(liquid=def_a, volume=vol_a)
        def_b = ctx.define_liquid(
            name="Reagent B", description=" ", display_color="#0000FF"
        )  ## Blue
        ...
        if num_col > 6:
            ...
            def_1 = ctx.define_liquid(
                name="Working Reagent", description=" ", display_color="#50C878"
            )  ## Green
    ...
    if dilution_on_deck == 1:
        ...
        if num_sample_initial > 6:
            def_undiluted_1 = ctx.define_liquid(
                name="Sample",
                description="Undiluted Samples, volume per tube (Slot C1)",
                display_color="#FF0000",
            )  ## Red
            def_undiluted_2 = ctx.define_liquid(
                name="Sample",
                description="Undiluted Samples, volume per tube (Slot B1)",
                display_color="#FF0000",
            )  ## Red
            for p in range(6):
                sample_rack_1.rows()[0][p].load_liquid(
                    liquid=def_undiluted_1, volume=vol_undiluted / 6
                )
            for q in range(num_sample_initial - 6):
                sample_rack_2.rows()[0][q].load_liquid(
                    liquid=def_undiluted_2,
                    volume=vol_undiluted / (num_sample_initial - 6),
                )
        else:
            def_undiluted_1 = ctx.define_liquid(
                name="Sample",
            ...
            for p in range(num_sample_initial):
                sample_rack_1.rows()[0][p].load_liquid(
                    liquid=def_undiluted_1, volume=vol_undiluted / num_sample_initial
                )
        def_buffer = ctx.define_liquid(
            name="Buffer",
            description="Buffer for sample dilution",
            display_color="#704848",
        )  ## Brown
        ...
        if num_sample_final > 24:
            def_1 = ctx.define_liquid(
                name="Sample",
                description="Samples, volume per tube (Slot C1)",
                display_color="#FF0000",
            )  ## Red
            def_2 = ctx.define_liquid(
                name="Sample",
                description="Samples, volume per tube (Slot B1)",
                display_color="#FF0000",
            )  ## Red
            for p in range(24):
                sample_rack_1.wells()[p].load_liquid(liquid=def_1, volume=vol_ / 24)
            for q in range(num_sample_final - 24):
                sample_rack_2.wells()[q].load_liquid(
                    liquid=def_2, volume=vol_ / (num_sample_final - 24)
                )
        else:
            def_1 = ctx.define_liquid(
                name="Sample",
            ...
            for p in range(num_sample_final):
                sample_rack_1.wells()[p].load_liquid(
                    liquid=def_1, volume=vol_ / num_sample_final
                )
    ...
    working_plate = ctx.load_labware(
        "corning_96_wellplate_360ul_flat", "C2", "WORKING PLATE"
    )
    working_plate_lid = ctx.load_labware(
        "corning_96_wellplate_360ul_flat", "D2", "PLATE LID"
    )
    ...
    if num_sample_final > 24:
        for tube in range(24):
            # p1k_1.flow_rate.aspirate = vol_sample*NUM_REPLICATE
            # p1k_1.flow_rate.dispense = vol_sample*NUM_REPLICATE
            # p1k_1.pick_up_tip(tips_200_loc[tube])
            # start = sample_1[tube]
            ...
            p1k_1.pick_up_tip(tips_200_loc[tube])
            start = sample_1[tube]
            p1k_1.mix(2, vol_sample * NUM_REPLICATE, start)
            p1k_1.blow_out(start.top(z=0))

            p1k_1.flow_rate.aspirate = vol_sample * NUM_REPLICATE
            p1k_1.flow_rate.dispense = vol_sample * NUM_REPLICATE
            p1k_1.aspirate(50, start.top(z=0))
            p1k_1.aspirate(vol_sample, start)
            ctx.delay(seconds=2)
            p1k_1.aspirate(50, start.top(z=0))
            p1k_1.aspirate(vol_sample, start)
            ctx.delay(seconds=2)
            end_1 = rxn_well[tube * 2 + 0]
            end_2 = rxn_well[tube * 2 + 1]
            p1k_1.dispense(vol_sample + 30, end_1.bottom(z=2))
            ctx.delay(seconds=2)
            p1k_1.dispense(vol_sample + 30, end_2.bottom(z=2))
            ctx.delay(seconds=2)
            p1k_1.blow_out(end_2.top(z=0))
        ...
        for tube in range(num_sample_final - 24):
            # p1k_1.flow_rate.aspirate = vol_sample*NUM_REPLICATE
            # p1k_1.flow_rate.dispense = vol_sample*NUM_REPLICATE
            # p1k_1.pick_up_tip(tips_200_loc[24+tube])
            # start = sample_2[tube]
            # p1k_1.aspirate(vol_sample*NUM_REPLICATE, start)
            # ctx.delay(seconds=2)
            # for count in range(NUM_REPLICATE):
            ...
            p1k_1.aspirate(vol_sample, start)
            ctx.delay(seconds=2)
            end_1 = rxn_well[48 + tube * 2 + 0]
            end_2 = rxn_well[48 + tube * 2 + 1]
            p1k_1.dispense(vol_sample + 30, end_1.bottom(z=2))
            ctx.delay(seconds=2)
            p1k_1.dispense(vol_sample + 30, end_2.bottom(z=2))
            ctx.delay(seconds=2)
    ...
    ctx.pause("Measure absorbance at 562 nm on a plate reader")
