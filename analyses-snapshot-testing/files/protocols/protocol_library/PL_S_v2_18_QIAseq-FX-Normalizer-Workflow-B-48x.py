...
metadata = {'protocolName': 'QIASeq FX with Normalizer Workflow B 48x v8','author': 'Opentrons <protocols@opentrons.com>','source': 'Protocol Library',}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    STEP_NORMALIZE        = True      # Set to 0 to skip block of commands
    #---------------------------
    ...
    AllSteps=[STEP_FXENZ,STEP_LIG,STEP_CLEANUP_1,STEP_CLEANUP_2,STEP_PCR,STEP_CLEANUP_3,STEP_NORMALIZE]
    ...
    if COLUMNS == 3:
        ...
        column_7_list = ['A9','A10','A11'] # Sample Plate 2: Final
        column_8_list = ['A9','A10','A11'] # Sample Plate 2: Final
        column_C_list = ['A9','A10','A11'] # Sample Plate 2: Final
        column_9_list = ['A9','A10','A11'] # Sample Plate 2: Final
        barcodes = ['A7','A8','A9']
        NWASH_list=['A8','A8','A8']
        NWASH_1 = ['A1','A2','A3']           # Heated Norm Wash on Wash Plate
        NWASH_2 = ['A4','A5','A6']        # Heated Norm Wash on Wash Plate
    if COLUMNS == 4:
    ...
    if COLUMNS == 6:
        ...
        column_7_list = ['A1','A2','A3','A4','A5','A6'] # Sample Plate 2: Final
        column_8_list = ['A1','A2','A3','A4','A5','A6'] # Sample Plate 2: Final
        column_C_list = ['A1','A2','A3','A4','A5','A6'] # Sample Plate 2: Final
        column_9_list = ['A1','A2','A3','A4','A5','A6'] # Sample Plate 2: Final
    ...
    if STEP_NORMALIZE == True:
        protocol.comment('==============================================')
        protocol.comment('--> Normalization')
        protocol.comment('==============================================')

        protocol.pause('SWAP PLATES')
        protocol.move_labware(labware=sample_plate_1,new_location=protocol_api.OFF_DECK,use_gripper=False)
        WashPlate = thermocycler.load_labware('opentrons_96_wellplate_200ul_pcr_full_skirt', 'WashPlate')
        #============================================================================================
        # GRIPPER MOVE (sample_plate_2) FROM OFF DECK --> HEATERSHAKER
        if ONDECK_HEATERSHAKER == True:
            heatershaker.open_labware_latch()
            protocol.move_labware(labware=sample_plate_2,new_location='A4',use_gripper=USE_GRIPPER,pick_up_offset=hs_pick_up_offset,drop_offset=deck_drop_offset)
            heatershaker.close_labware_latch()
        else:
            protocol.move_labware(labware=sample_plate_2,new_location='A4',use_gripper=USE_GRIPPER,pick_up_offset=deck_pick_up_offset,drop_offset=deck_drop_offset)
        #============================================================================================
        #tiprack_50_1        = protocol.load_labware('opentrons_flex_96_tiprack_50ul', 'A4')
        #tiprack_50_2        = protocol.load_labware('opentrons_flex_96_tiprack_50ul', 'B4')
        #tiprack_50_3        = protocol.load_labware('opentrons_flex_96_tiprack_50ul', 'C4')
        ...
        if REUSE_50_TIPS == True:
            RACKS_TO_DUMP.insert(0,tiprack_50_R)
        # Add A REUSABLE Tip Rack to the front of the list of Racks to Dump, Meaning that it's done being reused and can be cleared from the Deck.
        if REUSE_200_TIPS_ETOH_1 == True:
        ...
        for loop, X in enumerate(column_6_list):
            ...
            p50.dispense(NormBeadVol, CleanupPlate_2[column_3_list[loop]].bottom(z=1), rate=0.25)
            p50.blow_out(CleanupPlate_2[column_6_list[loop]].bottom(z=3))
        TipDone(50,None,loop,None)
        #===============================================
        ...
        if NWASH_AirMultiDis == True:
            ...
            for loop, X in enumerate(column_5_list):
                ...
                p1000.aspirate(NormWashVol/2, rate=0.25)
                p1000.dispense(NormWashVol, CleanupPlate_2[column_5_list[loop]].top(z=3), rate=0.6)
            TipDone(200,None,loop,None)
        else:
            for loop, X in enumerate(column_5_list):
                TipCheck(200,None,loop,None)
                ...
                p1000.move_to(WashPlate[NWASH_1[loop]].bottom(z=0.1))
                p1000.aspirate(NormWashVol/2, rate=0.25)
                p1000.dispense(NormWashVol, CleanupPlate_2[column_5_list[loop]].bottom(z=1), rate=0.5)
                if TIP_MIX == True:
                    WASHMix = 10
                if TIP_MIX == False:
                    WASHMix = 2
                for Mix in range(WASHMix):
                    p1000.aspirate(NormWashVol, CleanupPlate_2[column_5_list[loop]].bottom(z=Deepwell_Z_offset+1), rate=0.5)
                    p1000.dispense(NormWashVol, CleanupPlate_2[column_5_list[loop]].bottom(z=Deepwell_Z_offset+1), rate=1)
                TipDone(200,None,loop,None)
        #===============================================
        if TIP_MIX == False:
        ...
        for loop, X in enumerate(column_8_list):
            ...
            protocol.delay(minutes=0.1)
            p1000.move_to(CleanupPlate_2[X].bottom(z=0.25))
        ...
        for loop, X in enumerate(column_5_list):
            TipCheck(50,None,loop,None)
            p50.move_to(CleanupPlate_2[X].bottom(z=0))
            p50.aspirate(RemoveSup, rate=0.25)
            p50.default_speed = 100
            p50.move_to(CleanupPlate_2[X].top(z=-2))
        ...
        for loop, X in enumerate(column_5_list):
            ...
            p50.move_to(CleanupPlate_2.wells_by_name()[X].bottom(z=0.3))
            ...
            if TIP_MIX == False:
                ...
                for Mix in range(EluteMix):
                    p50.aspirate(NormEluteVol, CleanupPlate_2.wells_by_name()[X].bottom(z=Deepwell_Z_offset+1), rate=0.5)
                    p50.dispense(NormEluteVol, CleanupPlate_2.wells_by_name()[X].bottom(z=Deepwell_Z_offset+1), rate=1)
            p50.blow_out(CleanupPlate_2.wells_by_name()[X].top())
            TipDone(50,None,loop,None)
        #===============================================
        if TIP_MIX == False:
        ...
        #============================================================================================
        # GRIPPER MOVE (sample_plate_2) OFF DECK --> HEATHERSHAKER
        if ONDECK_HEATERSHAKER == True:
            heatershaker.open_labware_latch()
            protocol.move_labware(labware=sample_plate_2,new_location=heatershaker,use_gripper=USE_GRIPPER,pick_up_offset=deck_pick_up_offset,drop_offset=hs_drop_offset)
            heatershaker.close_labware_latch()
        ...
        for loop, X in enumerate(column_8_list):
            ...
            p50.dispense(TransferSup, sample_plate_2[column_9_list[loop]].bottom(z=1))
            TipDone(50,None,loop,None)
        #===============================================
