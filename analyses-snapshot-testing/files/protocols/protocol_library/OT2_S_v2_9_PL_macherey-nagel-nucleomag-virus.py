...
def run(ctx):
    ...
    [
        ...
        dry_time,
        tip_track,
    ] = [48, 414, 100, 600, 30, 500, 500, 0, 20, 15, 20, 5, 2, 5, 20, False]
    ...
    """
    if not 380 <= starting_vol <= 420:
        raise Exception("Starting volume should be from 360 - 420Âµl.")
    ...
    Define reagents
        ...
        Well 1 & 2 Binding Buffer MV2
        Well 4 & 5 Wash Buffer MV3
        Well 7 & 8 Wash Buffer MV4
        Well 9, 10 & 11 empty (Rinse replaced with Air dry)
        Well 12 Elution Buffer MV6

    """
    binding_buffer = buffers.wells()[:2]
    wash1 = buffers.wells()[3:5]
    wash2 = buffers.wells()[6:8]
    buffers.wells()[8:11]
    ...
    def bind(vol, bead_vol, reuse=True, restore=False):
        ...
        for i, (well, spot) in enumerate(zip(mag_samples_m, parking_spots)):
            ...
            for _x in range(mix_reps_bind - 5):
            ...
            if resuspend:
                ...
                for i, (m, spot) in enumerate(zip(mag_samples_m, parking_spots)):
                    ...
                    for _x in range(5):
                        m300.aspirate(250, loc_mix_asp_top)
                        m300.dispense(250, loc_mix_dis_pellet)
                    for _x in range(mix_reps):
                        m300.aspirate(250, loc_mix_asp_bottom)
                        m300.dispense(250, loc_mix_dis_top)
                    """ end of optimized mix """
                    m300.blow_out(m.bottom(11))
                    m300.air_gap(20)
                    m300.drop_tip(spot)
                m300.flow_rate.aspirate = 100
                m300.flow_rate.dispense = 200
