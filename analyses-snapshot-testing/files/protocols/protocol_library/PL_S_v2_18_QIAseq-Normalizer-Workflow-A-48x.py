def get_values(*names):
    import json
    _all_values = json.loads("""[{"name": "DRYRUN", "type": "dropDown", "label": "Dry Run", "options": [{"label": "no", "value": false}, {"label": "yes", "value": true}]}, {"name": "USE_GRIPPER", "type": "dropDown", "label": "Use Gripper", "options": [{"label": "yes", "value": true}, {"label": "no", "value": false}]}, {"name": "TIP_TRASH", "type": "dropDown", "label": "Trash Tips", "options": [{"label": "yes", "value": true}, {"label": "no", "value": false}]}, {"name": "DEACTIVATE_TEMP", "type": "dropDown", "label": "Deactivate Temperatures", "options": [{"label": "yes", "value": true}, {"label": "no", "value": false}]}, {"name": "COLUMNS", "type": "int", "label": "Columns of Samples", "default": 3}, {"name": "DEFAULT_OFFSETS", "type": "dropDown", "label": "Default Module Offsets", "options": [{"label": "yes", "value": true}, {"label": "no", "value": false}]}]""")
...
metadata = {'protocolName': 'QIAseq Normalizer Workflow A x48 v8','author': 'Opentrons <protocols@opentrons.com>','source': 'Protocol Library',}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    STEP_NORMCLEANUP      = True      # Set to 0 to skip block of commands
    #---------------------------
    ...
    AllSteps=[STEP_PCR,STEP_CLEANUP,STEP_NORMCLEANUP]
    ...
    if TIP_SETTING == 'Reusing Tips':
        ...
        NWASH_AirMultiDis     = True      # Default False   | When adding EtOH to multiple columns, dispense above the wells and reuse tips (Tip Saving)     
        REUSE_50_TIPS_RSB_1   = True      # Default False   | Reusing p50 tips
        REUSE_50_TIPS_RSB_2   = True      # Default False   | Reusing p50 tips
    ...
    DefinePosition(200,'C3','REUSE_200_1TIPS')
    # ========== FOURTH ROW ==========
    ...
    if TRASH_POSITION == 'BIN':
        ...
        sample_plate_2 = protocol.load_labware('opentrons_96_wellplate_200ul_pcr_full_skirt','D3', 'Sample Plate 2')
    ...
    if TRASH_POSITION == 'CHUTE':
        ...
        sample_plate_2 = protocol.load_labware('opentrons_96_wellplate_200ul_pcr_full_skirt','A3', 'Sample Plate 2')
        DefinePosition(None,'A4','OPEN')
        DefinePosition(None,'B4','OPEN')
        DefinePosition(None,'C4','OPEN')
        WashPlate = protocol.load_labware('nest_96_wellplate_2ml_deep', 'D4')
    ...
    # ========================== REAGENT PLATE_1 ============================
    DilutedPrimer       = reagent_plate_1.wells_by_name()['A1']
    PCR                 = reagent_plate_1.wells_by_name()['A2']
    NormBead            = reagent_plate_1.wells_by_name()['A3']
    NormElute           = reagent_plate_1.wells_by_name()['A4']
    #
    #
    #
    #
    ...
    # ========================== SAMPLE PLATE ===============================
    #NormWashBuffer_1    = sample_plate_1.wells_by_name()['A1']
    #NormWashBuffer_2    = sample_plate_1.wells_by_name()['A2']
    #NormWashBuffer_3    = sample_plate_1.wells_by_name()['A3']
    #NormWashBuffer_4    = sample_plate_1.wells_by_name()['A4']
    #NormWashBuffer_5    = sample_plate_1.wells_by_name()['A5']
    #NormWashBuffer_6    = sample_plate_1.wells_by_name()['A6']
    #NormWashBuffer_1    = sample_plate_1.wells_by_name()['A7']
    #NormWashBuffer_2    = sample_plate_1.wells_by_name()['A8']
    #NormWashBuffer_3    = sample_plate_1.wells_by_name()['A9']
    #NormWashBuffer_4    = sample_plate_1.wells_by_name()['A10']
    #NormWashBuffer_5    = sample_plate_1.wells_by_name()['A11']
    #NormWashBuffer_6    = sample_plate_1.wells_by_name()['A12']
    ...
    if COLUMNS == 1:
        column_1_list = ['A1']                              # Samples in Sample Plate
        column_2_list = ['A1']                              # Post PCR Samples in Bead Plate
        column_3_list = ['A4']                              # Beads to Bead Plate
        column_4_list = ['A4']                              # Reagent plate = Excess Samples to save
        column_5_list = ['A4']                              # Samples to Bead Plate
        column_6_list = ['A7']                              # sample plate - final samples
        NWASH_list=['A8']
        NWASH_1 = ['A7']                                    # Heated Norm Wash on Reagent Plate
        NWASH_2 = ['A8']                                    # Heated Norm Wash on Reagent Plate
    if COLUMNS == 2:
        column_1_list = ['A1','A2']                         # Sample_Plate_1 - input samples
        column_2_list = ['A1','A2']                         # CleanupPlate - Post PCR 
        column_3_list = ['A7','A8']                         # CleanupPlate - NormWash Beads
        column_4_list = ['A1','A2']                         # Sample_Plate_2 - Excess Samples to save
        column_5_list = ['A7','A8']                         # CleanupPlate - NormWash Beads + Samples
        column_6_list = ['A7','A8']                         # Sample plate - final samples
        ETOH_list = ['A3','A3']
        NWASH_list=['A8','A8']
        NWASH_1 = ['A1','A2']                               # Heated Norm Wash on Wash Plate
        NWASH_2 = ['A7','A8']                               # Heated Norm Wash on Wash Plate
    if COLUMNS == 3:
        column_1_list = ['A1','A2','A3']                    # Sample_Plate_1 - input samples
        column_2_list = ['A1','A2','A3']                    # CleanupPlate - Post PCR 
        column_3_list = ['A7','A8','A9']                    # CleanupPlate - NormWash Beads
        column_4_list = ['A1','A2','A3']                    # Sample_Plate_2 - Excess Samples to save
        column_5_list = ['A7','A8','A9']                    # CleanupPlate - NormWash Beads + Samples
        column_6_list = ['A7','A8','A9']                    # Sample plate - final samples
        ETOH_list = ['A3','A3','A3']
        NWASH_list=['A8','A8','A8']
        NWASH_1 = ['A1','A2','A3']                          # Heated Norm Wash on Wash Plate
        NWASH_2 = ['A7','A8','A9']                          # Heated Norm Wash on Wash Plate
    if COLUMNS == 4:
        column_1_list = ['A1','A2','A3','A4']               # Sample_Plate_1 - input samples
        column_2_list = ['A1','A2','A3','A4']               # CleanupPlate - Post PCR 
        column_3_list = ['A7','A8','A9','A10']              # CleanupPlate - NormWash Beads
        column_4_list = ['A1','A2','A3','A4']               # Sample_Plate_2 - Excess Samples to save
        column_5_list = ['A7','A8','A9','A10']              # CleanupPlate - NormWash Beads + Samples
        column_6_list = ['A7','A8','A9','A10']              # Sample plate - final samples
        ETOH_list = ['A3','A3','A3','A4']
        NWASH_list=['A8','A8','A9','A9']
        NWASH_1 = ['A1','A2','A3','A4']                     # Heated Norm Wash on Wash Plate
        NWASH_2 = ['A7','A8','A9','A10']                    # Heated Norm Wash on Wash Plate
    if COLUMNS == 5:
        column_1_list = ['A1','A2','A3','A4','A5']          # Sample_Plate_1 - input samples
        column_2_list = ['A1','A2','A3','A4','A5']          # CleanupPlate - Post PCR 
        column_3_list = ['A7','A8','A9','A10','A11']        # CleanupPlate - NormWash Beads
        column_4_list = ['A1','A2','A3','A4','A5']          # Sample_Plate_2 - Excess Samples to save
        column_5_list = ['A7','A8','A9','A10','A11']        # CleanupPlate - NormWash Beads + Samples
        column_6_list = ['A7','A8','A9','A10','A11']        # Sample plate - final samples
        ETOH_list = ['A3','A3','A3','A4','A4']
        NWASH_list=['A8','A8','A8','A9','A9']
        NWASH_1 = ['A1','A2','A3','A4','A5']                # Heated Norm Wash on Wash Plate
        NWASH_2 = ['A7','A8','A9','A10','A11']              # Heated Norm Wash on Wash Plate
    if COLUMNS == 6:
        column_1_list = ['A1','A2','A3','A4','A5','A6']     # Sample_Plate_1 - input samples
        column_2_list = ['A1','A2','A3','A4','A5','A6']     # CleanupPlate - Post PCR 
        column_3_list = ['A7','A8','A9','A10','A11','A12']  # CleanupPlate - NormWash Beads
        column_4_list = ['A1','A2','A3','A4','A5','A6']     # Sample_Plate_2 - Excess Samples to save
        column_5_list = ['A7','A8','A9','A10','A11','A12']  # CleanupPlate - NormWash Beads + Samples
        column_6_list = ['A7','A8','A9','A10','A11','A12']  # Sample plate - final samples
        ETOH_list = ['A3','A3','A3','A4','A4','A4']
        NWASH_list=['A8','A8','A8','A9','A9','A9']
        NWASH_1 = ['A1','A2','A3','A4','A5','A6']           # Heated Norm Wash on Wash Plate
        NWASH_2 = ['A7','A8','A9','A10','A11','A12']        # Heated Norm Wash on Wash Plate
    ...
    if STEP_PCR == True:
        protocol.comment('==============================================')
        protocol.comment('--> Normalization PCR')
        protocol.comment('==============================================')

        protocol.comment('--> Adding Diluted Primer')
        DilutedPrimerVol    = 10
        DilutedPrimerPremix = 2 if DRYRUN == False else 1
        ...
        for loop, X in enumerate(column_1_list):
            ...
            p50.mix(DilutedPrimerPremix,DilutedPrimerVol+3, DilutedPrimer.bottom(z=1), rate=0.25)
            p50.aspirate(DilutedPrimerVol+3, DilutedPrimer.bottom(z=1), rate=0.25)
            p50.dispense(3, DilutedPrimer.bottom(z=1), rate=0.25)
            p50.dispense(DilutedPrimerVol, sample_plate_1.wells_by_name()[X].bottom(z=1), rate=0.25)
        ...
        for loop, X in enumerate(column_1_list):
            ...
            p1000.mix(PCRPremix,PCRVol+3, PCR.bottom(z=1), rate=0.25)
            p1000.aspirate(PCRVol+3, PCR.bottom(z=1), rate=0.25)
            p1000.dispense(3, PCR.bottom(z=1), rate=0.25)
            p1000.dispense(PCRVol, sample_plate_1.wells_by_name()[X].bottom(z=1), rate=0.25)
            p1000.mix(PCRMixRep,PCRMixVol, sample_plate_1.wells_by_name()[X].bottom(z=1), rate=0.25)
    ...
    if STEP_PCRDECK == True:
        ...
        if ONDECK_THERMO == True:
            ...
            if DRYRUN == False:
                ...
                thermocycler.execute_profile(steps=profile_PCR_2, repetitions=2, block_max_volume=50)
                profile_PCR_3 = [
    ...
    if STEP_CLEANUP == True:
        ...
        for loop, X in enumerate(column_1_list):
            ...
            p50.move_to(sample_plate_1[X].bottom(z=0.1))
            p50.aspirate(TransferSup/2, rate=0.25)
            p50.dispense(TransferSup, CleanupPlate[column_2_list[loop]].bottom(z=1))
            TipDone(50,None,loop,None)
        #===============================================
        ...
        for loop, X in enumerate(column_2_list):
            TipCheck(200,None,loop,None)
            p1000.mix(CleanupBeadPremix,CleanupBeadVol+3, CleanupBead.bottom(z=1), rate=0.25)
            p1000.aspirate(CleanupBeadVol+3, CleanupBead.bottom(z=1), rate=0.25)
            p1000.dispense(3, CleanupBead.bottom(z=1), rate=0.25)
            ...
            #================================    
            p1000.dispense(CleanupBeadVol, CleanupPlate[X].bottom(z=1), rate=0.25)
            p1000.default_speed = 100
        ...
        for loop, X in enumerate(column_2_list):
            ...
            p1000.move_to(CleanupPlate[X].bottom(z=2))
            ...
            #================================ 
            p1000.dispense(200, Liquid_trash.top(z=-3), rate=0.6)
            protocol.delay(seconds=3)
            p1000.blow_out()
            #=====Reservoir Tip Touch========
            p1000.default_speed = 100
        ...
        for X in range(2):
            ...
            if ETOH_1_AirMultiDis == True:
                ...
                for loop, X in enumerate(column_2_list):
                    ...
                    #================================ 
                    p1000.move_to(CleanupPlate[X].top(z=-2))
                    p1000.dispense(ETOHMaxVol, rate=0.6)
                    protocol.delay(minutes=0.1)
                    p1000.blow_out()
                    p1000.move_to(CleanupPlate[X].top(z=5))
                    p1000.move_to(CleanupPlate[X].top(z=0))
            ...
            for loop, X in enumerate(column_2_list):
                TipCheck(200,'REUSE',loop,'ETOH_1')
                p1000.move_to(CleanupPlate[X].bottom(z=1))
                p1000.aspirate(RemoveSup-100, rate=0.25)
                protocol.delay(seconds=3)
                p1000.move_to(CleanupPlate[X].bottom(z=0))
                p1000.aspirate(100, rate=0.25)
                p1000.default_speed = 100
                p1000.move_to(CleanupPlate[X].top(z=-2))
                p1000.default_speed = 200
        ...
        for batch in range(BATCHREP):
            ...
            for loop, X in enumerate(column_2_list[batch*(int(len(column_2_list)/BATCHREP)):(1+batch)*int(len(column_2_list)/BATCHREP)]):
                TipCheck(50,None,loop,None)
                p50.move_to(CleanupPlate[X].bottom(z=0))
            ...
            H20Vol = 33
            H20MixRPM = 2000
            H20MixTime = 2*60 if DRYRUN == False else 0.1*60
            p50.flow_rate.aspirate = p50_flow_rate_aspirate_default*0.5
            p50.flow_rate.dispense = p50_flow_rate_dispense_default*0.5
            p50.flow_rate.blow_out = p50_flow_rate_blow_out_default*0.5
            #===============================================
            for loop, X in enumerate(column_2_list[batch*(int(len(column_2_list)/BATCHREP)):(1+batch)*int(len(column_2_list)/BATCHREP)]):
                ...
                p50.aspirate(H20Vol, H20.bottom(z=1), rate=0.75)
                p50.move_to(CleanupPlate.wells_by_name()[X].bottom(z=1))
                p50.dispense(H20Vol, rate=0.75)
                if TIP_MIX == True:
                    H20Mix = 10
                if TIP_MIX == False:
                    H20Mix = 2
                for Mix in range(H20Mix):
                    p50.aspirate(H20Vol, CleanupPlate.wells_by_name()[X].bottom(z=Deepwell_Z_offset+1), rate=0.5)
                    p50.dispense(H20Vol, CleanupPlate.wells_by_name()[X].bottom(z=Deepwell_Z_offset+1), rate=1)
                p50.blow_out(CleanupPlate.wells_by_name()[X].top())
                TipDone(50,'REUSE',loop,'RSB')
            #===============================================
            if TIP_MIX == False:
                heatershaker.set_and_wait_for_shake_speed(rpm=H20MixRPM)
                protocol.delay(H20MixTime)
                heatershaker.deactivate_shaker()
    ...
    if STEP_NORMCLEANUP == 1:

        if STEP_PCR == 0 and STEP_PCRDECK == 0 and STEP_CLEANUP == 0:
            #============================================================================================
            # GRIPPER MOVE (CleanupPlate) FROM HEATERSHAKER --> MAG BLOCK
            if ONDECK_HEATERSHAKER == True:
                heatershaker.open_labware_latch()
        ...
        if COLUMNS >= 4:
            ...
            #============================================================================================
            # GRIPPER MOVE (WashPlate) FROM OFF DECK --> THERMOCYCLER
            if ONDECK_THERMO == True:
                protocol.move_labware(labware=WashPlate,new_location=thermocycler,use_gripper=USE_GRIPPER,pick_up_offset=deck_pick_up_offset,drop_offset=tc_drop_offset)
            else:
                protocol.move_labware(labware=WashPlate,new_location='B1',use_gripper=USE_GRIPPER,pick_up_offset=deck_pick_up_offset,drop_offset=deck_drop_offset)
            #============================================================================================

        else:
            WashPlate = reagent_plate_1
        ...
        #===============================================
        TipCheck(200,'REUSE',loop,'ETOH_1')
        for loop, X in enumerate(NWASH_1):
            p1000.aspirate(NormWashVol, reservoir.wells_by_name()[NWASH_list[loop]].bottom(z=1), rate=0.5)
            p1000.dispense(NormWashVol/2, WashPlate[X].bottom(z=1), rate=0.5)
            p1000.dispense(NormWashVol/2, WashPlate[X].top(z=0), rate=0.5)
        for loop, X in enumerate(NWASH_2):
            p1000.aspirate(NormWashVol, reservoir.wells_by_name()[NWASH_list[loop]].bottom(z=1),rate=0.5)
            p1000.dispense(NormWashVol/2, WashPlate[X].bottom(z=1), rate=0.5)
            p1000.dispense(NormWashVol/2, WashPlate[X].top(z=0), rate=0.5)
        TipDone(200,'REUSE',loop,'ETOH_1')
        ...
        if DRYRUN == False:
            protocol.delay(minutes=3)
            thermocycler.set_block_temperature(56)

        protocol.comment('--> Dispensing NormBeads')
        NormBeadVol = 5
        NormBeadPremix = 15 if DRYRUN == False else 1
        p50.flow_rate.aspirate = p50_flow_rate_aspirate_default*0.5
        p50.flow_rate.dispense = p50_flow_rate_dispense_default*0.5
        p50.flow_rate.blow_out = p50_flow_rate_blow_out_default*0.5
        #===============================================
        TipCheck(50,'REUSE',loop,'RSB')
        p50.mix(NormBeadPremix,(5*COLUMNS)+3, NormBead.bottom(z=0.3),rate=0.5)
        for loop, X in enumerate(column_3_list):
            p50.aspirate(NormBeadVol, NormBead.bottom(z=0.3), rate=0.25)
            p50.dispense(NormBeadVol, CleanupPlate[column_3_list[loop]].bottom(z=1), rate=0.25)
            p50.blow_out(CleanupPlate[column_3_list[loop]].bottom(z=3))
        TipDone(50,'REUSE',loop,'RSB')
        #===============================================

        protocol.comment('--> Transferring Supernatant to Storage and Normalization')
        TransferSup = 30
        NormBeadPremix = 15 if DRYRUN == False else 1
        p50.flow_rate.aspirate = p50_flow_rate_aspirate_default*0.5
        p50.flow_rate.dispense = p50_flow_rate_dispense_default*0.5
        ...
        for loop, X in enumerate(column_2_list):
            TipCheck(50,'REUSE',loop,'RSB')
            p50.move_to(CleanupPlate[X].bottom(z=0.25))
            # Transferring 1/2 to Normalization Sample Wells
            p50.aspirate(TransferSup/2, rate=0.25)
            p50.dispense(TransferSup/2, CleanupPlate[column_5_list[loop]].bottom(z=0.3),rate=0.5)
            p50.move_to(CleanupPlate[X].bottom(z=0.2))
            # Transferring 1/2 to Excess Sample Wells
            p50.aspirate(TransferSup/2, rate=0.25)
            p50.dispense(TransferSup/2, sample_plate_2[column_4_list[loop]].bottom(z=1), rate=0.5)
            TipDone(50,'REUSE',loop,'RSB')
        #===============================================

        protocol.comment('--> Adding NWASH 1')
        NormWashVol = 180
        WashMixRPM = 1800
        WAshMixTime = 5*60 if DRYRUN == False else 0.1*60
        p1000.flow_rate.aspirate = p1000_flow_rate_aspirate_default*0.5
        p1000.flow_rate.dispense = p1000_flow_rate_dispense_default*0.5
        p1000.flow_rate.blow_out = p1000_flow_rate_blow_out_default*0.5
        #===============================================
        if NWASH_AirMultiDis == True:
            ...
            for loop, X in enumerate(column_5_list):
                p1000.move_to(WashPlate[NWASH_1[loop]].bottom(z=0.25))
                p1000.aspirate(NormWashVol/2, rate=0.25)
                protocol.delay(seconds=0.2)
                p1000.move_to(WashPlate[NWASH_1[loop]].bottom(z=0.1))
                p1000.aspirate(NormWashVol/2, rate=0.25)
                p1000.dispense(NormWashVol, CleanupPlate[column_5_list[loop]].top(z=3), rate=0.6)
            TipDone(200,None,loop,None)
        else:
            for loop, X in enumerate(column_5_list):
                TipCheck(200,None,loop,None)
                ...
                p1000.move_to(WashPlate[NWASH_1[loop]].bottom(z=0.1))
                p1000.aspirate(NormWashVol/2, rate=0.25)
                p1000.dispense(NormWashVol, CleanupPlate[column_5_list[loop]].bottom(z=1), rate=0.5)
                if TIP_MIX == True:
                    WASHMix = 10
                if TIP_MIX == False:
                    WASHMix = 2
                for Mix in range(WASHMix):
                    p1000.aspirate(NormWashVol, CleanupPlate[column_5_list[loop]].bottom(z=Deepwell_Z_offset+1), rate=0.5)
                    p1000.dispense(NormWashVol, CleanupPlate[column_5_list[loop]].bottom(z=Deepwell_Z_offset+1), rate=1)
                TipDone(200,None,loop,None)
        #===============================================
        if TIP_MIX == False:
            heatershaker.set_and_wait_for_shake_speed(rpm=WashMixRPM)
            protocol.delay(WAshMixTime)
            heatershaker.deactivate_shaker()
        ...
        protocol.comment('--> Adding NWASH 2')
        NormWashVol = 200
        WashMixRPM = 1800
        WAshMixTime = 5*60 if DRYRUN == False else 0.1*60
        p1000.flow_rate.aspirate = p1000_flow_rate_aspirate_default*0.5
        ...
        if NWASH_AirMultiDis == True:
            TipCheck(200,'REUSE',loop,'ETOH_1')
            for loop, X in enumerate(column_5_list):
                p1000.move_to(WashPlate[NWASH_2[loop]].bottom(z=0.25))
                p1000.aspirate(NormWashVol/2, rate=0.25)
                protocol.delay(seconds=0.2)
                p1000.move_to(WashPlate[NWASH_2[loop]].bottom(z=0.1))
                p1000.aspirate(NormWashVol/2, rate=0.25)
                p1000.dispense(NormWashVol, CleanupPlate[column_5_list[loop]].top(z=3), rate=0.6)
            TipDone(200,'REUSE',loop,'ETOH_1')
        else:
        ...
        for loop, X in enumerate(column_5_list):
            TipCheck(50,None,loop,None)
            p50.move_to(CleanupPlate[X].bottom(z=0))
            p50.aspirate(RemoveSup, rate=0.25)
            p50.default_speed = 100
            p50.move_to(CleanupPlate[X].top(z=-2))
            p50.default_speed = 200
            p50.touch_tip(speed=100)
            #======L Waste Volume Check======
            WASTEVOL+=(ActualRemoveSup*8)
            protocol.comment('Adding '+str((ActualRemoveSup*8))+'ul tp '+str(WASTEVOL))
            if WASTEVOL <14400:
            ...
            if WASTEVOL >=28800:
                Liquid_trash = Liquid_trash_well_3
            #================================ 
            p50.dispense(RemoveSup, Liquid_trash.top(z=-3), rate=0.6)
            protocol.delay(minutes=0.1)
            p50.blow_out()
            #=====Reservoir Tip Touch========
            p50.default_speed = 100
            p50.move_to(Liquid_trash.top().move(types.Point(x=4,z=-3)))
            p50.move_to(Liquid_trash.top().move(types.Point(x=-4,z=-3)))
        ...
        for loop, X in enumerate(column_5_list):
            ...
            p50.aspirate(NormEluteVol, NormElute.bottom(z=0.5), rate=0.5)
            p50.move_to(CleanupPlate.wells_by_name()[X].bottom(z=0.3))
            p50.dispense(NormEluteVol, rate=0.5)
            if TIP_MIX == True:
                EluteMix = 10
            if TIP_MIX == False:
                EluteMix = 2
            for Mix in range(EluteMix):
                p50.aspirate(NormEluteVol, CleanupPlate.wells_by_name()[X].bottom(z=Deepwell_Z_offset+1), rate=0.5)
                p50.dispense(NormEluteVol, CleanupPlate.wells_by_name()[X].bottom(z=Deepwell_Z_offset+1), rate=1)
            p50.blow_out(CleanupPlate.wells_by_name()[X].top())
        ...
        for loop, X in enumerate(column_5_list):
            TipCheck(50,'REUSE',loop+COLUMNS,'RSB')
            p50.move_to(CleanupPlate[X].bottom(z=0.1))
            p50.aspirate(TransferSup, rate=0.25)
            p50.dispense(TransferSup, sample_plate_2[column_6_list[loop]].bottom(z=0.3), rate=0.5)
            TipDone(50,'REUSE',loop+COLUMNS,'RSB')
        #===============================================
    ...
    if NOLABEL == False:
       # PROTOCOL SETUP - LABELING
        ...
        # ======== ESTIMATING LIQUIDS =======
        Sample_Volume = 15
        CleanupBead_Volume = COLUMNS*(50)
        EtOH_Volume = COLUMNS*(300)
        DilutedPrimer_Volume = COLUMNS*(10)
        PCR_Volume = COLUMNS*(25)
        NormBead_Volume = COLUMNS*(5)
        NormElute_Volume = COLUMNS*(25)
        NormWash_Volume = COLUMNS*(380)
        H20_Volume = COLUMNS*(33)
        ...
        DilutedPrimer = protocol.define_liquid(name="DilutedPrimer", description="Primer", display_color="#FFFB00")                             #FFFB00 = 'Base Yellow'
        ...
        Excess_Sample = protocol.define_liquid(name="Excess_Sample", description="Excess Sample", display_color="#82A9CF")                      #82A9CF = 'Placeholder Blue'
        ...
        Placeholder_NormWash = protocol.define_liquid(name="Placeholder_NormWash", description="Excess Sample", display_color="#82A9CF")        #82A9CF = 'Placeholder Blue'
        ...
        if RES_TYPE_96x == '12x15ml':
            ...
            reservoir.wells_by_name()['A3'].load_liquid(liquid=EtOH, volume=(EtOH_Volume*(1/2)))
            reservoir.wells_by_name()['A4'].load_liquid(liquid=EtOH, volume=(EtOH_Volume*(1/2)))
            reservoir.wells_by_name()['A7'].load_liquid(liquid=H20, volume=H20_Volume)
        ...
        if RES_TYPE_96x == '96x2ml':
            for loop, X in enumerate(TotalColumn):
                ...
                reservoir.wells_by_name()[X+'3'].load_liquid(liquid=EtOH, volume=(EtOH_Volume*(1/2)))
                reservoir.wells_by_name()[X+'4'].load_liquid(liquid=EtOH, volume=(EtOH_Volume*(1/2)))
                reservoir.wells_by_name()[X+'7'].load_liquid(liquid=H20, volume=H20_Volume)
        ...
        if COLUMNS >= 1:
            for loop, X in enumerate(TotalColumn):
                ...
                sample_plate_2.wells_by_name()[X+'1'].load_liquid(liquid=Excess_Sample, volume=0)
                sample_plate_2.wells_by_name()[X+'7'].load_liquid(liquid=Final_Sample, volume=0)
                WashPlate.wells_by_name()[X+'1'].load_liquid(liquid=Placeholder_NormWash, volume=0)
                WashPlate.wells_by_name()[X+'7'].load_liquid(liquid=Placeholder_NormWash, volume=0)
        if COLUMNS >= 2:
            for loop, X in enumerate(TotalColumn):
                sample_plate_1.wells_by_name()[X+'2'].load_liquid(liquid=Sample, volume=Sample_Volume)
                sample_plate_2.wells_by_name()[X+'2'].load_liquid(liquid=Excess_Sample, volume=0)
                sample_plate_2.wells_by_name()[X+'8'].load_liquid(liquid=Final_Sample, volume=0)
                WashPlate.wells_by_name()[X+'2'].load_liquid(liquid=Placeholder_NormWash, volume=0)
                WashPlate.wells_by_name()[X+'8'].load_liquid(liquid=Placeholder_NormWash, volume=0)
        if COLUMNS >= 3:
            for loop, X in enumerate(TotalColumn):
                sample_plate_1.wells_by_name()[X+'3'].load_liquid(liquid=Sample, volume=Sample_Volume)
                sample_plate_2.wells_by_name()[X+'3'].load_liquid(liquid=Excess_Sample, volume=0)
                sample_plate_2.wells_by_name()[X+'9'].load_liquid(liquid=Final_Sample, volume=0)
                WashPlate.wells_by_name()[X+'3'].load_liquid(liquid=Placeholder_NormWash, volume=0)
                WashPlate.wells_by_name()[X+'9'].load_liquid(liquid=Placeholder_NormWash, volume=0)
        if COLUMNS >= 4:
            for loop, X in enumerate(TotalColumn):
                sample_plate_1.wells_by_name()[X+'4'].load_liquid(liquid=Sample, volume=Sample_Volume)
                sample_plate_2.wells_by_name()[X+'4'].load_liquid(liquid=Excess_Sample, volume=0)
                sample_plate_2.wells_by_name()[X+'10'].load_liquid(liquid=Final_Sample, volume=0)
                WashPlate.wells_by_name()[X+'4'].load_liquid(liquid=Placeholder_NormWash, volume=0)
                WashPlate.wells_by_name()[X+'10'].load_liquid(liquid=Placeholder_NormWash, volume=0)
        if COLUMNS >= 5:
            for loop, X in enumerate(TotalColumn):
                sample_plate_1.wells_by_name()[X+'5'].load_liquid(liquid=Sample, volume=Sample_Volume)
                sample_plate_2.wells_by_name()[X+'5'].load_liquid(liquid=Excess_Sample, volume=0)
                sample_plate_2.wells_by_name()[X+'11'].load_liquid(liquid=Final_Sample, volume=0)
                WashPlate.wells_by_name()[X+'5'].load_liquid(liquid=Placeholder_NormWash, volume=0)
                WashPlate.wells_by_name()[X+'11'].load_liquid(liquid=Placeholder_NormWash, volume=0)
        if COLUMNS >= 6:
            for loop, X in enumerate(TotalColumn):
                sample_plate_1.wells_by_name()[X+'6'].load_liquid(liquid=Sample, volume=Sample_Volume)
                sample_plate_2.wells_by_name()[X+'6'].load_liquid(liquid=Excess_Sample, volume=0)
                sample_plate_2.wells_by_name()[X+'12'].load_liquid(liquid=Final_Sample, volume=0)
                WashPlate.wells_by_name()[X+'6'].load_liquid(liquid=Placeholder_NormWash, volume=0)
                WashPlate.wells_by_name()[X+'12'].load_liquid(liquid=Placeholder_NormWash, volume=0)
        for loop, X in enumerate(TotalColumn):
            reagent_plate_1.wells_by_name()[X+'1'].load_liquid(liquid=DilutedPrimer, volume=DilutedPrimer_Volume)
            reagent_plate_1.wells_by_name()[X+'2'].load_liquid(liquid=PCR, volume=PCR_Volume)
            reagent_plate_1.wells_by_name()[X+'3'].load_liquid(liquid=NormBead, volume=NormBead_Volume)
            reagent_plate_1.wells_by_name()[X+'4'].load_liquid(liquid=NormElute, volume=NormElute_Volume)
