metadata = {
    'protocolName': 'Pierce Bradford Protein Assay',
...
def add_parameters(parameters):
    parameters.add_int(variable_name="dilution_on_deck",
    ...
    parameters.add_int(variable_name="hs_on_deck",
...
def run(ctx):
    ...
    if num_rxn%8 != 0: num_col = num_col + 1

    if vol_sample == 10: vol_wr_per_rxn = 250
    elif vol_sample == 150: vol_wr_per_rxn = 150
    ...
    if vol_sample == 10: 
        tips_200_sample = ctx.load_labware('opentrons_flex_96_tiprack_200ul', 'C3', 'P50 TIPS')
        tips_sample_loc = tips_200_sample.wells()[:96]
    elif vol_sample == 150:
    ...
    if num_col > 6:
        ...
        def_1 = ctx.define_liquid(name="WORKING REAGENT", description="Coomassie Brilliant Blue G-250 solution ", display_color="#0000FF")  ## Blue
        wr_reservoir.wells()[0].load_liquid(liquid=def_1, volume=vol_1) 
        def_2 = ctx.define_liquid(name="WORKING REAGENT", description="Coomassie Brilliant Blue G-250 solution ", display_color="#0000FF")  ## Blue
    ...
    if dilution_on_deck == 1:
        ...
        if num_sample_initial > 6:
            def_undiluted_1 = ctx.define_liquid(name="SAMPLES", description="Undiluted Samples, volume per tube (Slot C1)", display_color="#FF0000")  ## Red
            def_undiluted_2 = ctx.define_liquid(name="SAMPLES", description="Undiluted Samples, volume per tube (Slot B1)", display_color="#FF0000")  ## Red
            for p in range(6):
