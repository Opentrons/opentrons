def get_values(*names):
    import json
    _all_values = json.loads("""[{"name": "num_samples", "type": "int", "label": "Number of Samples", "default": 8}, {"name": "deepwell_type", "type": "dropDown", "label": "Deepwell Type", "options": [{"label": "NEST 96 Wellplate 2mL", "value": "nest_96_wellplate_2ml_deep"}, {"label": "USA Scientific 96 Wellplate 2.4mL", "value": "usascientific_96_wellplate_2.4ml_deep"}]}, {"name": "res_type", "type": "dropDown", "label": "Reservoir Type", "options": [{"label": "NEST 12 Reservoir 15mL", "value": "nest_12_reservoir_15ml"}, {"label": "USA Scientific 12 Reservoir 22mL", "value": "usascientific_12_reservoir_22ml"}]}, {"name": "starting_vol", "type": "int", "label": "Starting Volume", "default": 400}, {"name": "binding_buffer_vol", "type": "int", "label": "Binding Buffer Volume", "default": 320}, {"name": "wash1_vol", "type": "int", "label": "Wash 1 Volume", "default": 400}, {"name": "wash2_vol", "type": "int", "label": "Wash 2 Volume", "default": 400}, {"name": "wash3_vol", "type": "int", "label": "Wash 3 Volume", "default": 300}, {"name": "elution_vol", "type": "int", "label": "Wash Mix Repetitions", "default": 50}, {"name": "settling_time", "type": "int", "label": "Settling Time (minutes)", "default": 7}, {"name": "mag_gen", "type": "dropDown", "label": "Mag Deck Generation", "options": [{"label": "GEN1", "value": "magdeck"}, {"label": "GEN2", "value": "magnetic module gen2"}]}, {"name": "park_tips", "type": "dropDown", "label": "Park Tips?", "options": [{"label": "Yes, park tips", "value": true}, {"label": "No, do not park tips", "value": false}]}, {"name": "tip_track", "type": "dropDown", "label": "Track Tips?", "options": [{"label": "Yes, track tips", "value": true}, {"label": "No, do not track tips", "value": false}]}, {"name": "flash", "type": "dropDown", "label": "Flash", "options": [{"label": "Yes, flash lights", "value": true}, {"label": "No, do not flash lights", "value": false}]}, {"name": "m300_mount", "type": "dropDown", "label": "P300 Multi Mount", "options": [{"label": "Left", "value": "left"}, {"label": "Right", "value": "right"}]}]""")
...
def run(ctx):
    ...
    wash1 = res1.wells()[3:7]
    wash2 = res1.wells()[7:11]
    elution_solution = res1.wells()[-1]
    dnase1 = [res2.wells()[0]]
    stopsolution = [res2.wells()[0:2]]
    wash3 = res2.wells()[3:6]
    wash4 = res2.wells()[6:10]
    ...
    def bind(vol, park=True):
        ...
        for i, (well, spot) in enumerate(zip(mag_samples_m, parking_spots)):
            ...
            if park:
                ...
                _drop(m300)
        ctx.pause("mix on heater/shaker for 5 minutes")
        BindIncubate = 5
        for _binddelay in np.arange(BindIncubate, 0, -1):
            ctx.delay(minutes=1, msg="Take plate off magdeck and vortex for 10 - 15 seconds.")
        magdeck.engage(height=MAG_HEIGHT)
        for bindi in np.arange(settling_time, 0, -0.5):
    ...
    def wash(vol, source, mix_reps=15, park=True, resuspend=True):
        ...
        for i, (m, spot) in enumerate(zip(mag_samples_m, parking_spots)):
            ...
            if park:
                ...
                _drop(m300)
        if source == wash3:
            ctx.delay(minutes=10, msg="Mix off-deck for 10 minutes.")
        ...
        if source == wash3:
            remove_supernatant(602, park=park)
        else:
            remove_supernatant(vol, park=park)
        if source == wash2:
            ctx.delay(minutes=5, msg="Let dry for 5 minutes.")
        ...
        for i, (m, spot) in enumerate(zip(mag_samples_m, parking_spots)):
            ...
            for n in range(num_trans):
                ...
                if n < num_trans - 1:
                    m300.air_gap(20)
            if resuspend:
                resuspend_pellet(m, m300, 150, reps=10)
            m300.blow_out(m.top())
            m300.air_gap(20)
            if park:
                m300.drop_tip(spot)
    ...
    def elute_one(vol, park=True):
        """
        `elute_one` will add elution from the reservoir to the deepwell extraciton plate.
        :param vol (float): The amount of volume to aspirate from the elution
                            buffer source and dispense to each well containing
                            beads.
        :param park (boolean): Whether to save sample-corresponding tips
        ...
        for i, (m, spot) in enumerate(zip(mag_samples_m, parking_spots)):
            ...
            if park:
                ...
                _drop(m300)
        for eone in np.arange(5, 0, -0.5):
            ctx.delay(minutes=0.5, msg="Vortex off-deck for 30 second increments. " + str(eone) + " minutes left.")
    ...
    elute_one(100, park=park_tips)
    dnase(52, dnase1, park=park_tips)
    stop_reaction(150, stopsolution, park=park_tips)
