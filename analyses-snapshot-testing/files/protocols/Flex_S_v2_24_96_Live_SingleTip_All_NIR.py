...
metadata = {"protocolName": "96 Channel Single Tip ALL"}
...
def run(ctx):
    ...
    tip_config = find_partial_tip_config("ninety_six_single_front_left")
    ...
    water_dest_well = dest_B2.wells_by_name()[WATER_DEST_WELL]
    ethanol_dest_well = dest_B2.wells_by_name()[ETHANOL_DEST_WELL]
    glycerol_dest_well = dest_B2.wells_by_name()[GLYCEROL_DEST_WELL]
    water_dest_column = dest_B2.columns()[WATER_DEST_COLUMN]
    ethanol_dest_column = dest_B2.columns()[ETHANOL_DEST_COLUMN]
    glycerol_dest_column = dest_B2.columns()[GLYCEROL_DEST_COLUMN]

    # Multi well and column variables
    water_dest_wells = [dest_B2.wells_by_name()[well] for well in WATER_DEST_WELLS]
    ethanol_dest_wells = [dest_B2.wells_by_name()[well] for well in ETHANOL_DEST_WELLS]
    glycerol_dest_wells = [dest_B2.wells_by_name()[well] for well in GLYCEROL_DEST_WELLS]
    water_dest_columns = [dest_B2.columns()[col] for col in WATER_DEST_COLUMNS]
    ethanol_dest_columns = [dest_B2.columns()[col] for col in ETHANOL_DEST_COLUMNS]
    glycerol_dest_columns = [dest_B2.columns()[col] for col in GLYCEROL_DEST_COLUMNS]

    # displays black in deck map
    dest_B2.load_empty(wells=dest_B2.wells())
    ...
    for liquid_class_name in liquid_class_names:
        ...
        comment_labware_well_volume_status(ctx, dest_B2)
        ...
        color_wells_with_liquid(dest_B2, filled)
