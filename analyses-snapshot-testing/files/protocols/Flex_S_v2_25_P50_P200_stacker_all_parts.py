...
def run(protocol: ProtocolContext) -> None:
    ...
    sample_plate_3 = protocol.load_labware(
        "stackable_opentrons_96_wellplate_200ul_pcr_full_skirt", "A2", "Sample Plate 2"
    )
    sample_plate_2 = sample_plate_3.load_labware("stackable_opentrons_96_wellplate_200ul_pcr_full_skirt", "Sample Plate 3")
    # ======================================================================
    ...
    lids = protocol.load_lid_stack("custom_opentrons_tough_pcr_auto_sealing_lid", "B3", 5)
    ...
    if ONDECK_TEMP:
        ...
        reagent_plate_1 = protocol.load_labware(
            "biorad_384_wellplate_50ul", "C1", "Reagent Plate 1"
        )
    ...
    if CUSTOM_OFFSETS:
        ...
        # MAG BLOCK OFFSETS
        mb_drop_offset = {"x": 2, "y": 2, "z": 2}
        mb_pick_up_offset = {"x": 2, "y": 2, "z": 2}
        # THERMOCYCLER OFFSETS
        tc_pick_up_offset = {"x": 2, "y": 2, "z": 2}
        # DECK OFFSETS
        deck_drop_offset = {"x": 2, "y": 2, "z": 2}
        deck_pick_up_offset = {"x": 2, "y": 2, "z": 2}
    else:
        PCRPlate_Z_offset = 2
        Deepwell_Z_offset = 2
        Deep384_Z_offset = 2
    ...
    if STEP_RNA:
        ...
        if MODESPEED != "QUICK":
            ...
            for loop, X in enumerate(column_list_1):
                ...
                p1000.dispense(
                    EPH3Vol, sample_plate_1[X].bottom(z=PCRPlate_Z_offset)
                )
    ...
    if STEP_POSTRNA:
        ...
        if MODETRASH == "CHUTE":
            protocol.comment(
                "MOVING: tiprack_50_SCP_4 = tiprack_A3_adapter --> TRASH"
            )
            protocol.move_labware(
    ...
    if STEP_CLEANUP_1:
        ...
        for loop, X in enumerate(column_list_1):
            ...
            p1000.dispense(
                ETOHMaxVol, CleanupPlate_1[X].bottom(z=Deepwell_Z_offset)
            )
        p1000.drop_tip()
        # ===============================================
        ...
        if MODETRASH == "CHUTE":
            protocol.comment(
                "MOVING: tiprack_200_10 = tiprack_A3_adapter --> TRASH"
            )
            protocol.move_labware(
                labware=tiprack_200_10,
                new_location=TRASH,
        ...
        if MODETRASH == "CHUTE":
            protocol.comment(
                "MOVING: tiprack_200_11 = tiprack_A3_adapter --> TRASH"
            )
            protocol.move_labware(
                labware=tiprack_200_11,
                new_location=TRASH,
        ...
        if MODESPEED != "QUICK":
            ...
            for loop, X in enumerate(column_list_1):
                ...
                p1000.dispense(
                    RSBVol, CleanupPlate_1[X].bottom(z=Deepwell_Z_offset)
                )
                p1000.drop_tip()
            # ===============================================
