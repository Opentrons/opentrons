...
metadata = {
    "protocolName": "Illumina DNA Prep 48x v8",
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    ETOH_1_AirMultiDis = (
        False  # Default False   | When adding EtOH to multiple columns, dispense above the wells and reuse tips (Tip Saving)
    )
    ...
    REPORT = (
        False  # Default False   | True = Include Extra Comments in the Protocol Step Preview for troubleshooting, False = Do Not Include
    )
    ...
    RSB = reagent_plate_1["A3"]
    if COLUMNS < 5:
        EPM = reagent_plate_1["A4"]
    if COLUMNS >= 5:
        EPM_1 = reagent_plate_1["A4"]
        EPM_2 = reagent_plate_1["A5"]
    ...
    if CUSTOM_OFFSETS == True:
        ...
        # MAG BLOCK OFFSETS
        mb_drop_offset = {"x": 0, "y": 1.0, "z": 0.5}
    ...
    if STEP_WASH == True:
        ...
        for X in range(3):
            ...
            if WASH_AirMultiDis == True:
                TipCheck(200, None, loop, None)
                for loop, X in enumerate(column_2_list):
                    p1000.aspirate(TWBMaxVol + 3, TWB.bottom(z=Deepwell_Z_offset + 1), rate=0.25)
                    p1000.dispense(3, TWB.bottom(z=Deepwell_Z_offset + 1), rate=0.25)
                    # =====Reservoir Tip Touch========
                    p1000.default_speed = 100
                    p1000.move_to(TWB.top().move(types.Point(x=4, z=-3)))
                    p1000.move_to(TWB.top().move(types.Point(x=-4, z=-3)))
        ...
        for loop, X in enumerate(column_2_list):
            TipCheck(50, None, loop, None)
            if COLUMNS < 5:
                p50.aspirate(EPMVol + 3, EPM.bottom(z=PCRPlate_Z_offset + 1), rate=0.25)
                p50.dispense(3, EPM.bottom(z=PCRPlate_Z_offset + 1), rate=0.25)
            if COLUMNS >= 5:
                if EPMVolCount < 5:
                    p50.aspirate(EPMVol + 3, EPM_1.bottom(z=PCRPlate_Z_offset + 1), rate=0.25)
                    p50.dispense(3, EPM_1.bottom(z=PCRPlate_Z_offset + 1), rate=0.25)
                if EPMVolCount >= 5:
                    p50.aspirate(EPMVol + 3, EPM_2.bottom(z=PCRPlate_Z_offset + 1), rate=0.25)
                    p50.dispense(3, EPM_2.bottom(z=PCRPlate_Z_offset + 1), rate=0.25)
                EPMVolCount += 1
    ...
    if STEP_PCRDECK == True:
        ...
        if ONDECK_THERMO == True:
            ...
            thermocycler.close_lid()
            if DRYRUN == False:
                profile_PCR_1 = [{"temperature": 68, "hold_time_seconds": 180}, {"temperature": 98, "hold_time_seconds": 180}]
    ...
    if STEP_CLEANUP == True:
        ...
        for loop, X in enumerate(column_3_list):
            ...
            for Y in range(CleanupBeadMix):
                p50.aspirate(SampleVol + 3, CleanupPlate[column_4_list[loop]].bottom(z=Deepwell_Z_offset + 0.75), rate=0.5)
                p50.dispense(SampleVol + 3, CleanupPlate[column_4_list[loop]].bottom(z=Deepwell_Z_offset + 0.75), rate=1)
            ...
            if ETOH_1_AirMultiDis == True:
                ...
                for loop, X in enumerate(column_4_list):
                    ...
                    if ETOHVOL >= 28800:
                        EtOH = EtOH_3
                    # ================================
                    p1000.aspirate(ETOHMaxVol, EtOH.bottom(z=Deepwell_Z_offset + 1))
    ...
    if NOLABEL == True:
        Sample_Volume = 40
        CleanupBead_Volume = COLUMNS * (80 + 50 + 50) * 1.1
        TAGSTOP_Volume = COLUMNS * ((10)) * 1.1
        ETOH_Volume = COLUMNS * ((150 * 2) * 3) * 1.1
        TWB_Volume = COLUMNS * ((150 * 2) * 3) * 1.1
        RSB_Volume = COLUMNS * (50 + 20 + 25) * 1.1
        TAGMIX_Volume = COLUMNS * (10) * 1.1
        EPM_Volume = COLUMNS * (45) * 1.1
        H20_Volume = COLUMNS * (5) * 1.1
        ...
        for loop, X in enumerate(TotalColumn):
            ...
            reagent_plate_1.wells_by_name()[X + "3"].load_liquid(liquid=RSB, volume=RSB_Volume)
            reagent_plate_1.wells_by_name()[X + "4"].load_liquid(liquid=EPM, volume=EPM_Volume)
            if COLUMNS >= 5:
