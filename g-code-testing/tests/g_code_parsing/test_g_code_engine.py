...
async def test_watcher_command_list_is_cleared(protocol_g_code_engine: GCodeEngine):
    ...
    """
    async with protocol_g_code_engine.run_protocol(PROTOCOL_PATH, APIVersion(2, 13)) as run_1:
        run_1_desc = run_1.get_text_explanation(SupportedTextModes.G_CODE)
    async with protocol_g_code_engine.run_protocol(PROTOCOL_PATH, APIVersion(2, 13)) as run_2:
