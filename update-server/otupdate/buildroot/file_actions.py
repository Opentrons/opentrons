...
SIG_NAME = 'rootfs.ext4.hash.sig'
HASH_NAME = 'rootfs.ext4.hash'
ROOTFS_NAME = 'rootfs.ext4'
UPDATE_FILES = [ROOTFS_NAME, SIG_NAME, HASH_NAME]
MANDATORY_FILES = [ROOTFS_NAME, HASH_NAME]
...
def unzip_update(filepath: str,
    ...
    :param progress_callback: A callable taking a number between 0 and 1 that
                              ...
                              exactly.
    :param require_sig: If ``True`` raise :py:class:`FileMissing` if there is
                        no signature. If ``False``, ignore
    ...
    file_paths: Dict[str, Optional[str]] = {fn: None for fn in UPDATE_FILES}
    file_sizes: Dict[str, int] = {fn: 0 for fn in UPDATE_FILES}
    LOG.info(f"Unzipping {filepath}")
    with zipfile.ZipFile(filepath, 'r') as zf:
        files = zf.infolist()
        remaining_filenames = copy.copy(UPDATE_FILES)
        for fi in files:
            if fi.filename in UPDATE_FILES:
                to_unzip.append(fi)
        ...
        for rfn in remaining_filenames:
            if rfn != SIG_NAME or require_sig:
                raise FileMissing(f"File {rfn} missing from zip")
...
def validate_update(filepath: str,
    ...
    files, sizes = unzip_update(filepath, zip_callback, bool(cert_path))
...
def write_update(rootfs_filepath: str,
    ...
    with open(rootfs_filepath, 'rb') as img, open(part_path, 'wb') as part:
        if None is file_size:
            file_size = img.seek(0, 2)
            img.seek(0)
            LOG.info(f'write_update: file size calculated as {file_size}B')
        LOG.info(f'write_update: writing {rootfs_filepath} ({file_size}B)'
                 f' to {unused.value.path} in {chunk_size}B chunks')
        while True:
