...
def _begin_write(
    session: UpdateSession, loop: asyncio.AbstractEventLoop, rootfs_file_path: str
):
    ...
    write_future = asyncio.ensure_future(
        loop.run_in_executor(
            None, file_actions.write_update, rootfs_file_path, session.set_progress
        )
    )
...
def _begin_validation(
    ...
    def validation_done(fut):
        ...
        if exc:
            ...
            loop.call_soon_threadsage(_begin_write, session, loop, rootfs_file)
...
async def file_upload(
    request: web.Request, session: session.UpdateSession
) -> web.Response:
    ...
    async for part in reader:
        if part.name != "ot3-system.zip":
            LOG.warning(f"Unknown field name {part.name} in file uploaded. ignoring")
            await part.release()
        else:
...
@require_session
async def commit(request: web.Request, session: session.UpdateSession) -> web.Response:
