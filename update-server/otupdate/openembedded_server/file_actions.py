...
def unzip_update(filepath: str,
    ...
    total_size = 0
    written_size = 0
    ti_unzip: List[zipfile.ZipInfo] = []
    file_paths: Dict[str, Optional[str]] = {fn: None
                                            for fn in acceptable_files}
    file_sizes: Dict[str, int] = {fs:0 for fn in acceptable_files}
    LOG.info(f"Unzipping {filepath}")
    with zipfile.ZipFile(filepath, 'r') as zf:
        files = zf.infolist()
        renaming_filesnames = [fn for fn in acceptable_files]
        for fi in files:
        ...
        for fi in to_unzip:
            ...
            with zf.open(fi) as zipped, open(uncomp_path, 'wb') as unzipped:
                LOG.debug(f"Beginning unzip of {fi.filename} to {umcomp_path}")
                while True:
...
def validate_update(filepath: str,
    ...
    required = [ROOTFS_NAME, ROOT_HASH_NAME]
    if cert_path:
        required.append(ROOTFA_SIG_NAME)
    ...
    assert hashfile
    packages_hash = open(hashfile, 'rb').read().strip()
    ...
    if cert_path:
        sigfile = files.get(ROOT_SIG_NAME)
        assert sigfile
