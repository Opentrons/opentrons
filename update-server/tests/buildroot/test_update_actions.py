...
def test_validate_catches_bad_sig(downloaded_update_file, testing_cert):
    ...
    with pytest.raises(file_actions.SignatureMismatch):
        updater.validate_update(downloaded_update_file, cb,
                                testing_cert)
...
def test_commit_update(monkeypatch):
    ...
    monkeypatch.setattr(update_actions, '_find_unused_partition',
                        lambda: unused)
    monkeypatch.setattr(update_actions, '_switch_partition',
                        lambda: new)
...
def test_mount_update(monkeypatch, tmpdir):
    ...
    monkeypatch.setattr(update_actions, '_find_unused_partition',
                        lambda: unused)
    monkeypatch.setattr(update_actions, '_mountpoint_root',
                        lambda: tmpdir)
    mountpoint = None
