...
#
# Unlike system-server, this server has none: every endpoint that changes something
# is behind `require_scopes()`.
IGNORED_ENDPOINTS: set[tuple[str, str]] = set()
...
async def test_access_control_coverage(test_cli: UpdateServerClient) -> None:
    """Look for endpoints that should have access control but don't."""
    endpoint_info = get_declared_security(test_cli.asgi_app.openapi())
