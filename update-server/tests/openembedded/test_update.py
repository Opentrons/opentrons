...
from otupdate.common import update, config
from otupdate.common.update_actions import UpdateActionsInterface
from otupdate.openembedded import RootFSInterface, PartitionManager, Updater
...
async def test_updater_chain(
    ...
    fut = update._begin_validation(
        session, conf, loop, downloaded_update_ot3_file, updater
    )
...
async def test_session_catches_validation_fail(
    otupdate_config, downloaded_update_ot3_file, loop
):
    conf = config.load_from_path(otupdate_config)
    session = UpdateSession(conf.download_storage_path)
    rfs = RootFSInterface()
    part_mgr = PartitionManager()
    handler = Updater(rfs, part_mgr)
    fut = update._begin_validation(
        session, conf, loop, downloaded_update_ot3_file, handler
    )
...
async def test_update_happypath(
    ...
    monkeypatch.setattr(UpdateActionsInterface, "from_request", lambda x: updater)
    # Upload
    ...
    while body["stage"] == "validating":
        ...
        resp = await test_cli.get(session_endpoint(update_session, "status"))
        assert resp.status == 200
        body = await resp.json()
