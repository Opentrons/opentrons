...
metadata = {'protocolName': 'SMC High Sensitivity Immunoassay - Sample Dilution',
...
def add_parameters(parameters):
    ...
    choices = [{"display_name": f"1:{value}", "value": value} for value in dilution_ratios]
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    mount_single        = ctx.params.mount_single
    dry_run             = ctx.params.dry_run #minimizes incubation times and mix reps- also returns tip to box instead of trash
    dil_ratio           = ctx.params.dil_ratio   
    num_samples         = ctx.params.num_samples  # max 20 (in triplicate)
   
    try:
        [mount_single,dry_run,dil_ratio,num_samples] = get_values(  # noqa: F821
        'mount_single','dry_run','dil_ratio','num_samples')
    ...
    tips_1000_2                  = ctx.load_labware('opentrons_flex_96_filtertiprack_1000ul','B3','1000ul Tip Box')
   
    S1000                       = ctx.load_instrument('flex_1channel_1000', mount_single, tip_racks=[tips_1000_2]) # Single-channel pipette
    ...
    if dil_ratio in (30, 50, 64, 100):
        if  num_samples > 10:
            vol_sd_1 = 12000
            vol_sd_2 = 5000
            def_sd_1 = ctx.define_liquid(name="Sample Diluent", description=" ", display_color="#9966FF")  ## light
            def_sd_2 = ctx.define_liquid(name="Sample Diluent", description=" ", display_color="#9966FF")  ## light
    ...
    if dil_ratio in (30, 50, 64, 100, 200, 300, 1000):
        for well in samples:
    ...
    # sample_dil_scheme = { ratio: [manual sample input, sample in c1, diluent in c1, sample from c1 into E1, diluent from c1 into E1, sample from e1 into g1, diluent from e1 into g1] }
    sample_dil_scheme = {
    ...
    sample_src = (sample_plate.rows_by_name()["A"][:]+ sample_plate.rows_by_name()["B"][:8])[:num_samples]
    step1_src = (intermediate.rows_by_name()["C"][:]+ intermediate.rows_by_name()["D"][:8])[:num_samples]
    step2_src = (intermediate.rows_by_name()["E"][:]+ intermediate.rows_by_name()["F"][:8])[:num_samples]
    step3_src = (intermediate.rows_by_name()["G"][:]+ intermediate.rows_by_name()["H"][:8])[:num_samples]
