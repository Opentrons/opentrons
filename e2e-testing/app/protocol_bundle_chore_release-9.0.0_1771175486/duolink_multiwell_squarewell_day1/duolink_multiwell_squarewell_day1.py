...
metadata = {
    'protocolName': 'Duolink PLA for Microscopy (Multiwell Plate Assay with 96 Square Well Culture Plate) - Day 1',
...
def run(ctx):
    ...
    if num_well_last_col > 0:
        rxn_remainder = working_plate.wells()[num_col_full*8:num_col_full*8+num_well_last_col]
    ...
    # volume info 
    vol_ab = 80 * num_col_full + 80 
    vol_ab_plus_one = 80 * (num_col_full + 1) + 80
    def_ab= ctx.define_liquid(name="ANTIBODY SOLUTION", description="", display_color="#98FB98")  ## green
    if num_well_last_col > 0:
        [reagent_plate.rows()[row][0].load_liquid(liquid=def_ab, volume=vol_ab_plus_one)
    ...
    vol_re = 80 * num_col_total + 80 
    def_block = ctx.define_liquid(name="BLOCKING SOLUTION", description="", display_color="#FFC300")  ## yellow
    ...
    def transfer_remainder(start, vol):
