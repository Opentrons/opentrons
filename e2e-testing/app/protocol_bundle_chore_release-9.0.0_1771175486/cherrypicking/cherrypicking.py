...
def run(ctx):
    [pipette_type, pipette_mount, tip_type, tip_reuse, transfer_csv] = get_values(
        "pipette_type", "pipette_mount", "tip_type", "tip_reuse", "transfer_csv"
    )
    tiprack_map = {
        "p10_single": {"standard": "opentrons_96_tiprack_10ul", "filter": "opentrons_96_filtertiprack_20ul"},
        "p50_single": {"standard": "opentrons_96_tiprack_300ul", "filter": "opentrons_96_filtertiprack_200ul"},
        "p300_single": {"standard": "opentrons_96_tiprack_300ul", "filter": "opentrons_96_filtertiprack_200ul"},
        "p1000_single": {"standard": "opentrons_96_tiprack_1000ul", "filter": "opentrons_96_filtertiprack_1000ul"},
        "p20_single_gen2": {"standard": "opentrons_96_tiprack_20ul", "filter": "opentrons_96_filtertiprack_20ul"},
        "p300_single_gen2": {"standard": "opentrons_96_tiprack_300ul", "filter": "opentrons_96_filtertiprack_200ul"},
        "p1000_single_gen2": {"standard": "opentrons_96_tiprack_1000ul", "filter": "opentrons_96_filtertiprack_1000ul"},
    }
    transfer_info = [
    ...
    for line in transfer_info:
        ...
        if float(vol) < 5:
            pip.aspirate(5, source.bottom(float(h)), rate=0.8)
            pip.dispense(5, source.bottom(float(h)), push_out=0)

            pip.aspirate(5, source.bottom(float(h)), rate=0.5)
            ctx.delay(seconds=1)
            pip.touch_tip(v_offset=-2, speed=20)     

            pip.dispense(float(vol), dest.bottom(z=2), push_out=0)
            ctx.delay(seconds=1)
            pip.touch_tip(v_offset=-2, speed=20)  
            pip.blow_out(source.top(z=-2))
            pip.touch_tip(speed=60)  
        else:
            pip.aspirate(float(vol), source.bottom(float(h)), rate=0.8)
            pip.dispense(float(vol), source.bottom(float(h)), push_out=0)

            pip.aspirate(float(vol), source.bottom(float(h)), rate=0.5)
            ctx.delay(seconds=1)
            pip.touch_tip(v_offset=-2, speed=20)     
