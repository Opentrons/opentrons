...
metadata = {
    'protocolName': 'Duolink PLA for Microscopy (Component Testing Assay with 96 Round Well Culture Plate) - Day 2',
...
def run(ctx):
    ...
    # volume info
    vol_re = [VOL_PROBE*4+40, VOL_LIGATION*4+40, VOL_AMP*4+40, VOL_DAPI*4+40, VOL_AF*4+40]
    ...
    descrip_re = '40uL*4 +40uL per well'
    ...
    def wash(buffer_wash, run, min, tips):
        ...
        for n in range(run):
            ...
            for j in range(NUM_COL):    
                ...
                p1k_8.dispense(VOL_WASH-10, rxn[j].top(z=-3).move(Point(x=D_1K, y=D_1K)))
                p1k_8.move_to(rxn[j].top(z=0))
            if dry_run: p1k_8.return_tip()
            else: p1k_8.drop_tip()      
            p1k_8.flow_rate.dispense = DEFAULT_RATE 
