...
metadata = {
    'protocolName': 'Duolink PLA for Microscopy (Multiwell Plate Assay with 96 Square Well Culture Plate) - Day 2',
...
def run(ctx):
    ...
    wash_other_plate = ctx.load_labware('nest_12_reservoir_15ml', 'B2', 'WASH BUFFER B, B(0.01x), and PBS')  
    ...
    tips_200_reuse = [ctx.load_labware('opentrons_flex_96_tiprack_200ul', slot, '200uL TIPS REUSE')
    ...
    for col, (name_info, color_info) in enumerate(zip(name_re, color_re)):
        def_re = ctx.define_liquid(name=name_info, description=" ", display_color=color_info)
        [reagent_plate.rows()[row][col].load_liquid(liquid=def_re, volume=vol_re_dark)
         for row in range(num_well_last_col)]
        def_re = ctx.define_liquid(name=name_info, description=" ", display_color=color_info)
        [reagent_plate.rows()[row][col].load_liquid(liquid=def_re, volume=vol_re_light)
         for row in range(num_well_last_col, 8)]
        
    vol_wa_a = VOL_WASH * 6 * num_sample + 24000
    def_wa_a = ctx.define_liquid(name='WASH BUFFER A', description=" ", display_color='#0080bf') ## Blue
    wash_a_plate.wells()[0].load_liquid(liquid=def_wa_a, volume=vol_wa_a)

    def_wa_b = ctx.define_liquid(name='WASH BUFFER B', description=" ", display_color='#00acdf') ## Blue
    if num_col_total > 6:
        vol_wa_b = [VOL_WASH * (6-1) * 8 + 2000, VOL_WASH * (num_col_total-6-1) * 8 + 2000]
        [wash_other_plate.wells()[n].load_liquid(liquid=def_wa_b, volume=vol_wa_b[n])
         for n in [0, 1]]
        [wash_other_plate.wells()[n].load_liquid(liquid=def_wa_b, volume=vol_wa_b[n-2])
         for n in [2, 3]]
    else:
        vol_wa_b = VOL_WASH * (num_col_total-1) * 8 + 2000
        [wash_other_plate.wells()[n].load_liquid(liquid=def_wa_b, volume=vol_wa_b)
         for n in [0, 2]]

    def_wa_b_diluted = ctx.define_liquid(name='WASH BUFEER B (0.01x)', description=" ", display_color='#7ce8ff') ## Blue
    if num_col_total > 6:
        vol_wa_b_diluted = [VOL_WASH * (6-1) * 8 + 2000, VOL_WASH * (num_col_total-6-1) * 8 + 2000]
        [wash_other_plate.wells()[n].load_liquid(liquid=def_wa_b_diluted, volume=vol_wa_b_diluted[n-4])
         for n in [4, 5]]  
    else:
        vol_wa_b_diluted = VOL_WASH * (num_col_total-1) * 8 + 2000
        wash_other_plate.wells()[4].load_liquid(liquid=def_wa_b_diluted, volume=vol_wa_b_diluted)

    def_wa_pbs = ctx.define_liquid(name='PBS', description=" ", display_color='#ccf9ff') ## Blue
    if num_col_total > 6:
        vol_wa_pbs = [VOL_WASH * (6-1) * 8 + 2000, VOL_WASH * (num_col_total-6-1) * 8 + 2000]
        [wash_other_plate.wells()[n].load_liquid(liquid=def_wa_pbs, volume=vol_wa_pbs[n-6])
         for n in [6, 7]]  
    else:
        vol_wa_pbs = VOL_WASH * (num_col_total-1) * 8 + 2000
        wash_other_plate.wells()[6].load_liquid(liquid=def_wa_pbs, volume=vol_wa_pbs) 
    ...
    def wash(run, min, tips):
        ...
        for n in range(run):
            ...
            for mm in range(m):
                p1k_8.aspirate(VOL_WASH*4, wash_a.bottom(z=1).move(Point(x=(n*6+mm)*9-49.5)))
                ctx.delay(seconds=1)
                p1k_8.air_gap(10)
                ...
                for j in range(4): 
                    ...
                    p1k_8.dispense(VOL_WASH-10, rxn_total[mm*4+j].top(z=-2).move(Point(x=D_1K, y=D_1K)))
                    p1k_8.move_to(rxn_total[mm*4+j].top(z=0))
                    ctx.delay(seconds=1)
            if l > 0:
                p1k_8.aspirate(VOL_WASH*l, wash_a.bottom(z=1).move(Point(x=(m+1)*9-49.5)))
                ctx.delay(seconds=1)
                p1k_8.air_gap(10)
                p1k_8.flow_rate.dispense = 80  
                ...
                for j in range(l):      
                    ...
                    p1k_8.dispense(VOL_WASH-10, rxn_total[m*4+j].top(z=-2).move(Point(x=D_1K, y=D_1K)))
                    p1k_8.move_to(rxn_total[m*4+j].top(z=0))
                    ctx.delay(seconds=1)               
            ...
            for k in range(num_col_total):            
                ...
                p1k_8.aspirate(VOL_WASH, rxn_total[k].bottom(z=0.8).move(Point(x=D_200, y=D_200))) 
                ctx.delay(seconds=2)
                p1k_8.move_to(rxn_total[k].top(z=0))    
        ...
        for _ in range(2):
            ...
            if num_col_total > 6:
                ...
                for j in range(6):      
                    ...
                    p1k_8.dispense(vol_half-10, rxn_total[j].top(z=-2).move(Point(x=D_1K, y=D_1K)))
                    p1k_8.move_to(rxn_total[j].top(z=0))
                    ctx.delay(seconds=1) 
                ...
                for j in range(num_col_total-6):        
                    ...
                    p1k_8.dispense(vol_half-10, rxn_total[6+j].top(z=-2).move(Point(x=D_1K, y=D_1K)))
                    p1k_8.move_to(rxn_total[6+j].top(z=0))
                    ctx.delay(seconds=1)  
            else:
                ...
                for j in range(num_col_total):        
                    p1k_8.move_to(rxn_total[j].top(z=0))              
                    p1k_8.dispense(vol_half, rxn_total[j].top(z=-2).move(Point(x=D_1K, y=D_1K)))
                    p1k_8.move_to(rxn_total[j].top(z=0))
                    ctx.delay(seconds=1) 
    ...
    wash_2(wash_b_diluted, MIN_WASH_B_DILUTED if not dry_run else 0.1, tips_200_reuse[2]) 
