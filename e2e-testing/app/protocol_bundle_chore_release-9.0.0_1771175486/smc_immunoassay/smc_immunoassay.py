...
metadata = {'protocolName': 'SMC High Sensitivity Immunoassay',
...
def add_parameters(parameters):
    ...
    parameters.add_str(
        ...
        description="Select the analyte for the assay",
        default="IL-22",
        choices=[
                 ...
                 {"display_name": "VEGF-A", "value": "VEGF-A"},
    ...
    parameters.add_bool(
        variable_name="sealing_plate",
        display_name="Seal Plate for Capture Step",
        description="Allow pause after plate set-up to seal for capture step, otherwise proceeds to capture incubation",
        default=False
    )
...
def run(ctx):
    ...
    jitterbug_4         = 875
    jitterbug_5         = 1000
    Z_OFFSET_RESERVOIR  = 2.0
    Z_OFFSET_ELU        = 0.6

    dil_ratio = 2

    dry_run             = ctx.params.dry_run #minimizes incubation times and mix reps- also returns tip to box instead of trash
    dilute_samples      = ctx.params.dilute_samples
    detection_AB        = ctx.params.detection_AB 
    # reuse_elute_tips    = True
    num_samples         = ctx.params.num_samples  # max 20 (in triplicate)
    mount_multi         = ctx.params.mount_multi
    sealing_plate         = ctx.params.sealing_plate
    run_type               = ctx.params.run_type   
    
    try:
        [mount_multi, detection_AB, run_type, sealing_plate, dry_run, dilute_samples, num_samples] = get_values(  # noqa: F821
        'mount_multi','detection_AB','run_type', 'sealing_plate', 'dry_run','dilute_samples','num_samples')

    except (NameError):
        pass


    # if num_samples == 0: run_type = "Standards"
    # else: run_type = "Standards+Samples"
    ...
    if detection_AB != "IL-6" and detection_AB != "IL-17A" and detection_AB != "cTnI" and detection_AB != "IL-22" and detection_AB != "IL-2" and detection_AB != "NF-L" and detection_AB != "NPTX2" and detection_AB != "PD-1" and detection_AB != "IFN-a2" and detection_AB != "IL-5" and detection_AB != "IL-13" and detection_AB != "MCP-1" and detection_AB != "BDNF" and detection_AB != "GFAP" and detection_AB != "IL-23" and detection_AB != "IL-12p70" and detection_AB != "IL-17F" and detection_AB != "IL-17A/F" and detection_AB != "pTau217" and detection_AB != "pTau231" and detection_AB != "IL-1a" and detection_AB != "IL-1b" and detection_AB != "PD-L1" and detection_AB != "Glucagon" and detection_AB != "IFN-b1" and detection_AB != "IL-18" and detection_AB != "NGFb" and detection_AB != "IFN-L1" and detection_AB != "IFN-g" and detection_AB != "UCHL1" and detection_AB != "IL-4" and detection_AB != "IL-10" and detection_AB != "TNFa" and detection_AB != "VEGF-A" and detection_AB != "p-aSynuclein" and detection_AB != "total aSynuclein" and detection_AB != "AB1-40" and detection_AB != "AB1-42" and detection_AB != "pTau181" and detection_AB != "total Tau" and detection_AB != "SNAP-25" and detection_AB != "TDP-43":
        raise Exception('No assay associated with the chosen Antibody, please pick another.')

    ctx.comment(f"Detection Incubation: {detection_incubation}; Capture Incubtation: {capture_incubation}; Sample Vol: {volume_s}")
    
    sample_vol                  = 3.5*volume_s

    h_s                         = ctx.load_module('heaterShakerModuleV1','D3')
    ...
    elution_plate               = ctx.load_labware('smc_384_read_plate', 'D1','Read plate without skirted lid')
    standard_plate              = ctx.load_labware('nest_96_wellplate_2ml_deep', 'D2','Starting standards + sample plate')
    ...
    tips_1000                   = ctx.load_labware('opentrons_flex_96_filtertiprack_1000ul','A2','1000ul Tip Box')
    
    tips_50                     = ctx.load_labware('opentrons_flex_96_filtertiprack_50ul','A1','50ul tip box #1')
    tips_51                     = ctx.load_labware('opentrons_flex_96_filtertiprack_50ul','B1','50ul tip box #2')
   

    # pipettes
    M1000                       = ctx.load_instrument('flex_8channel_1000', mount_multi) # Multi-channel pipette
    S1000                       = ctx.load_instrument('flex_1channel_1000', mount_single, tip_racks=[tips_1000]) # Single-channel pipette
    ...
    if run_type == "Standards+Samples":
        samples_liq = ctx.define_liquid(
            name='Samples',
            description='Samples',
            display_color='#f833ff'
        )
    standard_1_liq = ctx.define_liquid(
        name='Standard 1',
        description=stdrd_descrip,
        display_color='#D0BF15'
    )
    ...
    if dilute_samples == True:
        sample_diluent.load_liquid(sample_diluent_liq, volume=((dil_ratio*num_samples)*volume_s)+1500)
    ...
    standard_dest_sets = [col[:3] for col in assay_plate.columns()] # creates list of 12 lists, with 3 items each
    ...
    for i, (vol, d) in enumerate(zip(standard_dil_vols, standard_plate.rows()[0][1:])):
        S1000.aspirate(550 if i == 0 else 500,standard_diluent.bottom(Z_OFFSET_RESERVOIR))
    ...
    for s, dest_set in zip(standard_plate.rows()[0],standard_dest_sets):
        S1000.pick_up_tip()
        num_asp = math.ceil(len(dest_set)*volume_s/1000)
    ...
    if run_type == "Standards+Samples":
        ...
        sample_start_list = [3,27,51,75,4,28,52,76,5,29,53,77,6,30,54,78,7,31,55,79] 
        #all of the possible starting wells for the samples in assay plate
        s_list = []

        for i in range(num_samples):
            s_list.append(sample_start_list[i]) #creates list of actual starting wells depending on number of samples

        for i, s in zip(s_list,samples):
            S1000.pick_up_tip()
            if dilute_samples:
                S1000.aspirate(volume_s*2*(dil_ratio-1),sample_diluent.bottom(1))
                S1000.dispense(S1000.current_volume,s.top(-3))
                S1000.mix(7 if not dry_run else 1,(volume_s*dil_ratio)-10,s.bottom(1))
    ...
    M1000.move_to(coated_beads.top().move(types.Point(x=-2.5,z=4)))
    M1000.default_speed *= 16 
    for scheme in wells_to_pip:
        ...
        for well in scheme:
            ...
            ctx.delay(seconds=1)
            M1000.default_speed /= 16
            M1000.move_to(well.top().move(types.Point(x=-well.diameter/2+0.1,z=-2))) 
            ctx.delay(seconds=0.5)
            M1000.move_to(well.top().move(types.Point(x=-well.diameter/2+2,z=4)))
            M1000.default_speed *= 16
    ...
    if sealing_plate:
        ...
        ctx.pause('Please add seal to plate. Heater-shaker incubation will begin afterwards.')
    h_s.close_labware_latch()
    
    h_s.set_and_wait_for_shake_speed(jitterbug_3)
    ctx.delay(minutes=capture_incubation*60 if not dry_run else 0.25,msg=f'Please allow {capture_incubation} hour(s) for incubation at room temperature.')
    ctx.pause(msg='When back, press resume to continue.')
    h_s.deactivate_shaker()
    h_s.open_labware_latch()
    ctx.pause(f'The Incubation is complete- perform the post-capture wash and return plate to the Heater-Shaker afterwards. Please add Detection AB ({detection_AB}) to reservoir.')
    h_s.close_labware_latch()
    ...
    M1000.aspirate(260,detection_antibody.bottom(Z_OFFSET_RESERVOIR)) #aspirate whole plate worth of antibody
    ...
    for i,d in enumerate((assay_plate.rows()[0])):
        M1000.dispense(20, d.bottom().move(types.Point(x=-0.75 if i % 2 == 0 else 0.8,z=1.1)))
        ctx.delay(seconds=1)
    ...
    ctx.pause('Starting Detection incubation on Flex. Please add seal to plate. Heater-shaker incubation will begin afterwards.')
    
    h_s.close_labware_latch()
    h_s.set_and_wait_for_shake_speed(jitterbug_5)
    ctx.delay(minutes=detection_incubation*60 if not dry_run else 0.25,msg=f'Please allow {detection_incubation} hour(s)\
        for incubation at room temperature.')
    ctx.pause(msg='When back, press resume to continue.')
    h_s.deactivate_shaker()
    h_s.open_labware_latch()

    ctx.pause(msg='Carefully remove seal. Perform post-detection wash then return plate (without seal) to heater-shaker for final\
        shake before final aspiration.')

    h_s.close_labware_latch()
    h_s.set_and_wait_for_shake_speed(jitterbug_3)
    ctx.delay(minutes=1.5 if not dry_run else 0.1,msg=f'Shaking for 90s at {jitterbug_3} rpm.')
    h_s.deactivate_shaker()
    h_s.open_labware_latch()
    ctx.pause(msg='Place plate on plate washer for final aspiration, then return the plate to the Heater-Shaker (without seal).')
    h_s.close_labware_latch()
    h_s.open_labware_latch()
    ctx.move_labware(
        assay_plate,
    ...
    for i, (tip, d) in enumerate(zip(elutionB_tips,samples_e)):
        ...
        M1000.move_to(d.bottom(0.75))
        M1000.default_speed /= 60
        M1000.dispense(11,d.bottom().move(types.Point(x=-0.5 if i % 2 == 0 else 0.5,z=0.75)),rate=0.02)
        ctx.delay(seconds=2)
        M1000.default_speed *= 60
    ...
    h_s.set_and_wait_for_shake_speed(jitterbug_5)
    ctx.delay(minutes=10 if not dry_run else 0.25,msg='Please allow 10 minute incubation at 1000 rpm.')
    h_s.deactivate_shaker()
    ...
    ctx.delay(minutes=2 if not dry_run else 0.25,msg='Please allow 2 minutes for beads to settle.')
    ...
    for tip, s, d in zip(elutionD_tips,samples_e, elution_dests):
        ...
        ctx.delay(seconds=0.5)
        M1000.move_to(d.bottom(8))
        M1000.dispense(17,d.bottom().move(types.Point(x=d.length/2-0.9,z=5)),rate=0.02) 
        ctx.delay(seconds=1)
        M1000.move_to(d.top().move(types.Point(x=d.length/2-0.9,z=-2.5)))
        M1000.default_speed /= 10
        M1000.move_to(d.top().move(types.Point(x=d.length/2-0.2,z=-2.5)))
        M1000.move_to(d.top().move(types.Point(x=d.length/2-0.2,z=2.5))) 
        M1000.default_speed *= 10 
        if not dry_run:
            M1000.dispense(S1000.current_volume, waste)
