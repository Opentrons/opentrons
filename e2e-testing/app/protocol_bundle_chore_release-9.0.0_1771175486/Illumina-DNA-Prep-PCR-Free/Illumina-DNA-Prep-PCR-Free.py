...
################################################################################
	...
	if REUSE_RSB_TIPS:
		...
		if report:
			ctx.comment(f'\nAdding 50 ul tip rack #{p50_rack_count}\n')
	...
	if REUSE_REMOVE_TIPS:
		...
		if report:
			ctx.comment(f'\nAdding 200 ul tip rack #{p200_rack_count} to slot A2\n')
		for x in range(num_cols):
		...
		if report:
			ctx.comment(f'\nAdding 200 ul tip rack #{p200_rack_count} to slot {Available_on_deck_slots[0]}\n')
		tt_200 += 12
	...
	if REUSE_ETOH_TIPS and not REUSE_REMOVE_TIPS:
		...
		p200.tip_racks.append(tip200_reuse)
		if report:
			ctx.comment(f'\nAdding 200 ul tip rack #{p200_rack_count}\n')
		for x in range(num_cols):
...
################################################################################
	...
	if num_cols >= 4:
		...
		# elm 		= elm1 + elm2
		for well in elm1:
			well.load_liquid(liquid=elm_liq,volume=(elm_vol * 3) * 1.1)
		for well in elm2:
			well.load_liquid(liquid=elm_liq,volume=(elm_vol * (3-(6-num_cols))) * 1.1)
	else:
		elm 		= elm1
		for well in elm1:
			well.load_liquid(liquid=elm_liq,volume=(elm_vol * num_cols) * 1.1)
	...
	for well in blt:
		well.load_liquid(liquid=blt_liq,volume=(blt_v * num_cols) * 1.1)
		well.load_liquid(liquid=tb1_liq,volume=(tb1_vol * num_cols) * 1.1)
	...
	for well in st2:
		well.load_liquid(liquid=st2_liq,volume=(st2_vol * num_cols) * 1.1)
...
################################################################################
		...
		if rack == tip50:
			if tt_50 == 0 and reuse == None: #If this is the first column of tip box and these aren't reused tips
				if len(Available_on_deck_slots) > 0: 
					...
					if report:
						ctx.comment(f'\nAdding 50 ul tip rack #{p50_rack_count} to slot {Available_on_deck_slots[0]}\n')
					Available_on_deck_slots.pop(0)
	...
	def TipSwap(tipvol):
		...
		if report:
			ctx.comment(f'\nThrew out: {rack_to_dispose} and placed {rack_to_add} to {deck_slot}\n')

	def run_tag_profile():
		#Presetting Thermocycler Temps
		ctx.comment("\n****Starting TAG Profile (41C for 5 minutes with 100C lid)****\n")
		...
		#Move Plate to TC
		ctx.comment('\n****Moving Plate to Pre-Warmed TC Module Block****\n')
		if heater_shaker:
	...
	def run_elm_profile():
		...
		#Presetting Thermocycler Temps
		ctx.comment("\n****Starting ELM Profile (37C for 5 minutes, then 50C for 5 minutes with 100C lid)****\n")
		tc_mod.set_lid_temperature(100)
		...
		#Move Plate to H-S
		ctx.comment("\n****Moving Sample Plate back to H-S****\n")
		ctx.move_labware(
			sample_plate,
			h_s if heater_shaker else 'D1',
			use_gripper = USE_GRIPPER
	...
	def remove_supernatant(well,vol,waste_,column):
		ctx.comment("\n-------Removing "+str(vol)+ "ul of Supernatant-------\n")
		...
		for x in range(num_trans):
			...
			if REUSE_REMOVE_TIPS:
				p200.return_tip()
				ctx.comment('\n****Dropping Tip Back in Tip Box****\n')
			else:
				if trash_tips:
					p200.drop_tip()
					ctx.comment('\n****Dropping Tip in Waste shoot****\n')
				else:
					p200.return_tip()
					ctx.comment('\n****Dropping Tip Back in Tip Box****\n')
...
################################################################################
	...
	def post_tag_cleanup():
		...
		for i in range(num_cols):
			ctx.comment('\n**** Adding Stop Solution to Column ' + str(i+1) + ' to Stop Tagmentation ****\n')
		...
		ctx.comment('\n****Moving Elution Plate from Magnet to TC****\n')
		ctx.move_labware(
			FLP_plate,
			tc_mod if ondeck_thermo else "A1",
			use_gripper=USE_GRIPPER
...
################################################################################
	...
	def index_ligation():
		...
		if heater_shaker:
			...
			h_s.deactivate_shaker()
			h_s.open_labware_latch()
		ctx.comment("\n****Moving Sample Plate to Magnet for Pelleting****\n")
		ctx.move_labware(
			sample_plate,
			magblock,
			use_gripper=USE_GRIPPER
		...
		if heater_shaker:
			h_s.open_labware_latch()
		ctx.comment("\n****Moving Plate back to Heater-Shaker for Wash****\n")
		ctx.move_labware(
			sample_plate,
			h_s if heater_shaker else 'D1',
			use_gripper = USE_GRIPPER
		...
		if heater_shaker:
			h_s.open_labware_latch()
		ctx.comment("\n****Removing Sample Plate from Magnet****\n")
		ctx.move_labware(
			sample_plate,
			h_s if heater_shaker else 'D1',
			use_gripper = USE_GRIPPER
...
################################################################################
	...
	def lib_cleanup():
		...
		global threshold
		ctx.comment("\n-------Starting Cleanup-------\n")
		ctx.comment("\n-------Adding and Mixing Cleanup Beads-------\n")
		...
		if heater_shaker:
			h_s.open_labware_latch()
		ctx.comment("\n****Moving Labware to Magnet for Pelleting****\n")
		ctx.move_labware(
			sample_plate,
			magblock,
			use_gripper=USE_GRIPPER
		...
		if standard_input:
			ctx.comment('\n-------Mixing Beads and Transferring to LP2 Wells-------')
			...
			if heater_shaker:
				h_s.close_labware_latch()
			ctx.comment('\n-------Mixing Beads in LP2 Wells with LP1 Supernatant-------\n')
...
################################################################################
		...
		if heater_shaker:
			h_s.open_labware_latch()
		ctx.comment("\n****Moving sample plate off of Magnet****\n")
		ctx.move_labware(
			sample_plate,
			h_s if heater_shaker else 'D1',
			use_gripper=USE_GRIPPER
...
################################################################################			
		...
		if heater_shaker:
			h_s.open_labware_latch()
		ctx.comment("\n****Move Samples to Magnet for Pelleting****\n")
		ctx.move_labware(
			sample_plate,
			magblock,
			use_gripper=USE_GRIPPER
