...
metadata = {
    'protocolName': 'Duolink PLA for Microscopy (Component Testing Assay with 96 Square Well Culture Plate) - Day 1',
...
def run(ctx):
    ...
    # volume info 
    vol_to_be_transfer_ab = 80 * 8 + 80 
    vol_to_be_transfer_re = 80 * 4 + 80 
    ...
    def_block = ctx.define_liquid(name="BLOCKING SOLUTION", description="80uL*4 + 80uL per well", display_color="#FFC200")  ## yellow
    ...
    for ab_count in range(NUM_COL):
        ...
        for well_count in range(8):  
            ...
            p1k_1.dispense(VOL_AB, end.top(z=-2).move(Point(x=D_1K, y=D_1K)))
            p1k_1.move_to(end.top(z=0))
        p1k_1.blow_out()
