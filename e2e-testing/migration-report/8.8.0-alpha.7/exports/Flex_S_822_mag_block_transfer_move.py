...
def run(protocol: protocol_api.ProtocolContext) -> None:
    ...
    tip_rack_3 = protocol.load_labware(
        "opentrons_flex_96_filtertiprack_50ul",
        location="A2",
        label="Opentrons Flex 96 Filter Tip Rack 50 µL (3)",
        namespace="opentrons",
        version=1,
    )
    tip_rack_4 = protocol.load_labware(
        "opentrons_flex_96_filtertiprack_50ul",
        location="A3",
        label="Opentrons Flex 96 Filter Tip Rack 50 µL (4)",
        namespace="opentrons",
        version=1,
    )
    ...
    pipette_left.configure_for_volume(40)
    ...
    pipette_left.mix(
        ...
        dispense_flow_rate=52,
        final_push_out=2,
    )
    pipette_left.drop_tip(waste_chute)

    # Step 3: pause
    protocol.delay(seconds=300, msg="Moving DNA onto beads. Next step: magnet separation. ")

    # Step 4: move
    protocol.move_labware(well_plate_1, magnetic_block_1, use_gripper=True)

    # Step 5: pause
    protocol.delay(seconds=300, msg="Magnet separation 1")
    ...
    # Step 13: pause
    protocol.delay(seconds=300, msg="Bead drying")

    # Step 14: move
    protocol.move_labware(well_plate_1, "C1", use_gripper=True)
    ...
    pipette_left.pick_up_tip(location=tip_rack_1)
    pipette_left.configure_for_volume(50)
    pipette_left.prepare_to_aspirate()
    pipette_left.mix(
        repetitions=10,
        volume=50,
    ...
    # Step 17: pause
    protocol.delay(seconds=120, msg="Eluting DNA from beads")
    ...
    # Step 24: pause
    protocol.delay(seconds=300, msg="Pushing DNA onto beads. Next step: magnet separation. ")

    # Step 25: move
    protocol.move_labware(well_plate_1, magnetic_block_1, use_gripper=True)

    # Step 26: pause
    protocol.delay(seconds=300, msg="Magnet separation")
    ...
    # Step 38: pause
    protocol.delay(seconds=120, msg="Eluting DNA")

    # Step 39: move
    protocol.move_labware(well_plate_1, magnetic_block_1, use_gripper=True)

    # Step 40: pause
    protocol.delay(seconds=300, msg="Removing beads")
