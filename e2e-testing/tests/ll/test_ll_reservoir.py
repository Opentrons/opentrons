"""Reservoir creation test for the Labware Creator.

Ported from legacy Cypress test:
    labware-library/cypress/e2e/labware-creator/reservoir.cy.js
"""
...
EXPECTED_FIXTURE = LABWARE_FIXTURES / "testpro_10_reservoir_250ul.json"


@pytest.mark.llE2E
def test_ll_create_reservoir(page: Page, ll_base_url: str) -> None:
    """Create a reservoir and verify the exported definition."""
    creator = LabwareCreator(page)
    creator.navigate(ll_base_url)

    # -- Select labware type --
    creator.select_labware_type("Reservoir")
    ...
    creator.set_grid_rows("1")
    creator.expect_rows_required_error(visible=False)

    # Row spacing radio should not exist for a 1-row reservoir
    creator.expect_row_spacing_radio_not_visible()
    ...
    creator.set_well_y_dimension("60")
    ...
    creator.set_well_depth("70")
    creator.expect_depth_required_error(visible=False)

    # -- Grid spacing (X only for reservoir) --
    ...
    creator.expect_display_name_placeholder("TestPro 10 Reservoir 250 µL")
    creator.expect_filename_placeholder("testpro_10_reservoir_250ul")
