"""E2E regression tests for Protocol Designer customer-reported protocols.

Covers RQA-5529 (Maor) and RQA-5354 (post-tagmentation): import spot-checks
and post-import wizard steps that remain export-clean.
"""
...
def _import_maor(page: Page) -> tuple[ProtocolEditorPage, TransferPage, Timeline]:
...
def _import_post_tagmentation(page: Page) -> tuple[ProtocolEditorPage, TransferPage, Timeline]:
...
def _clear_maor_d4_neighbors(editor: ProtocolEditorPage) -> None:
    ...
    for label, labware in (
        ...
        editor.move_labware_with_gripper(labware, "Off-deck")
...
def _prepare_post_tag_clean_tiprack(editor: ProtocolEditorPage) -> None:
    ...
    editor.move_labware_with_gripper(POST_TAG_TIPRACK_200_B3, POST_TAG_TIPRACK_SAFE_SLOT)


def _assert_timeline_and_export_clean(
    page: Page,
    ...
    *,
    ...
    """Assert timeline stays clean after wizard steps and export downloads a protocol file."""
    timeline.wait_for_timeline_steps(min_steps=min_steps)
    timeline.expect_no_known_regression_errors()
    assert_export_downloads_clean_protocol(page, editor, exports_dir, filename=filename)


@pytest.mark.pdE2E
@pytest.mark.slow
@pytest.mark.timeout(600)
def test_pd_maor_protocol_reservoir_transfer_steps_have_no_errors(page: Page, pd_exports_dir: Path) -> None:
    """RQA-5529: import stays clean and reservoir transfer steps remain error-free in the step editor."""
    editor, _, timeline = _import_maor(page)
    ...
    assert_export_downloads_clean_protocol(
        ...
        editor,
        pd_exports_dir,
        filename="maor_reservoir_transfer_spot_check.py",
    )
...
@pytest.mark.slow
@pytest.mark.timeout(600)
def test_pd_post_tagmentation_reservoir_wash_steps_have_no_errors(page: Page, pd_exports_dir: Path) -> None:
    """RQA-5354: import stays clean and reservoir wash steps with automatic tip tracking remain error-free."""
    editor, _, timeline = _import_post_tagmentation(page)
    ...
    assert_export_downloads_clean_protocol(
        ...
        editor,
        pd_exports_dir,
        filename="post_tagmentation_reservoir_wash_spot_check.py",
    )
...
def test_pd_maor_96ch_reservoir_column_transfer(page: Page, pd_exports_dir: Path) -> None:
    ...
    _assert_timeline_and_export_clean(
        page,
        editor,
        timeline,
        pd_exports_dir,
        min_steps=MAOR_MIN_TIMELINE_STEPS + MAOR_NEIGHBOR_CLEAR_STEPS + 1,
        filename="maor_96ch_reservoir_column_transfer.py",
    )
...
@pytest.mark.slow
@pytest.mark.timeout(900)
def test_pd_post_tagmentation_96ch_wash_like_single_transfer(page: Page, pd_exports_dir: Path) -> None:
    ...
    _assert_timeline_and_export_clean(
        ...
        timeline,
        pd_exports_dir,
        min_steps=POST_TAGMENTATION_MIN_TIMELINE_STEPS + POST_TAG_DECK_PREP_STEPS + 1,
        filename="post_tagmentation_96ch_wash_like_single_transfer.py",
    )
...
@pytest.mark.slow
@pytest.mark.timeout(900)
def test_pd_post_tagmentation_96ch_lp1_single_once(page: Page, pd_exports_dir: Path) -> None:
    ...
    _assert_timeline_and_export_clean(
        ...
        pd_exports_dir,
        min_steps=POST_TAGMENTATION_MIN_TIMELINE_STEPS + POST_TAG_DECK_PREP_STEPS + 1,
        filename="post_tagmentation_96ch_lp1_single_once.py",
    )
...
@pytest.mark.slow
@pytest.mark.timeout(900)
def test_pd_post_tagmentation_96ch_single_always_distribute(page: Page, pd_exports_dir: Path) -> None:
    ...
    _assert_timeline_and_export_clean(
        ...
        pd_exports_dir,
        min_steps=POST_TAGMENTATION_MIN_TIMELINE_STEPS + POST_TAG_DECK_PREP_STEPS + 1,
        filename="post_tagmentation_96ch_single_always_distribute.py",
    )
