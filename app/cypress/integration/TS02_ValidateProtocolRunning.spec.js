...
describe('Test Suite 02 - Validate Protocol Running', () => {
    ...
    it('Run JSON protocol using magnetic and temperature modules', () => {
        ...
        cy.get('.upload-panel__file_input__1Mo7Z').attachFile(filePath).then(() => {
                ...
                cy.waitUntil(() => cy.get('.kIChyh').should('be.visible')).then(() => {
                    cy.get('[aria-describedby="Tooltip__4"] > .Svg-sc-1lpozsw-0 > path').click().then(() => {
                        ...
                        cy.waitUntil(() => cy.get(':nth-child(2) > .lists__title_bar__3n-Et').should('be.visible')).then(() => {
                            cy.get('.buttons__button_outline__3z7qv').click().then(() => {
                                
                                // Confirm the protocol run finished successfully
                                cy.get('.alerts__title_bar__Pad5-', { timeout: 45000 }).should('have.text', 'Run complete! Reset run to run protocol again.').then(() => {
                ...
                cy.waitUntil(() => cy.get('.kIChyh').should('be.visible')).then(() => {
                    cy.get('[aria-describedby="Tooltip__4"] > .Svg-sc-1lpozsw-0 > path').click().then(() => {
                        cy.get('.buttons__button_outline__3z7qv').click().then(() => {
                            ...
                            // Confirm the protocol run finished successfully
                            cy.get('.alerts__title_bar__Pad5-', { timeout: 140000 }).should('have.text', 'Run complete! Reset run to run protocol again.').then(() => {
