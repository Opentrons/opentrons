...
describe('PickUpTip', () => {
  ...
  it('renders correct copy when preparing space', () => {
    ...
    getByText(matchTextWithSpans('Place a full Mock TipRack Definition into slot 1'))
    getByRole('link', { name: 'Need help?' })
    getByRole('button', { name: 'Confirm placement' })
  })
  it('renders correct copy when confirming position', () => {
    when(mockCreateRunCommand).calledWith(
      {
        command: {
    ...
    getByText("Ensure that the pipette nozzle closest to you is centered above and level with the top of the tip in the A1 position. If it isn't, use the controls below or your keyboard to jog the pipette until it is properly aligned")
    getByRole('link', { name: 'Need help?' })
  })
  it('executes correct chained commands when confirm placement CTA is clicked', () => {
    ...
    expect(props.createRunCommand).toHaveBeenNthCalledWith(3, {
      command: { commandType: 'savePosition', params: { pipetteId: 'pipetteId1' } },
      waitUntilComplete: true,
    }, { onSuccess: expect.any(Function) })
  })
  ...
  it('executes correct chained commands when confirm position CTA is clicked and user tries again', () => {
    ...
    expect(props.registerPosition).toHaveBeenNthCalledWith(1, { type: 'tipPickUpOffset', offset: { x: -1, y: -1, z: -1 } })
    ...
    expect(props.registerPosition).toHaveBeenNthCalledWith(2, { type: 'tipPickUpOffset', offset: null })
    ...
    expect(props.createRunCommand).toHaveBeenNthCalledWith(10, {
      command: { commandType: 'savePosition', params: { pipetteId: 'pipetteId1' } },
      waitUntilComplete: true,
    }, { onSuccess: expect.any(Function) })
    expect(props.registerPosition).toHaveBeenNthCalledWith(3, { type: 'tipPickUpOffset', offset: { x: -1, y: -1, z: -1 } })
    expect(props.createRunCommand).toHaveBeenNthCalledWith(11, {
      command: {
        commandType: 'pickUpTip',
        params: {
          pipetteId: 'pipetteId1',
