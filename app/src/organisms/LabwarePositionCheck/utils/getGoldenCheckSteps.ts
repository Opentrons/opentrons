...
function getPrimaryPipetteId(pipettes: LoadedPipette[]): string{
  if (pipettes.length < 1) {
    throw new Error('no pipettes in protocol, cannot determine primary pipette for LPC')
  }
  return pipettes.reduce((acc, pip) => {
    return (getPipetteNameSpecs(acc.pipetteName)?.channels ?? 0) > (getPipetteNameSpecs(pip.pipetteName)?.channels ?? 0)
      ? pip
      : acc
  }, pipettes[0]).id
}

export const getGoldenCheckSteps = (protocolData: CompletedProtocolAnalysis): LabwarePositionCheckStep[] => {
  return [
    { section: SECTIONS.BEFORE_BEGINNING },
    { section: SECTIONS.ATTACH_PROBE, pipetteId: getPrimaryPipetteId(protocolData.pipettes) },
    ...getAllCheckSectionSteps(protocolData),
    { section: SECTIONS.DETACH_PROBE, pipetteId: getPrimaryPipetteId(protocolData.pipettes)},
    { section: SECTIONS.RESULTS_SUMMARY },
  ]
}

function getAllCheckSectionSteps(protocolData: CompletedProtocolAnalysis): CheckPositionsStep[] {
