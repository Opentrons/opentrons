...
import { AllPipetteOffsetCalibrations, GripperData, Instruments, PipetteData, PipetteOffsetCalibration } from '@opentrons/api-client'

export function getProtocolUsesGripper(analysis: CompletedProtocolAnalysis): boolean {
  return analysis?.commands.some(c => c.commandType === 'moveLabware' && c.params.strategy === 'usingGripper') ?? false
}
export function getAttachedGripper(attachedInstruments: Instruments): GripperData | null {
  return (attachedInstruments?.data ?? []).find((i): i is GripperData => (
    i.instrumentType === 'gripper'
    && i.data.calibratedOffset != null
  )) ?? null
}

export function getPipetteMatch(loadedPipette: LoadedPipette, attachedInstruments: Instruments): PipetteData | null {
  return (attachedInstruments?.data ?? []).find((i): i is PipetteData => (
...
export function getCalibrationDataForPipetteMatch(
  ...
  return allPipettesCalibrationData?.data.find(cal => (
...
export function getAreInstrumentsReady(
  ...
  const allSpeccedPipettesReady = speccedPipettes.every(loadedPipette => {
    const attachedPipetteMatch = getPipetteMatch(loadedPipette, attachedInstruments)
    const calibrationData = attachedPipetteMatch != null
      ? getCalibrationDataForPipetteMatch(attachedPipetteMatch, allPipettesCalibrationData) 
      : null
