...
import { closeProtocol, loadProtocol } from '../../../redux/protocol/actions'
...
describe('ProtocolUpload', () => {
  ...
  it('calls ingest protocol if handleUpload', () => {
    ...
    const protocolFile = new File([JSON.stringify(withModulesProtocol)], 'fixture_protocol.json')
    const input = getByTestId('file_input')
    fireEvent.change(input, {target: {files: [protocolFile]}})
    expect(ingestProtocolFile).toHaveBeenCalledWith(protocolFile, expect.any(Function), expect.any(Function))
  })
