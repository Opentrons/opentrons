...
const StepComponentMap: Record<StepKey, JSX.Element> = {
 [ROBOT_CALIBRATION_STEP_KEY]: <Text marginTop={SPACING_3}>TODO: robot calibration step contents</Text>,
 [MODULE_SETUP_KEY]: <Text marginTop={SPACING_3}>TODO: module setup step contents</Text>,
...
export function RunSetupCard(): JSX.Element | null {
  ...
  if (protocolData === null ||
    ('metadata' in protocolData && Object.keys(protocolData).length === 1)) return null

  let stepsKeysInOrder: StepKey[] = [ROBOT_CALIBRATION_STEP_KEY]
  if (protocolHasModules(protocolData as JsonProtocolFile)){
    stepsKeysInOrder = [...stepsKeysInOrder, MODULE_SETUP_KEY, LABWARE_SETUP_KEY]
  } else {
      ...
      {stepsKeysInOrder.map((stepKey, index) =>
        <React.Fragment key={stepKey}>
          ...
          <CollapsibleStep
            ...
            toggleExpanded={() => stepKey === expandedStepKey ? setExpandedStepKey(null) : setExpandedStepKey(stepKey)}
            >
