...
export function MetadataCard(): JSX.Element {
  ...
  return (
    <Card width="100%" padding={SPACING_3}>
      <Flex>
        <LabeledValue flex={3} label={t('organization_and_author')} value={author || '-'} />
        <LabeledValue flex={3} label={t('last_updated')} value={lastUpdated ? format(lastUpdated, DATE_FORMAT) : '-'} />
        <LabeledValue flex={2} label={t('creation_method')} value={method || '-'} />
      </Flex>
      <Flex marginTop={SPACING_2}>
        <LabeledValue flex={6} label={t('description')} value={description || '-'} />
