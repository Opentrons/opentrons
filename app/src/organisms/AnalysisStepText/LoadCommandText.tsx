...
export const LoadCommandText = ({
  ...
  switch (command.commandType) {
    case 'loadPipette': {
      const pipetteModel = getPipetteNameOnMount(robotSideAnalysis, command.params.mount)
      commandText = t('load_pipette_protocol_setup', {
        pipette_name: pipetteModel != null ? getPipetteNameSpecs(pipetteModel)?.displayName ?? '' : '',
        mount_name: command.params.mount === 'left' ? t('left') : t('right'),
      })
      break
    }
    case 'loadModule': {
      const occludedSlotCount = getOccludedSlotCountForModule(getModuleType(command.params.model, robotSideAnalysis.robotType ?? OT2_STANDARD_MODEL))
      commandText = (
        t("load_module_protocol_setup", {
    ...
    case 'loadLabware': {
      if (command.params.location !== 'offDeck' && 'moduleId' in command.params.location) {
        ...
        commandText = t("load_labware_info_protocol_setup", {
          ...
          slot_name: getModuleDisplayLocation(robotSideAnalysis, command.params.location.moduleId),
