...
export function SelectAuthenticationType({
  ...
  const handleClickBack = (): void => {
    if (fromWifiList != null) {
      history.push('/network-setup/wifi')
    } else {
  ...
  return (
    <>
      {isShowSetWifiCred ? (
        <SetWifiCred ssid={ssid} authType={selectedAuthType} />
      ) : (
        <Flex
        ...
        >
          ...
          >
            ...
            <TertiaryButton
              ...
              borderRadius="42px"
              onClick={() => setIsShowSetWifiCred(true)}
            >
              {t('shared:next')}
            </TertiaryButton>
          </Flex>
          <Flex alignItems={ALIGN_CENTER} flexDirection={DIRECTION_COLUMN}>
            ...
            >
              <Btn
                backgroundColor={
                  selectedAuthType === 'wpa' ? COLORS.medBlue : ''
                }
              ...
              <Btn
                backgroundColor={
                  selectedAuthType === 'none' ? COLORS.medBlue : ''
                }
                borderRadius="3.5625rem"
                padding={`${String(SPACING.spacing4)} ${String(
                  SPACING.spacing6
            ...
            {/* Note kj 12/27/2022 I hard-coded MAC Address because we have text item that is with bold style, if that is reuseable for the hi-fi, I will switch.
