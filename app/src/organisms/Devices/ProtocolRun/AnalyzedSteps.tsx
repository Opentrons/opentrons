...
export const AnalyzedSteps = React.forwardRef(({ runId, jumpedIndex, makeHandleJumpToStep }: AnalyzedStepsProps, ref: React.ForwardedRef<ViewportListRef>): JSX.Element | null => {
  ...
  const [currentCommandDirection, setCurrentCommandDirection] = React.useState<-1 | 0 | 1>(0)

  if (robotSideAnalysis == null) return null
  const currentRunCommandIndex = robotSideAnalysis.commands.findIndex(c => c.key === currentRunCommandKey) ?? 0
