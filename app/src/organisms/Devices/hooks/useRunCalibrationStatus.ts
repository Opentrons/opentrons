...
import { useDeckCalibrationStatus, useIsOT3, useRunPipetteInfoByMount, useStoredProtocolAnalysis } from '.'
import { DeckCalibrationStatus } from '../../../redux/calibration/api-types'
import { useMostRecentCompletedAnalysis } from '../../LabwarePositionCheck/useMostRecentCompletedAnalysis'
import { CompletedProtocolAnalysis, LoadedPipette, ProtocolAnalysisOutput } from '@opentrons/shared-data'
...
export function useRunCalibrationStatus(
  ...
  return isOT3
    ? getFlexRunCalibrationStatus(mostRecentAnalysis, storedProtocolAnalysis, instrumentsQueryData)
...
function getFlexRunCalibrationStatus(
  ...
  const requiredPipettes: LoadedPipette[] = robotAnalysis?.pipettes ?? storedAnalysis?.pipettes ?? []
  const wrongPipettesAttached = requiredPipettes.some((speccedPipette) => {
    const pipetteOnThisMount = instrumentsQueryData?.data.find((i): i is PipetteData => i.instrumentType === 'pipette' && i.mount === speccedPipette.mount)
    return pipetteOnThisMount?.instrumentName !== speccedPipette.pipetteName
  })
  if (wrongPipettesAttached) return { complete: false, reason: 'attach_pipette_failure_reason' }

  const pipettesNotCalibrated = requiredPipettes.some((speccedPipette) => {
    const pipetteMatch = instrumentsQueryData?.data.find((i): i is PipetteData => (
      i.instrumentType === 'pipette' && i.mount === speccedPipette.mount && i?.instrumentName === speccedPipette.pipetteName
    ))
    return pipetteMatch?.data.calibratedOffset?.last_modified == null
  })
  if (pipettesNotCalibrated) return { complete: false, reason: 'calibrate_pipette_failure_reason' }

  const protocolRequiresGripper = isGripperInCommands(robotAnalysis?.commands ?? storedAnalysis?.commands ?? [])
  if (protocolRequiresGripper) {
    const attachedGripper = attachedInstruments.find(i => i.instrumentType === 'gripper')
...
function getOT2RunCalibrationStatus(deckCalStatus: DeckCalibrationStatus | null, runPipetteInfoByMount: ReturnType<typeof useRunPipetteInfoByMount>): ProtocolCalibrationStatus {
