  ...
  describe('action creators', () => {
    test('fetchPipettes calls GET /pipettes', () => {
      ...
      return store
        ...
        .then(() =>
          expect(client).toHaveBeenCalledWith(robot, 'GET', 'pipettes')
        )
    })
    ...
    test('fetchPipettes with refresh calls GET /pipettes?refresh=true', () => {
      ...
      return store
        .dispatch(fetchPipettes(robot, true))
        .then(() =>
          expect(client).toHaveBeenCalledWith(
            robot,
            'GET',
            'pipettes?refresh=true'
          )
        )
    })
  ...
  describe('selectors', () => {
    ...
    test('makeGetRobotPipettes with existing robot', () => {
      ...
      expect(getRobotPipettes(state, robot)).toEqual(
        state.api.api[NAME].pipettes
      )
      expect(getRobotPipettes(state, { name: 'foo' })).toEqual({
        inProgress: false,
      })
    })
