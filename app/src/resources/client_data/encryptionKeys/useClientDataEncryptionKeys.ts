...
export function useClientDataEncryptionKeys(
  ...
  const { keyDisplayRequestedNonce: keyDisplayRequestedData } = data?.data ?? {}
  const keyDisplayRequestedNonce = keyDisplayRequestedData ?? null

  return { keyDisplayRequestedNonce }
}
