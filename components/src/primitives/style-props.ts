...
const layoutStyles = (props: Types.StyleProps): CSSProperties => {
  const pickedProps = pick(props, LAYOUT_PROPS)
  const { size, ...styles } = pickedProps as CSSProperties & {
    size?: string | number
  }

  if (size != null) {
    styles.width = styles.width ?? (size as string | number)
    styles.height = styles.height ?? (size as string | number)
  }
