...
import { DeckSlot, RobotType, getDeckDefFromRobotType, ModuleModel, ModuleLocation, getModuleDef2, LabwareDefinition2, inferModuleOrientationFromXCoordinate, LabwareLocation, OT2_ROBOT_TYPE } from '@opentrons/shared-data'
...
export function BaseDeck(props: BaseDeckProps): JSX.Element {
  const { robotType, trashSlotName, moduleLocations, labwareLocations, children } = props
  ...
  return (
    <RobotCoordinateSpace height="400px" viewBox={`${deckDef.cornerOffsetFromOrigin[0]} ${deckDef.cornerOffsetFromOrigin[1]} ${deckDef.dimensions[0]} ${deckDef.dimensions[1]}`}>
      ...
      {moduleLocations.map(({ moduleModel, moduleLocation, nestedLabwareDef, innerProps }) => {
        const slotDef = deckDef.locations.orderedSlots.find(s => s.id === moduleLocation.slotName)
        return slotDef != null ? (
          <Module
            def={getModuleDef2(moduleModel)}
            x={slotDef.position[0]}
            y={slotDef.position[1]}
            orientation={inferModuleOrientationFromXCoordinate(slotDef.position[0])}
      ...
      {labwareLocations.map(({ labwareLocation, definition }) => {
        const slotDef = deckDef.locations.orderedSlots.find(s => labwareLocation !== 'offDeck' && 'slotName' in labwareLocation && s.id === labwareLocation.slotName)
        return slotDef != null ? (
