...
export function ProtocolTimelineScrubber(
  ...
  const fileMetadata = {
    protocolName: `Protocol snippet`,
    source: 'Protocol timeline scrubber',
  }

  const onDownloadClick = (): void => {
    const filteredCommands = commands.filter(command =>
      selected.includes(command.id)
    )
    const python = filteredCommands.map((command, index) =>
      getPythonFromSelectedCommands(
        filteredCommands.slice(0, index + 1),
        invariantContextFromRunCommands,
        initialRobotState,
        command.id,
        filteredCommands
    ...
    const invariantContextFromSequence = constructInvariantContextFromRunCommands(
      filteredCommands
    )

    const error = python.find(py => py.includes('Error'))
    if (error == null) {
      const pythonContents =
        [
          ...
          getLoadAdapters(
            invariantContextFromSequence.moduleEntities,
            invariantContextFromSequence.labwareEntities,
          ...
          getLoadPipettes(
            invariantContextFromSequence.pipetteEntities,
            invariantContextFromSequence.labwareEntities,
            initialRobotState.pipettes
          ),
          ...[
            getLoadTrashBins(invariantContextFromSequence.trashBinEntities),
            getLoadWasteChute(
              invariantContextFromSequence.wasteChuteEntities
            ),
          ],
          ...python,
        ]
          ...
          .join('\n\n') + '\n'
      const blob = new Blob([pythonContents], { type: 'text/x-python' }) // Create a blob
      const url = URL.createObjectURL(blob) // Generate a URL for the blob

      const a = document.createElement('a')
      a.href = url
      a.download = 'script.py' // Set file name
