...
def sidebar_nav_link(page: Page, *, href_fragment: str, label: str) -> Locator:
    ...
    def _sidebar_by_container() -> Locator:
        return page.locator('[class*="nav_container"]').locator(
            f'a[href="{hash_href}"], a[href="{slash_href}"]'
        )

    def _sidebar_by_navbar_class() -> Locator:
        return page.locator(
            f'[class*="navbar_link"][href="{hash_href}"], '
            f'[class*="navbar_link"][href="{slash_href}"]'
        )
    ...
    def _any_matching_link() -> Locator:
        return page.locator(
            f'a[href="{hash_href}"], a[href="{slash_href}"]'
        ).filter(has_not=page.locator('[class*="crumb_link"]'))
