...
from pages.device_cards_page import THERMOCYCLER, HEATER_SHAKER, TEMPERATURE, DeviceCardHelper
...
def test_thermocycler_module_card(device_cards: DeviceCardHelper) -> None:
    ...
    if not device_cards.has_module_card(prefix):
        log_step(
            f"Thermocycler card with prefix '{prefix}' "
            "not found or disabled — skipping"
        )
        pytest.skip(
            f"Thermocycler card with prefix '{prefix}' not found or disabled."
        )
...
def test_heater_shaker_module_card(device_cards: DeviceCardHelper) -> None:
    ...
    if not device_cards.has_module_card(prefix):
        log_step(
            f"Heater-Shaker card with prefix '{prefix}' "
            "not found or disabled — skipping"
        )
        pytest.skip(
            f"Heater-Shaker card with prefix '{prefix}' not found or disabled."
        )
...
def test_temperature_module_card(device_cards: DeviceCardHelper) -> None:
    ...
    if not device_cards.has_module_card(prefix):
        log_step(
            f"Temperature module card with prefix '{prefix}' "
            "not found or disabled — skipping"
        )
        pytest.skip(
            f"Temperature module card with prefix '{prefix}' "
