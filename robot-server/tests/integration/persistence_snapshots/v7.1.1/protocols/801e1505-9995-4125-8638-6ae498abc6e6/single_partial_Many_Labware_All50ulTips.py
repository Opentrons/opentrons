...
metadata = {"protocolName": "single_partial_many_labware_All50ulTips"}
requirements = {
    "robotType": "Flex",
    "apiLevel": "2.16",
}
...
def run(protocol_context: protocol_api.ProtocolContext):
    ...
    # load the labware
    armadillo_96 = protocol_context.load_labware(
        "armadillo_96_wellplate_200ul_pcr_full_skirt", "B1"
    )
    ...
    bio_384 = protocol_context.load_labware(
        "appliedbiosystemsmicroamp_384_wellplate_40ul", "D1"
    )
    ...
    # load the trashes
    trashA1 = protocol_context.load_trash_bin(
        "A1"
    )  # since this is the first trash loaded, it is treated as the default trash
