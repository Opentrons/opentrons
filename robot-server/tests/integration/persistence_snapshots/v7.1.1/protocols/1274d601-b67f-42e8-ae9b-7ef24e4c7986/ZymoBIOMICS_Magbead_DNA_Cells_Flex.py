...
def run(ctx):
    ...
    def lysis(vol, source):
        ...
        ctx.delay(
            ...
            msg="Shake at 1800 rpm for 30 minutes.",
        )
    ...
    def bind(vol1, vol2):
        ...
        ctx.delay(
            minutes=10 if not dry_run else 0.25, msg="Shake at 1800 rpm for 10 minutes."
        )
        h_s.deactivate_shaker()
        ...
        h_s.set_and_wait_for_shake_speed(2000)
        ctx.delay(
            minutes=1 if not dry_run else 0.25, msg="Shake at 2000 rpm for 1 minutes."
        )
        h_s.deactivate_shaker()
    ...
    def elute(vol):
        ...
        ctx.delay(
            ...
            msg="Shake on H-S for 5 minutes at 2000 rpm.",
        )
        h_s.deactivate_shaker()
        ...
        for i, (m, e) in enumerate(zip(samples_m, elution_samples_m)):
            ...
            m1000.transfer(
                vol, m.bottom(0.3), e.bottom(5), air_gap=20, new_tip="never"
            )  # original = 0.15
