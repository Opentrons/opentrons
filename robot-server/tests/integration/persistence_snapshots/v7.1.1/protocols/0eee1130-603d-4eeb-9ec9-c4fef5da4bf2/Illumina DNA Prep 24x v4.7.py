...
def run(protocol: protocol_api.ProtocolContext):
    ...
    protocol.comment("THIS IS A DRY RUN") if DRYRUN == True else protocol.comment(
        "THIS IS A REACTION RUN"
    )
    protocol.comment(
        "USED TIPS WILL GO IN TRASH"
    ) if TIP_TRASH == True else protocol.comment("USED TIPS WILL BE RE-RACKED")
    ...
    sample_plate_1 = hs_adapter.load_labware(
    ...
    for loop in range(RUN):
        ...
        if STEP_TAG == 1:
            ...
            for loop, X in enumerate(column_1_list):
                ...
                p50.dispense(
                    TAGSTOPVol, sample_plate_1[X].bottom(z=0.5)
                )  # original = ()
            ...
            if DRYRUN == False:
                ...
                thermocycler.execute_profile(
                    steps=profile_TAGSTOP, repetitions=1, block_max_volume=50
                )
                thermocycler.set_block_temperature(10)
            thermocycler.open_lid()
            ############################################################################################################################################
        ...
        if STEP_WASH == 1:
            ...
            for loop, X in enumerate(column_1_list):
                ...
                p50.move_to(
                    (
                        sample_plate_1.wells_by_name()[X]
                        .center()
            ...
            for loop, X in enumerate(column_1_list):
                ...
                p50.aspirate(
                    BarcodeVol + 1,
                    reagent_plate.wells_by_name()[barcodes[loop]].bottom(z=0.5),
                    rate=0.25,
                )
                p50.dispense(
                    1,
        ...
        if STEP_CLEANUP == 1:
            ...
            for loop, X in enumerate(column_1_list):
                ...
                p50.dispense(
                    AMPureVol,
                    sample_plate_1[column_2_list[loop]].bottom(z=0.75),
                    rate=1,
                )
                ...
                p50.aspirate(
                    SampleVol + 3,
                    sample_plate_1[column_1_list[loop]].bottom(z=0.75),
                    rate=0.5,
                )
                p50.dispense(
                    SampleVol + 3,
            ...
            for loop, X in enumerate(column_2_list):
                ...
                p50.dispense(
                    TransferSup + 1, sample_plate_1[column_3_list[loop]].bottom(z=1)
                )
                p50.return_tip() if TIP_TRASH == False else p50.drop_tip()
                p50_tips += 1
            # ===============================================
