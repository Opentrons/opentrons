...
def run(ctx):
    ...
    def tiptrack(pip, tipbox):
        ...
        if drop_count >= 250:
            ...
            if ABR_TEST == False:
                ctx.pause(
                    "Please empty the waste bin of all the tips before continuing."
                )
    ...
    def TL_lysis(vol, source):
        ...
        ctx.delay(
            minutes=40 if not dry_run else 0.25, msg="Shake at 1800 rpm for 30 minutes."
        )
    ...
    def AL_lysis(vol, source):
        ...
        ctx.delay(
            minutes=15 if not dry_run else 0.25, msg="Shake at 2000 rpm for 4 minutes."
        )
        h_s.deactivate_shaker()
    ...
    def wash(vol, source):
        ...
        ctx.delay(
            ...
            msg="Please wait 5 minutes while wash buffer shakes on H-S",
        )
        h_s.deactivate_shaker()
        ...
        if dry_run:
            h_s.open_labware_latch()
            ctx.move_labware(sample_plate, 4, use_gripper=USE_GRIPPER)
            h_s.close_labware_latch()
