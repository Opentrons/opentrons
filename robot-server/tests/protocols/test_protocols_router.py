...
def enable_protocol_subprocess(monkeypatch: pytest.MonkeyPatch) -> None:
    """Existing router tests cover the protocol-subprocess queue path."""
    monkeypatch.setattr(
        "robot_server.protocols.router.feature_flags.protocol_subprocess_enabled",
        lambda: True,
    )
...
async def test_create_existing_protocol(
    ...
    decoy.when(
        await analyses_manager.start_analysis_if_rtps_differ(
            analysis_id="analysis-id",
            protocol_resource=stored_protocol_resource,
            run_time_param_values={},
            run_time_param_paths={},
            last_analysis_summary=completed_analysis,
        )
    ).then_return(None)
...
async def test_create_existing_protocol_with_different_run_time_params(
    ...
    decoy.when(
        await analyses_manager.start_analysis_if_rtps_differ(
            ...
            last_analysis_summary=completed_summary,
        )
...
async def test_create_existing_protocol_with_same_run_time_params(
    ...
    decoy.when(
        await analyses_manager.start_analysis_if_rtps_differ(
            ...
            run_time_param_values={"vol": 123, "dry_run": True, "mount": "left"},
            run_time_param_paths={},
            last_analysis_summary=analysis_summaries[-1],
        )
    ).then_return(None)
...
async def test_create_protocol_without_protocol_subprocess(
    decoy: Decoy,
    protocol_store: ProtocolStore,
    analysis_store: AnalysisStore,
    data_files_store: DataFilesStore,
    ...
    """Without protocol subprocess, initialize on the request then start analysis."""
    monkeypatch.setattr(
        "robot_server.protocols.router.feature_flags.protocol_subprocess_enabled",
        lambda: False,
    )
    ...
    decoy.when(
        await file_reader_writer.read(files=[protocol_file])  # type: ignore[list-item]
    ).then_return([buffered_file])
    decoy.when(await file_hasher.hash(files=[buffered_file])).then_return("abc123")
    decoy.when(
        await protocol_reader.save(
    ...
    assert result.status_code == 201
    assert result.content.data.analysisSummaries == [pending_analysis]
    decoy.verify(
        await analyses_manager.enqueue_analysis(
            analysis_id="analysis-id",
            protocol_resource=protocol_resource,
...
async def test_create_existing_protocol_pending_without_protocol_subprocess(
    decoy: Decoy,
    protocol_store: ProtocolStore,
    analysis_store: AnalysisStore,
    data_files_store: DataFilesStore,
    ...
    """Without protocol subprocess, a pending last analysis still 503s after initialize."""
    monkeypatch.setattr(
        "robot_server.protocols.router.feature_flags.protocol_subprocess_enabled",
        lambda: False,
    )
    ...
    decoy.when(
        await analysis_store.matching_rtp_values_in_analysis(
            last_analysis_summary=pending_summary,
            new_parameters=[],
        )
    ).then_raise(AnalysisIsPendingError("analysis-id"))
