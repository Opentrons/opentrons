...
def test_command_type_validation_jog():
    c = session.SessionCommand(**{'command': session.CommandName.jog,
                                  'data': {'vector': [1, 2, 3]}})
...
def test_command_type_validation_jog_fail():
    with pytest.raises(ValueError):
        session.SessionCommand(**{'command': session.CommandName.jog,
                                  'data': {}})
...
def test_command_type_empty():
    ...
    c = session.SessionCommand(
        **{'command': session.CommandName.prepare_pipette,
           'data': {}})
