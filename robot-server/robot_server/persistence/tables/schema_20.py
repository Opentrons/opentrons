...
class AnalysisResultSQLEnum(enum.Enum):
    """Analysis result sql enum."""
...
analysis_table = sqlalchemy.Table(
    ...
    # Added via ADD COLUMN migration, so this must stay last to match sqlite_schema.
    sqlalchemy.Column(
        "result",
        sqlalchemy.Enum(
            AnalysisResultSQLEnum,
            values_callable=lambda obj: [e.value for e in obj],
            validate_strings=True,
            nullable=True,
        ),
