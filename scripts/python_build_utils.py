...
def _ref_from_sha(sha):
    ...
    allrefs = (
        subprocess.check_output(["git", "show-ref", "--tags", "--heads"], cwd=CWD)
        .strip()
        .decode("utf-8")
        .split("\n")
    )
    ...
    matching = [
        this_ref
        for this_sha, this_ref in [  # the refs
            ref_plus_sha.split(" ") for ref_plus_sha in allrefs if ref_plus_sha
        ]
...
def dump_br_version(package, project, extra_tag="", git_dir=None):
    ...
    return json.dumps(
        {pref + "_version": normalized, pref + "_sha": sha, pref + "_branch": branch}
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        dest="package", metavar="PACKAGE", type=str, choices=package_entries.keys()
    )
    parser.add_argument(
        dest="project", metavar="PROJECT", type=str, choices=project_entries.keys()
    )
    parser.add_argument(
        dest="task",
        metavar="TASK",
