#!/usr/bin/env python3
"""
Deploy Labware Library to S3
Deploys the labware library build artifacts to staging/production/sandbox S3 buckets
"""
...
def deploy_labware(environment, branch=None, aws_profile=None, source_dir="dist"):
    """Deploy labware library to S3"""
    # Environment-specific bucket configuration
    buckets = {
        "sandbox": "opentrons.sandbox.labware",
    ...
    if not source_path.exists():
        ...
        print("Make sure you've run 'make dist' first.")
        sys.exit(1)
    ...
    if environment == "sandbox":
        ...
        url = f"http://{bucket}.s3-website.us-east-2.amazonaws.com/{branch}/"
    else:
        s3_path = f"s3://{bucket}/"
        if environment == "staging":
            url = f"https://staging.labware.opentrons.com/"
        else:  # production
            url = f"https://labware.opentrons.com/"

    # Use --delete flag for labware to ensure clean deployments
    cmd = [
        "aws", "s3", "sync",
        str(source_path) + "/",
        s3_path,
    ...
    print(f"  Mode: Clean deployment (--delete flag enabled)")
...
def main():
    parser = argparse.ArgumentParser(description="Deploy Labware Library to S3")
    parser.add_argument(
        "environment",
    ...
    parser.add_argument(
        "--branch",
        help="Branch name (defaults to 'edge' for sandbox, 'main' for staging/production)"
    )
    parser.add_argument(
        "--aws-profile",
    ...
    parser.add_argument(
        "--source-dir",
        default="dist",
        help="Source directory to deploy (default: dist)"
    )
