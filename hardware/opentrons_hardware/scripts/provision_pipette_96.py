...
async def run(
    ...
    try:
        ...
        async with build.driver(build_settings(args)) as driver, CanMessenger(
            ...
            await flash_serials(messenger, which_pipette, base_log, trace_log,ppNo)
        print("FlashNum=Pass")
    except:
        print("FlashNum=Fail")
...
async def flash_serials(
    ...
    should_quit = await get_and_update_serial_once(
        messenger, which_pipette, base_log, trace_log,pipetteNo
    )
...
async def get_and_update_serial_once(
    ...
    name, model, data = await get_serial(pipetteNo
        , base_log
    )
    try:
...
def main() -> None:
    ...
    parser.add_argument(
        "--pipetteNo", type=str, help="P1KHV34", default="P1KHV34202430908A01"
    )
    add_can_args(parser)
