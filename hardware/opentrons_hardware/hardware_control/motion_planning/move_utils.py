...
def targets_to_moves(initial: Coordinates, targets: Iterator[MoveTarget]) -> Iterator[Move]:
    """Transform a list of MoveTargets into a list of Moves"""
    for target in targets:
        displacement = np.array(target.position) - np.array(initial)
        distance = np.linalg.norm(displacement)
        m = Move(
            unit_vector=tuple(displacement / distance),
            distance=distance,
...
def find_initial_speed(
    ...
    for axis in Axis.get_all_axes():
        ...
        if not axis_component:
        ...
        elif (not prev_move.unit_vector[axis.value]) or prev_move.final_speed == 0:
            ...
            previous_axis_final_speed = prev_move.final_speed * prev_move.unit_vector[axis.value]
            axis_initial_limit = max(previous_axis_final_speed, axis_constraints.max_speed_discont)
            axis_speed_limit = axis_initial_limit / axis_component
            log.debug(
            ...
            axis_speed_limit = axis_constraints.max_direction_change_speed_discont / axis_component
...
def find_final_speed(
    ...
    for axis in Axis.get_all_axes():
        ...
        if not axis_component:
        ...
        elif (not next_move or not next_move.unit_vector[axis.value] or next_move.initial_speed == 0) and axis_component:
            ...
            next_initial_speed = next_move.initial_speed * next_move.unit_vector[axis.value]
            axis_initial_limit = max(axis_component.max_speed_discont, next_initial_speed)
            axis_speed_limit = axis_initial_limit / axis_component
    
            log.debug(f'same dir, using axis speed {axis_speed_limit} from next move {next_initial_speed}')
...
def achievable_final(constraints: SystemConstraints, move: Move, initial_speed: float, final_speed: float) -> float:
    ...
    ## TODO: element wise comparison for both initial_speed & final_Speed
    #     if initial_speed <= constraints.max_speed_discont and final_speed <= constraints.max_speed_discont:
    #         # this will work fine - the entire thing is underneath the discontinuity speed and we can jump
    #         # around however
    #         return final_speed
    ...
    max_acceleration_vector = [constraints[axis].max_acceleration for axis in Axis.get_all_axes()]
    
    acceleration = np.dot(max_acceleration_vector, move.unit_vector)
    acceleration = math.copysign(acceleration, final_speed - initial_speed)
    
    ## TODO: doublecheck the acceleration elementwise is  == max_acceleration_vector
    
    max_achievable_final_speed_sq = initial_speed **2 + 2 * acceleration * move.distance
...
def build_blocks(
    ...
    distance: float,
    max_speed: float,
    constraints: SystemConstraints) -> Tuple[Block, Block, Block]:
...
#         "check preconstraints for delta-v"
    ...
    max_acceleration_vector = [constraints[axis].max_acceleration for axis in Axis.get_all_axes()]
    max_acceleration_scale = np.dot(max_acceleration_vector, unit_vector)

    initial_speed_sq = initial_speed ** 2
    final_speed_sq = final_speed ** 2
    
    max_achievable_speed = math.sqrt(
        0.5 * (2*max_acceleration_scale * distance + initial_speed_sq + final_speed_sq))
    max_speed = min(max_achievable_speed, max_speed)
    max_speed_sq = max_speed ** 2

    log.debug(f"build blocks: {initial_speed}mm/s to {final_speed}mm/s in {distance}mm "
    ...
    if max_achievable_speed > constraint_max_speed:
        ...
        coast = Block(
            initial_speed=final.initial_speed, acceleration=0, distance=distance-first.distance-final.distance)
...
def build_move(
    ...
    next_move: Optional[Move],
    constraints: SystemConstraints) -> Move:
    ...
    m = Move(
        ...
        move.distance,
        move.max_speed,
        build_blocks(move.unit_vector, initial_speed, final_speed, move.distance, move.max_speed, constraints))
    log.debug(f'applied constraints to {move} generating {m}')
    return m


def blended(constraints: SystemConstraints, first: Move, second: Move):
    # have these actually had their blocks built?
    if sum(b.distance for b in first.blocks) != first.distance: return False
    if sum(b.distance for b in second.blocks) != second.distance: return False
    # do their junction velocities match constraints?

    for axis in Axis.get_all_axes():
        final_speed = abs(first.blocks[-1].final_speed * first.unit_vector[axis.value])
        initial_speed = abs(second.blocks[0].initial_speed * second.unit_vector[axis.value])
        if np.sign(first.unit_vector[axis.value]) == np.sign(second.unit_vector[axis.value]):
            ...
            return final_speed <= constraints[axis].max_direction_change_speed_discont \
...
def all_blended(constraints: SystemConstraints, moves: List[Move]) -> bool:
    ...
    while True:
        try:
            ...
            if not blended(constraints, prev, current): return False
            prev = current
