...
def _create_pipetting_heights(start_mm: float, end_mm: float,
                              liquid_class: LiquidClassSettings) -> PipettingHeights:
    # Calculates the:
...
class PipettingLiquidSettings:
    ...
    def __init__(self, ctx: ProtocolContext, cfg: PipettingLiquidSettingsConfig) -> None:
    ...
    def _submerge(self) -> None:
        ...
        self._cfg.pipette.move_to(self._cfg.well.bottom(start_above),
                                  force_direct=False)
        submerged_loc = self._cfg.well.bottom(self._cfg.heights.end.below)
        self._cfg.pipette.move_to(submerged_loc, force_direct=True,
                                  speed=self._cfg.settings.submerge.speed)
    ...
    def _dispense(self) -> None:
        if self._cfg.dispense:
            ...
            if self._cfg.pipette.current_volume > 0:
                ...
                self._cfg.pipette.flow_rate.dispense = self._cfg.settings.wet_air_gap.flow_rate
                self._cfg.pipette.dispense()
                # go back to previous speed
                apply_pipette_speeds(self._cfg.pipette, self._cfg.settings)  # back to defaults
    ...
    def _retract(self) -> None:
        self._cfg.pipette.move_to(self._cfg.well.bottom(self._cfg.heights.end.above),
                                  force_direct=True, speed=self._cfg.settings.retract.speed)
    ...
    def run(self,
            ...
            on_post_emerge: Optional[Callable[[], None]] = None) -> None:
        self._approach()
