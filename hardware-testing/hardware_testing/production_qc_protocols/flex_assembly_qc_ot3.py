...
def _get_user_answer_sync(
    ...
    for _ in range(3):
        ...
        if result:
            ...
            ctx.pause("Scan failed. Try again.")
    raise RuntimeError("Error scanning Yes/No QR code.")


def _confirm_ready(api: SyncHardwareAPI, ctx: ProtocolContext, prompt: str) -> None:
    while not _get_user_answer_sync(api, ctx, prompt):
        ctx.comment("Operator scanned No. Waiting for Yes before continuing.")
...
def _move_along_axis_and_record_test_results(
    axis: Axis, api: SyncHardwareAPI, report: CSVReport, section: str
) -> None:
    ...
    _record_gantry_test_status(f"{ax_str}-min", api, report, section, axis=axis)
    print("quickly move to endstop")
    api.move_rel(mount, _move_rel_point_for_axis(axis, rel_distance))
    _record_gantry_test_status(f"{ax_str}-max", api, report, section, axis=axis)
    print("homing")
    api.home([axis])
...
def _run_gantry(
    api: SyncHardwareAPI, report: CSVReport, section: str, ctx: ProtocolContext, cfg: TestConfig
) -> None:
    ...
    try:
        ...
        ctx.comment("GANTRY: homing")
        api.home(GANTRY_AXES)
        _record_gantry_test_status("home-start", api, report, section)
        for ax in GANTRY_AXES:
            ctx.comment(f"GANTRY: Axis {ax.name} MIN/MAX")
            _move_along_axis_and_record_test_results(ax, api, report, section)
        ctx.comment("GANTRY: homing")
        api.home(GANTRY_AXES)
        _record_gantry_test_status("home-end", api, report, section)
    finally:
...
async def _move_and_interrupt_with_signal(self: OT3API, sig_name: str) -> None:
    ...
    stop = MoveStopCondition.sync_line if sig_name == "nsync" else MoveStopCondition.none
...
def _signal_home(api: SyncHardwareAPI, ctx: ProtocolContext) -> None:
    try:
        ...
        print(e)
        if not api.is_simulator:
            _confirm_ready(api, ctx, "Release the E-STOP.")
...
def _run_signals(
    ...
    for sig_name in SIGNAL_TEST_NAMES:
        ctx.comment(f"SIGNALS: {sig_name.upper()}")
        _signal_home(api, ctx)
        ...
        if not api.is_simulator and "external" in sig_name:
            _confirm_ready(api, ctx, f"Connect {sig_name.upper()}.")
            _confirm_ready(api, ctx, "Prepare to hit the E-STOP.")
...
def _get_plunger_positions(api: SyncHardwareAPI, mount: OT3Mount) -> Tuple[float, float]:
...
def _test_current_position_and_record_result(
    api: SyncHardwareAPI, mount: OT3Mount, report: CSVReport, section: str, tag: str
) -> None:
...
def _probe_mount_and_record_result(
    ...
    if mount == OT3Mount.GRIPPER:
        ...
        if not api.is_simulator:
            _confirm_ready(api, ctx, f"Attach {probe.name} calibration probe.")
    ...
    if not api.is_simulator:
        _confirm_ready(api, ctx, "About to probe down. Touch the sensor to stop probing motion.")
    ...
    if mount == OT3Mount.GRIPPER:
        if not api.is_simulator:
            _confirm_ready(api, ctx, "Remove calibration probe.")
...
def _wait_for_instrument_presence(
    ...
    if not api.is_simulator:
        _confirm_ready(
            api,
            ctx,
            f"{verb.capitalize()} a {instr_str} {direction} the {mount.name} mount.",
        )
    api.reset()
    api.cache_instruments()
    found = api.has_gripper() if is_gripper else api.hardware_pipettes[mount.to_mount()] is not None
    if found == presence:
        print(f"{instr_str} {verb} {direction} {mount.name}")
        return True
    print(f"ERROR: unable to detect {instr_str} was {verb}ed {direction} {mount.name} mount")
    return False
...
def _run_instruments(
    ...
    for mount in [OT3Mount.LEFT, OT3Mount.RIGHT]:
        ctx.comment(f"INSTRUMENTS: PIPETTE {mount.name}")
    ...
    ctx.comment("INSTRUMENTS: GRIPPER")
...
def _test_ethernet(
    ...
    if not api.is_simulator:
        _confirm_ready(api, ctx, "Connect ethernet cable.")
        eth_ip, eth_mac = _run_hw_coroutine(
            api, _get_ethernet_status_async
        )
    else:
        eth_ip = "0.0.0.0"
        eth_mac = "AA:AA:AA:AA:AA:AA"
    report(section, "ethernet", [eth_ip, eth_mac, CSVResult.from_bool(bool(eth_ip))])


async def _test_wifi_async(
    report: CSVReport, section: str, cfg: TestConfig
) -> None:
...
def _test_wifi(api: SyncHardwareAPI, report: CSVReport, section: str, cfg: TestConfig) -> None:
    _run_hw_coroutine(api, _test_wifi_async, report, section, cfg)
...
def _test_usb_a_ports(
    ...
    if not api.is_simulator:
        _confirm_ready(api, ctx, "Insert USB drives into all nine USB-A ports.")
...
def _test_aux(
    ...
    for test_name, test_config in AUX_PORT_TESTS.items():
        if test_name == "CAN":
            if not api.is_simulator:
                _confirm_ready(api, ctx, "Release ESTOP 2.")
                _confirm_ready(api, ctx, "Prepare CAN analyzer and PCAN software.")
            for can_test_name in AUX_CAN_TESTS:
                if api.is_simulator:
                    ...
                    inp = _get_user_answer_sync(
                        api,
                        ctx,
                        f"Does {can_test_name.upper()} count TRANSMIT = RECEIVE?",
                    )
            ...
            if api.is_simulator:
                ...
                continue
            _confirm_ready(api, ctx, test_config[APT_PROMPT])
            ...
            report(section, test_name, [test_result[1], CSVResult.from_bool(test_result[0])])
    if not api.is_simulator:
        _confirm_ready(api, ctx, "Unplug all AUX cables.")


def _run_connectivity(
    api: SyncHardwareAPI, report: CSVReport, section: str, ctx: ProtocolContext, cfg: TestConfig
) -> None:
    ctx.comment("CONNECTIVITY: ETHERNET")
    _test_ethernet(api, report, section, ctx)
    ctx.comment("CONNECTIVITY: WIFI")
    if not api.is_simulator:
        _test_wifi(api, report, section, cfg)
    else:
        report(section, "wifi", ["", "", "0.0.0.0", "AA:AA:AA:AA:AA:AA", CSVResult.PASS])
        assert nmcli.iface_info is not None
        assert nmcli.configure is not None
        assert nmcli.wifi_disconnect is not None
    ctx.comment("CONNECTIVITY: USB-B-REAR")
    if not api.is_simulator:
        inp = _get_user_answer_sync(
            api, ctx, "Connect USB-B to computer, does computer detect device?"
        )
        result = CSVResult.from_bool(inp)
    else:
        result = CSVResult.PASS
    report(section, "usb-b-rear", [result])
    ctx.comment("CONNECTIVITY: USB-A")
    _test_usb_a_ports(api, report, section, ctx)
    ctx.comment("CONNECTIVITY: AUX")
...
def _run_image_check_server(
    ...
    if api.is_simulator:
        _run_hw_coroutine(api, _run_image_check_server_async, async_api, report, section, file_path)
        return
    result: Optional[CSVResult] = CSVResult.FAIL
    server_process: Optional[Popen] = None
    ...
    try:
        ...
        address = f"http://{server_address}/{file_path.name}"
        print(f"\n\nopen your web browser, and go to:\n\n\t{address}\n\n")
        inp = _get_user_answer_sync(api, ctx, "Is image OK?")
        result = CSVResult.from_bool(inp)
    finally:
        if server_process:
            server_process.kill()
...
def _run_peripherals(
    ...
    ctx.comment("PERIPHERALS: DISPLAY")
    result = _get_user_answer_sync(api, ctx, "Is ODD on?")
    report(section, "screen-on", [CSVResult.from_bool(result)])
    result = _get_user_answer_sync(api, ctx, "Is ODD touchscreen working?")
    report(section, "screen-touch", [CSVResult.from_bool(result)])

    ctx.comment("PERIPHERALS: DECK LIGHTS")
    api.set_lights(rails=True)
    result = _get_user_answer_sync(api, ctx, "Are the DECK-LIGHTS on?")
    report(section, "deck-lights-on", [CSVResult.from_bool(result)])
    api.set_lights(rails=False)
    result = _get_user_answer_sync(api, ctx, "Are the DECK-LIGHTS off?")
    report(section, "deck-lights-off", [CSVResult.from_bool(result)])
    api.set_lights(rails=True)

    ctx.comment("PERIPHERALS: STATUS LIGHT")
    try:
        for color, state in COLOR_TO_STATE.items():
            ...
            result = _get_user_answer_sync(api, ctx, f"Is the STATUS-LIGHT {color}?")
            report(section, f"status-light-{color}", [CSVResult.from_bool(result)])
    finally:
        api.set_status_bar_state(StatusBarState.IDLE)

    ctx.comment("PERIPHERALS: DOOR SWITCH")
    door_timeout_seconds = 10
    print("CLOSE the front door")
    if not api.is_simulator:
        _confirm_ready(api, ctx, "Close the front door.")
    ...
    if not api.is_simulator:
        _confirm_ready(api, ctx, "Open the front door.")
    start_time_seconds = monotonic()
    ...
    ctx.comment("PERIPHERALS: CAMERA")
    ...
    if sku and sku in FLEX_SKUS_NO_CAMERA:
        ...
        try:
            cam_pic_path = _run_hw_coroutine(
                api, _take_picture, async_api, report, section
            )
        except Exception as e:
...
def _build_report(test_name: str) -> CSVReport:
    return CSVReport(
        ...
        sections=[
            ...
            CSVSection(title=TestSection.SIGNALS.value, lines=_build_signal_csv_lines()),
            CSVSection(
...
def add_parameters(parameters: ParameterContext) -> None:
    ...
    parameters.add_str(
        ...
        description="Select ALL or one assembly QC section to run.",
    )
    parameters.add_str(
        ...
        description="Optional SKU. Use 999-00279 for no-camera Flex peripherals testing.",
    )
    parameters.add_str(
        ...
        description="Optional SSID used by CONNECTIVITY when WiFi is not already connected.",
    )
    parameters.add_str(
        ...
        default=UNUSED,
        choices=[{"display_name": UNUSED, "value": UNUSED}],
        description="Optional WiFi password used by CONNECTIVITY.",
    )
...
def _build_config(ctx: ProtocolContext) -> TestConfig:
    ...
    return TestConfig(
        ...
        wifi_password="" if args["wifi_password"] == UNUSED else str(args["wifi_password"]),
    )
...
def run(ctx: ProtocolContext) -> None:
    ...
    ctx.comment("starting Flex assembly QC.")
    ...
    for section, test_run in _selected_tests(cfg.only_test).items():
        ctx.comment(f"running {section.value}.")
