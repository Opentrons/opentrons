...
def _includes_result(current: float, speed: float, cfg: TestConfig) -> bool:
    ...
    return current >= pass_currents[cfg.pipette_serial[:4]]


def build_current_speed_csv_sections(cfg: TestConfig) -> List[CSVSection]:
    """Build CSV Lines."""
    sections = [
        CSVSection(
            title=_get_cs_section_tag(current),
            lines=[
                CSVLine(
                    _get_cs_test_tag(current, speed, trial, direction, pos),
                    [float, float, float, float, CSVResult]
                    if _includes_result(current, speed, cfg)
                    else [float, float, float, float],
                )
                for speed in sorted(PLUNGER_CURRENTS_SPEED[current], reverse=False)
                for trial in range(DEFAULT_TRIALS)
...
def _set_default_and_home_plunger(api: SyncHardwareAPI, mount: OT3Mount) -> None:
...
def _record_plunger_alignment(
    ...
    pipette_ax = Axis.of_main_tool_actuator(cfg.mount)
    _current_pos = api.current_position_ot3(cfg.mount)
    est = _current_pos[pipette_ax]
    if not api.is_simulator:
        _encoder_poses = api.encoder_current_position_ot3(cfg.mount)
    ...
    report(
        _get_cs_section_tag(current),
        _get_cs_test_tag(current, speed, trial, direction, position),
        data,
    )
    return _did_pass
...
def _test_direction(
    ...
    plunger_poses = helpers_ot3.get_plunger_positions_ot3(api, cfg.mount)
    top, _, bottom, _ = plunger_poses
    # check that encoder/motor align
    aligned = _record_plunger_alignment(
        api, cfg, report, trial, current, speed, direction, "start"
    )
    if not aligned:
        return False
    # move the plunger
    ...
    try:
        _move_plunger(api, cfg.mount, _plunger_target, speed, current, acceleration)
        # check that encoder/motor still align
        aligned = _record_plunger_alignment(
            api, cfg, report, trial, current, speed, direction, "end"
        )
    except StallOrCollisionDetectedError:
        aligned = False
        _set_default_and_home_plunger(api, cfg.mount)
    return aligned
        ...
        for speed in speeds:
            for trial in range(trials):
                ...
                for direction in ["down", "up"]:
                    ...
                    if not test_pass:
                        if _includes_result(current, speed, cfg):
                            raise RuntimeError("Current Speed test failed.")
...
def _reset_gantry(api: SyncHardwareAPI) -> None:
    api.home(
        [
            Axis.Z_L,
            Axis.Z_R,
    ...
    home_pos = api.gantry_position(OT3Mount.RIGHT, CriticalPoint.MOUNT)
    test_pos = helpers_ot3.get_slot_calibration_square_position_ot3(5)
    test_pos = test_pos._replace(z=home_pos.z)
    api.move_to(OT3Mount.RIGHT, test_pos, critical_point=CriticalPoint.MOUNT)
...
def test_current_speed(
    ...
    """Run the pipette current speed test."""
    try:
        ...
        helpers_ot3.enable_stall_detection(api, True)


# ----------- END TEST Current Speed -----------
...
def build_report(test_name: str, cfg: TestConfig) -> CSVReport:
    ...
    return CSVReport(
        ...
        + build_current_speed_csv_sections(cfg)
        + [
            CSVSection(
                title="PRESSURE-DATA",
...
# Re-order elements in this list to change the order in which they run
TESTS = [
    (
        TestSection.CURRENT_SPEED,
        test_current_speed,
    ),
    (
...
def add_parameters(parameters: ParameterContext) -> None:
    ...
    # TODO: Remove all these options that may or may not be necessary anymore
    parameters.add_str(
    ...
    parameters.add_int(
        ...
        maximum=10,
        default=2,
        description="Number of trials to run the droplet test.",
    )
    parameters.add_int(
...
def run(ctx: ProtocolContext) -> None:
    ...
    assert pipette, "Need to have at least a left pipette."
    serial = helpers_ot3.get_pipette_serial_ot3(pipette)
    config = TestConfig(
    ...
    api.move_to(OT3Mount.LEFT, attach_pos._replace(z=current_pos.z))
