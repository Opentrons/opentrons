...
def run(ctx: ProtocolContext) -> None: 
    ...
    if not ctx.is_simulating(): 
        ctx.pause("set the dwelling current to 0.9A to heat up faster, then click resume") 
    ...
    if not ctx.is_simulating(): 
        ...
        with open(csv_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["timestamp", "elapsed_seconds", "avg_temp", "target_temp", "is_engaged"])
        is_engaged = False
        while True: 
            current_temp_1 = ot3api.read_sensor(primary) 
            current_temp_2 = ot3api.read_sensor(secondary) 
            ...
            if avg_temp >= target: 
                ...
                is_engaged = False
                ctx.delay(seconds=10*60, msg=f"Waiting for 10 minutes to ensure the temperature is stable, current temperature {avg_temp}, target={target}")
            else: 
                get_motors_hot(ot3api)  # type: ignore [arg-type]
                is_engaged = True
