...
RETURN_TIP = True
DEFAULT_PLATE_COUNT = 5
LIQUID_CLASS_TIPRACK_SLOT = "D1"
RUN_TIPRACK_SLOTS = ["C1", "C2", "C3", "B1", "B2"]
...
def add_parameters(parameters: ParameterContext) -> None:
    ...
    parameters.add_int(
        ...
        default=DEFAULT_PLATE_COUNT,
        minimum=1,
        maximum=len(RUN_TIPRACK_SLOTS),
        description="How many plates to fill in this run",
    )
...
def _prime_destination_plate(ctx: ProtocolContext, plate: Labware) -> None:
    ...
    # Seed liquid tracking with a minimal virtual volume so meniscus-referenced
    # liquid-class moves have a valid surface to reference.
...
def _verify_source_volume(
    ...
    if measured_volume_ul < minimum_required_volume_ul:
        ctx.pause(
            ...
            f"Refill reservoir {SOURCE_WELL} and resume. The protocol will probe again automatically."
        )
        return False
    return True


def _probe_until_sufficient(
    ctx: ProtocolContext,
    pipette: InstrumentContext,
    probe_tips: Labware,
    reservoir: Labware,
    ...
    """Probe until enough liquid is detected to continue the run."""
    attempt = 1
    while True:
        ctx.comment(f"Liquid sufficiency check attempt {attempt}.")
        measured_volume_ul = _probe_source_volume(
            ctx, pipette, probe_tips, reservoir, assumed_volume_ul
        )
        if _verify_source_volume(
            ctx, measured_volume_ul, minimum_required_volume_ul, plate_count
        ):
            return measured_volume_ul
        attempt += 1
...
def run(ctx: ProtocolContext) -> None:
    """Run."""
    plate_count = ctx.params.plate_count  # type: ignore[attr-defined]
    if plate_count > len(RUN_TIPRACK_SLOTS):
        raise ValueError("plate_count exceeds configured 96-tiprack slots")

    liquid_class_tips = ctx.load_labware(
        "opentrons_flex_96_tiprack_200uL",
        LIQUID_CLASS_TIPRACK_SLOT,
    ...
    minimum_required_volume_ul = _ul_per_plate(target_volume, pipette.channels) * plate_count
    ...
    ctx.comment(
        ...
        f"({_format_ml(per_plate_liquid_ul)} per plate)."
    )
    ctx.pause(
        f"Add {_format_ml(recommended_starting_volume_ul)} of {LIQUID_NAME} to reservoir {SOURCE_WELL} "
        f"(minimum {_format_ml(minimum_required_volume_ul+22000)} required), load a liquid-class tiprack in {LIQUID_CLASS_TIPRACK_SLOT}, "
        f"load {plate_count} fresh run tipracks in {', '.join(RUN_TIPRACK_SLOTS[:plate_count])}, "
        "and place the first plate in D3. The protocol will probe liquid height first and then "
        "decide whether the liquid is sufficient."
    )
    measured_starting_volume_ul = _probe_until_sufficient(
        ctx,
        pipette,
        liquid_class_tips,
        reservoir,
        recommended_starting_volume_ul,
    ...
    liquid_class = _get_transfer_settings(
        ctx, pipette, liquid_class_tips, target_volume
    )

    for i in range(plate_count):
        pipette.configure_for_volume(target_volume)
        pipette.pick_up_tip(run_tips[i].wells_by_name()["A1"])
        ...
        if RETURN_TIP:
            ...
            pipette.drop_tip()
        if i < plate_count - 1:
            ctx.pause(f"Replace the filled plate with plate {i + 2} of {plate_count} in D3.")
