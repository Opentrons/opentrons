...
def add_parameters(parameters: ParameterContext) -> None:
    ...
    parameters.add_str(
        ...
        default="96channel",
        choices=[
            {"display_name": "96channel", "value": "96channel"},
            {"display_name": "8channel", "value": "8channel"},
            {"display_name": "1channel", "value": "1channel"},
        ],
...
metadata = {"protocolName": "all Pre-heating protocol"}
...
def run(ctx: ProtocolContext) -> None:
    ...
    _ = ctx.load_instrument(
        f"flex_{ctx.params.pipette_type}_{ctx.params.model_type}", "left"  # type: ignore [attr-defined]
    )
    if not ctx.is_simulating():
