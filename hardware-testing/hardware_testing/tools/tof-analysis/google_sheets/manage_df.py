...
def append_file(
    ...
    if google_drive:
        file_path = google_drive.download_single_file(
            os.curdir, file_path, file_name, None
        )
        ...
        if axis == "x":
            file_details = (
                file_details[2:3] + ["X-Axis"] + file_details[5:-1] + [labware_num]
            )
        elif axis == "z":
            file_details = (
                file_details[2:3]
    ...
    for i, col in enumerate(
        columns[1:-1]
    ):  # Exclude the 'Hash_id' and'Values' column from this loop
        if i < len(labels):
    ...
    new_row["Values"] = json.dumps(
        matrix.tolist()
    )  # Store the matrix as a single entry
...
def get_files(credentials_path, drive_folder, email, stacker_configuration=None):
    ...
    google_drive = google_drive_helper.google_drive(
        credentials_path, drive_folder, email
    )
        ...
        while folder_paths:
            ...
            if folder_name not in exclude:
                get_files(
                    ...
                    folder_path,
                    email,
                    stacker_configuration + [folder_name],
                )
    else:
...
if __name__ == "__main__":
    ...
    drive_folder = (
        f"~/Library/CloudStorage/GoogleDrive-{email}"
