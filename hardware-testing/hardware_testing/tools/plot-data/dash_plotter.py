...
class RealTimePlotter:
    ...
    def read_csv_files(self, filenames: List[str]) -> dict:
        ...
        for filename in filenames:
            ...
            try:
                results[filename] = pd.read_csv(target, header=0)  # Ensure the first row is treated as headers
    ...
    def setup_layout(self):
        ...
        self.app.layout = html.Div([
            ...
            html.Div([
                html.Div([
                    html.H3("Files", style={'marginBottom': 10, 'display': 'inline-block'}),
                    html.Button("Select All", id='select-all-btn', n_clicks=0,
                                style={'marginLeft': 20, 'padding': '4px 12px', 'cursor': 'pointer'}),
                    ...
                    html.Button("Plot", id='plot-btn', n_clicks=0,
                                style={'marginLeft': 20, 'padding': '4px 14px', 'cursor': 'pointer',
    ...
    def setup_callbacks(self):
        ...
        def update_plot(n_clicks, selected_files, start_idx, end_idx):
            ...
            if not selected_files:
                if not csv_files:
                    empty_fig = go.Figure()
                    empty_fig.update_layout(title="No Data Available", xaxis_title="Time(s)", yaxis_title="Values")
                    return empty_fig, f"Status: No CSV files found in {self.data_path}", "Current File: None", "Data Points: 0"
            ...
            if not dataframes:
                empty_fig = go.Figure()
                empty_fig.update_layout(title="No Data Available", xaxis_title="Time(s)", yaxis_title="Values")
                return empty_fig, "Status: No readable files", "Current File: None", "Data Points: 0"
            ...
            fig.update_layout(
                ...
                yaxis2=dict(title="Flow Rate", side='right', overlaying='y', showgrid=False),
                hovermode='x unified',
                showlegend=True,
                height=600,
            ...
            file_info = f"Plotting {len(dataframes)} file(s): {', '.join(selected_files)}"
