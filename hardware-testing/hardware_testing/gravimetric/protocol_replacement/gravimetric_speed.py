...
metadata = {"protocolName": "Gravimetric QC V3 speed"}
...
def _run_and_store_trial(
    ...
    with _batch_measurement_report_update(fixture_settings.test_report):
        ...
        if fixture_settings.ctx.is_simulating():
            ...
            cur_height = liquid_height  # type: ignore[assignment]
        report.store_trial(
            fixture_settings.test_report,
            trial,
            volume,
...
def _run_fixture(
    ...
    try:
        with _right_pipette_heat_guard(
            ...
            ctx.comment(
                "fixed scale measurement wait: "
                f"{fixture_settings.scale_delay:g} seconds"
            )
            ctx.comment(
                "pipette gantry moves use CSV speed "
                f"{fixture_settings.gantry_speed:g} mm/s; "
                "liquid-class submerge and retract speeds are unchanged"
            )
            if fixture_settings.fast_simulate:
...
def run(ctx: ProtocolContext) -> None:
    ...
    if dual_mount_no_trash:
        ...
        ctx.comment(
            ...
            f"Z=+40 mm at CSV gantry speed {csv_settings.gantry_speed:g} mm/s; "
            "do not return tips"
        )
    if dual_mount_no_trash:
        ctx.comment(
            "dual tiprack and other gantry moves use CSV speed "
            f"{csv_settings.gantry_speed:g} mm/s"
        )
    elif dual_multi_extension_deck:
        ctx.comment(
            "dual P50M/P1000M tiprack and A3 disposal moves use CSV speed "
            f"{csv_settings.gantry_speed:g} mm/s"
        )
