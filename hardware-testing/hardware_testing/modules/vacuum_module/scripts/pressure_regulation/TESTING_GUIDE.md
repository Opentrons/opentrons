...
Host tools (from this directory, or via `-m`):

```bash
python3 cli.py {report,compare,convert} ...
python3 -m hardware_testing.modules.vacuum_module.scripts.pressure_regulation {report,compare,convert} ...
```
...
```bash
python3 cli.py report [--input PATH] [--output PATH] [--format html|pdf|both] [--refresh] [--title TEXT]
```
...
| `--title` | `Vacuum module pressure report` | Report heading |
...
```bash
python3 cli.py compare [--runs-dir DIR] [--output PATH] [--format html|pdf|both] [--refresh] [--title TEXT]
```
...
| `--refresh` | off | Embed an HTML auto-refresh tag |
| `--title` | `Vacuum module pressure report` | Report heading |
...
```bash
python3 cli.py convert --input PATH [--output PATH]
```
...
```bash
python3 cli.py convert --input runs/23_1888194c_water_3x0p2mm/results.json
python3 cli.py convert --input results.csv --output results.json
```
...
(
  while true; do
    ...
    python3 cli.py report --input results.json --output index.html --refresh 2>/dev/null || true
    python3 cli.py compare --runs-dir runs --output compare.html --refresh 2>/dev/null || true
    sleep 3
  done
) &
...
With `--refresh`, the HTML auto-reloads (3 s while `running`, 30 s when done). Generated pages have no auto-refresh unless that flag is set.
...
python3 cli.py compare --runs-dir $ART/runs --output $ART/compare.html
```
...
Waste rows also show trip / Trip t / expect / pass.
