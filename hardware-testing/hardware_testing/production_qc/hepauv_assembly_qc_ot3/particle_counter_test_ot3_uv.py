...
async def _main(arguments: argparse.Namespace) -> None:
    try:
        ...
        UVSN = input("Enter UV LIGHT Barcode Number(输入UV灯管条码):: ").strip()
        ...
        for iii in range(1):
            ...
            #获取数据UV get uv data
            print(f"Test Slot {grip_slot2} ...(开始测试位置{grip_slot2}")
            ...
            print(f"************测试结果: 位置-{test_data2['SLOT']} = {test_data2['uvdata']} test_result = {test_result_val} (测试通过标准_UV值>=175)************")
            # LOOP THROUGH FORCES
            if iii == 4:
                tetsuv = True
