...
def _ensure_hardware_testing_loaded() -> None:
    ...
    global RETEST_PRESSURE_CFG, RETEST_PRESSURE_EVENT
    ...
    from hardware_testing.production_qc.pipette_assembly_qc_ot3.pressure import (
        PRESSURE_FIXTURE_EVENT_CONFIGS as _RETEST_PRESSURE_CFG,
        PressureEvent as _RETEST_PRESSURE_EVENT,
    )
    ...
    RETEST_PRESSURE_CFG = _RETEST_PRESSURE_CFG
    RETEST_PRESSURE_EVENT = _RETEST_PRESSURE_EVENT
...
NON_RETESTABLE_PRESSURE_FAILED = False
RETEST_FIXTURE_WELL = "H1"
RETEST_FIXTURE_CHANNEL_INDEX = 0
RETEST_FIXTURE_CHANNEL_LABEL = "CH1"
...
RETEST_PARTIAL_PICKUP_CURRENT = PARTIAL_CURRENTS[1]
RETEST_PARTIAL_PICKUP_DISTANCE_MM = 13.0
RETEST_PARTIAL_PICKUP_SPEED_MM_PER_S = 5.0
...
@dataclass
class ChannelRetestResult:
    """Single-channel fixture retest result for one original failure row."""
...
def _write_single_channel_retest_result(
    write_cb: Callable, result: ChannelRetestResult
) -> None:
    write_cb(
        [
            "single-channel-retest-result",
            result.phase_name,
            f"CH{result.channel}",
            result.nozzle,
            result.retest_kind,
            f"{result.volume_ul}ul",
            result.sample_count,
            "insert-average-pressure",
            _format_optional_float(result.insert_average),
            "insert-threshold",
            _format_optional_float(result.insert_threshold),
            "insert-pass",
            "" if result.insert_passed is None else _bool_to_pass_fail(result.insert_passed),
            "leak-min",
            (
                _format_optional_float(result.minimum)
                if result.retest_kind != RETEST_KIND_INSERT_PRESSURE
                else ""
            ),
            "leak-max",
            (
                _format_optional_float(result.maximum)
                if result.retest_kind != RETEST_KIND_INSERT_PRESSURE
                else ""
            ),
            "leak-rate",
            (
                _format_optional_float(result.leak_rate)
                if result.retest_kind != RETEST_KIND_INSERT_PRESSURE
                else ""
            ),
            "leak-threshold",
            result.leak_threshold,
            "pass",
            _bool_to_pass_fail(result.passed),
            result.error or "",
        ]
    )
...
def _connect_to_single_channel_fixture(simulate: bool = False) -> PressureFixtureBase:
    """Connect to the single-channel pressure fixture used for retest."""
    if simulate:
        return connect_to_fixture(simulate=True)
    ...
    if not ports:
        raise RuntimeError(
            "No serial ports found for the single-channel pressure fixture retest."
        )
    ...
    for serial_port in ports:
        ...
        try:
            ui.print_info(
                f"Trying to connect to single-channel Pressure fixture on port {port}"
            )
            fixture = PressureFixture.create(port=port, slot_side="left")
            fixture.connect()
            ui.print_info(f"Found single-channel fixture on port {port}")
            return fixture
        except Exception as err:
            if fixture is not None:
                try:
    ...
    raise RuntimeError(
        "No single-channel pressure fixture found. Checked ports: "
        + "; ".join(errors)
    )


def _single_channel_retest_event() -> Any:
    return RETEST_PRESSURE_EVENT.ASPIRATE_P1000


async def _read_single_fixture_channel(
    api: OT3API,
    ...
    for sample_index in range(sample_count):
        ...
        if len(readings) <= RETEST_FIXTURE_CHANNEL_INDEX:
            raise RuntimeError(
                f"single-channel fixture returned {len(readings)} values; "
                f"expected at least {RETEST_FIXTURE_CHANNEL_INDEX + 1}"
            )
        values.append(readings[RETEST_FIXTURE_CHANNEL_INDEX])
        print(
            f"single-channel-retest sample {sample_index + 1}/{sample_count}: "
            f"{RETEST_FIXTURE_CHANNEL_LABEL}@{RETEST_FIXTURE_WELL}="
            f"{values[-1]:.2f}"
        )
        if not api.is_simulator and sample_index < sample_count - 1:
            await asyncio.sleep(sample_delay)
    return values


def _single_fixture_leak_rate(values: List[float]) -> Tuple[float, float, float]:
    sorted_values = sorted(values)
    calc_values = sorted_values[1:-1] if len(sorted_values) >= 3 else sorted_values
    minimum = min(calc_values)
    maximum = max(calc_values)
...
async def _run_single_channel_fixture_retest_for_row(
    api: OT3API,
    mount: OT3Mount,
    single_fixture: PressureFixtureBase,
    ...
    is_insert_phase = _is_insert_pressure_phase(phase_name)
    is_leak_phase = volume_ul is not None
    if is_insert_phase and is_leak_phase:
        retest_kind = RETEST_KIND_INSERT_AND_LEAK
    elif is_insert_phase:
        retest_kind = RETEST_KIND_INSERT_PRESSURE
    elif is_leak_phase:
        retest_kind = RETEST_KIND_LEAK_RATE
    else:
        retest_kind = "unsupported"

    nozzle = _channel_to_nozzle(channel)
    leak_threshold = (
        leak_thresholds.get(_phase_volume_key(phase_name), leak_thresholds.get("default", 1.0))
        if is_leak_phase
        else INSERT_PRESSURE_MIN
    )
    if retest_kind == "unsupported":
        return ChannelRetestResult(
            ...
            leak_rate=math.inf,
            passed=False,
            error=f"unsupported retest phase {phase_name}",
        )
    event_config = RETEST_PRESSURE_CFG[_single_channel_retest_event()]
    write_cb(
        [
            "single-channel-retest-start",
            phase_name,
            f"CH{channel}",
            nozzle,
            retest_kind,
            f"{volume_ul or 0.0}ul",
            RETEST_FIXTURE_CHANNEL_LABEL,
            RETEST_FIXTURE_WELL,
        ]
    )
    try:
        retest_fixture_point = (
            fixture_a1_point
            + _tiprack_well_offset(RETEST_FIXTURE_WELL)
            - _channel_tiprack_offset(channel)
        )
        write_cb(
            [
                "single-channel-retest-target",
                phase_name,
                f"CH{channel}",
                nozzle,
                f"{RETEST_FIXTURE_CHANNEL_LABEL}@{RETEST_FIXTURE_WELL}",
                round(retest_fixture_point.x, 3),
                round(retest_fixture_point.y, 3),
        ...
        write_cb(
            [
                ...
                "distance-mm",
                RETEST_PARTIAL_PICKUP_DISTANCE_MM,
                "speed-mm-s",
        ...
        insert_values = await _read_single_fixture_channel(
            api,
            single_fixture,
            RETEST_PRESSURE_CFG[RETEST_PRESSURE_EVENT.INSERT].sample_count,
            RETEST_PRESSURE_CFG[RETEST_PRESSURE_EVENT.INSERT].sample_delay,
        )
        insert_average = statistics.fmean(insert_values)
        insert_passed = insert_average > INSERT_PRESSURE_MIN
        write_cb(
            [
                "single-channel-retest-insert-values",
                phase_name,
                f"CH{channel}",
                "|".join(f"{value:.2f}" for value in insert_values),
            ]
        )
        write_cb(
            [
                "single-channel-retest-insert-average",
                phase_name,
                f"CH{channel}",
                round(insert_average, 4),
                "threshold",
                INSERT_PRESSURE_MIN,
                _bool_to_pass_fail(insert_passed),
            ]
        )
        if not is_leak_phase:
            return ChannelRetestResult(
                phase_name=phase_name,
                channel=channel,
                nozzle=nozzle,
                ...
                volume_ul=0.0,
                leak_threshold=leak_threshold,
                sample_count=len(insert_values),
                minimum=0.0,
                maximum=0.0,
                leak_rate=0.0,
                passed=insert_passed,
                insert_average=insert_average,
                insert_threshold=INSERT_PRESSURE_MIN,
                insert_passed=insert_passed,
            )
        assert volume_ul is not None
        await api.aspirate(mount, volume_ul)
        if not api.is_simulator:
            await asyncio.sleep(event_config.stability_delay)
        values = await _read_single_fixture_channel(
            api,
            single_fixture,
        ...
        await api.dispense(mount, volume_ul)
        await asyncio.sleep(1)
        return ChannelRetestResult(
            phase_name=phase_name,
            channel=channel,
...
async def _run_single_channel_fixture_retests(
    ...
    retest_rows = [
        ...
        if _phase_volume_key(str(row[0])) in {"1ul", "50ul", "200ul"}
        or _is_insert_pressure_phase(str(row[0]))
    ]
    if not retest_rows:
        return {}
    ...
    try:
        try:
            single_fixture = _connect_to_single_channel_fixture(simulate=cfg.simulate)
        except Exception as err:
            error = f"{type(err).__name__}: {err}"
            write_cb(["single-channel-retest-error", error])
            for row in retest_rows:
                result = _failed_single_channel_retest_result(
                    row, leak_thresholds, error
                )
                results[(result.phase_name, result.channel)] = result
                _write_single_channel_retest_result(write_cb, result)
            return results
        for row in retest_rows:
            result = await _run_single_channel_fixture_retest_for_row(
                api,
                mount,
                single_fixture,
                fixture_a1_point,
...
async def _read_pressure_and_check_results(
    ...
    global NON_RETESTABLE_PRESSURE_FAILED
...
def add_parameters(parameters: ParameterContext) -> None:
    ...
    parameters.add_float(
        ...
        maximum=30.0,
        description=(
            "Extra Z offset for calibrated B3 pressure fixture and B2 retest fixture pickup."
        ),
    )
...
def _load_calibrated_retest_fixture_a1_point(
    ...
    return retest_a1_point, retest_labware


def _load_protocol_deck_labware(
    ctx: ProtocolContext, z_offset: float
) -> Tuple[Point, Point, Any, Any]:
    """Load protocol-visible labware and return calibrated fixture points."""
    pickup_point, fixture_labware = _load_calibrated_fixture_pickup_point(
        ctx, z_offset
    )
    retest_a1_point, retest_labware = _load_calibrated_retest_fixture_a1_point(
        ctx, z_offset
    )
    ctx.comment(
...
def _slot_name_to_ot2_slot_number(slot_name: str) -> int:
    slot_name = slot_name.strip().upper()
    row = slot_name[0]
    col = int(slot_name[1:])
    row_offset = {"D": 0, "C": 3, "B": 6, "A": 9}[row]
...
def _phase_volume_ul(phase_name: str) -> Optional[float]:
    if "1.0ul" in phase_name:
        return 1.0
    if "50.0ul" in phase_name:
        return 50.0
...
def _channel_to_nozzle(channel: int) -> str:
    if channel < 1 or channel > 96:
        raise ValueError(f"channel must be 1-96, got {channel}")
    row_index = (channel - 1) // 12
    col_index = (channel - 1) % 12
    return f"{'ABCDEFGH'[row_index]}{col_index + 1}"


def _tiprack_well_offset(well: str) -> Point:
    well = well.strip().upper()
    row_index = "ABCDEFGH".index(well[0])
    col_index = int(well[1:]) - 1
    return Point(x=col_index * 9, y=row_index * -9)


def _channel_tiprack_offset(channel: int) -> Point:
    return _tiprack_well_offset(_channel_to_nozzle(channel))


def _summary_row_key(row: List[Any]) -> Tuple[str, int]:
    channel_text = str(row[1]).replace("CH", "")
    return str(row[0]), int(channel_text)


def _failed_single_channel_retest_result(
    row: List[Any],
    leak_thresholds: Dict[str, float],
    error: str,
) -> ChannelRetestResult:
    phase_name, channel = _summary_row_key(row)
    volume_value = _phase_volume_ul(phase_name)
    volume_ul = volume_value or 0.0
    is_insert_phase = _is_insert_pressure_phase(phase_name)
    is_leak_phase = volume_value is not None
    if is_insert_phase and is_leak_phase:
        retest_kind = RETEST_KIND_INSERT_AND_LEAK
    elif is_insert_phase:
        retest_kind = RETEST_KIND_INSERT_PRESSURE
    ...
    return ChannelRetestResult(
        ...
        passed=False,
        error=error,
        insert_threshold=INSERT_PRESSURE_MIN if is_insert_phase else None,
        insert_passed=False if is_insert_phase else None,
    )
...
def _summarize_channel_leak_history(
    ...
    fail_count_threshold: int,
    retest_mode: bool,
    retest_results: Optional[Dict[Tuple[str, int], ChannelRetestResult]] = None,
    ...
    for phase_name, channel_histories in sorted(
        ...
        for channel_index, channel_values in enumerate(channel_histories, start=1):
            ...
            original_passed = channel_passed
            original_status = _channel_status_label(original_passed, False)
            retest_result = retest_results.get((phase_name, channel_index))
    ...
    for channel_index, insert_fail_count in sorted(INSERT_PRESSURE_FAILED_COUNTS.items()):
        if insert_fail_count < fail_count_threshold:
            continue
        original_passed = False
        original_status = _channel_status_label(original_passed, False)
        phase_name = "insert-pressure"
...
def _collect_abnormal_channel_rows(
    leak_thresholds: Dict[str, float],
    cv_thresholds: Dict[str, float],
    fail_count_threshold: int,
    retest_mode: bool,
) -> List[List[Any]]:
    def _discard_write(_: List[Any]) -> None:
        return None

    _, abnormal_rows = _summarize_channel_leak_history(
        write_cb=_discard_write,
...
def _print_abnormal_channels_terminal_only(abnormal_rows: List[List[Any]]) -> None:
    ...
    for row in abnormal_rows:
        ...
        original_status = row[9] if len(row) >= 14 else row[-2]
        retest_pass = row[11] if len(row) >= 14 else "NOT_RUN"
        status = row[-2]
        print(
            f" - {phase} {channel}: status={status}, "
            f"original-status={original_status}, retest-pass={retest_pass}, "
...
async def _main(
    ...
    retest_fixture_a1_point: Optional[Point] = None,
    progress_cb: Optional[Callable[[str], None]] = None,
) -> None:
    """Run."""
    ...
    try:
        ...
        csv_cb.results(
            "pressure", channel_summary_passed and not NON_RETESTABLE_PRESSURE_FAILED
        )
        csv_cb.write(["-------------"])
...
def run(ctx: ProtocolContext) -> None:
    ...
    future = asyncio.run_coroutine_threadsafe(
        _main(
            ...
            retest_fixture_a1_point=retest_fixture_a1_point,
