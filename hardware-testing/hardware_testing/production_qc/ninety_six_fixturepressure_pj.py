...
CHANNEL_INVALID_TRIAL_COUNTS: Dict[str, List[int]] = {}
...
def _calculate_linear_regression_slope(
    samples: List[float], total_window_seconds: float
) -> float:
    if len(samples) <= 1 or math.isclose(total_window_seconds, 0.0, abs_tol=1e-9):
        return 0.0
    if len(samples) >= 5:
        trim_count = max(1, len(samples) // 5)
        fit_samples = samples[trim_count:-trim_count]
    ...
    if len(fit_samples) <= 1:
        return 0.0
    time_step_seconds = total_window_seconds / max(len(samples) - 1, 1)
    offset = (len(samples) - len(fit_samples)) // 2
    x_values = [(offset + idx) * time_step_seconds for idx in range(len(fit_samples))]
    x_mean = statistics.fmean(x_values)
    y_mean = statistics.fmean(fit_samples)
    numerator = sum((x - x_mean) * (y - y_mean) for x, y in zip(x_values, fit_samples))
...
async def _read_pressure_and_check_results(
    ...
    valid_leak_trial_per_channel = [True for _ in range(channels)]
    test_pass_stability = True
    for c in range(channels):
        chronological_samples = _samples_per_channel[c]
        sorted_samples = sorted(chronological_samples)
        trimmed_samples = sorted_samples[1:] if len(sorted_samples) > 1 else sorted_samples
        _c_min = min(trimmed_samples)
        _c_max = max(trimmed_samples)
        leak_rate = abs(
            _calculate_linear_regression_slope(
                chronological_samples,
                LEAK_RATE_WINDOW_SECONDS,
            )
        )
        ...
        csv_data_leak_method = [
            f"pressure-{tag.value}-channel-{c + 1}",
            "leak-rate-method",
            "stable-window-linear-slope",
        ]
        print(csv_data_leak_method)
        write_cb(csv_data_leak_method)
        csv_data_avg = [
            f"pressure-{tag.value}-channel-{c + 1}",
            "average",
        ...
        if _c_max - _c_min > pressure_event_config.stability_threshold:
            ...
            valid_leak_trial_per_channel[c] = False
    csv_data_stability = [
    ...
    if not test_pass_accuracy:
        valid_leak_trial_per_channel = [False for _ in range(channels)]
    csv_data_accuracy = [
        f"pressure-{tag.value}",
        "accuracy",
    ...
    if previous and previous_average_per_channel:
        ...
        if phase_name:
            _record_channel_leak_values(
                phase_name,
                leak_rates_per_channel,
                channels,
                valid_leak_trial_per_channel,
            )
...
def _record_channel_leak_values(
    phase_name: str, leak_rates: List[float], channels: int, valid_mask: List[bool]
) -> None:
    phase_history = CHANNEL_LEAK_HISTORY.setdefault(
        phase_name, [[] for _ in range(channels)]
    )
    invalid_trial_counts = CHANNEL_INVALID_TRIAL_COUNTS.setdefault(
        phase_name, [0 for _ in range(channels)]
    )
    if len(phase_history) < channels:
        phase_history.extend([[] for _ in range(channels - len(phase_history))])
    if len(invalid_trial_counts) < channels:
        invalid_trial_counts.extend([0 for _ in range(channels - len(invalid_trial_counts))])
    for channel_index, leak_rate in enumerate(leak_rates[:channels]):
        if channel_index < len(valid_mask) and valid_mask[channel_index]:
            phase_history[channel_index].append(leak_rate)
        else:
            invalid_trial_counts[channel_index] += 1
...
def _summarize_channel_leak_history(
    ...
    for phase_name, channel_histories in sorted(CHANNEL_LEAK_HISTORY.items()):
        ...
        invalid_trial_counts = CHANNEL_INVALID_TRIAL_COUNTS.get(
            phase_name, [0 for _ in range(len(channel_histories))]
        )
        for channel_index, channel_values in enumerate(channel_histories, start=1):
            invalid_trial_count = (
                invalid_trial_counts[channel_index - 1]
                if channel_index - 1 < len(invalid_trial_counts)
                else 0
            )
            if not channel_values:
                overall_pass = False
                channel_tag = f"channel-leak-{phase_name}-ch{channel_index}"
                write_cb([channel_tag, "invalid-trial-count", invalid_trial_count])
                write_cb([channel_tag, "decision-reason", "no-valid-trials"])
                write_cb([channel_tag, "status", _channel_status_label(False, retest_mode)])
                write_cb([channel_tag, "pass", _bool_to_pass_fail(False)])
                compact_summary_rows.append(
                    [
                        ...
                        0,
                        invalid_trial_count,
                        _channel_status_label(False, retest_mode),
                        _bool_to_pass_fail(False),
                    ]
                )
                continue
...
async def _main(
    ...
    global CHANNEL_INVALID_TRIAL_COUNTS
    global FINAL_TEST_RESULTS
    LOG_GING = ''
    FINAL_TEST_FAIL_INFOR = []
    PRESSURE_DATA_CACHE = []
    CHANNEL_LEAK_HISTORY = {}
    CHANNEL_INVALID_TRIAL_COUNTS = {}
    FINAL_TEST_RESULTS = []
    ...
    csv_cb.write(["leak-rate-method", "stable-window-linear-slope"])
