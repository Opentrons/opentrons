...
def file_setup(test_data, details):
    ...
    test_name = "{}-multi-channel-tip_presence_test".format(
...
async def _main() -> None:
    ...
    print(OT3Axis.of_main_tool_actuator(mount))
    details = [pipette_model]
    data_frame = {"pipette_model": None,
                "plunger_position(mm)": None,
                "ejector_status": None,
                "trial": None
                }
    ...
    try:
        ...
        for t in range(1, trials+1):
            ...
            while not tip_sense:
                ...
                tip_presence = await hw_api.get_tip_status(mount)
                print(f"tip state: {tip_presence}")
                print(current_position[OT3Axis.of_main_tool_actuator(mount)])
                tip_state = tip_presence[OT3Axis.of_main_tool_actuator(mount)]
                d_str = f"{pipette_model}, {current_position[OT3Axis.of_main_tool_actuator(mount)]}, {tip_state}, {t} \n"
                data.append_data_to_file(test_n, test_f, d_str)
                if tip_state == False:
                    tip_sense = True
                await move_plunger_relative_ot3(hw_api, mount, step_size)
            input("Measure with Caliper")
