...
def build_arg_parser():
    arg_parser = argparse.ArgumentParser(
        description="OT-3 8-Channel Partial Tip Pick-up Test"
    )
    ...
    arg_parser.add_argument(
        ...
        choices=["1CH", "8CH"],
        required=False,
        help="Sets the pipette number of channels",
        default="8CH",
    )
    arg_parser.add_argument(
        "-d",
        ...
        choices=["front", "rear"],
        required=False,
        help="Sets the pipette nozzles direction",
        default="front",
    )
    arg_parser.add_argument(
        "-p",
        ...
        choices=["P50", "P1K"],
        required=False,
        help="Sets the pipette type",
        default="P1K",
    )
    arg_parser.add_argument(
        "-i",
        "--tip_size",
        choices=["T20", "T50", "T200", "T1K"],
        required=False,
        help="Sets tip size",
        default="T1K",
    )
    ...
    arg_parser.add_argument(
        ...
        type=str,
        required=False,
        help="Sets the concentricity slot",
        default="D1",
    )
    arg_parser.add_argument(
        "-v",
        ...
        type=float,
        required=False,
        help="Sets the leak test volume",
        default=1000,
    )
    arg_parser.add_argument(
        "-o",
        ...
        type=float,
        required=False,
        help="Sets the liquid offset for aspiration",
        default=25,
    )
    arg_parser.add_argument(
        "-a",
        ...
        action="store_true",
        required=False,
        help="Calibrates tiprack position",
    )
    arg_parser.add_argument(
        "-y",
        "--concentricity",
        action="store_true",
        required=False,
        help="Enables concentricity test",
    )
    arg_parser.add_argument(
        "-s",
        "--simulate",
...
class Eight_Channel_Partial_Pickup_Test:
    def __init__(
        ...
        self.deck_slot: Optional[Dict] = None
        self.tiprack_position: Optional[Point] = None
        self.drop_position: Optional[Point] = None
        self.gauge_position: Optional[Point] = None
        self.trough_position: Optional[Point] = None
        self.feel_position: Optional[Point] = None
        self.hole_plate_position: Optional[Point] = None
        self.motor_current: Optional[float] = None
        self.pick_up_distance: Optional[float] = None
        self.pick_up_speed: Optional[float] = None
        self.tip_overlap: Optional[float] = None
        self.tip_diff = None
        self.end_effector: Optional[float] = None
        ...
        self.gauges: Dict[
            str, mitutoyo_digimatic_indicator.Mitutoyo_Digimatic_Indicator
        ] = {}
        self.gauge_ports = {
            "Z": "/dev/ttyUSB0",
            # "Y":"/dev/ttyUSB1",
        ...
        if self.channels == "8CH":
            if self.direction == "front":
                self.nozzles = [
                    i + "1" for i in reversed(list(string.ascii_uppercase))
                ][-8:]
            else:
        ...
        self.nozzle_encoders = {nozzle: "None" for nozzle in self.nozzles}
    ...
    def file_setup(self):
        ...
        self.nozzle_file = self.test_file.replace(".csv", "") + self.nozzle_file
        self.nozzle_path = (
            f"/data/testing_data/{self.test_name}/{self.test_date}/{self.nozzle_file}"
        )
        if not os.path.exists(self.nozzle_path):
            ...
            data.append_data_to_file(
                test_name=self.test_name,
                run_id=self.test_date,
                file_name=self.nozzle_file,
                data=self.nozzle_header,
            )
    ...
    async def pipette_setup(self):
        ...
        self.pipette_id = (
            "SIMULATION"
            if self.simulate
            else self.api._pipette_handler.get_pipette(self.mount).pipette_id
        )
        self.tip_diff = float(
            input("Tip Lenght Difference (mm) [Default = 0.2 mm]: ") or "0.2"
        )
        self.motor_current = float(
            input("Motor Current (Amps) [Default = 550 mA]: ") or "0.55"
        )
        self.pick_up_distance = float(
            input("Pick-up Distance (mm) [Default = 13 mmm]: ") or "13"
        )
        self.pick_up_speed = float(
            input("Pick-up Tip Speed (mm/s) [Default = 10 mm/s]: ") or "10"
        )
        self.test_data["Current"] = str(self.motor_current)
        self.test_data["Distance"] = str(self.pick_up_distance)
    ...
    async def gauge_setup(self):
        for key, value in self.gauge_ports.items():
            self.gauges[
                key
            ] = mitutoyo_digimatic_indicator.Mitutoyo_Digimatic_Indicator(port=value)
    ...
    async def _update_nozzle_manager(
        ...
        if self.direction == "front":
            await api.update_nozzle_configuration_for_mount(
                mount, self.nozzles[(tip_num - 1)], self.nozzles[0]
            )
        else:
            await api.update_nozzle_configuration_for_mount(
                mount, self.nozzles[0], self.nozzles[-1]
            )
    ...
    async def _calibrate_tiprack(self, api: OT3API, mount: OT3Mount) -> None:
        ...
        await api.pick_up_tip(
            mount, tip_length=(self.tip_length[self.tip_size] - self.tip_overlap_ref)
        )
        final_press = await api.encoder_current_position_ot3(mount, cp)
        print(f"Final Press Position: {final_press[Axis.by_mount(mount)]}")
        self.tip_overlap = abs(
            final_press[Axis.by_mount(mount)] - initial_press[Axis.by_mount(mount)]
        )
        print(f"Tip Overlap: {self.tip_overlap}")
        self.end_effector = self.tip_length[self.tip_size] - self.tip_overlap
        ...
        self.test_data["Overlap"] = str(self.tip_overlap)
        self.test_data["End Effector"] = str(self.end_effector)
        self.deck_slot["deck_slot"][self.tiprack_slot][Axis.X.name] = tiprack_point.x
    ...
    async def _calibrate_trash(self, api: OT3API, mount: OT3Mount) -> None:
        await api.home_z(mount)
        cp = CriticalPoint.TIP
        print(f"\nCalibrating Drop Tip Position at Slot {self.trash_slot}!")
        assert self.deck_slot is not None
        drop_tip_point = Point(
            self.deck_slot["deck_slot"][self.trash_slot]["X"],
            self.deck_slot["deck_slot"][self.trash_slot]["Y"],
            self.deck_slot["deck_slot"][self.trash_slot]["Z"],
        )
        await self._move_to_point(api, mount, drop_tip_point, cp)
        ...
        self.deck_slot["deck_slot"][self.trash_slot][Axis.X.name] = drop_tip_point.x
    ...
    async def _calibrate_gauge(self, api: OT3API, mount: OT3Mount) -> None:
        await api.home_z(mount)
        cp = CriticalPoint.TIP
        print(f"\nCalibrating Gauge at Slot {self.gauge_slot}!")
        assert self.deck_slot is not None
        gauge_point = Point(
            self.deck_slot["deck_slot"][self.gauge_slot]["X"],
            self.deck_slot["deck_slot"][self.gauge_slot]["Y"],
            self.deck_slot["deck_slot"][self.gauge_slot]["Z"],
        )
        await self._move_to_point(api, mount, gauge_point, cp)
        current_position = await api.current_position_ot3(mount, cp)
        gauge_loc = await self.jog(api, mount, current_position, cp)
        gauge_point = Point(
            gauge_loc[Axis.X], gauge_loc[Axis.Y], gauge_loc[Axis.by_mount(mount)]
        )
        self.deck_slot["deck_slot"][self.gauge_slot][Axis.X.name] = gauge_point.x
    ...
    async def _calibrate_trough(self, api: OT3API, mount: OT3Mount) -> None:
        await api.home_z(mount)
        cp = CriticalPoint.TIP
        print(f"\nCalibrating Trough at Slot {self.trough_slot}!")
        assert self.deck_slot is not None
        trough_point = Point(
            self.deck_slot["deck_slot"][self.trough_slot]["X"],
            self.deck_slot["deck_slot"][self.trough_slot]["Y"],
            self.deck_slot["deck_slot"][self.trough_slot]["Z"],
        )
        await self._move_to_point(api, mount, trough_point, cp)
        current_position = await api.current_position_ot3(mount, cp)
        trough_loc = await self.jog(api, mount, current_position, cp)
        trough_point = Point(
            trough_loc[Axis.X], trough_loc[Axis.Y], trough_loc[Axis.by_mount(mount)]
        )
        self.deck_slot["deck_slot"][self.trough_slot][Axis.X.name] = trough_point.x
        ...
        self.feel_position = Point(
            self.deck_slot["deck_slot"][self.feel_slot]["X"],
            self.deck_slot["deck_slot"][self.feel_slot]["Y"],
            self.deck_slot["deck_slot"][self.feel_slot]["Z"],
        )

    async def _calibrate_hole_plate(self, api: OT3API, mount: OT3Mount) -> None:
        await api.home_z(mount)
        cp = CriticalPoint.TIP
        print(f"\nCalibrating Hole Plate at Slot {self.center_slot}!")
        assert self.deck_slot is not None
        hole_plate_point = Point(
            self.deck_slot["deck_slot"][self.center_slot]["X"],
            self.deck_slot["deck_slot"][self.center_slot]["Y"],
            self.deck_slot["deck_slot"][self.center_slot]["Z"],
        )
        await self._move_to_point(api, mount, hole_plate_point, cp)
        current_position = await api.current_position_ot3(mount, cp)
        hole_plate_loc = await self.jog(api, mount, current_position, cp)
        hole_plate_point = Point(
            hole_plate_loc[Axis.X],
            hole_plate_loc[Axis.Y],
            hole_plate_loc[Axis.by_mount(mount)],
        )
        self.deck_slot["deck_slot"][self.center_slot][Axis.X.name] = hole_plate_point.x
    ...
    async def _get_nozzles_offset(self, api: OT3API, mount: OT3Mount) -> None:
        await api.home()
        for i, (k, v) in enumerate(self.nozzle_offsets.items()):
            print(f"\n-> Measuring Nozzle {k}")
            tip = i + 1
            await self._select_tip(api, mount, tip, True)
            if len(self.gauges) > 0:
                gauge, encoder = await self._measure_gauge(api, mount, "Z", True)
                encoder_z = round(encoder[Axis.by_mount(mount)], 2)
                self.nozzle_offsets[k] = str(gauge)
                self.nozzle_encoders[k] = str(encoder_z)
        await api.home_z()
        print(self.nozzle_offsets)
        print(self.nozzle_encoders)
    ...
    async def _update_settings(
        ...
        map_key = pipette._nozzle_manager.current_configuration.valid_map_key
        press_fit_config = pipette._config.pick_up_tip_configurations.press_fit
        current_setting = press_fit_config.configuration_by_nozzle_map[map_key][
            "default"
        ]
        current_setting.speed = speed
        current_setting.current = current
        current_setting.distance = distance
        pipette._config.pick_up_tip_configurations.press_fit.configuration_by_nozzle_map[
            map_key
        ][
            "default"
        ] = current_setting
        print(
            f"Pick up settings. {pipette._config.pick_up_tip_configurations.press_fit.configuration_by_nozzle_map[map_key]['default']}"
        )
    ...
    async def _pick_up_tips(self, api: OT3API, mount: OT3Mount, cycle) -> None:
        await api.home_z()
        cp = CriticalPoint.NOZZLE
        print(f"\nPicking up Tips at Column {cycle}/12!")
        x_offset = self.tip_distance * (cycle - 1)
        assert self.tiprack_position is not None
        tiprack_loc = self.tiprack_position._replace(
            x=self.tiprack_position.x + x_offset
        )
    ...
    async def _measure_tips(self, api: OT3API, mount: OT3Mount, cycle: int) -> None:
        for i in range(self.tip_num):
            tip = i + 1
            print(f"\n-> Measuring Tip {tip}/{self.tip_num}")
            await self._select_tip(api, mount, tip)
            assert self.tip_overlap is not None
            assert self.tip_overlap_ref is not None
            assert self.tip_diff is not None
            if len(self.gauges) > 0:
                gauge, encoder = await self._measure_gauge(api, mount, "Z")
                encoder_z = round(encoder[Axis.by_mount(mount)], 2)
                nozzle_offset = float(self.nozzle_offsets[self.nozzles[i]])
    ...
    async def _select_tip(
        self, api: OT3API, mount: OT3Mount, tip: int, nozzle: bool = False
    ) -> None:
        self.test_data["Tip"] = str(tip)
        y_offset = self.tip_distance * (tip - 1)
        if self.direction == "front":
            y_offset = y_offset * (-1)
        assert self.gauge_position is not None
    ...
    async def _measure_gauge(
        self, api: OT3API, mount: OT3Mount, axis: str, nozzle: bool = False
    ) -> Tuple[float, Dict[Axis, float]]:
        # Jog gauge
        print(f"Reading {axis} Gauge...")
        await api.move_rel(mount, self.gauge_offsets[axis], speed=10)
        cp = CriticalPoint.NOZZLE
        # Read encoder
        encoder = await api.encoder_current_position_ot3(
            mount, critical_point=cp, refresh=True
        )
        encoder_z = round(encoder[Axis.by_mount(mount)], 2)
        print(f"{axis} Encoder = {encoder_z}mm")
        # Read gauge
        ...
        return gauge, encoder

    async def _leak_test(self, api: OT3API, mount: OT3Mount) -> None:
        await api.home_z(mount)
        cp = CriticalPoint.TIP
        assert self.liquid_offset is not None
        assert self.trough_position is not None
    ...
    async def _feel_test(self, api: OT3API, mount: OT3Mount) -> None:
        await api.home_z(mount)
        cp = CriticalPoint.TIP
        ...
        if self.concentricity:
            await self._concentricity_test(api, mount)
        await self._move_to_point(api, mount, self.drop_position, cp)
        await api.drop_tip(mount)
        await api.home_z(mount)

    async def _concentricity_test(self, api: OT3API, mount: OT3Mount) -> None:
        await api.home_z(mount)
        cp = CriticalPoint.TIP
        await self._move_to_point(api, mount, self.hole_plate_position, cp)
        center_result = float(
            input(
                "\nConcentricity Test Result [Default = 1]: \n1 = Pass \n2 = Fail \nAnswer:"
            )
            or "1"
        )
        self.test_data["Concentricity"] = str(center_result)

    def _reset_data(self):
        self.test_data["Leak"] = "None"
        self.test_data["Feel"] = "None"

    def _save_config(self, filename: str, data: str) -> None:
    ...
    def _load_config(self, tip_num):
        ...
        if self.direction == "front":
            self.tiprack_position = Point(
                self.deck_slot["deck_slot"][self.tiprack_slot]["X"],
                self.deck_slot["deck_slot"][self.tiprack_slot]["Y"],
                self.deck_slot["deck_slot"][self.tiprack_slot]["Z"],
            )

            self.drop_position = Point(
                self.deck_slot["deck_slot"][self.trash_slot]["X"],
                self.deck_slot["deck_slot"][self.trash_slot]["Y"],  # - y_offset,
                self.deck_slot["deck_slot"][self.trash_slot]["Z"],
            )

            self.gauge_position = Point(
                self.deck_slot["deck_slot"][self.gauge_slot]["X"],
                self.deck_slot["deck_slot"][self.gauge_slot]["Y"],  # - y_offset,
                self.deck_slot["deck_slot"][self.gauge_slot]["Z"],
            )

            self.trough_position = Point(
                self.deck_slot["deck_slot"][self.trough_slot]["X"],
            ...
            self.hole_plate_position = Point(
                self.deck_slot["deck_slot"][self.center_slot]["X"],
    ...
    async def _countdown(self, count_time: float) -> None:
        """
    ...
    async def _reset(self, api: OT3API, mount: OT3Mount) -> None:
        # Home Z
        await api.home_z()
        ...
        try:
            ...
            if self.api and self.mount:
                ...
                if self.calibrate:
                    ...
                    if self.concentricity:
                        await self._calibrate_hole_plate(self.api, self.mount)
                else:
...
if __name__ == "__main__":
    ...
    test = Eight_Channel_Partial_Pickup_Test(
        args.simulate,
        args.calibrate,
        args.concentricity,
        args.cycles,
