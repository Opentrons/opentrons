...
class Plot:
    def __init__(self, data):
        ...
        self.PLOT_FONT = 16
        self.PLOT_PATH = "plot_gripper_deck_drop_offset/"
    ...
    def create_plot(self):
        ...
        print("Plotting Gripper Offset Percentage...")
        self.gripper_offset_percentage_plot()
        print("Plots Saved!")
    ...
    def gripper_offset_cycle_plot(self):
        ...
        self.plot_param["filename"] = "plot_gripper_deck_offset"
        self.plot_param["title"] = "Gripper Deck Drop XY Offsets"
        self.plot_param["x_title"] = "Cycle"
        self.plot_param["y_title"] = "XY Offset (mm)"
    ...
    def gripper_offset_plot(self):
        for i in range(len(self.axes)):
            df = self.df_data[self.df_data["Axis"] == self.axes[i]]
            x_axis = "Type"
            y_axis = "Offset"
            x_start = df[x_axis].iloc[0]
            x_end = df[x_axis].iloc[-1]
            ...
            self.set_legend(fig, [f"{self.axes[i]} Pick-Up Offset", f"{self.axes[i]} Drop Offset"])
            fig.update_layout(
                xaxis_categoryorder="array",
                xaxis_categoryarray=self.offset_types
            )
            self.plot_param["figure"] = fig
            self.plot_param["filename"] = f"plot_gripper_offset_{self.axes[i].lower()}"
            self.plot_param["title"] = f"Gripper Deck Drop Offset ({self.axes[i]}-axis Error) [Z-axis = -0.75mm]"
            self.plot_param["x_title"] = "Offset Type"
            self.plot_param["y_title"] = f"{self.axes[i]}-axis Offset (mm)"
    ...
    def gripper_offset_percentage_plot(self):
        df = self.df_data
        x_axis = "Axis"
        y_axis = "Offset"
        x_start = df[x_axis].iloc[0]
        ...
        y_start = df[y_axis].iloc[0]
        y_end = df[y_axis].iloc[-1]
        fig = px.histogram(df, x=x_axis, y=[y_axis], color="Axis", barnorm="percent")
        self.plot_param["figure"] = fig
        self.plot_param["filename"] = "plot_gripper_axis_percentage"
        self.plot_param["title"] = "Gripper Deck Drop Offset (XY Percentage)"
        self.plot_param["x_title"] = "Axis"
        self.plot_param["y_title"] = "Percentage (%)"
        self.plot_param["x_range"] = None
