...
def get_user_input():
    ...
    while robot not in robot_list:
        print(
            f"Your input was {robot}. Expected input is one of the following: {robot_list}"
        )
        robot = input("Robot: ")
    ...
    while labware not in labware_list:
        print(
            f"Your input was {labware}. Expected input is one of the following: {labware_list}"
        )
    ...
    try:
        ...
        while not (is_stable == "True"):
            ...
            if bool(is_stable) == 1:
                ...
                if max_weight < 1:
                    print(
                        f"You should remeasure. The maximum mass captured was {max_weight} grams."
                    )
                    break
                df.to_csv(savepath)
                break
