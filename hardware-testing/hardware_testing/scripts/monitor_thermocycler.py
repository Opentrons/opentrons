...
async def _loop_til_temp(
    ...
    debug_info: List[Tuple[DebugPlateTemperature, float]] = []
    time_to_hit = 0.0
    plt.suptitle("Temperature.")
    ax = plt.gca()
    ax.set_ylim(BLOCK_TARGET_MIN - 5, BLOCK_TARGET_MAX + 5)
    ...
    while (
        ...
        debug_info.append((await _get_temperature_debug(connection), time.time()))
        ...
        times = [d[1] - start for d in debug_info]
        ramp_factor = (
        ...
        # hs = [d[0].heatsink for d in debug_info]
        fr = [d[0].front_right for d in debug_info]
        fl = [d[0].front_left for d in debug_info]
        fc = [d[0].front_center for d in debug_info]
        br = [d[0].back_right for d in debug_info]
        bl = [d[0].back_left for d in debug_info]
        bc = [d[0].back_center for d in debug_info]

        plt.plot(times, target, linestyle=":")
        # plt.plot(times, hs)
        plt.plot(times, fr)
        plt.plot(times, fl)
        plt.plot(times, fc)
        ...
        plt.plot(abbrv_times, ideal, linestyle="--")
        plt.pause(0.05)

    print(
        f"Start temp {start_temp.current} time to hit it {time_to_hit} effective ramp rate {abs((start_temp.current - set_temp) / time_to_hit)}"
    )
    plt.show()
