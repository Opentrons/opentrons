...
async def _main(is_simulating: bool, mount: types.OT3Mount) -> None:
    ...
    if(not args.nc):
        ...
        if(os.path.exists(path)):
            with open('/home/root/.opentrons/testing_data/calibrated_slot_locations.json', 'r') as openfile:
    ...
    for key in calibrated_slot_loc.keys():
        ...
        await api.move_to(mount, Point(calibrated_slot_loc[key][0],
                    calibrated_slot_loc[key][1], calibrated_slot_loc[key][2]))
            ...
            for row in range(ROWS):
                ...
                for pick_up in range(PICKUPS_PER_TIP):
                    ...
                    if(tip_presence_eject):
                        ...
                        cycle_data = [test_robot, test_pip['name'], rack, pick_up+1,
                ...
                # await api.move_to(mount, Point(calibrated_slot_loc[key][0]+9*col,
                #             calibrated_slot_loc[key][1]-9*(row+1), calibrated_slot_loc[key][2]))
            ### adjust column increment
            print("Moving to next column...\n")
            # await api.move_to(mount, Point(calibrated_slot_loc[key][0]+9*(col+1),
            #             calibrated_slot_loc[key][1], calibrated_slot_loc[key][2]))
