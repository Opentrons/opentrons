...
async def _bowtie_move(api, homed_position_left: types.Point, homed_position_right: types.Point) -> tuple:
    ...
    for key in high_tp_points.keys():
        if type(key) == int:
            ...
            try:
                ...
                await api.move_to(OT3Mount.LEFT, Point(high_tp_points[key][0], high_tp_points[key][1],
                                                       home_z_pos[OT3Axis.Z_L]))
                await api.move_to(OT3Mount.LEFT, high_tp_points[key])
            ...
            await api.move_to(OT3Mount.LEFT, Point(tip_pick_up_pos[OT3Axis.X], tip_pick_up_pos[OT3Axis.Y],
                                                   tip_pick_up_pos[OT3Axis.Z_L] - 30.0))
            tip_len = 57
            await api.pick_up_tip(OT3Mount.LEFT, tip_len, prep_after=False)
            await api.remove_tip(OT3Mount.LEFT)
            await api.move_to(OT3Mount.LEFT, Point(tip_pick_up_pos[OT3Axis.X], tip_pick_up_pos[OT3Axis.Y],
                                                   tip_pick_up_pos[OT3Axis.Z_L]))
...
async def _main(is_simulating: bool, cycles: int, mount: types.OT3Mount) -> None:
    ...
    test_robot = 'gantry lifetime robot'
    ...
    for i in range(cycles):
        ...
        stall_count, xy_count, y_count, z_l_count, z_r_count, final_encoder_pos, open_loop_pos = \
            await _bowtie_move(api, homed_pos_left, homed_pos_right)
