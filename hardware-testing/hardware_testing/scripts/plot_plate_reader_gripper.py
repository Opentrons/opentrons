"""Plot Plate Reader Gripper Test Results."""
import os
import sys
import argparse
import pandas as pd
...
class Plot:
    def __init__(self, data):
        ...
        self.PLOT_FONT = 16
        self.PLOT_PATH = "plot_plate_reader_gripper/"
    ...
    def create_plot(self):
        print("Plotting Plate Presence...")
        self.plate_presence_plot()
        print("Plotting Lid Status...")
        self.lid_status_plot()
    ...
    def plate_presence_plot(self):
        df = self.df_data
        x_axis = "Cycle"
        y_axis = "Plate State"
        fig = px.line(df, x=x_axis, y=[y_axis], color="Step", markers=True)
        legend = ["Add Plate","Close Lid","Open Lid","Remove Plate"]
        self.set_legend(fig, legend)
        self.plot_param["figure"] = fig
        self.plot_param["filename"] = "plot_plate_presence"
        self.plot_param["title"] = f"Plate Reader Module Data (Plate Presence)"
        self.plot_param["x_title"] = "Cycle"
        self.plot_param["y_title"] = "Plate State"
        self.plot_param["x_range"] = [0, 500]
        self.plot_param["y_range"] = None
        self.plot_param["legend"] = "Move State"
        self.plot_param["annotation"] = None
        self.write_plot(self.plot_param)

    def lid_status_plot(self):
        df = self.df_data
        x_axis = "Cycle"
        y_axis = "Lid State"
        fig = px.line(df, x=x_axis, y=[y_axis], color="Step", markers=True)
        ...
        self.set_legend(fig, legend)
        self.plot_param["figure"] = fig
        self.plot_param["filename"] = "plot_lid_status"
        self.plot_param["title"] = f"Plate Reader Module Data (Lid Status)"
        self.plot_param["x_title"] = "Cycle"
        self.plot_param["y_title"] = "Lid State"
        self.plot_param["x_range"] = [0, 500]
        self.plot_param["y_range"] = None
        self.plot_param["legend"] = "Move State"
        self.plot_param["annotation"] = None
