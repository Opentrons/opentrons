"""Single-Channel Pipette Dynamic Tip Overlap Test."""
import argparse
import asyncio
import csv
import time
...
async def _main(args: argparse.Namespace, cfg: TestConfig) -> None:
    ...
    try:
        ...
        if (args.calibrate):
            ...
            # await move_to_point(hw_api, mount, initial_dial_loc, CriticalPoint.NOZZLE)
        ...
        for tip in range(1, total_tips+1):
            ...
            if args.dial_indicator:
                ...
                for tip in range(1, tips_to_use + 1):
                    ...
                    tip_dist = (await hw_api.encoder_current_position_ot3(mount, CriticalPoint.NOZZLE))[Axis.by_mount(mount)]
                    print(f"Encoder Position: {tip_dist}")
                    encoder_pos.append(tip_dist)
                    time.sleep(0.5)
                    ...
                    if tip_count % num_of_columns == 0:
                        x_offset = 0
                print(f'Tip Position_1: {measurements}')
                print(f'Tip Position_2: {measurements_2_map}')
                print(f'Delta tip pos:{tip_measurement-tip_measurement_2}')
            ...
            measurement_map.append(tip_dist)
            ...
            measurement_map.append(tip_measurement_2)
            d_str = ''
