...
class Plot:
    ...
    def center_offset_plot(self):
        ...
        y_min = round(df_sort[y_axis].iloc[0], 3)
        # y_min_id = df[y_axis].idxmin()
        y_min_xpos = df_sort.iloc[0][x_axis].item()
        y_min_text = f"Min Offset = {y_min}mm"

        y_max = round(df_sort[y_axis].iloc[-1], 3)
        # y_max_id = df[y_axis].idxmax()
        y_max_xpos = df_sort.iloc[-1][x_axis].item()
        y_max_text = f"Max Offset = {y_max}mm"

        y_range = round((y_max - y_min)/2, 3)
        y_avg = round(df_sort[y_axis].mean(), 3)
        y_avg_xpos = 0
        ...
        self.plot_param["title"] = "Gripper Y-Axis Center Offset"
