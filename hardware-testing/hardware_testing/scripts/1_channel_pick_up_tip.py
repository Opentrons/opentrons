...
async def _main() -> None:
    ...
    directory = f'/data/1ch_pick_up_tip_{datetime.datetime.now().strftime("%m_%d_%y")}'
    ...
    file_name = f"{directory}/full_pu_1ch_pipette_%s-%s.csv" % (
        "standard",
    ...
    if (args.measure_nozzles):
        ...
        measurements = []
        measurement_map = {}
        nozzle_fname = f"{directory}/nozzle_data_1ch_pipette_%s-%s.csv" % (
        "standard",
        datetime.datetime.now().strftime("%m-%d-%y_%H-%M"),
        )
        with open(nozzle_fname, 'w', newline='') as csvfile:
            ...
            test_details.writerow({'1ch Nozzle Flatness Testing'})
    ...
    with open(file_name, 'w', newline='') as pu_csvfile:
        ...
        test_details.writerow({'test details'})
        test_details.writerow({pipette_model})
        test_details.writerow({'1ch Pick up Tip Testing-Default Settings'})
