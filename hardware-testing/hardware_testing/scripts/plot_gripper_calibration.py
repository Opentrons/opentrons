...
class Plot:
    def __init__(self, data):
        ...
        self.PLOT_FONT = 16
        self.PLOT_PATH = "plot_gripper_calibration/"
    ...
    def import_file(self, file):
        ...
        if "X" in self.axes:
            ...
            df["X Zeroed"] = round(df["X Gauge"] - df["X Zero"] - (self.PROBE_RAD), 3)
        if "Y" in self.axes:
            self.gauge_zero["Y"] = df["Y Zero"].iloc[0]
            df["Y Zeroed"] = round(df["Y Gauge"] - df["Y Zero"] - (self.PROBE_RAD), 3)
        if "Z" in self.axes:
    ...
    def deck_height_plot(self):
        ...
        y_avg_xpos = 0
        y_avg_text = f"Deck Avg = {y_avg}mm ± {y_range}mm"
    ...
    def gauge_plot(self, axis, index, zero=False):
        ...
        y_range = round((y_max - y_min)/2, 2)
        y_avg = round(df[y_axis].mean(), 2)
        y_avg_xpos = 0
