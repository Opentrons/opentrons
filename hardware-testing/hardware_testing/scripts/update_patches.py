...
def _update_patches(upstream: str) -> None:
    ...
    _run_cmd(f"git checkout {args.upstream}".split())
    _run_cmd("make -C hardware-testing apply-patches-gravimetric".split())
    _run_cmd("git add -A".split())
    _run_cmd(["git", "commit", "-m", "temp commit, will reverted (can be rebased away)"])
    ...
    _run_cmd(f"make -C hardware-testing update-patches-gravimetric upstream={cur_hash}".split())
    _run_cmd("git add hardware-testing/hardware_testing/gravimetric/overrides/*".split())
    _run_cmd(["git", "commit", "-m", "chore(hardware-testing): update gravimetric patch files"])
    ...
    print(f"git rebase -i {args.upstream}")
    print(f"and delete the lines with the shortsha \n{cp_hash}and \n{revert_hash}")

def _main() -> None:
    parser = argparse.ArgumentParser(
        prog='update patches', description='updates the patches for hardware testing', parents=parents)
    parser.add_argument(
        '--upstream',
        type=str,
        default='origin/edge',
