...
def run(ctx: ProtocolContext) -> None:
    ...
    tiprack_200 = ctx.load_labware("opentrons_ot3_96_tiprack_200uL", "10")
    ...
    multi.dispense(200 - TEST_VOLUME, plate[f"A{COLUMN}"].top(-1)).blow_out()
    multi.drop_tip()
    # DYE
    wells = [plate[f"{row}{COLUMN}"] for row in "ABCDEFGH"]
    for well in wells:
        pipette.pick_up_tip()
        pipette.aspirate(TEST_VOLUME, reservoir[RESERVOIR_DYE_WELL].bottom(2))
        pipette.dispense(TEST_VOLUME, well.top(-1)).blow_out()
        pipette.drop_tip()
